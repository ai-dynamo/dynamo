#!/usr/bin/env bash
# Deploy and benchmark MX P2P vs disk loading.
#
# Usage:
#   ./run.sh              # Run both p2p and disk, fetch results, plot comparison
#   ./run.sh p2p          # Run only p2p
#   ./run.sh disk         # Run only disk
#   ./run.sh both         # Same as no argument
#   ./run.sh clean        # Clean up all resources for this RUN_ID
#   ./run.sh plot         # Re-generate comparison plot from existing artifacts
#   ./run.sh table        # Print mx-target startup timing table from existing logs
#
# Environment variables:
#   NAMESPACE        Kubernetes namespace (default: hwoo)
#   RUN_ID           Unique ID for resource isolation in parallel runs (default: default)
...
#   SCALEUP_DELAY    Seconds to wait before scaling up mx-target (default: 130)
#   STAT             Statistic to plot: avg, p50, p90, p99, min, max (default: avg)
#   TP_SIZE          Tensor parallel size (default: 8)
#   DP_SIZE          Data parallel size (default: 1). Total GPUs per worker = TP_SIZE * DP_SIZE
#   ENABLE_EP        Set to 1 to add --enable-expert-parallel to vLLM args
...
#   VLLM_EXTRA_ARGS  Space-separated extra vLLM args (e.g., "--quantization fp8 --max-model-len 8192")
#   VLLM_USE_DEEP_GEMM   Set to 0 to disable DeepGEMM (avoids weight repacking overhead in P2P)
#   VLLM_USE_FLASHINFER_MOE_FP8  Set to 0 to disable FlashInfer MoE (avoids w13→w31 swap overhead in P2P)
#   ENABLE_TORCH_COMPILE_CACHE   Set to 0 to disable torch.compile cache reuse on target (default: 1)
#   DEEPGEMM_SOURCE_WARMUP_MODE         DeepGEMM warmup mode on source: full or relax (default: full)
#   DEEPGEMM_TARGET_WARMUP_MODE  DeepGEMM warmup mode on target: skip, relax, or full (default: skip)
#                                skip: reuse source's JIT cache from PVC; relax/full: target warms up independently
#   ENABLE_REQUEST_LOGGING       Set to 1 to log per-request routing on frontend and arrival on workers
#   RDMA_RESOURCE        RDMA resource name for k8s scheduling (default: rdma/ib; nscale uses rdma/shared_ib)
#   KUBE_CONTEXT         Kubernetes context to use (creates isolated kubeconfig for parallel runs)
#
# Dataset generation (used when DATASET trace file doesn't exist):
#   INPUT_TRACE      Base trace for synthesis (default: conversation_trace.jsonl)
#   MAX_ISL          Max input sequence length filter (default: 8000)
#   MAX_OSL          Max output sequence length filter (default: 8000)
#   STEP_RATES       Comma-separated rate values (default: 4,4,10,12,12,10,4,4)
#   STEP_DURATIONS   Comma-separated per-step durations in seconds (default: 60,60,60,60,60,60,60,60)
#   CONSTANT_RATE    Set to 1 to use constant-rate scheduling instead of Poisson arrival
set -euo pipefail
...
SCALEUP_DELAY="${SCALEUP_DELAY:-130}"
STAT="${STAT:-avg}"
TP_SIZE="${TP_SIZE:-8}"
DP_SIZE="${DP_SIZE:-1}"
...
dgd_for_mode() {
  local m="$1"
  if [[ "$m" == "p2p" ]]; then
    echo "vllm-agg-mx-p2p-${RUN_ID}"
  else
    echo "vllm-agg-disk-${RUN_ID}"
  fi
}
...
scale_service() {
  ...
  # Try DGDSA scale subresource first, fall back to DGD spec patch
  local adapter_name="${dgd_name}-$(echo "$service_name" | tr '[:upper:]' '[:lower:]')"
  if kubectl patch "dgdsa/${adapter_name}" -n "$NAMESPACE" \
      --type merge -p "{\"spec\":{\"replicas\":${replicas}}}" 2>/dev/null; then
    echo "Scaled DGDSA $adapter_name to $replicas replicas"
  else
    echo "DGDSA $adapter_name not found, patching DGD directly"
    kubectl patch "dgd/${dgd_name}" -n "$NAMESPACE" \
      --type merge -p "{\"spec\":{\"services\":{\"${service_name}\":{\"replicas\":${replicas}}}}}"
  fi

  # Wait for DGD ready (up to 30 min)
  echo "Waiting for DGD $dgd_name to be ready after scaling..."
  local elapsed=0
  local timeout=1800
  while true; do
    sleep 10
    ...
    if [[ "$ready" == "True" ]]; then
      echo "  DGD ready after ${elapsed}s"
      return
    fi
    if [[ "$elapsed" -ge "$timeout" ]]; then
      echo "WARNING: Timed out waiting for DGD ready after ${timeout}s"
      return
    fi
    if (( elapsed % 60 == 0 )); then
      echo "  waiting for DGD ready... (${elapsed}s)"
    fi
  done
}
...
check_target_resources() {
  ...
  while true; do
    ...
    gpu_alloc=$(kubectl get pods --all-namespaces --field-selector="spec.nodeName=$target_node,status.phase=Running" \
    ...
    if [[ "$m" == "p2p" ]]; then
      ...
      rdma_alloc=$(kubectl get pods --all-namespaces --field-selector="spec.nodeName=$target_node,status.phase=Running" \
...
save_logs() {
  ...
  for svc in mx-source mx-target frontend; do
    local pod
...
run_experiment() {
  ...
  # Deploy DGD (sed the default DGD name to the RUN_ID-scoped name)
  local dgd_tmp="/tmp/dgd-${RUN_ID}-${m}.yaml"
  model_sed < "$dgd_yaml" \
  ...
  # Wait, then sanity-check target and scale up
  echo "Waiting ${SCALEUP_DELAY}s before scaling up mx-target..."
  sleep "$SCALEUP_DELAY"
  check_target_resources "$m"
  scale_service "$dgd_name" "mx-target" 1

  # Wait for aiperf trace replay to complete
  wait_for_aiperf "$m"
...
clean_all() {
  ...
  kubectl delete pod "$BENCHMARK_POD" -n "$NAMESPACE" --ignore-not-found --wait
  kubectl delete "dgd/vllm-agg-mx-p2p-${RUN_ID}" -n "$NAMESPACE" --ignore-not-found --wait
  kubectl delete "dgd/vllm-agg-disk-${RUN_ID}" -n "$NAMESPACE" --ignore-not-found --wait
  kubectl delete "deployment/${MX_SERVER_NAME}" -n "$NAMESPACE" --ignore-not-found --wait
...
# Setup source and target node image caches
./setup-image-cache.sh source
./setup-image-cache.sh target

# Taint source and target nodes to prevent GPU theft during benchmark
...
echo "Tainting target node with ${TAINT_KEY}=target:NoSchedule..."
