#!/usr/bin/env bash
# Deploy and benchmark disaggregated prefill/decode with planner-driven scale-up.
#
# Unlike the aggregated scripts (run_planner.sh) which use mx-source/mx-target
# workers, this script deploys separate VllmPrefillWorker and VllmDecodeWorker
# with KV-aware routing. The planner scales both prefill and decode independently
# based on TTFT/ITL SLA breaches via the KV Router's per-worker gauges.
#
# Usage:
#   ./run_disagg_planner.sh              # Run both p2p and disk
#   ./run_disagg_planner.sh p2p          # Run only p2p
#   ./run_disagg_planner.sh disk         # Run only disk
#   ./run_disagg_planner.sh both         # Same as no argument
#   ./run_disagg_planner.sh clean        # Clean up all resources for this RUN_ID
#   ./run_disagg_planner.sh plot         # Re-generate comparison plot from existing artifacts
#   ./run_disagg_planner.sh table        # Print mx-target startup timing table from existing logs
#
...
# Disagg planner-specific settings
TTFT_TARGET="${TTFT_TARGET:-2000}"
ITL_TARGET="${ITL_TARGET:-50}"
MIN_REPLICAS="${MIN_REPLICAS:-1}"
MAX_GPU_BUDGET="${MAX_GPU_BUDGET:-32}"
PLANNER_INTERVAL="${PLANNER_INTERVAL:-5}"
TARGET_NODE_COUNT="${TARGET_NODE_COUNT:-2}"
...
echo "=== Configuration (disagg planner mode) ==="
echo "  Mode:      $MODE"
...
echo "  ITL target:       ${ITL_TARGET}ms"
echo "  Min replicas:     $MIN_REPLICAS"
echo "  Max GPU budget:   $MAX_GPU_BUDGET"
...
# Substitute disagg planner-specific values in the --config JSON string
planner_sed() {
  sed -e "s|\"ttft\":2000|\"ttft\":$TTFT_TARGET|g" \
      -e "s|\"itl\":50|\"itl\":$ITL_TARGET|g" \
      -e "s|\"min_endpoint\":1|\"min_endpoint\":$MIN_REPLICAS|g" \
      -e "s|\"max_gpu_budget\":32|\"max_gpu_budget\":$MAX_GPU_BUDGET|g" \
      -e "s|\"decode_engine_num_gpu\":8|\"decode_engine_num_gpu\":$TOTAL_GPUS|g" \
      -e "s|\"prefill_engine_num_gpu\":8|\"prefill_engine_num_gpu\":$TOTAL_GPUS|g" \
...
dgd_for_mode() {
  local m="$1"
  if [[ "$m" == "p2p" ]]; then
    echo "vllm-disagg-p2p-planner-${RUN_ID}"
  else
    echo "vllm-disagg-disk-planner-${RUN_ID}"
  fi
}
...
default_dgd_for_mode() {
  local m="$1"
  if [[ "$m" == "p2p" ]]; then
    echo "vllm-disagg-p2p-planner"
  else
    echo "vllm-disagg-disk-planner"
  fi
}
...
yaml_for_mode() {
  local m="$1"
  if [[ "$m" == "p2p" ]]; then
    echo "disagg_p2p_planner.yaml"
  else
    echo "disagg_disk_planner.yaml"
  fi
}
...
wait_for_aiperf() {
  local m="$1"
  local dgd_name bench_mode
  dgd_name=$(dgd_for_mode "$m")
  bench_mode="disagg-${m}"
  local timeslice_path="/models/artifacts/${MODEL_BASE}/${DATASET}/${RUN_TAG}/${bench_mode}/profile_export_aiperf_timeslices.json"

  echo "Waiting for aiperf to finish (disagg-$m)..."
  local pod
  pod=$(kubectl get pods -n "$NAMESPACE" --no-headers -o custom-columns=":metadata.name" | grep "${dgd_name}.*vllmprefillworker" | head -1 || true)
  if [[ -z "$pod" ]]; then
    echo "WARNING: No prefill worker pod to poll, sleeping 300s instead"
    sleep 300
    return
  fi
...
save_logs() {
  ...
  dgd_name=$(dgd_for_mode "$m")
  bench_mode="disagg-${m}"
  local logs_dir="${LOCAL_ARTIFACTS}/${bench_mode}/logs"
  mkdir -p "$logs_dir"

  echo "Collecting disagg-$m service logs..."
  for svc in vllmprefillworker vllmdecodeworker frontend planner; do
    local pod
...
fetch_aiperf_artifacts() {
  local m="$1"
  local bench_mode="disagg-${m}"
  local remote_path="/perf-cache/artifacts/${MODEL_BASE}/${DATASET}/${RUN_TAG}/${bench_mode}"
  local local_path="${LOCAL_ARTIFACTS}/${bench_mode}"

  if ! kubectl get pod "$BENCHMARK_POD" -n "$NAMESPACE" &>/dev/null; then
    echo "WARNING: Benchmark pod not found, skipping aiperf artifact fetch for disagg-$m"
    return
  fi
  ...
  mkdir -p "$local_path"
  echo "Fetching aiperf artifacts (disagg-$m)..."
  local files
...
run_experiment() {
  local m="$1"
  local dgd_name default_dgd dgd_yaml frontend bench_mode
  dgd_name=$(dgd_for_mode "$m")
  ...
  echo ""
  echo "===== Running disagg-$m experiment (planner mode) ====="
  ...
  local dgd_tmp="/tmp/dgd-${RUN_ID}-disagg-${m}-planner.yaml"
  model_sed < "$dgd_yaml" \
  ...
  if [[ "$ENABLE_TORCH_COMPILE_CACHE" == "0" ]]; then
    sed -i '/VllmDecodeWorker:/,$ { s|/models/.cache/vllm/compile_cache|/tmp/vllm_compile_cache|g }' "$dgd_tmp"
  fi
  ...
  if [[ "$DEEPGEMM_TARGET_WARMUP_MODE" != "skip" ]]; then
    ...
    sed -i '/VllmDecodeWorker:/,$ { s|/models/.cache/deepgemm|/tmp/deepgemm|g }' "$dgd_tmp"
  fi
  ...
  # Upload trace file to prefill worker pod
  local prefill_pod
  prefill_pod=$(kubectl get pods -n "$NAMESPACE" --no-headers -o custom-columns=":metadata.name" | grep "${dgd_name}.*vllmprefillworker" | head -1 || true)
  if [[ -z "$prefill_pod" ]]; then
    echo "ERROR: No prefill worker pod found"
    kubectl get pods -n "$NAMESPACE" --no-headers
    exit 1
  fi
  # Sync DeepGEMM JIT cache from prefill worker (ephemeral /tmp) to PVC for target reuse
  echo "Syncing DeepGEMM JIT cache to PVC..."
  kubectl exec "$prefill_pod" -n "$NAMESPACE" -- \
  ...
  echo "Uploading trace to ${prefill_pod}:/models/traces/${DATASET}.jsonl ..."
  kubectl exec "$prefill_pod" -n "$NAMESPACE" -- mkdir -p /models/traces
  kubectl cp "$TRACE_FILE" "$NAMESPACE/${prefill_pod}:/models/traces/${DATASET}.jsonl"
  ...
  model_sed < perf.yaml \
    | sed -e "s/value: p2p/value: $bench_mode/" \
  ...
  local remote_artifact_dir="/perf-cache/artifacts/${MODEL_BASE}/${DATASET}/${RUN_TAG}/${bench_mode}"
...
clean_all() {
  ...
  kubectl delete pod "$BENCHMARK_POD" -n "$NAMESPACE" --ignore-not-found --wait
  kubectl delete "dgd/vllm-disagg-p2p-planner-${RUN_ID}" -n "$NAMESPACE" --ignore-not-found --wait
  kubectl delete "dgd/vllm-disagg-disk-planner-${RUN_ID}" -n "$NAMESPACE" --ignore-not-found --wait
  kubectl delete "deployment/${MX_SERVER_NAME}" -n "$NAMESPACE" --ignore-not-found --wait
...
if [[ "$MODE" == "plot" ]]; then
  if [[ -d "$LOCAL_ARTIFACTS/disagg-p2p" ]] && [[ -d "$LOCAL_ARTIFACTS/disagg-disk" ]]; then
    ...
    echo "ERROR: Need both $LOCAL_ARTIFACTS/disagg-p2p and $LOCAL_ARTIFACTS/disagg-disk to generate plot"
    exit 1
  fi
  exit 0
fi
...
if [[ "$MODE" == "table" ]]; then
  P2P_SUBDIR="disagg-p2p" DISK_SUBDIR="disagg-disk" \
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  for m in p2p disk; do
    ...
    default_dgd=$(default_dgd_for_mode "$m")
    dgd_yaml=$(yaml_for_mode "$m")
    dgd_out="$outdir/dgd-disagg-${m}.yaml"
    model_sed < "$dgd_yaml" \
      | sed "s|name: ${default_dgd}|name: ${dgd_name}|g" \
      | planner_sed \
      | strip_run_tolerations \
      > "$dgd_out"
    if [[ "$ENABLE_TORCH_COMPILE_CACHE" == "0" ]]; then
      sed -i '/VllmDecodeWorker:/,$ { s|/models/.cache/vllm/compile_cache|/tmp/vllm_compile_cache|g }' "$dgd_out"
    fi
    if [[ "$DEEPGEMM_SOURCE_WARMUP_MODE" != "full" ]]; then
    ...
    if [[ "$DEEPGEMM_TARGET_WARMUP_MODE" != "skip" ]]; then
      ...
      sed -i '/VllmDecodeWorker:/,$ { s|/models/.cache/deepgemm|/tmp/deepgemm|g }' "$dgd_out"
    fi
    if [[ "$ENABLE_REQUEST_LOGGING" == "1" ]]; then
      sed -i '0,/^      envs:$/{/^      envs:$/a\        - name: DYN_LOG\
          value: "info,dynamo_runtime::pipeline::network::egress::push_router=trace"
  ...
  for m in p2p disk; do
    ...
    frontend="${dgd_name}-frontend"
    bench_mode="disagg-${m}"
    perf_out="$outdir/perf-disagg-${m}.yaml"
    model_sed < perf.yaml \
      | sed -e "s/value: p2p/value: $bench_mode/" \
            -e "s/value: vllm-agg-mx-p2p-frontend/value: $frontend/" \
            -e "s/value: staircase_trace/value: $DATASET/" \
...
# Reserve TARGET_NODE_COUNT target nodes upfront (taint to prevent GPU theft)
./setup-image-cache.sh target "$TARGET_NODE_COUNT"
