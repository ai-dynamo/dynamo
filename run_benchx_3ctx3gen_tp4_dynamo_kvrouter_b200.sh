#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.3ctx3gen_tp4.dynamo.kvrouter
#SBATCH --nodes=2
...
#SBATCH --output=bench/logs/run_benchx_3ctx3gen_tp4_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_3ctx3gen_tp4_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 3 ctx TP1 + 3 gen TP4 with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 3 ctx worker(s) (GPUs 0-2) TP1
#   NODE0 — gen worker 0 GPUs 3,4,5,6 (TP4)
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx3gen_tp4_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx3gen_tp4_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_3ctx3gen_tp4_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
GEN_PIDS=()
# --- decode worker 0 on $NODE0 GPUs 3,4,5,6 (TP4) ---
GEN_PORT=8085
echo "[$(date +%H:%M:%S)] Starting gen worker 0 on $NODE0 GPUs 3,4,5,6 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=4 --ntasks-per-node=4 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
  --output="$RESULTS_DIR/gen_worker_g0.log" \
  --container-image="$CONTAINER_IMAGE" --container-mounts="$CONTAINER_MOUNTS" \
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=3,4,5,6 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
...
# --- 3 ctx worker(s) on $NODE0 GPUs 0-2 (prefill, TP1) ---
CTX_PIDS=()
for GPU in 0 1 2; do
...
# ==============================================================================
echo "[$(date +%H:%M:%S)] Waiting for servers (3 prefill + 3 decode)..."
if ! wait_for_dynamo_workers "${NODE0}" "${FRONTEND_PORT}" 3 3 2700 60; then
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE0}:8085,${NODE1}:8086,${NODE1}:8087" \
    --labels "ctx_g0,ctx_g1,ctx_g2,gen_g0,gen_g1,gen_g2" \
