...
class PerformanceConfig:
    def __init__(self):
        ...
        self.nats_max_payload_mb = 8  # Default NATS max payload in MB
...
class HighCapacityNatsServer(ManagedProcess):
    """NATS server configured for large payloads (up to 8MB recommended)"""

    def __init__(self, request, port=4222, timeout=300, max_payload_mb=8):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            if len(json_bytes) > payload_size_threshold:
                logger.debug(
                    f"Using BytesIO for large payload ({payload_size_mb:.2f} MB)"
                )

                # Use BytesIO for large payloads
...
Examples:
  ...
  python test_request_plane_performance.py --min-size 200000 --max-size 400000 --step 50000
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="NATS server max payload size in MB (default: 8, max recommended: 8)",
    )
