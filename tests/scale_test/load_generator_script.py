...
async def run_load_test_worker(
    ...
    print(f"[Process {process_id}] Starting: {duration}s, {qps:.2f} QPS, {len(urls)} targets")

    clients = {url: AsyncOpenAI(base_url=f"{url}/v1", api_key="not-needed", timeout=duration + 30) for url in urls}
    ...
    return {"process_id": process_id, "stats": stats, "actual_duration": time.time() - start_time}


def worker_process(
    process_id: int, qps: float, duration: int, urls: list, model: str, max_tokens: int, result_queue: multiprocessing.Queue
) -> None:
    try:
        result = asyncio.run(run_load_test_worker(process_id, qps, duration, urls, model, max_tokens))
...
def print_results(agg_stats: dict, actual_duration: float) -> int:
    ...
    for url, s in sorted(agg_stats.items()):
        ...
        if total > 0:
            ...
            print(f"\nFrontend {url}:")
            print(f"  Requests: {total}, Success: {s['success']}, Errors: {s['failed']} ({s['failed'] / total * 100:.1f}%)")
            print(f"  Latency: avg={avg_lat:.1f}ms, p50={p50_lat:.1f}ms, p99={p99_lat:.1f}ms")
...
def main() -> int:
    ...
    print("=" * 70)
    print(f"Pod {os.environ.get('JOB_COMPLETION_INDEX', '0')}/{total_pods}, {num_processes} process(es)")
    print(f"QPS: {qps_per_pod:.2f}/pod, {qps_per_pod / num_processes:.2f}/process, {qps_per_pod * total_pods:.2f} total")
    print(f"Duration: {duration}s, Frontends: {len(urls)}")
    print("=" * 70 + "\n")

    if num_processes == 1:
        result = asyncio.run(run_load_test_worker(0, qps_per_pod, duration, urls, model, max_tokens))
        results = [result]
    else:
        ...
        for i in range(num_processes):
            p = multiprocessing.Process(
                target=worker_process, args=(i, qps_per_process, duration, urls, model, max_tokens, result_queue)
            )
            p.start()
            processes.append(p)
