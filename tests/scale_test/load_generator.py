...
class LoadGenerator:
    ...
    async def send_request(self, url: str) -> RequestResult:
        ...
        try:
            ...
            return RequestResult(url=url, latency=latency, success=True, tokens_generated=tokens)

        except Exception as e:
            latency = time.time() - start_time
            return RequestResult(url=url, latency=latency, success=False, error=str(e))
    ...
    async def generate_load(self, duration_sec: int, qps: float) -> Dict[str, LoadGeneratorStats]:
        logger.info(f"Generating load for {duration_sec}s at {qps} QPS across {len(self.frontend_urls)} frontends...")
        ...
        logger.info(f"Sent {request_count} requests in {actual_duration:.1f}s (actual QPS: {actual_qps:.1f})")
