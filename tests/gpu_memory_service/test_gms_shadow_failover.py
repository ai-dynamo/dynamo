...
# 1. Shadow A starts as the explicit RW weights publisher, then quiesces.
# 2. Shadow B starts in explicit RO mode from the committed weights layout, then quiesces.
# 3. Primary starts in explicit RO mode and owns the next RW KV layout.
...
def _start_primary(
    ...
    *,
    ...
    primary = manager.start_engine(
        "primary",
        weights_lock_mode=RequestedLockType.RO,
    )
    assert_completion_ok(
        frontend_port,
        "Primary test",
...
def _run_shadow_failover_test(
    ...
    with GMSProcessManager(request, engine_cls) as manager:
        ...
        shadow_a = manager.start_engine(
            "shadow-a",
            weights_lock_mode=RequestedLockType.RW,
        )
        assert_completion_ok(
            frontend_port,
            "Hello",
