...
def pytest_ignore_collect(collection_path, config):
    """Skip collecting integration test files if required dependencies aren't installed."""
    filename = collection_path.name

    # Skip prompt_embeds integration tests if openai or torch aren't available
    if filename == "test_prompt_embeds_integration.py":
