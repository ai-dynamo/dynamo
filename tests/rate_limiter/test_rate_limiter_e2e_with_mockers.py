...
class RateLimitTestFrontend(ManagedProcess):
    ...
    def __init__(self, request, frontend_port: int, rate_limit_duration_ms: int, max_queue_depth: int):
        command = [
            "python",
            "-m",
            "dynamo.frontend",
...
async def test_rate_limiter_e2e(request, runtime_services):
    ...
    frontend = RateLimitTestFrontend(request, FRONTEND_PORT, RATE_LIMIT_DURATION_MS, MAX_QUEUE_DEPTH)
    mocker_processes = []
    ...
    try:
        ...
        assert status == 429, f"Expected 429 (rate limited), got {status}. Response: {response}"
        ...
        status, response = await send_test_request(base_url)
        assert status == 200, f"Expected 200 after rate limit expired, got {status}. Response: {response}"
        ...
        for i in range(3):
            ...
            assert status == 200, f"Request {i+1} after expiry: Expected 200, got {status}"
...
async def test_rate_limiter_multiple_events(request, runtime_services):
    ...
    frontend = RateLimitTestFrontend(request, FRONTEND_PORT + 1, RATE_LIMIT_DURATION_MS, MAX_QUEUE_DEPTH)
    mocker_processes = []
    ...
    try:
        ...
        mocker = MockerProcess(request, "dyn://test-namespace.mocker.generate", mocker_args_file)
        mocker_processes.append(mocker)
        mocker.__enter__()
        ...
        logger.info("=== Triggering second rate limiting event (should extend period) ===")
        await publish_all_workers_busy_event()
        await asyncio.sleep(0.2)
...
async def test_rate_limiter_disabled(request, runtime_services):
    ...
    frontend = ManagedProcess(
        ...
        health_check_urls=[
            (f"http://localhost:{FRONTEND_PORT + 2}/v1/models", lambda r: r.status_code == 200)
        ],
        log_dir=request.node.name,
        terminate_existing=False,
    )
