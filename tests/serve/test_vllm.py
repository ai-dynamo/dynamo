...
vllm_configs = {
    ...
    # Audio multimodal test using the backend-native AudioLoader via agg_multimodal.sh.
    # Uses Qwen2-Audio-7B — self-contained (no base model dep), has tokenizer.json.
    # DYN_CHAT_PROCESSOR=vllm is required: the Rust Jinja engine cannot render
    # multimodal content arrays (audio_url) — the template does string + array
    # concatenation which fails. This applies to all audio models, not just Qwen3.
