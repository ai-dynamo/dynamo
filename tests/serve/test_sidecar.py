...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "Qwen/Qwen3-0.6B",
            id="replicas",
            marks=pytest.mark.model("Qwen/Qwen3-0.6B"),
        ),
        pytest.param(
            True,
            1,
            ...
            id="dep",
            marks=pytest.mark.model("silence09/DeepSeek-R1-Small-2layers"),
        ),
    ],
...
def test_sidecar_kv_routing(
    ...
    with reserved_ports(
        ...
        if dep:
            devices = list(engine_env.values())
            assert len(set(devices)) == 2, "DEP requires two distinct GPUs"
