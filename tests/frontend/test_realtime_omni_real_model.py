...
(``OpenAIServingRealtime.transcribe_realtime`` calls
``model_cls.buffer_realtime_audio``), and in vLLM-Omni exactly one model does:
``Qwen3OmniMoeForConditionalGeneration``. Every published Qwen3-Omni checkpoint
is 30B-A3B, so there is no small stand-in -- ``Qwen2.5-Omni-3B`` is supported by
vLLM-Omni generally but does not implement ``SupportsRealtime``. CI runs on
shared 24 GB L4s; ``tests/serve/test_vllm_omni.py`` already skips the *7B* omni
as needing ~80 GB.
...
and run::
    ...
    pytest tests/frontend/test_realtime_omni_real_model.py -s

``TENSOR_PARALLEL_SIZE`` and the ``gpu_4`` marker below are a starting point,
not a measured configuration -- neither has been validated against real
hardware. Profile with ``tests/utils/profile_pytest.py`` and add the resulting
``profiled_vram_gib`` / ``requested_vllm_kv_cache_bytes`` markers before wiring
this into any automated tier (see ``.ai/test-model-size-guardrails.md``).
"""
...
from tests.frontend.test_realtime_omni_bridge import _drain_until, _recv_json
...
# Weights alone are ~60 GB in bf16; the thinker/talker stages and the audio
# encoder add more on top.
SKIP_REASON = (
...
def _speech_like_input() -> bytes:
    """One second of 16 kHz PCM16 audio to open and commit a turn.

    The content is not asserted on -- what matters is that a committed turn
    reaches the engine and comes back as a completed response. A tone keeps the
    test hermetic (no sample-clip download).
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.ws_connect(f"ws://127.0.0.1:{port}/v1/realtime") as ws:
                ...
                json.dumps(
                    {
                        ...
                        "audio": base64.b64encode(_speech_like_input()).decode("utf-8"),
                    }
                )
            )
            ...
            while response_done_status is None:
                ...
                if etype == "error":
                    # The failure mode this test exists for: an engine-output
                    # shape the handler mishandles surfaces here, and the turn
                    # then terminates as failed rather than completed.
            ...
            assert audio_b64_parts, "talker produced no audio deltas"

            # Real audio, unlike the mock's echo, is only checked for shape: the
            # deltas must decode as PCM16 and carry a non-silent signal.
            out_bytes = b"".join(base64.b64decode(p) for p in audio_b64_parts)
            assert len(out_bytes) % 2 == 0, len(out_bytes)
            samples = np.frombuffer(out_bytes, dtype=np.int16)
