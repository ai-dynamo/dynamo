...
"""Smoke the AIC core payload and native engine in the shipped frontend image."""
...
def _assert_installed_aic_matches_frontend_manifest() -> None:
    """Validate the core package source selected by the frontend manifest."""
    ...
    matches = [
        ...
        if Requirement(value).name == "aiconfigurator-core"
    ]
    assert len(matches) == 1, matches
    requirement = matches[0]
    installed_version = Version(metadata.version("aiconfigurator-core"))
    ...
    expected_ref = expected_ref.partition("#")[0]

    direct_url_text = metadata.distribution("aiconfigurator-core").read_text(
        "direct_url.json"
    )
