...
# exists on the local volume, create a lightweight symlink inside the HF
# cache that points back to the local copy.  The action.yml pre-copies
# models from the S3 FUSE mount to a regular directory on the shared work
# volume *before* starting the test container, so the symlink always points
# at local disk (never at FUSE — safetensors mmap triggers SIGBUS on FUSE).
# This lets from_pretrained() and snapshot_download() resolve the model
# without any network I/O, while still allowing HF to download any model
# that is *not* staged locally.
#
...
def _link_local_model_to_hf_cache(model_id: str, local_models_dir: str) -> bool:
    """Symlink a locally available model into the HF cache.

    Creates the huggingface_hub cache structure so that from_pretrained() and
    snapshot_download() resolve the model without a network round-trip.

    The caller is responsible for ensuring the local directory contains
    regular files (not a FUSE mount).  In CI the action.yml pre-copies
    models from the S3 FUSE mount to a local directory on the work volume
    before starting the test container, so the symlink target is always
    local disk where mmap works safely.
    ...
    try:
        ...
        logging.warning(
            f"Failed to link local model '{model_id}' to HF cache: {e}. "
            f"Will fall back to HuggingFace download."
        )
        try:
            os.unlink(tmp_link)
        except OSError:
