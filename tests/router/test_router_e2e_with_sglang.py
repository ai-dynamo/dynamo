...
# Disaggregated SGLang configuration (single GPU, memory-constrained)
SGLANG_DISAGG_ARGS: Dict[str, Any] = {
    "page_size": PAGE_SIZE,
    "model": MODEL_NAME,
    "mem_fraction_static": 0.4,  # Tighter memory for 2 workers on same GPU
    "context_length": 1024,
...
class SGLangDisaggProcess:
    """Manages disaggregated SGLang workers (prefill or decode) for testing.

    Similar to SGLangProcess but configured for disaggregated serving mode.
    Prefill and decode workers share the same namespace but use different
    component names ("prefill" vs "backend").
    """
    ...
    def __init__(
        ...
        request_plane: str = "tcp",
        store_backend: str = "etcd",
        bootstrap_port: int = 12345,
    ):
        """Initialize disaggregated SGLang workers.

        Args:
            request: pytest request fixture for log directory
            namespace: Shared namespace for prefill and decode workers
            mode: "prefill" or "decode" disaggregation mode
            sglang_args: Configuration dict (page_size, model, mem_fraction_static, etc.)
            num_workers: Number of worker processes
            request_plane: Request plane to use ("nats", "tcp", or "http")
            store_backend: Storage backend to use ("etcd" or "file")
            bootstrap_port: Port for NIXL bootstrap communication
        """
        ...
        self.mode = mode
        # Prefill workers register as "prefill" component, decode as "backend"
        self.component_name = "prefill" if mode == "prefill" else "backend"
        self.endpoint = f"dyn://{self.namespace}.{self.component_name}.generate"
        self.num_workers = num_workers
        self.worker_processes = []
        self.store_backend = store_backend
        ...
        page_size = sglang_args.get("page_size", PAGE_SIZE)
        model = sglang_args.get("model", MODEL_NAME)
        mem_fraction_static = sglang_args.get("mem_fraction_static", 0.4)
        context_length = sglang_args.get("context_length")
        disable_cuda_graph = sglang_args.get("disable_cuda_graph", True)

        self.model_name = model

        for worker_idx in range(num_workers):
            # All workers share GPU 0 for single-GPU disaggregated testing
            gpu_device = "0"
            ...
            # Add per-worker KV events config for ZMQ publishing
            # Use different port ranges for prefill vs decode
            base_port = 20080 if mode == "prefill" else 20090
            kv_events_port = base_port + worker_idx
            kv_events_config = f'{{"publisher":"zmq","topic":"kv-events","endpoint":"tcp://*:{kv_events_port}"}}'
            command.extend(["--kv-events-config", kv_events_config])
            ...
            self.worker_processes.append(process)
            logger.info(
                f"Created SGLang {mode} worker {worker_idx} on GPU {gpu_device} "
                f"(mem_frac={mem_fraction_static}, kv_port={kv_events_port}) "
                f"with endpoint: {self.endpoint}"
            )

    def __enter__(self):
        """Start all worker processes with sequential initialization."""
        logger.info(
            f"[SGLangDisaggProcess] Starting {len(self.worker_processes)} {self.mode} workers..."
        )

        for i, process in enumerate(self.worker_processes):
            logger.info(f"[SGLangDisaggProcess] Starting {self.mode} worker {i}...")
            try:
                process._logger = logging.getLogger(process.__class__.__name__)
                process._command_name = process.command[0]
                os.makedirs(process.log_dir, exist_ok=True)
                log_name = f"{process._command_name}-{self.mode}.log.txt"
                process._log_path = os.path.join(process.log_dir, log_name)
                ...
                logger.info(
                    f"[SGLangDisaggProcess] {self.mode} worker {i} launched with PID: "
                    f"{process.proc.pid if process.proc else 'unknown'}"
                )
                time.sleep(process.delayed_start)
                ...
                if i < len(self.worker_processes) - 1:
                    init_delay = 5
                    logger.info(
                        f"[SGLangDisaggProcess] Waiting {init_delay}s for {self.mode} worker {i} to initialize..."
                    )
                    time.sleep(init_delay)

            except Exception:
                logger.exception(
                    f"[SGLangDisaggProcess] Failed to start {self.mode} worker {i}"
                )
                try:
                    process.__exit__(None, None, None)
                except Exception as cleanup_err:
                    logger.warning(
                        f"[SGLangDisaggProcess] Error during cleanup: {cleanup_err}"
                    )
                raise

        logger.info(f"[SGLangDisaggProcess] All {self.mode} workers launched.")

        # Wait for health checks
        for i, process in enumerate(self.worker_processes):
            logger.info(
                f"[SGLangDisaggProcess] Checking health for {self.mode} worker {i}..."
            )
            try:
                elapsed = process._check_ports(process.timeout)
                process._check_urls(process.timeout - elapsed)
                process._check_funcs(process.timeout - elapsed)
                logger.info(
                    f"[SGLangDisaggProcess] {self.mode} worker {i} health checks passed"
                )
            except Exception:
                logger.error(
                    f"[SGLangDisaggProcess] {self.mode} worker {i} health check failed"
                )
                self.__exit__(None, None, None)
                raise

        logger.info(f"[SGLangDisaggProcess] All {self.mode} workers ready!")
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        """Stop all worker processes gracefully."""
        for i, process in enumerate(self.worker_processes):
            logger.info(f"Stopping SGLang {self.mode} worker {i}")
            process.__exit__(exc_type, exc_val, exc_tb)

        logger.info(f"Waiting for SGLang {self.mode} worker resources to clean up...")
        time.sleep(2)
...
@pytest.mark.gpu_1
@pytest.mark.parametrize("request_plane", ["nats", "tcp"], indirect=True)
@pytest.mark.timeout(300)  # TODO: benchmark this
def test_router_decisions_disagg_sglang(
    request,
    runtime_services_dynamic_ports,
    predownload_models,
    set_ucx_tls_no_mm,
    ...
    """Test disaggregated prefill-decode routing with SGLang on single GPU.

    This test validates that:
    1. Prefill and decode workers can run on the same GPU with memory sharing
    2. The KV router correctly routes prefill requests to prefill workers
    3. Prefix reuse works correctly (same prefill worker handles related prefixes)
    4. Prefill and decode workers are distinct (true disaggregation)
    """
    logger.info(
        f"Starting disaggregated SGLang router test with request_plane={request_plane}"
    )
    ...
    try:
        # Start prefill worker first
        logger.info("Starting SGLang prefill worker")
        prefill_workers = SGLangDisaggProcess(
            request,
            namespace=shared_namespace,
            mode="prefill",
            sglang_args=SGLANG_DISAGG_ARGS,
            num_workers=1,
            request_plane=request_plane,
        )
        prefill_workers.__enter__()

        # Wait for prefill worker to fully initialize before starting decode
        # This prevents memory contention during model loading
        logger.info(
            "Waiting for prefill worker to stabilize before starting decode worker..."
        )
        time.sleep(5)

        # Start decode worker
        logger.info("Starting SGLang decode worker")
        decode_workers = SGLangDisaggProcess(
            request,
            namespace=shared_namespace,
            mode="decode",
            sglang_args=SGLANG_DISAGG_ARGS,
        ...
        # Run disaggregated routing test
        _test_router_decisions_disagg(
        ...
        logger.info("Disaggregated SGLang router test completed successfully")

    finally:
        # Clean up in reverse order (decode first, then prefill)
