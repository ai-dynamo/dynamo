...
def valkey_index_key(
    namespace: str, component: str, index_scope: str, block_size: int
) -> str:
    """Mirror the Rust Valkey index-key encoding for black-box assertions."""

    def pct(value: str) -> str:
        return quote(value, safe="-._~")

    return (
        f"dynamo:kv-router:{pct(namespace)}:component-{pct(component)}:"
        f"scope-{pct(index_scope)}:block-size-{block_size}"
    )
