...
async def _send_cost_policy_request(frontend_port: int) -> dict:
    """Send a single short request and return the parsed response."""
    completions_url = f"http://localhost:{frontend_port}/v1/completions"
    payload = {
        "model": MODEL_NAME,
        "prompt": "cost-policy-probe",
        "max_tokens": 2,
        "stream": False,
        "nvext": {"extra_fields": ["worker_id", "timing"]},
    }
    async with aiohttp.ClientSession() as session:
        async with session.post(completions_url, json=payload) as response:
            ...
            assert (
                response.status == 200
            ), f"Cost policy request failed with status {response.status}: {body}"
            return json.loads(body)
...
@pytest.mark.parametrize("request_plane", ["tcp"], indirect=True)
@pytest.mark.parametrize(
    "transfer_cost_blocks,expect_bypass",
    [
        # transfer_cost=0 + idle pools → LHS == RHS → cost equation says no bypass.
        (0, False),
        # transfer_cost>0 + idle pools → LHS < RHS → cost equation says bypass.
        (10, True),
    ],
    ids=["transfer_cost_zero_no_bypass", "transfer_cost_high_bypass"],
)
def test_conditional_prefill_cost_policy_e2e_with_mockers(
    request,
    runtime_services_dynamic_ports,
    predownload_tokenizers,
    request_plane,
    transfer_cost_blocks,
    expect_bypass,
):
    """E2E: cost-policy conditional prefill consults the cost equation.

    On an idle prefill+decode pool with no decode-side overlap:
        LHS = potential_prefill_block(d_chosen) = prompt_blocks
        RHS = potential_prefill_block(p) + transfer_cost = prompt_blocks + transfer_cost
    So bypass fires iff transfer_cost > 0. We exercise both branches via the
    transfer_cost_blocks knob since prompt length alone doesn't discriminate
    under the cost equation.
    """
    logger.info(
        "Starting conditional prefill COST policy E2E test (transfer_cost=%d, "
        "expect_bypass=%s)",
        transfer_cost_blocks,
        expect_bypass,
    )
    ...
    with _launch_disagg_workers(
        ...
        with FrontendRouterProcess(
            ...
            use_kv_events=False,
            conditional_prefill_enabled=True,
            conditional_prefill_policy="cost",
            conditional_prefill_transfer_cost_blocks=transfer_cost_blocks,
        ):
            frontend_url = f"http://localhost:{frontend_port}"
            asyncio.run(
                wait_for_frontend_ready(
            ...
            response = asyncio.run(_send_cost_policy_request(frontend_port))
            logger.info("Cost policy E2E response: %s", response)

            worker_id = response.get("nvext", {}).get("worker_id", {})
            assert worker_id.get("decode_worker_id") is not None, (
                "Request should route to a decode worker, " f"got worker_id={worker_id}"
            )
            if expect_bypass:
                assert worker_id.get("prefill_worker_id") is None, (
                    "Cost policy with transfer_cost>0 should bypass remote prefill, "
                    f"got worker_id={worker_id}"
                )
            else:
                assert worker_id.get("prefill_worker_id") is not None, (
                    "Cost policy with transfer_cost=0 on idle pool should NOT "
                    f"bypass (LHS == RHS), got worker_id={worker_id}"
                )
