...
async def test_router_returns_valid_dp_rank(request):
    ...
    with MockerProcess(request, namespace, dp_size=DP_SIZE):
        try:
            ...
            endpoint = (
                runtime.namespace(namespace).component("prefill").endpoint("generate")
            )
            ...
            if hasattr(kv_push_router, "best_worker"):
                ...
                assert (
                    0 <= dp_rank < DP_SIZE
                ), f"dp_rank {dp_rank} out of range [0, {DP_SIZE})"
    ...
    with MockerProcess(request, namespace, dp_size=DP_SIZE):
        try:
            ...
            if hasattr(kv_push_router, "best_worker"):
                ...
                logger.info(
                    f"✅ Router coverage: {num_ranks}/{DP_SIZE} ranks used - {sorted(dp_ranks_used)}"
                )
            else:
                logger.warning("Router API doesn't support best_worker - skipping test")
