...
async def test_gpu_xid79_nvsentinel_eviction(
    ...
    async with ManagedDeployment(
        ...
        if InferenceLoadTester:
            ...
            if successful < 3:
                logger.error(
                    f"  ✗ Baseline failed ({successful}/5) - CUDA passthrough broken!"
                )
                pytest.fail(
                    f"Baseline inference failed ({successful}/5) - check CUDA passthrough"
                )
        else:
        ...
        if fault_id:
            ...
            logger.error(
                "  ✗ XID injection failed - fault-injection-api not available!"
            )
            logger.error("    - Check: kubectl get pods -n fault-injection-system")
            logger.error(
                "    - This test REQUIRES XID injection for NVSentinel to detect"
            )
        ...
        if successful >= 4:
            logger.error(
                f"  ✗ CUDA faults not working! ({successful}/5 still succeeded)"
            )
        ...
        if not cordoned:
            ...
            logger.error(
                "    - Verify NVSentinel is deployed with syslog-health-monitor"
            )
            logger.error(
                "    - Check: kubectl logs -n nvsentinel -l app.kubernetes.io/name=syslog-health-monitor"
            )
            pytest.fail(
                "NVSentinel did not cordon node within 30s - check syslog-health-monitor"
            )
        ...
        while asyncio.get_event_loop().time() - start_time < eviction_timeout:
            ...
            logger.info(
                f"  [{int(elapsed)}s] Pods on cordoned node: {current_on_target}"
            )

            if current_on_target == 0:
                evicted = True
                logger.info(
                    f"  ✓ All pods evicted from cordoned node after {int(elapsed)}s"
                )
                break
        ...
        if not evicted:
            ...
            logger.error(
                "    - Check: kubectl logs -n nvsentinel -l app.kubernetes.io/name=node-drainer"
            )
            logger.error(
                "    - Verify deleteAfterTimeoutMinutes=2 in node-drainer config"
            )
        ...
        await deployment.wait_for_pods_on_healthy_nodes(
            exclude_node=target_node, timeout=1000
        )
        ...
        for service_name, service_pods in new_pods.items():
            for pod in service_pods:
                if hasattr(pod, "spec"):
                    ...
                    init_containers = getattr(
                        pod.spec, "init_containers", None
                    ) or getattr(pod.spec, "initContainers", None)
                    if init_containers:
                        for ic in init_containers:
                            ic_name = getattr(ic, "name", "")
                            if "cuda-fault" in ic_name or "compile-cuda" in ic_name:
                                logger.error(
                                    f"  ✗ Pod {pod.metadata.name} still has CUDA init container!"
                                )
                    # Check node is not the cordoned one (use getattr for both naming styles)
                    node_name = getattr(pod.spec, "node_name", None) or getattr(
                        pod.spec, "nodeName", None
                    )
                    if node_name == target_node:
                        logger.error(
                            f"  ✗ Pod {pod.metadata.name} scheduled on cordoned node!"
                        )
        ...
        logger.info("  ✓ CUDA passthrough (toggle-based): PASS")
        logger.info("  ✓ NVSentinel cordoned node: PASS")
        logger.info("  ✓ node-drainer evicted pods (no force delete): PASS")
        logger.info("  ✓ New pods clean (no CUDA lib, healthy node): PASS")
        logger.info("")
