...
# Re-use the SetBusyThreshold event from the load-shedding test —
# it's a small inline helper that POSTs /busy_threshold to the FE.
from tests.fault_tolerance.deploy.test_scenario_load_shedding_maintains_sla import (
    SetBusyThreshold,
)
...
from tests.fault_tolerance.deploy.checks import Check, _iter_server_metric
...
async def test_decode_overload_disagg(runtime_env, request, arm):
    ...
    if arm == "T1":
        spec["VllmDecodeWorker"].set_env_var(
    ...
    if arm == "S":
        ...
        checks = [
            ...
            SlaViolation(
                load_name="sustain", e2e_p99_ms=30000.0, ttft_p99_ms=10000.0
            ),
    ...
    if arm == "S":
        ...
        checks.append(
            _KvCacheStaysBelow(
                services=["VllmDecodeWorker"], ceiling=0.85
            )
        )
    if arm in ("F2", "F3"):
