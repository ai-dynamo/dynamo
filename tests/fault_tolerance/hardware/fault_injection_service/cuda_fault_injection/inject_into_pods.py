...
def _patch_service_for_injection(
    ...
    if enable:
        ...
        print("      ✓ Added LD_PRELOAD and CUDA fault env vars")
    ...
    if use_configmap and enable:
        ...
        service["extraPodSpec"]["volumes"] = [
            ...
            if v.get("name")
            not in ["cuda-fault-lib-source", "cuda-fault-lib", "node-fault-marker"]
        ]
    ...
    if target_node and enable:
        ...
        if soft_affinity:
            ...
            print(
                f"      ✓ Added SOFT node affinity (prefer {target_node}, allow spillover)"
            )
        else:
            ...
            print(
                f"      ✓ Added HARD node affinity (ALL pods MUST be on {target_node})"
            )
...
Examples:
  ...
  python inject_into_pods.py --deployment vllm-v1-disagg-router --namespace dynamo-oviya --verify
    ...
    if args.node:
        ...
        affinity_mode = (
            "HARD (required)"
            if args.hard_affinity
            else "SOFT (preferred, allows spillover)"
        )
