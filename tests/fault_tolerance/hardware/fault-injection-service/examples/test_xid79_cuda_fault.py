...
class CUDAFaultInjector:
    ...
    def build_library(self) -> bool:
        ...
        result = subprocess.run(
            ["make"], cwd=CUDA_FAULT_LIB_DIR, capture_output=True, text=True
        )
    ...
    def create_configmap_with_library(self, namespace: str) -> bool:
        ...
        cuda_injection_dir = os.path.join(
            os.path.dirname(__file__), "..", "cuda-fault-injection"
        )
    ...
    def patch_deployment_for_cuda_fault(
        self, deployment_name: str, namespace: str, target_node: str = None
    ) -> bool:
        ...
        try:
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    if not has_artifacts:
                        ...
                        print(
                            f"    ... {(attempt+1)*5}s: CUDA fault artifacts still in spec: {', '.join(artifact_details)}"
                        )
            ...
            if force_delete_pods:
                ...
                try:
                    ...
                    if deleted_count > 0:
                        print(
                            f"    ✓ Deleted {deleted_count} pod(s) - will restart with clean spec"
                        )
                    else:
    ...
    def wait_for_crash_loop_backoff(
        self, deployment_name: str, namespace: str, node_name: str, timeout: int = 120
    ) -> bool:
    ...
    def cleanup(self, deployment_name: str, namespace: str):
        ...
        try:
            ...
            k8s_apps.patch_namespaced_deployment(deployment_name, namespace, deployment)

            print("    ✓ Removed CUDA fault injection from deployment")
...
class InferenceLoadTester:
    ...
    def send_inference_request(
        self, prompt: str = "Hello, world!", timeout: int = INFERENCE_TIMEOUT
    ) -> Dict:
...
def restart_gpu_driver_on_node(node_name: str, wait_timeout: int = 300) -> bool:
    ...
    try:
        ...
        old_pod = k8s_core.read_namespaced_pod(
...
def cleanup_on_exit():
    ...
    try:
        ...
        try:
            ...
            if cordoned_nodes:
                print(
                    f"    Found {len(cordoned_nodes)} test-cordoned node(s): {', '.join(cordoned_nodes)}"
                )
                for node_name in cordoned_nodes:
                    ...
                    try:
                        ...
                        if node_name == cleanup_state.get("target_node"):
                            restore_cordon = cleanup_state.get(
                                "node_was_initially_cordoned", False
                            )
                ...
                print("[✓] All test-cordoned nodes processed")
            else:
            ...
            if cleanup_state["target_node"]:
                print(
                    f"[→] Fallback: Uncordoning target node: {cleanup_state['target_node']}"
                )
                try:
                    k8s_core.patch_node(
                        ...
                        {
                            "spec": {
                                "unschedulable": cleanup_state[
                                    "node_was_initially_cordoned"
                                ]
                            },
                            "metadata": {
                                "labels": {
...
def test_xid79_with_cuda_fault_injection(cleanup_on_exit):
    ...
    try:
        ...
        print(
            "  [MANUAL]    Node cordoning (NVSentinel fault-quarantine-module in production)"
        )
        print(
            "  [MANUAL]    Node draining (NVSentinel node-drainer-module in production)"
        )
        ...
        if not pods.items:
            pytest.fail(
                f"[FAILED] No worker pods found for deployment: {TARGET_DEPLOYMENT}"
            )

        target_node = pods.items[0].spec.node_name
        cleanup_on_exit["target_node"] = target_node  # Register for cleanup
        baseline_pod_count = len(
            [p for p in pods.items if p.spec.node_name == target_node]
        )
        ...
        try:
            ...
            # Check decode worker
            decode_worker = (
                dgd.get("spec", {}).get("services", {}).get("VllmDecodeWorker", {})
            )
            decode_env = (
                decode_worker.get("extraPodSpec", {})
                .get("mainContainer", {})
                .get("env", [])
            )
        ...
        cleanup_on_exit["node_was_initially_cordoned"] = (
            node_was_initially_cordoned  # Register for cleanup
        )

        if node_was_initially_cordoned:
            print(
                f"[⚠] Target node {target_node} is already cordoned - uncordoning for test"
            )
        ...
        if not baseline_result["success"]:
            ...
            print("    Continuing test, but inference monitoring may not work")
        else:
        ...
        print(
            f"[✓] Baseline load started: {initial_stats['success']}/{initial_stats['total']} requests successful"
        )
        ...
        if response.status_code != 200:
            pytest.fail(
                f"[FAILED] Injection failed ({response.status_code}): {response.text}"
            )
        ...
        if post_xid_result["success"]:
            print("[✓] Inference still working - GPU redundancy effective")
            print("    (Other workers on healthy nodes handling requests)")
        else:
            print(f"[⚠] Inference failing after XID: {post_xid_result['error'][:80]}")
        print(
            f"    Inference success rate so far: {post_xid_stats['success_rate']:.1f}%"
        )
        ...
        if not cuda_injector.patch_deployment_for_cuda_fault(
            TARGET_DEPLOYMENT, NAMESPACE, target_node=target_node
        ):
        ...
        for i in range(14):  # 14 iterations * 30s = 7 minutes max
            ...
            for pod in check_pods.items:
                ...
                if pod.status.container_statuses:
                    ...
                    pod_status_details.append(
                        f"{pod_name}: {phase} (no container status)"
                    )

            print(
                f"    ... {elapsed}s: {crashed_count}/{len(check_pods.items)} crashed, {running_count} running, {pending_count} pending"
            )
            if len(check_pods.items) == 0:
                print(
                    f"        ⚠ No pods found on {target_node} - node affinity may not be working!"
                )
            else:
        ...
        print(
            f"    Success rate during crash phase: {crash_stats['success_rate']:.1f}%"
        )
        print("    ⚠ CUDA errors causing inference failures (expected)")
        ...
        print("    (In production: no LD_PRELOAD to remove - pods crash naturally)")
        print("    Removing test artifact so new pods will start clean")
        ...
        print(
            "    CRITICAL: Must delete ALL pods (including Pending) with old node affinity"
        )
        ...
        print(
            "    In production: NVSentinel node-drainer-module would drain at this point"
        )
        print(
            "    Pods will reschedule to healthy nodes with clean spec (no node affinity)"
        )
        ...
        driver_restart_success = restart_gpu_driver_on_node(
            target_node, wait_timeout=300
        )
        ...
        for i in range(30):  # 30 iterations * 30s = 15 minutes max
            ...
            ready_count = len(
                [
                    p
                    for p in check_pods.items
                    if p.status.phase == "Running"
                    and p.status.container_statuses
                    and p.status.container_statuses[0].ready
                    and p.spec.node_name != target_node
                ]
            )  # Must be on healthy node

            total_count = len(check_pods.items)
            on_healthy_nodes = len(
                [p for p in check_pods.items if p.spec.node_name != target_node]
            )

            print(
                f"    ... {elapsed}s: {ready_count}/{total_count} ready, {on_healthy_nodes}/{total_count} on healthy nodes"
            )
        ...
        if not all_ready:
            print("[⚠] Not all pods ready after 15 minutes, continuing anyway...")
        ...
        if pods_on_healthy_nodes >= 3:
            print("[✓] All pods successfully rescheduled to healthy nodes")
        else:
            print(
                f"[⚠] Expected 3 pods on healthy nodes, found {pods_on_healthy_nodes}"
            )

        # Show inference stats during rescheduling
        resched_stats = load_tester.get_stats()
        print(
            f"    Inference success rate during rescheduling: {resched_stats['success_rate']:.1f}%"
        )
        ...
        print(
            f"  ✓ GPU driver restart: {'succeeded' if driver_restart_success else 'failed'}"
        )
        ...
        print(
            "  • Node cordoned (fault-quarantine-module/pkg/reconciler/node_quarantine.go:274)"
        )
        print(
            "  • Pods drained (node-drainer-module/pkg/informers/informers.go:471-535)"
        )
        print(
            "  • GPU driver restarted (fault-remediation-module/pkg/reconciler/reconciler.go:204)"
        )
        print(
            "  • Node uncordoned (fault-quarantine-module/pkg/reconciler/reconciler.go:840-844)"
        )
