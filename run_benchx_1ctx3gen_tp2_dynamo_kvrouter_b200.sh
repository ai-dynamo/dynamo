#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.1ctx3gen_tp2.dynamo.kvrouter
#SBATCH --nodes=1
...
#SBATCH --output=bench/logs/run_benchx_1ctx3gen_tp2_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_1ctx3gen_tp2_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 1 ctx TP1 + 3 gen TP2 with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 1 ctx worker(s) (GPUs 0-0) TP1
#   NODE0 — gen worker 0 GPUs 1,2 (TP2)
#   NODE0 — gen worker 1 GPUs 3,4 (TP2)
#   NODE0 — gen worker 2 GPUs 5,6 (TP2)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_1ctx3gen_tp2_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_1ctx3gen_tp2_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_1ctx3gen_tp2_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
GEN_PIDS=()
# --- decode worker 0 on $NODE0 GPUs 1,2 (TP2) ---
GEN_PORT=8085
echo "[$(date +%H:%M:%S)] Starting gen worker 0 on $NODE0 GPUs 1,2 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
  --output="$RESULTS_DIR/gen_worker_g0.log" \
  --container-image="$CONTAINER_IMAGE" --container-mounts="$CONTAINER_MOUNTS" \
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=1,2 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
...
# --- decode worker 1 on $NODE0 GPUs 3,4 (TP2) ---
GEN_PORT=8086
echo "[$(date +%H:%M:%S)] Starting gen worker 1 on $NODE0 GPUs 3,4 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
  --output="$RESULTS_DIR/gen_worker_g1.log" \
  --container-image="$CONTAINER_IMAGE" --container-mounts="$CONTAINER_MOUNTS" \
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=3,4 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
    export DYN_SYSTEM_PORT=${GEN_PORT} && \
    trtllm-llmapi-launch python3 -m dynamo.trtllm \
      --model-path $MODEL_PATH --served-model-name $MODEL \
      --disaggregation-mode decode \
...
# --- decode worker 2 on $NODE0 GPUs 5,6 (TP2) ---
GEN_PORT=8087
echo "[$(date +%H:%M:%S)] Starting gen worker 2 on $NODE0 GPUs 5,6 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
  --output="$RESULTS_DIR/gen_worker_g2.log" \
  --container-image="$CONTAINER_IMAGE" --container-mounts="$CONTAINER_MOUNTS" \
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=5,6 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
    export DYN_SYSTEM_PORT=${GEN_PORT} && \
    trtllm-llmapi-launch python3 -m dynamo.trtllm \
      --model-path $MODEL_PATH --served-model-name $MODEL \
      --disaggregation-mode decode \
...
# --- 1 ctx worker(s) on $NODE0 GPUs 0-0 (prefill, TP1) ---
CTX_PIDS=()
for GPU in 0; do
