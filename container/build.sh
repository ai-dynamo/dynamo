...
# If build.sh is invoked for dev/local-dev (or no --target, dev implied), delegate to dev_build.sh.
# IMPORTANT: This happens *after* BUILD_ARGS has been fully constructed so dev_build.sh can
# pass through all --build-arg flags generically.
if [[ -z "${TARGET:-}" || "${TARGET:-}" == "dev" || "${TARGET:-}" == "local-dev" ]]; then
    export DEV_BUILD_DEFAULT_TAG="${TAG#--tag }"
    export DEV_BUILD_DOCKER_BUILD_ARGS="${BUILD_ARGS}"
    export DEV_BUILD_DOCKER_BUILD_CONTEXT_ARGS="${BUILD_CONTEXT_ARG:-}"
    export DEV_BUILD_DOCKERFILE="${DOCKERFILE}"
    exec "${SOURCE_DIR}/dev/dev_build.sh" "$@"
fi
