...
if [[ -z "${DEV_IMAGE_INPUT:-}" ]]; then
    ...
    if [[ $FRAMEWORK != "NONE" ]]; then
        ...
        echo "======================================"
        $RUN_PREFIX docker buildx build --load -f "${SOURCE_DIR}/Dockerfile" --target dev $PLATFORM $BUILD_ARGS $DYNAMO_BASE_CACHE_FROM $DYNAMO_BASE_CACHE_TO --tag $DYNAMO_BASE_IMAGE $BUILD_CONTEXT_ARG $BUILD_CONTEXT $NO_CACHE
        # Start framework build
        echo "======================================"
        echo "Starting Build 2: Framework Image"
        echo "======================================"
        BUILD_ARGS+=" --build-arg DYNAMO_BASE_IMAGE=${DYNAMO_BASE_IMAGE}"
        $RUN_PREFIX docker buildx build --load -f $DOCKERFILE $TARGET_STR $PLATFORM $BUILD_ARGS $FRAMEWORK_CACHE_FROM $FRAMEWORK_CACHE_TO $TAG $LATEST_TAG $BUILD_CONTEXT_ARG $BUILD_CONTEXT $NO_CACHE
    else
        $RUN_PREFIX docker buildx build --load -f $DOCKERFILE $TARGET_STR $PLATFORM $BUILD_ARGS $CACHE_FROM $CACHE_TO $TAG $LATEST_TAG $BUILD_CONTEXT_ARG $BUILD_CONTEXT $NO_CACHE
    fi
fi
