...
# upstream omni image does. git is only pulled in when VLLM_OMNI_GIT_URL is set
# (installing vllm-omni from an unreleased git ref) — the upstream vllm runtime
# image does not ship git.
#
...
# above) points imageio at the LGPL CLI copied from wheel_builder, so the omni
# video-export path encodes through that instead of a bundled GPL binary. The
