...
#######################################
#
# Collects source archives for vllm's third-party deps on top of the
# vllm-runtime baseline. Same shape as trtllm_runtime / dynamo_runtime.
# Gated on ENABLE_SOURCE_ARCHIVAL — default off so PR builds stay fast;
# CI flips it on for push/release/workflow_dispatch (see
