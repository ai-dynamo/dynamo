...
RUN --mount=type=secret,id=aws-web-identity-token,target=/run/secrets/aws-token \
    ...
    if [ "$ENABLE_MEDIA_FFMPEG" = "true" ]; then \
        maturin build --release --features "media-ffmpeg,kv-indexer,slot-tracker,lightseek-mm,aic-forward-pass" --out /opt/dynamo/dist; \
    else \
        maturin build --release --features "kv-indexer,slot-tracker,lightseek-mm,aic-forward-pass" --out /opt/dynamo/dist; \
    fi && \
