...
RUN --mount=type=secret,id=aws-web-identity-token,target=/run/secrets/aws-token \
    ...
    if [ "$ENABLE_MEDIA_FFMPEG" = "true" ]; then \
        maturin build --release --features "media-ffmpeg,kv-indexer,lightseek-mm" --auditwheel skip --out "${RUNTIME_WHEEL_DIR}"; \
    else \
        maturin build --release --features "kv-indexer,lightseek-mm" --auditwheel skip --out "${RUNTIME_WHEEL_DIR}"; \
    fi && \
    ARCH_ALT=$([ "${TARGETARCH}" = "amd64" ] && echo "x86_64" || echo "aarch64") && \
    auditwheel repair \
        --exclude libnixl.so \
