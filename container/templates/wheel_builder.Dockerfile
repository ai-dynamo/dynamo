...
{% if framework == "sglang" %}    maturin build --release --features "kv-indexer,slot-tracker,select-service,mm-routing,aic-forward-pass,request-trace-s3,nvtx" --out /opt/dynamo/dist && \
...
{% endif %}
        maturin build --release --features "media-ffmpeg,kv-indexer,slot-tracker,select-service,mm-routing,aic-forward-pass,request-trace-s3,nvtx" --auditwheel skip --out target/wheels && \
        auditwheel repair \
        ...
        maturin build --release --features "kv-indexer,slot-tracker,select-service,mm-routing,aic-forward-pass,request-trace-s3,nvtx" --out /opt/dynamo/dist; \
    fi && \
