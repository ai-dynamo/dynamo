...
RUN --mount=type=secret,id=aws-web-identity-token,target=/run/secrets/aws-token \
    ...
    if [ "$ENABLE_MEDIA_FFMPEG" = "true" ]; then \
        maturin build --release --features "media-ffmpeg,kv-indexer,slot-tracker,select-service,mm-routing,aic-forward-pass,mocker-kvbm-offload" --out /opt/dynamo/dist; \
    else \
        maturin build --release --features "kv-indexer,slot-tracker,select-service,mm-routing,aic-forward-pass,mocker-kvbm-offload" --out /opt/dynamo/dist; \
    fi && \
