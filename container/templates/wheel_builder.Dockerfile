...
RUN --mount=type=secret,id=aws-key-id,env=AWS_ACCESS_KEY_ID \
    ...
    if [ "$ENABLE_KVBM" == "true" ]; then \
        ...
        if [ "$ENABLE_KVBM_NCCL" = "true" ]; then \
            KVBM_FEATURES="nccl"; \
            echo "Building KVBM with NCCL support for replicated mode"; \
        fi; \
        if [ -n "$KVBM_FEATURES" ]; then \
            maturin build --release --features "$KVBM_FEATURES" --out target/wheels; \
        else \
