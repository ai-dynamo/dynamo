...
from aisimulate.spica.replay import RunnerCapabilities
...
def test_runner_incompatible_backend_is_removed_before_perf_lookup(monkeypatch):
    ...
    monkeypatch.setattr(
        "aisimulate.spica.search_space.parallel_configs_for", fake_parallel_configs
    )
    config = _config(
...
def test_all_modes_infeasible_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        "aisimulate.spica.search_space.parallel_configs_for", always_raise
    )
    config = _config(
        deployment_mode=["agg", "disagg"],
        backend=["trtllm"],
