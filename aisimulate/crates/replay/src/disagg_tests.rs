...
use super::*;
use crate::components::{AdmissionQueue, NoReplayMetadata, ReplayEngineObservation};
use crate::core::EngineEventBatch;
use crate::core::round_robin::PoolRoundRobinPlacement;
use crate::engine::{ReplayEngineConfig, ReplayEngineFactory, ReplayRoleConfig};
...
impl TestDisaggConfig {
    fn runtime_config(&self, emit_kv_events: bool) -> anyhow::Result<OfflineDisaggReplayConfig> {
        ...
        let prefill_factory =
            factory.role_factory(&engine, crate::WorkerStage::Prefill, emit_kv_events)?;
        let decode_factory =
            factory.role_factory(&engine, crate::WorkerStage::Decode, emit_kv_events)?;
...
fn test_cancellation_during_transfer_ignores_retired_completion_event() {
    for mode in [
        ...
        assert!(runtime.events.iter().all(|event| !matches!(
            &event.kind,
            crate::events::SimulationEventKind::TransferComplete { .. }
        )));
