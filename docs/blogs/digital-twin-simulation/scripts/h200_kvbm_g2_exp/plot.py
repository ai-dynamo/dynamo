...
"""
Plot the H200 KVBM G2 host-memory cache experiment.

Reads data.csv (long format: one row per (source, concurrency) with sources
`baseline` and `g2`) and emits h200_kvbm_g2_exp.png to ../../images/. The
figure has two panels:
...
def main():
    ...
    ap.add_argument(
        "--out",
        default=os.path.join(HERE, "..", "..", "images", "h200_kvbm_g2_exp.png"),
    )
    args = ap.parse_args()
    ...
    for c, y, r in zip(concs, g2_ttft, ttft_red):
        label = f"-{r:.1f}%" if r >= 0 else f"+{-r:.1f}%"
        ax_ttft.annotate(
            label,
            (c, y),
            ...
            xytext=(10, -4),
            fontsize=10,
            color=NV_GREEN if r >= 0 else WARN,
            fontweight="bold",
        )
    ...
    x_pad = max(2.0, (max(all_x) - min(all_x)) * 0.12)
    y_pad = max(5.0, (max(all_y) - min(all_y)) * 0.12)
    ax_pareto.set_xlim(max(0, min(all_x) - x_pad), max(all_x) + x_pad)
    ax_pareto.set_ylim(min(all_y) - y_pad, max(all_y) + y_pad)
    apply_nv_style(ax_pareto)
    ax_pareto.legend(fontsize=10, loc="upper right", frameon=False)

    add_title(
        fig,
        "H200 KVBM G2 Host-Memory Cache Tier",
        sub_subtitle="H200 Kimi-K2.5   TP=4   1 worker   Mooncake trace   vLLM 0.19.0   offline replay",
    )
