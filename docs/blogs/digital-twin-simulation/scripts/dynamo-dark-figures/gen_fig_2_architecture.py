...
def arrow_down(
    x1: float, y1: float, x2: float, y2: float, color: str, label: str | None = None
) -> str:
...
def build_svg() -> str:
    ...
    # Router hexagon.
    parts.append(
        hexagon(ROUTER_CX, INNER_Y, 46, fill="#151515", stroke=FLUORITE, stroke_w=2)
    )
    ...
    parts.append(
        cylinder(
            AXIS_X, COLL_CY, COLL_W, COLL_H, fill=NEUTRAL_FILL, stroke=NEUTRAL_STROKE
        )
    )
