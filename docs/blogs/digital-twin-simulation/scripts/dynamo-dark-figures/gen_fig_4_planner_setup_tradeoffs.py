...
def _planner_point(fig, row, col, y_value):
    fig.add_trace(
        go.Scatter(
            x=[PLANNER_SLA["gpu_hours"]],
            y=[y_value],
            mode="markers",
            name="Planner (SLA target)",
            legendgroup="planner",
            showlegend=(row == 1 and col == 1),
            marker=dict(
                symbol="diamond",
...
def main() -> None:
    ...
    fig.update_yaxes(
        title=dict(
            text="p90 ITL (ms)", font=dict(family=SANS, size=11, color=TEXT_MUTED)
        ),
