...
LRU sees only recency. In a high traffic environment, a wait for the completion of a called tool (2-30 seconds while the agent waits for an external API) might cause the agent's blocks to age out and when the agent resumes, the entire prefix must be recomputed.

![Without cache control, KV blocks are evicted during tool calls, forcing a full re-prefill on the next turn.](./tool-call-eviction.svg)

With retention APIs, the harness can pin blocks through the tool call gap, eliminating the redundant prefill entirely:

![With the retention API, cache is pinned through the tool call and decode resumes without re-prefill.](./tool-call-pinned.svg)

To solve this, we need to provide the harness a granular API to control which blocks should be retained, where they should live, and for how long.
...
Multi-tier storage solves sharing and persistence, but blocks still arrive on GPU only after the request hits the worker. The missing piece for agentic systems is prefetch. The harness has two useful signals: first, it can observe tool call completion and fire a prefetch signal the moment the response arrives, before it even begins composing the next LLM request. That head start -- even a few hundred milliseconds -- is enough to begin moving blocks from storage to GPU. Second, the harness can track historical latency distributions per tool type (local file reads return in ~100ms, external API calls in ~2s) and issue speculative prefetches before the tool call completes. We are building prefetch hooks so the harness can signal "bring these blocks from storage to GPU ahead of the next request." Combined with the retention APIs (below), this gives the harness full lifecycle control: pin blocks to prevent eviction, set priority to control eviction ordering, and prefetch blocks proactively before they are needed.
