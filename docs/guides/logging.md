...
| `POD_NAME`                         | Service name for OpenTelemetry traces (default: `dynamo`) | `POD_NAME=dynamo-frontend-abc123` |
| `OTEL_TRACES_EXPORTER`             | Enable OTLP trace exporting (set to `otlp` to enable) | `OTEL_TRACES_EXPORTER=otlp` |
| `OTEL_EXPORTER_OTLP_ENDPOINT`      | OTLP exporter endpoint (default: http://localhost:4317) | `OTEL_EXPORTER_OTLP_ENDPOINT=http://tempo:4317` |
...
- **With `OTEL_TRACES_EXPORTER=otlp`**: Same as above, plus traces are exported to an OTLP collector for visualization.

### Configuration

To enable OTLP trace exporting:

1. Set `OTEL_TRACES_EXPORTER=otlp` to enable trace export
2. Optionally configure the endpoint using `OTEL_EXPORTER_OTLP_ENDPOINT` (default: `http://localhost:4317`)
3. Optionally set `POD_NAME` to identify the service (useful in Kubernetes)

**Export Settings:**
- **Protocol**: gRPC (Tonic)
- **Service Name**: Value of `POD_NAME` env var, or `dynamo` if not set
