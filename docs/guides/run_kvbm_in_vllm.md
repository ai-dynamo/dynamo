...
# start vllm with DYN_SYSTEM_ENABLED set to true and DYN_SYSTEM_PORT port to 9998.
# NOTE: Make sure port 9998 (for KVBM worker metrics) and port 9999 (for KVBM leader metrics) are available.
DYN_SYSTEM_ENABLED=true DYN_SYSTEM_PORT=9998 vllm serve --kv-transfer-config '{"kv_connector":"DynamoConnector","kv_role":"kv_both", "kv_connector_module_path": "dynamo.llm.vllm_integration.connector"}' deepseek-ai/DeepSeek-R1-Distill-Llama-8B
