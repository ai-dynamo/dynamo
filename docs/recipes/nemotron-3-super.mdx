...
All four targets run aggregated vLLM 0.21.0 (runtime image `vllm-runtime:1.3.1-nemotron-super-dev.1`) with two TP4 + EP worker replicas, MTP speculative decoding (DL=3), and KV-aware routing. They differ in checkpoint, kernel backends, and the trace they are benchmarked against:
...
- This is a Day-0 recipe on a dedicated dev runtime image (`vllm-runtime:1.3.1-nemotron-super-dev.1`); it is functional and benchmarked but not yet promoted to a release runtime image.
