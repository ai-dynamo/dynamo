...
- An NGC image pull secret named `nvcr-secret` for `nvcr.io/nvidia/ai-dynamo/vllm-runtime:1.3.1-nemotron-ultra-dev.1`.
...
All targets serve `nvidia/NVIDIA-Nemotron-3-Ultra-550B-A55B-NVFP4` on the dedicated dev runtime image `vllm-runtime:1.3.1-nemotron-ultra-dev.1` with KV-aware routing and a 262144 max model length:
...
- This is a Day-0 recipe on a dedicated dev runtime image (`vllm-runtime:1.3.1-nemotron-ultra-dev.1`); it is functional and benchmarked but not yet promoted to a release runtime image.
