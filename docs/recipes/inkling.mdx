...
Audio rides as an `audio_url` content part — a public HTTP(S) URL (the worker pod fetches it, so it needs egress) or a base64 `data:` URI. Inkling expects 16 kHz audio; WAV is ideal, and MP3/FLAC/OGG also decode in this image. This sample is 16 kHz mono WAV, matching the model card spec:
...
For air-gapped clusters, send local files as `data:` URIs instead — e.g. `{"type": "audio_url", "audio_url": {"url": "data:audio/wav;base64,'$(base64 -i sample.wav)'"}}` — and mix multiple media parts in one message as the context budget allows.
