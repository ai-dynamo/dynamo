...
| draft | Cluster-specific networking and fabric setup | DeepSeek V4 Pro, DeepSeek-R1, GLM-5 | GLM-5 now has ComputeDomain, NIXL/UCX, startup, and recovery notes; DeepSeek V4 Pro and DeepSeek-R1 still need homes. |
...
| draft | `glm-5-nvfp4.mdx` | `recipes/glm-5-nvfp4/README.md` | SGLang disagg | Topology, published runtime image, EAGLE MTP, KV cache, NIXL/UCX, recovery/rollouts, 1K/8K benchmark result, deploy/verify/benchmark/operate commands | Generic storage/HF setup | Visually checked; still needs owner review of production rollout caveat. |
| draft | `gpt-oss-120b.mdx` | `recipes/gpt-oss-120b/README.md`, `recipes/gpt-oss-120b/trtllm/disagg/README.md` | TRT-LLM agg, TRT-LLM disagg | Available configs, P/D topology, engine config differences, KV transfer, quantization, benchmark commands, deployment-vs-benchmark GPU-count caveat | Repeated top-level quickstart | Needs owner review of disagg `DEPLOYMENT_GPU_COUNT=6` vs 5-GPU deploy topology. |
...
| draft | GLM-5 NVFP4 SGLang disagg | `recipes/glm-5-nvfp4/sglang/disagg` | Yes | Yes | Represented with read-only configuration, deploy, verify, benchmark evidence, operate, and recovery notes. |
| draft | GPT-OSS-120B TRT-LLM agg | `recipes/gpt-oss-120b/trtllm/agg` | Yes | Yes | Represented with selector, deploy, verify, benchmark shape, artifacts, and config notes. |
| draft | GPT-OSS-120B TRT-LLM disagg | `recipes/gpt-oss-120b/trtllm/disagg` | Yes | Yes | Represented with selector, deploy, verify, benchmark shape, KV transfer, quantization, and GPU-count caveat. |
