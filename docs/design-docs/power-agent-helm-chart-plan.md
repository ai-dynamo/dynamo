...
| v1.5 | 2026-06-23 | CI/build follow-ups (PR #9682 review by @nv-anants): (1) bumped `deploy/power-agent/Dockerfile` base from `python:3.11-slim-bookworm` → `python:3.12-slim-bookworm` (repo default). (2) Mirrored both test jobs (`power-agent-test` + `power-agent-helm-tests`) from `pr.yaml` into `post-merge-ci.yml` so post-merge runs the same gates as PR CI, not just the image build. (3) Reverted all `release.yml` changes (NGC image copy + standalone chart `helm cm-push` + summary lines) — publishing the agent image/chart as a public release artifact is deferred to a separate process coordinated with @dagil-nvidia, so this PR no longer touches `release.yml`. (4) **Unit tests now run *inside the container image*** rather than a pip-installed runner: the Dockerfile gained a `test` stage (`FROM runtime` + pytest + tests, `RUN python -m pytest tests -v`) so the suite validates the exact shipped image (Python 3.12 + baked `pynvml`/`kubernetes`/`prometheus-client`); the build/push job now passes `target: runtime` so only the slim stage is published. (5) Added a standalone `deploy/power-agent/pytest.ini` (with `pythonpath = .`) so the relocated unit is its own pytest rootdir — both the in-image test stage and a bare local `pytest deploy/power-agent/tests/` resolve `from power_agent import …` without inheriting the monorepo-root `pyproject.toml` `filterwarnings = ["error", …]` (TRTLLM/pydantic filters that fail to import in the minimal env). (6) Renamed both test jobs to `name: Power Agent` (matching the build job) so all three group under one umbrella in the checks UI. Verified `57 passed` on an isolated CPython 3.12 with exactly the image's deps (docker not available locally; the `docker build --target test` path is exercised by CI). |
