...
class AICPowerOptimizer:
    ...
    def optimize(self) -> PowerAwareConfig:
        """AIC sweep — first call per process ~3-6s (DB load), subsequent
        calls ~10-15 ms (cached). Always run via asyncio.to_thread.
        See aic_power_optimizer.py:optimize() docstring for measured numbers."""
...
| 14 | AIC `power_w` non-physical (extrapolation artefact) | Inside `optimize()` after `estimate_perf` | AIC's per-kernel 3D-cubic interpolator extrapolates out-of-grid queries and can return values above measured peak. Empirically observed on `h200_sxm` + vLLM 0.19.1 + `generation_attention` at `batch >= 256` (aggregate `power_w` returned by `estimate_perf` was ~1275 W vs 700 W TDP, 721 W measured peak). The optimizer clamps the value used to derive caps at TDP × 1.1; the **raw** value is preserved on `PowerAwareConfig.aic_power_w_*` so `update_correction()` sees the true mismatch and `c_power_*` coefficients converge below 1.0 instead of being silently masked. Cap clamping is independent of the EMA peg signal (#6), so #14 + #6 firing together is a strong "AIC data needs backfill" signal. | WARNING per clamp; `dynamo_aic_power_w_clamped_total{side=...}` |
...
| 12 | **Hysteresis: `aic_drift_consecutive_ticks` (default 3)** | Suppresses transient p95 spikes from cache misses, GC pauses, model warm-up — which would otherwise cause spurious sweeps. EMA smooths coefficients; hysteresis controls when to re-sweep. Note: the actual sweep CPU cost is ~10-15 ms once the perf database is cached (cold-start is 3-6 s); the real cost of a spurious sweep is downstream — a fresh annotation PATCH on every worker pod, a frontend `/busy_threshold` re-POST fanout, and a momentary config flap that the EMA must re-stabilise around. |
