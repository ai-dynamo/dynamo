...
| File | Lines | Change |
|------|-------|--------|
| `planner_core.py` | 276, 759, 772, 926, 951 | `scaling_mode != NOOP` for init; `scaling_mode == ACTIVE` for execute |
| `disagg_planner.py` | 64, 85, 159, 244 | Same pattern |
| `agg_planner.py` | 86, 107, 325 | Same pattern |
...
| `components/src/dynamo/planner/utils/planner_config.py` | Add new fields to `PlannerConfig`. Add backward compat validation. Add file output validation |
| `components/src/dynamo/planner/utils/planner_core.py` | Add 15 metrics to `PlannerPrometheusMetrics`. Add `_emit_advisory_metrics()`, `_estimate_sla_with_replicas()`, `_build_path_recommendation()`, `_safe_gauge_set()`, `_write_advisory_jsonl()` to `BasePlanner`. Replace `no_operation` checks with `scaling_mode` branches. Add `/advisory/status` HTTP handler |
| `components/src/dynamo/planner/utils/disagg_planner.py` | Replace 4 `no_operation` checks. Add advisory emission in `_throughput_loop` and `_load_loop` after GPU budget application |
| `components/src/dynamo/planner/utils/agg_planner.py` | Replace 3 `no_operation` checks. Add advisory emission in `_load_loop` |
| `components/src/dynamo/planner/utils/prefill_planner.py` | No structural changes. Advisory metrics emitted from parent `DisaggPlanner` |
| `components/src/dynamo/planner/utils/decode_planner.py` | No structural changes. Advisory metrics emitted from parent `DisaggPlanner` |
