...
`fastokens` is the default tokenizer backend in this Dynamo fork for BPE `tokenizer.json` models. It uses the Rust encoder from the [`fastokens` GitHub repository](https://github.com/crusoecloud/fastokens) for text-to-token-ID conversion while Dynamo continues to use HuggingFace `tokenizers` for decoding and streaming output.
...
Opt out to the HuggingFace backend with `--tokenizer default` if:
...
`fastokens` is enabled by default. You can also set it explicitly with either the CLI flag or the environment variable. The CLI flag takes precedence.
...
To use the HuggingFace tokenizer backend, set `DYN_TOKENIZER=default` or pass `--tokenizer default`.
...
|---|---|---|---|
| `--tokenizer` | `DYN_TOKENIZER` | `default`, `fastokens` | `fastokens` |
...
Check the frontend startup logs after startup.
...
**I expect `fastokens`, but the logs do not show `Using fastokens tokenizer backend`.**
