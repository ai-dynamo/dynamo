...
| `DYN_MM_ENABLE_LIBJPEG` | `1` | Enable libjpeg-turbo for frontend JPEG decoding. Set to `0` to force Rust `image::ImageReader`. Dynamo backend runtime images include `libturbojpeg`; custom images must provide `libturbojpeg.so.0` or Dynamo logs a one-time warning and falls back to `image::ImageReader`. |
