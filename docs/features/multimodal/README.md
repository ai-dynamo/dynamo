...
| `DYN_MM_IMAGE_DECODER_BACKEND` | *(empty)* | Optional frontend image decoder backend. Leave unset for the default Rust `image::ImageReader`; set to `libjpeg_turbo` to opt into libjpeg-turbo for JPEG inputs. Dynamo runtime images include `libturbojpeg`; custom images must provide `libturbojpeg.so.0`. |
