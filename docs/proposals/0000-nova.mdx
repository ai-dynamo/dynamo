...
{/*
  ...
  Swap for a real in-repo DEP (or delete this page) before the Proposals tab
...
> [!WARNING]
> **Status: Draft — Demo Content.** This page mirrors an in-flight DEP from the sibling [`ai-dynamo/enhancements`](https://github.com/ai-dynamo/enhancements/pull/61) repository (PR #61 by [@ryanolson](https://github.com/ryanolson) on branch `ryan/nova`) so the Proposals tab can render real design prose against a real GitHub review thread. It is illustrative only — real Dynamo DEPs live under `docs/proposals/` in this repository. The line-level comments below and the "Revision discussion" section further down are the actual design debate on the enhancements PR.
