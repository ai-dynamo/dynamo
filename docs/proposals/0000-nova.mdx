...
subtitle: "Demo mirror of ai-dynamo/enhancements#61 — a transport-agnostic active messaging layer for Dynamo."
# GitHub sources are CROSS-REPO for this demo: the mirrored PR lives in
# ai-dynamo/enhancements, not ai-dynamo/dynamo. The runtime reads
# owner/repo/pr/path from the <PrInlineComments> mount div at the bottom
# of this page. Keep this front-matter and the mount div in sync.
pr: 61
tracking-issue:
---

{/*
  DEMO CONTENT — mirrors an IN-FLIGHT DEP from the sibling
  ai-dynamo/enhancements repository (PR #61, "Nova: Active Messaging as a
  Foundational Network Primitive", by @ryanolson on branch ryan/nova) so the
  Proposals tab can render real design prose against real GitHub review
  discussion.

  Body text below is preserved verbatim from
  https://github.com/ai-dynamo/enhancements/blob/ryan/nova/deps/0000-nova.md
  (only two changes: the source's # H1 title is removed because Fern renders
  the front-matter title as H1, and headings are demoted one level so the
  page hierarchy nests under the Fern H1). Do NOT paraphrase headings or
  body text — the inline PR review comments below anchor to those exact
  spans via TextQuoteSelector and will drift otherwise.

  Swap for a real DEP (or delete this page) before the Proposals tab
  is announced. This page is illustrative infrastructure only.
*/}

import { DepMetadata } from "@/components/DepMetadata";
import { PrInlineComments } from "@/components/PrInlineComments";

{/* Cross-repo card: owner/repo point at ai-dynamo/enhancements so the
    "View PR #61" action resolves to the source PR, not a PR in this repo. */}
...
<DepMetadata
  ...
  sponsor="TBD"
  requiredReviewers="[@grahamking](https://github.com/grahamking), [@biswapanda](https://github.com/biswapanda)"
  reviewDate="TBD"
  owner="ai-dynamo"
  repo="enhancements"
  pr={61}
...
> [!WARNING]
> **Status: Draft — Demo Content.** This page mirrors an in-flight DEP from the sibling [`ai-dynamo/enhancements`](https://github.com/ai-dynamo/enhancements/pull/61) repository (PR #61 by [@ryanolson](https://github.com/ryanolson) on branch `ryan/nova`) so the Proposals tab can render real design prose against a real GitHub review thread. It is illustrative only — real Dynamo DEPs live in the `ai-dynamo/enhancements` repository and are rendered here. The line-level comments below and the "Revision discussion" section further down are the actual design debate on the enhancements PR.
...
#### If We Delay This Work

- **ServerStreaming remains the only paradigm**: All communication must follow the streaming response pattern, even when inappropriate (e.g., simple request/response operations)
- **Distributed KVBM is blocked**: The KV Block Manager cannot be implemented in a distributed fashion without bidirectional communication primitives

#### If We Never Do This

- **Architectural rigidity**: Dynamo becomes locked into a single communication pattern, forcing all future features to shoehorn into ServerStreaming regardless of fit
- **Competitive disadvantage**: Inference is becoming increasingly distributed in nature. A system built on a single-tool communication model will struggle to adapt as new patterns emerge (streaming audio, real-time bidirectional protocols, etc.)
- **Missed optimization opportunities**: Many use cases (health checks, metadata queries, cache lookups) are naturally unary RPC - forcing them through streaming adds unnecessary overhead
- **Limited future adaptability**: Building on a flexible foundation now enables responding to unforeseen communication requirements; retrofitting later is significantly more costly
...
This approach involves tradeoffs that should be acknowledged:

#### Loss of Full-Stack Tooling (vs. gRPC)

With gRPC, `protoc` automatically generates client-side accessors, strongly-typed stubs, and the wire protocol has well-defined semantics with extensive tooling support. Nova does not provide this level of code generation.

**Mitigation**: Protobufs can still be used with Nova since handlers accept and return raw bytes - any serialization format works. However, this requires more manual effort than a full-stack gRPC solution where everything is generated from `.proto` files.

#### Risk of Transport Over-Coupling

Directly exposing multiple transport implementations creates risk that application code becomes coupled to transport-specific behaviors. Unwinding such coupling is painful.

**Mitigation**: The `Transport` trait provides a clear abstraction boundary. Application code should interact with Nova's handler registration and sending APIs, not transport internals.

#### Constrained Feature Set

Supporting common features across multiple transport implementations (TCP, gRPC, UCX, NATS) is inherently harder than optimizing for a single stack. Nova's feature set will necessarily be the intersection of what's feasible across all backends, rather than the union of each backend's capabilities.

**Mitigation**: Nova focuses on the active message primitive which maps naturally to all transports. Transport-specific optimizations (e.g., UCX rendezvous) are exposed where possible but not required.
...
**Reason Rejected:** gRPC's overhead is acceptable for external APIs but too high for internal hot-path communication like KVBM operations. A gRPC only solution eliminates numerous optimizations from the NVIDIA networking team that can provide custom optimization for specific problems.
...
* [Active Messages: a Mechanism for Integrated Communication and Computation](https://www2.eecs.berkeley.edu/Pubs/TechRpts/1992/5569.html) - von Eicken et al., 1992
...
{/* Cross-repo comment mirror: owner + repo + path tell fern/js/dep-pr-comments.js
    to fetch from ai-dynamo/enhancements PR #61 instead of the default
    ai-dynamo/dynamo. Path filters inline review comments to the single file
    changed on that PR. issue is omitted, so the "Design discussion" section
    below renders its empty-state "open a tracking issue" deep-link — that is
    expected: no tracking issue exists yet for the Nova DEP. */}
