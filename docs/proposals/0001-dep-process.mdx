...
subtitle: "How Dynamo proposes, reviews, discusses, and ratifies cross-cutting design and process decisions."
pr: 11687
tracking-issue:
---

{/*
  DRAFT MIRROR of DEP-0001. The authoritative source lives at:
      https://github.com/ai-dynamo/dynamo-tpm  (private / internal)
  and will be published to ai-dynamo/enhancements once the DEP-0001 PR
  is opened. Until then, this page renders the current draft on the
  docs preview so reviewers can leave line-level comments HERE — via
  the <PrInlineComments> mount at the bottom, which points at this
  Dynamo repo's PR #11687 and its `docs/proposals/0001-dep-process.mdx`
  path so comments land on this file.

  Section vocabulary is DEP-0001's own; the file is not summarized. The
  in-body Field/Value metadata table from the source is dropped here
  because <DepMetadata> below renders that surface.
*/}
...
<DepMetadata
  ...
  requiredReviewers="Dynamo maintainers — engineering leadership and docs owners"
  owner="ai-dynamo"
  repo="dynamo"
  pr={11687}
/>
...
> [!WARNING]
> **Status: Draft.** This DEP proposes the DEP process itself and is under active discussion. It is not yet a ratified decision — the lifecycle, front-matter, roles, and phases below are the proposed starting point and will be finalized in review. Leave line-level review by highlighting text on this page (see the "💬 Comment on GitHub" affordance) or on [PR #11687](https://github.com/ai-dynamo/dynamo/pull/11687) directly.
...
Neelay Shah raised the need for a formal DEP process. Dynamo had no agreed way to propose a cross-cutting design or process change, discuss it in the open, and record the decision. Design content, line-level review, and open-ended debate lived in three places at once: a separate repository, pull-request threads, and meetings. A reader had no single place to find a proposal's current state, and a contributor without repository access had no way to weigh in.

Three constraints came out of that discussion, and any process we adopt has to meet all three:

* **Shareability.** Harry Kim asked for maximum shareability — a clean, public link a PM or partner can open without a GitHub account or repo access.
* **Line-level design discussion.** Neelay Shah noted that GitHub issues track work well but are poor for line-level design discussion; reviewers need to comment on specific lines of a proposal.
* **Discoverability with the code.** David Zier flagged that storing planning docs away from the code leads to confusion and missed documentation. Proposals have to stay easy to find and cross-linked to the code they affect.

{/* Editorial note: before this DEP goes public, reframe the named attributions (David Zier, Neelay Shah, Harry Kim here; Ryan Olson in Validation) as "raised in the originating discussion" rather than naming individuals. */}
...
The strongest argument against a dedicated repository is David Zier's: planning docs stored away from the code lead to confusion and missed documentation. That is a real risk, and it is the reason single-repo projects colocate proposals with code.
...
The lifecycle below is the proposed model, shown as an example. Exact state names get finalized during DEP review — the current dynamo docs README banner uses a slightly different enum ("Approved", and no "Implemented" state) that we would reconcile. As proposed, a DEP moves through these states, and the banner and the `status` header carry the current one:
...
Each DEP would carry front-matter like the example below. The keys are proposed and match the docs-site template so a DEP stays portable between the repository and the rendered page; the final set gets settled in review.

| Key | Meaning |
| :--- | :--- |
| `number` | DEP number, zero-padded to four digits (`0001`). |
| `title` | Full title, prefixed with the DEP number on the rendered page. |
...
| `tracking-issue` | URL of the anchor issue in `ai-dynamo/enhancements`. |
| `pr` | The pull request that adds or edits the DEP. Drives the inline review and revision mirror. |

As proposed, the `status` value maps to the docs-site banner: Draft to a warning callout, Under Review to an info callout, Accepted or Implemented to a note callout, and Rejected, Deferred, or Replaced to an error callout. Keep the `status` header and the banner in sync.

## Implementation Details

The render and mirror already exist on draft PR `ai-dynamo/dynamo#11687`. It adds a Proposals tab, a `DepMetadata` component for the status pill and metadata card, a `PrInlineComments` component that renders a mount div, and the client runtime `fern/js/dep-pr-comments.js`, injected site-wide. The runtime reads the DEP's `pr` and `issue` from the mount and fetches three sources from GitHub's public REST API:
...
We validated the render and mirror on the live Fern preview against a real DEP: Nova, "Active Messaging as a Foundational Network Primitive," `ai-dynamo/enhancements` PR #61 by Ryan Olson, `deps/0000-nova.md`, about 509 lines.
...
* Bot and CI accounts — Copilot, copy-pr-bot, github-actions — are filtered from every surface, so the page shows human review only (authors dzier, ryanolson, brminich, oandreeva-nv, nv-kmcgill53, richardhuo-nv).
...
* **Automated numbering and index.** DEP numbering and an auto-generated index are manual today.
...
* Proposals tab, DEP template, status-banner convention, `DepMetadata` and `PrInlineComments` components, and the comment-mirror runtime, validated against a real DEP.
...
* Automated DEP numbering and index generation.

### Phase 2: Backfill and Index
...
* Backfill existing enhancements DEPs onto the tab and generate a DEP index.
...
* **Relationship to `deps/0000-dep-process.md`.** `ai-dynamo/enhancements` already holds an approved `0000-dep-process`. Whether DEP-0001 extends and formalizes it, or supersedes it, is undecided here. The review picks one.
* **Lifecycle state names.** The proposed states (Draft, Under Review, Accepted, Rejected, Deferred, Implemented, Replaced) need reconciling with the current dynamo docs README banner enum, which differs.
...
* **SIG rollout dependency.** DEP-0001 references SIGs as the ownership layer but does not define them. Standing up SIGs — their names, chairs, and scope — is a separate governance effort this process depends on.
...
{/* Cross-repo comment mirror: this page lives in ai-dynamo/dynamo (draft
    PR #11687), so we point the mirror at THIS repo's PR and filter by
    this file's path so only comments on this DEP page show up here. No
    tracking issue yet — the empty-state "open a tracking issue" prompt
    below is expected until one is filed. */}

<PrInlineComments
  owner="ai-dynamo"
  repo="dynamo"
  pr={11687}
  path="docs/proposals/0001-dep-process.mdx"
/>
