...
<Note>
This is **Stage 0** of the [Well-Lit Path](/dynamo/dev/well-lit-paths) — the tested, end-to-end route from your first request to a production service. Get a response below, then follow the path stage by stage.
</Note>
...
You've finished Stage 0. The [Well-Lit Path](/dynamo/dev/well-lit-paths) takes it from here, one tested stage at a time:

<CardGroup cols={2}>
  <Card title="Stage 1 · Run a Real Model as a Service" icon="regular server" href="/dynamo/dev/well-lit-paths/aggregated-service">
    Stand up a persistent endpoint serving a model you actually care about.
  </Card>
  <Card title="See the whole path" icon="regular map" href="/dynamo/dev/well-lit-paths">
    The full 1→100 journey: routing, disaggregation, and production on Kubernetes.
  </Card>
</CardGroup>
