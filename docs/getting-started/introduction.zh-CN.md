...
- **在任意引擎之上的系统级优化** -- 推理引擎优化单 GPU 前向传递。Dynamo 增加分布式层：解耦式服务、智能路由、跨内存层级的 KV 缓存管理，以及自动扩缩容。
- **可组合的性能提升技术** -- 解耦式服务、KV 缓存感知路由和 KV 缓存卸载等技术各自都能提升性能；组合使用时会带来叠加收益。
...
Dynamo 进行系统级设计的动机之一，是支持解耦式服务：在不同设备上运行 prefill 和 decode，使两者能够独立扩展和并行化。解耦式服务需要三项能力：(1) 调度，用于在不相互干扰的情况下分配 prefill 和 decode 阶段；(2) 内存管理，用于 KV 缓存卸载和载入；(3) 低延迟数据传输，用于在节点之间以及跨内存层级移动 KV 缓存。

![Dynamo 基础：调度、内存管理和数据传输](../assets/img/intro-foundations.svg)

Dynamo 的基础能力最初面向解耦式服务，随后扩展到用于多模态的 EPD 解耦，现在支持 diffusion、RL 和 agents 等工作负载。
...
| | [Grove](https://github.com/ai-dynamo/grove) | 支持 Kubernetes 多节点解耦式服务所需的 gang scheduling 和拓扑感知 |
| | [Model Express](https://github.com/ai-dynamo/model-express) | 通过缓存模型权重并经由 NIXL 将其传输到其他 GPU，快速加载模型权重。未来也将用于容错 |
| **性能** | [AIConfigurator](https://github.com/ai-dynamo/aiconfigurator) | 基于模型、ISL/OSL、HW 等估算聚合式与解耦式服务的性能。以前称为 LLMPet |
...
Dynamo 通过组合三项核心技术实现先进的 LLM 性能：解耦式服务、KV 缓存感知路由和 KV 缓存卸载。这些技术由 NIXL 支撑，NIXL 是一种低延迟数据传输层，可在节点之间无缝移动 KV 缓存。
...
- [Disaggregated serving](../design-docs/disagg-serving.zh-CN.md) 在“设计原则”部分，我们介绍了解耦式服务的概念。[InferenceX](https://newsletter.semianalysis.com/p/inferencex-v2-nvidia-blackwell-vs) 展示了它的性能。借助解耦式服务和大规模专家并行，DeepSeek V3 可以达到约 7 倍吞吐量/GPU。
此外，当这三项技术组合在一起时，会产生如下图所示的叠加收益。

![解耦式服务、KV 缓存感知路由和 KV 缓存卸载的性能可组合性](../assets/img/intro-perf.svg)

- **Disaggregated Serving + KV Cache-Aware Routing** -- KV 缓存感知路由同时针对计算（prefill）和内存（decode）进行负载均衡，从而同时优化延迟和吞吐量。
- **Disaggregated Serving + KV Cache Offloading** -- KV 缓存卸载带来更快的 TTFT，并且可以减少 prefill worker 数量以降低 TCO。
- **KV Cache-Aware Routing + KV Cache Offloading** -- 卸载会增加总体可寻址缓存大小，提高 KV 缓存命中率，进而加速 TTFT。

> [!TIP]
> 准备尝试这些技术？请参阅 [Dynamo recipes](https://github.com/ai-dynamo/dynamo/tree/main/recipes)，了解组合解耦式服务、路由和卸载的分步部署示例。

## 从配置到生产级部署

### 使用 AIConfigurator 在 30 秒内找到最佳配置

手动为解耦式服务寻找最佳并行度可能需要数天的穷举配置扫描，而这一挑战在大规模环境中只会更加严峻。
...
- [Recipes](https://github.com/ai-dynamo/dynamo/tree/main/recipes) -- 组合解耦式服务、路由和卸载
