...
This page catalogs the environment variables that govern Dynamo observability. Set them on **every** Dynamo process (frontend, router, workers) that you want to emit signals — a variable set on workers alone does not configure the frontend. For how-to walkthroughs, see the observability guides on [Metrics](../../observability/metrics.md), [Tracing](../../observability/tracing.md), [Logging](../../observability/logging.md), and [Health Checks](../../observability/health-checks.md).
...
<Tabs>
  ...
  <Tab title="Kubernetes">
    Set the variables on the [`DynamoGraphDeployment`](../../kubernetes/dgd-reference.mdx) (DGD) — at graph level in `spec.envs` (prepended to every component) or per-component under `services.<name>.extraPodSpec.mainContainer.env`. A component-level entry overrides a graph-level one of the same name.
...
<ParamField path="OTEL_EXPORT_ENABLED" type="boolean" default="false">
  Master switch for OTLP export. Gates **both** traces and logs. Without it, traces and logs never leave the process — Tempo and Loki stay empty even when healthy.
</ParamField>

<ParamField path="OTEL_EXPORTER_OTLP_TRACES_ENDPOINT" type="string" default="http://localhost:4317">
  OTLP gRPC endpoint for traces. Must be a gRPC listener — Dynamo's exporter does not speak OTLP/HTTP, even though the OTel Collector also listens on `:4318`.
</ParamField>

<ParamField path="OTEL_EXPORTER_OTLP_LOGS_ENDPOINT" type="string" default="same as traces endpoint">
  OTLP gRPC endpoint for logs. Defaults to `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` when unset. Same gRPC-only constraint as the traces endpoint.
</ParamField>

<ParamField path="OTEL_SERVICE_NAME" type="string" default="dynamo">
  Service name attached to traces and spans. Set a per-component value (e.g. `dynamo-frontend`) so services are distinguishable in Tempo.
</ParamField>
...
<ParamField path="DYN_SYSTEM_USE_ENDPOINT_HEALTH_STATUS" type="string">
  JSON array of endpoints that must be served before the component reports `ready`, e.g. `["generate"]`. Unset by default. Marked deprecated in the runtime source (`environment_names.rs`) — verify current behavior against your release before relying on it.
...
## Request replay tracing

Per-request JSONL capture for [Mooncake replay](../../observability/request-tracing.md). Primarily a local-development workflow.

<ParamField path="DYN_REQUEST_TRACE" type="boolean">
  Truthy master switch that enables request-replay capture. Unset by default.
</ParamField>

<ParamField path="DYN_REQUEST_TRACE_SINKS" type="string" default="jsonl_gz">
  Comma-separated sinks: `jsonl`, `jsonl_gz`, or `stderr`.
</ParamField>

<ParamField path="DYN_REQUEST_TRACE_OUTPUT_PATH" type="string" default="/tmp/dynamo-request-trace">
  Literal JSONL path or gzip segment prefix for captured rows.
</ParamField>
...
<ParamField path="DYN_REQUEST_TRACE_JSONL_BUFFER_BYTES" type="integer" default="1048576">
  JSONL or gzip batching threshold in bytes.
</ParamField>

<ParamField path="DYN_REQUEST_TRACE_JSONL_FLUSH_INTERVAL_MS" type="integer" default="1000">
  Periodic flush interval in milliseconds.
</ParamField>

<ParamField path="DYN_REQUEST_TRACE_JSONL_GZ_ROLL_BYTES" type="integer" default="268435456">
  Gzip roll threshold in uncompressed bytes.
</ParamField>

<ParamField path="DYN_REQUEST_TRACE_JSONL_GZ_ROLL_LINES" type="integer">
  Optional gzip roll threshold in records. Unset by default.
</ParamField>

<ParamField path="DYN_REQUEST_TRACE_TOOL_EVENTS_ZMQ_ENDPOINT" type="string">
  Optional ZMQ PULL bind address for harness tool events. Set on the process that should own tool-event ingress, usually the frontend. Unset by default.
</ParamField>

<ParamField path="DYN_REQUEST_TRACE_TOOL_EVENTS_ZMQ_TOPIC" type="string" default="agent-tool-events">
  First-frame ZMQ topic filter, used when the ZMQ endpoint is configured.
</ParamField>
...
- [Local Resource Monitor (Local)](local-resource-monitor.mdx) — host-side per-process exporter.
