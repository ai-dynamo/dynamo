...
`vllm`, `sglang` and `trtllm` production grade engines are available in `components/src/dynamo`. They run as Python components, using the Rust bindings. See the main README.
...
The [dynamo](https://pypi.org/project/ai-dynamo/) Python library allows you to build your own engine and attach it to Dynamo. All of the main backend components in `components/src/dynamo` work like this.
...
More fully-featured Python engines are in `components/src/dynamo`.
