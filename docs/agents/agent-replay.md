...
- which requests belonged to the same agent session; and
...
- investigate latency or cache regressions with a repeatable request schedule; and
...
| **Offline simulation with DynoSim** | The request graph against Dynamo's simulated scheduler, router, and KV cache | Fast topology, routing, capacity, and what-if exploration |
...
The sink writes rotating `request-trace.NNNNNN.jsonl.gz` files. Stop the frontend, or otherwise allow the sink to flush, before using the files. See [Agent Tracing](agent-tracing.md) for configuration, the record schema, Perfetto conversion, and optional tool-event ingestion.
...
- every request has input length, output length, and sequence hashes; and
- the final trace file was flushed cleanly.

These checks catch a wrong frontend, missing session headers, and an incomplete shutdown before those errors become misleading replay results.
...
- replayed output lengths match the capture; and
...
Replay is intentionally content-free. Synthetic prompts preserve block-sharing topology, not the original text. Wire input length can differ slightly because the target tokenizer and chat template reconstruct the request. Service time can also differ even against the same model. Tool rows are not dispatched by AIPerf. Their elapsed time remains represented by the gap between model requests, and DynoSim can retain it as tool-wait time. If a changed model response would make the agent choose another tool or branch, that requires another live agent run rather than static trace replay.
