...
|---|---|---|
| **You provide** | Full deployment spec (components, parallelism, replicas, resource limits) | Model, backend, workload, hardware, and optional SLA targets |
...
Use a DGD directly when you have a hand-crafted configuration for a specific model/hardware combination. Most [recipes](https://github.com/ai-dynamo/dynamo/tree/main/recipes) are tuned DGD manifests. Use a DGDR when you want Dynamo to generate the DGD for you.
...
All hardware fields are auto-detected from cluster GPU nodes when omitted (requires cluster-wide mode with GPU discovery enabled). `gpuSku` is a selector that restricts which nodes are considered; the other fields are overrides passed to the profiler. If all fields are set, discovery is skipped.
...
<ParamField path="modelCache" type="ModelCacheSpec">
  Optional PVC configuration for pre-downloaded model weights. When set, weights are loaded from the PVC instead of downloading from HuggingFace. See [Model Caching](model-caching.md).
</ParamField>
