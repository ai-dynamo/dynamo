...
- Set `DYN_ENFORCE_DISAGG=true` (default: `false`) for disaggregated deployments. This controls EPP startup behavior:
  - **`true` (recommended for disaggregated serving):** The EPP waits indefinitely at startup for prefill workers to register before accepting requests. This prevents a race condition where the EPP starts before prefill workers are ready (e.g., during model weight loading which can take up to 30 minutes for large models) and permanently creates a disabled prefill router. The K8s StartupProbe is the timeout mechanism — if prefill workers never appear, the probe's `failureThreshold × periodSeconds` determines when the EPP pod is killed and restarted.
  - **`false` (default):** The EPP waits up to 5 minutes for prefill workers at startup, returning immediately as soon as one registers. If none are found within 5 minutes, it falls back to aggregated mode (no prefill/decode separation). This is appropriate for aggregated deployments or when disaggregated mode is optional.
