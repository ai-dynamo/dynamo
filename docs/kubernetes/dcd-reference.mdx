...
Configures the GPU Memory Service (GMS) for a worker component. Referenced by `experimental.gpuMemoryService`.
...
Configures active-passive failover for a worker component. Referenced by `experimental.failover`.
...
Configures checkpointing for a DGD component. Referenced by `experimental.checkpoint`.
...
Declares an additional GMS client pod for inter-pod GMS. Referenced by [GPUMemoryServiceSpec](#gpumemoryservicespec) `extraClientPods`.
...
Customizes the checkpoint Job created for a DGD component. Referenced by [ComponentCheckpointConfig](#componentcheckpointconfig) `job`.
...
Legacy checkpoint identity, referenced by [ComponentCheckpointConfig](#componentcheckpointconfig) `identity`.
