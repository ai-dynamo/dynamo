...
| `deployMocker` _boolean_ | DeployMocker indicates whether to automatically create a mocker DynamoGraphDeployment<br />after profiling completes. The mocker deployment uses simulated engines that don't require<br />GPUs, making it useful for testing planner behavior and infrastructure without GPU resources.<br />When true, a mocker DGD named "<dgdr-name>-mocker" is created. The mocker DGD uses<br />profiling data for realistic timing simulation. | false |  |
...
| `generatedMockerDeployment` _[RawExtension](#rawextension)_ | GeneratedMockerDeployment contains the generated mocker DynamoGraphDeployment<br />specification for testing purposes. The mocker uses simulated engines that don't<br />require GPUs, using profiling data for realistic timing simulation.<br />Created automatically when deployMocker is true. |  | EmbeddedResource: {} <br />Optional: {} <br /> |
...
| `mockerDeployment` _[DeploymentStatus](#deploymentstatus)_ | MockerDeployment tracks the auto-created mocker DGD when DeployMocker is true.<br />The mocker deployment uses simulated engines that don't require GPUs. |  | Optional: {} <br /> |
