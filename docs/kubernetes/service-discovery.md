...
| **KV Store (etcd)** | No | etcd cluster | Deprecated on Kubernetes; local, bare-metal, or legacy deployments only |
...
> [!WARNING]
> **Deprecated on Kubernetes.** Use Kubernetes discovery for operator-managed deployments. etcd discovery remains
> available for supported local, bare-metal, and legacy deployments.
