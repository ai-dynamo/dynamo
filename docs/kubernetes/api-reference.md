...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `gpuSku` _string_ | GPUSKU is the AIC hardware system identifier (e.g., "h100_sxm", "h200_sxm", "b200_sxm").<br />When omitted, the operator auto-detects this from cluster GPU labels via InferHardwareSystem.<br />Recognized raw GPU product names (e.g., "NVIDIA-B200") are also accepted and automatically<br />normalized to the AIC identifier; unrecognized values that cannot be mapped are rejected. |  | Optional: \{\} <br /> |
