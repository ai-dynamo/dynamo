...
- A Gateway API implementation that supports GAIE `InferencePool` resources and `endpointPickerRef`
  calls. This walkthrough shows the same verified agentgateway and Istio paths as the GAIE
...
- Access to a Dynamo EPP image with experimental router-only on-ramp support. The example manifests
  use the `<router-only-epp-image>` placeholder because the normal Dynamo release image does not
  imply this under-development mode.
...
From the repository root, pick one topology. Both examples are namespace-neutral. Before applying
one, replace `<router-only-epp-image>` in the selected manifest with an EPP image that implements
router-only on-ramp support.
...
| `DYN_EPP_KV_EVENT_TOPIC` | KV cache aware routing | vLLM `--kv-events-config` topic (`""` = no prefix). |
| `DYN_MODEL_NAME` | All on-ramp deployments | Model name for the tokenizer because there is no Dynamo model card in router-only mode. |
...
If this namespace is only for the on-ramp experiment, delete it:
