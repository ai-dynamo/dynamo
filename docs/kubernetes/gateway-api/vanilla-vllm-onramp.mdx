...
- Access to the Dynamo EPP image and the vLLM image used by the example.
...
Dynamo and model-serving prerequisite, not a GAIE-specific resource.
...
Check the EPP logs for pod discovery and routing decisions:

```bash
kubectl logs -n "$NAMESPACE" deployment/qwen-epp -c epp --tail=100
```
