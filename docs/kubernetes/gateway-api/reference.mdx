...
```mermaid
flowchart LR
    Worker["Dynamo worker"] -->|"publishes KV events"| NATS["Dynamo runtime<br/>NATS Core event plane"]
...
| `DYN_ROUTER_DECODE_ACTIVE_REQUEST_WEIGHT` | `0.0` | Adds block-equivalent routing cost for each active request. Tune only when decode step latency depends materially on active batch size. |
...
| `DYN_ROUTER_REPLICA_SYNC` | `false` | Publishes and subscribes KV active-sequence state across router replicas. |
| `DYN_ROUTER_SESSION_AFFINITY_TTL_SECS` | unset | Enables session affinity and best-effort binding sync with this router-local idle TTL. |
...
| `DYN_ACTIVE_DECODE_BLOCKS_THRESHOLD` | unset | Decode worker is busy above this active-block fraction. Setting a numeric value independently enables this rejection check. |
| `DYN_ACTIVE_PREFILL_TOKENS_THRESHOLD` | unset | Worker is busy above this absolute active-prefill-token count. Setting a numeric value independently enables this rejection check. |
| `DYN_ACTIVE_PREFILL_TOKENS_THRESHOLD_FRAC` | unset | Worker is busy above this fraction of `max_num_batched_tokens`. Setting a numeric value independently enables this rejection check. |
...
For the native Rust ext_proc implementation, see
[Experimental: Rust EPP](./experimental-rust-epp.mdx).
