...
<Warning>
Patch the default `AgentgatewayParameters` resource in `agentgateway-system` only as a cluster-wide
policy decision. Gateways without `spec.infrastructure.parametersRef` inherit that default.
</Warning>

## Startup Probes

Large models, slow storage, or model-cache misses can make workers load longer than the default
startup probe budget. Increase the affected component's `startupProbe.failureThreshold` when the
long startup time is expected.
...
Apply this to the model worker component that is timing out. Use pod events to confirm that the
failure is a startup probe failure rather than an image pull, scheduling, or model download problem.

## Developer References

Image build commands belong with the component source, not in this user reference. Use these source
