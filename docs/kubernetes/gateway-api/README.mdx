...
The DGD's EPP component configures the EndpointPicker plugins and the runtime values required by the
embedded router. Start from the recipe manifests unless you are intentionally changing routing
behavior:

- `eppConfig.config.plugins` defines filters, scorers, and pickers.
- `eppConfig.config.schedulingProfiles` composes those plugins and assigns weights. Changing weights
  changes endpoint scoring; keep the filter, scorer, and picker chain consistent with the topology.
- `DYN_MODEL_NAME` must match the served model name used by the workers so the EPP tokenizes prompts
  with the same model context.
- `DYN_KV_CACHE_BLOCK_SIZE` must match the worker backend's `--block-size`.
- `DYN_ENFORCE_DISAGG=true` is used by disaggregated deployments to fail closed when the expected
  prefill/decode topology is not present.

See the GAIE recipe manifests for concrete aggregated and disaggregated EPP configurations:
[aggregated](https://github.com/ai-dynamo/dynamo/blob/main/recipes/llama-3-70b/vllm/agg/gaie/deploy.yaml)
and
[disaggregated](https://github.com/ai-dynamo/dynamo/blob/main/recipes/llama-3-70b/vllm/disagg-single-node/gaie/deploy.yaml).
