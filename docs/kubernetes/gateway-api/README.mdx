...
- A Gateway API implementation that supports GAIE. The quickstart shows agentgateway and Istio. See
  the upstream [GAIE gateway implementation list](https://github.com/kubernetes-sigs/gateway-api-inference-extension/blob/main/site-src/implementations/gateways.md)
...
Use a Gateway implementation that supports the GAIE EndpointPicker flow. The quickstart includes
agentgateway and Istio examples. Envoy Gateway and other Gateway API implementations also work when
their installed version implements GAIE.
...
<Tabs>
  ...
  <Tab title="Envoy Gateway">
    Use Envoy Gateway when its installed version supports GAIE EndpointPicker. Keep the Dynamo
    resources the same: the route still targets the generated `InferencePool`, and the pool points
    at the Dynamo EPP service.
  </Tab>
  <Tab title="Other GAIE">
    Use another Gateway API implementation if it supports GAIE `InferencePool` resources and the
    EndpointPicker extension. Check the upstream
