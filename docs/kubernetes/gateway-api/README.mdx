    ...
    spec:
      containers:
        - name: main
          env:
            - name: DYN_MODEL_NAME
              value: RedHatAI/Llama-3.3-70B-Instruct-FP8-dynamic # Match the worker model name.
            - name: DYN_KV_CACHE_BLOCK_SIZE
              value: "128" # Match the worker backend's --block-size.
            - name: DYN_ENFORCE_DISAGG
              value: "false" # Use "true" for disaggregated fail-closed behavior.
```

See the GAIE recipe manifests for full aggregated and disaggregated EPP configurations:
