...
adds inference-specific endpoint selection: the gateway calls an Endpoint Picker Plugin before it
forwards the request. Dynamo's EPP uses Dynamo routing logic to choose the model endpoint, including
KV cache aware routing when KV events are available.
