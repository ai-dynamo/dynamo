...
- Gateway API and GAIE CRDs installed. The quickstart installs them explicitly from pinned upstream
  release manifests.
- A Gateway API implementation that supports GAIE. The quickstart uses tested agentgateway and Istio
  paths. See the upstream
...
Use a Gateway implementation that supports the GAIE EndpointPicker flow. The quickstart tests
agentgateway and Istio. Envoy Gateway and other Gateway API implementations may work when their
installed version supports GAIE `InferencePool` resources and EndpointPicker calls; validate the
controller behavior before using that path.
...
<Tabs>
  ...
  <Tab title="Envoy Gateway">
    Use Envoy Gateway when its installed version supports GAIE `InferencePool` resources and
    EndpointPicker calls. Keep the Dynamo resources the same: the route still targets the generated
    `InferencePool`, and the pool points at the Dynamo EPP service.
  </Tab>
  <Tab title="Other GAIE">
    Use another Gateway API implementation if it supports GAIE `InferencePool` resources and the
    EndpointPicker extension. This quickstart does not test that path. Check the upstream
    [GAIE gateway implementation list](https://github.com/kubernetes-sigs/gateway-api-inference-extension/blob/main/site-src/implementations/gateways.md)
    before choosing a controller.
  </Tab>
</Tabs>

The quickstart walks through the two implementation paths shown in this table:
