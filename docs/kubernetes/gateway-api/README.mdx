...
For existing GAIE stacks that already run stock vLLM pods, the experimental
[vanilla vLLM on-ramp](./vanilla-vllm-onramp.mdx) shows how Dynamo EPP routing can fit into that
shape without making it the default operator-managed flow.
