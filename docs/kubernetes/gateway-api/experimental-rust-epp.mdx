...
| `DYN_ROUTER_*` | varies | Standard Dynamo router settings, including cache-overlap credit, prefill load scale, temperature, replica sync, and tracking flags. |

For router tuning details, see [GAIE Reference](./reference.mdx#router-tuning).

## Limitations

- Pod-level Kubernetes discovery only. `DYN_KUBE_DISCOVERY_MODE=container` is rejected.
