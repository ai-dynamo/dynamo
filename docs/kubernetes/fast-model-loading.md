...
This is complementary to [Model Caching](model-caching.md): caching gets the weights onto a shared PVC once; this page makes each worker *read* them fast. It applies to the **PVC + Download Job** path, not the ModelExpress object-storage path (which streams from S3/GCS/Azure Blob instead — see [ModelExpress](modelexpress.md)).
...
For SGLang and TensorRT-LLM, the striping fix alone still helps; there is no `runai_streamer` equivalent to add. (SGLang's fast-distribution path is ModelExpress `remote_instance` — a P2P pull from an already-loaded worker, not a faster first read from storage. See [Model Caching](model-caching.md#option-2-modelexpress-p2p-distribution).)
...
### 1. Stripe the cache directory *before* downloading
...
Run the normal [PVC + Download Job](model-caching.md#step-2-download-the-model). Because the directory default is now wide, every shard lands striped across all OSTs.

### 3. Enable the streaming loader in the DGD
...
- [Model Caching](model-caching.md) — get the weights onto a shared PVC once (download Job, mount config, ModelExpress)
