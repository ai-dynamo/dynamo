...
> **Responsibility and setup matrix:** For cluster-admin vs application-user responsibilities and exact `install` vs `enabled` Helm settings for Grove/KAI, see [Detailed Installation Guide - Responsibility Split](../installation-guide.md#responsibility-split-admin-vs-user-vs-operator).
...
Users submit `DynamoGraphDeployment` resources; they do not manually create Grove resources. When the Grove pathway is active, the Dynamo operator creates and reconciles the underlying Grove objects.
