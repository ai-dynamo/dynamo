...
Counters for requests that end early: [cancellations](../../design-docs/request-cancellation.md) (the client or frontend aborts an in-flight request) and [rejections](../../fault-tolerance/request-rejection.md) (the frontend sheds load with HTTP 503 when all workers are busy). The worker-side counterparts are under [Component metrics](#component-metrics).
...
<ParamField path="dynamo_frontend_model_rejection_total" type="counter">
  Total requests rejected because all workers were busy (surfaced to the client as HTTP 503). Incremented when the router returns `ResourceExhausted`. Labeled by `model` and `endpoint` (the API route).
</ParamField>
...
<ParamField path="dynamo_frontend_router_queue_pending_requests" type="gauge">
  Requests pending in the router scheduler queue. Registered only when `--router-queue-threshold` is set. Labeled by `worker_type` (`prefill` or `decode`) to distinguish queues in disaggregated mode.
</ParamField>

### KV indexer metrics

<ParamField path="dynamo_component_kv_cache_events_applied" type="counter">
  KV cache events applied to the router's radix-tree index. Appears only when `--router-kv-overlap-score-credit` is greater than 0 (default) and workers publish KV events. Labeled by `status` and `event_type` — see [Metric-specific labels](metric-labels.mdx#metric-specific-labels).
</ParamField>
...
| `dynamo_frontend_router_queue_*` | Registered; populated when `--router-queue-threshold` set | Registered; populated when `--router-queue-threshold` set | **Not registered** | **Not created** |
