...
<ParamField path="dynamo_frontend_router_queue_wait_seconds" type="histogram">
  Time requests spend pending in the router scheduler queue before worker admission.
</ParamField>
...
Router queue wait P99:

```promql
histogram_quantile(
  0.99,
  sum by (le, model, worker_type, policy_class) (
    rate(dynamo_frontend_router_queue_wait_seconds_bucket[5m])
  )
)
```
