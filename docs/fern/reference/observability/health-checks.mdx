...
During shutdown, `/live` returns `503 Service Unavailable` with status `shutting_down`.

The frontend `/health` endpoint returns `200 OK` and lists the endpoints and instances currently
...
does not change the frontend response status by itself.
...
After startup, both paths return `200 OK` with status `ready`:
