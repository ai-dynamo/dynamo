...
This page catalogs the environment variables that govern Dynamo observability. Set them on **every** Dynamo process (frontend, router, workers) that you want to emit signals — a variable set on workers alone does not configure the frontend. For how-to walkthroughs, see the observability guides on [Metrics](../../observability/metrics.mdx), [Tracing](../../observability/tracing.mdx), [Logging](../../observability/logging.mdx), and [Health Checks](../../observability/health-checks.mdx).
