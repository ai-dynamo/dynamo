...
Programs are keyed by the normalized `session_id`. Use the canonical headers defined in [Session IDs](../../agents/session-ids.mdx):
...
For request-level tracing, enable `DYN_REQUEST_TRACE=1` on the frontend. Explicit harness tool spans additionally require `DYN_REQUEST_TRACE_TOOL_EVENTS_ZMQ_ENDPOINT` and a configured publisher. See [Request Trace Reference](../observability/request-tracing.mdx).

## Related Pages

- [Run the ThunderAgent Scheduler](../../agents/thunderagent-quickstart.md)
- [ThunderAgent Scheduler Design](../../agents/thunderagent-router.md)
