...
Choose a backend and follow its tab. Each example uses `Qwen/Qwen3-0.6B` and one GPU. The vLLM
launch scripts start both the Dynamo frontend and the worker, and stop both processes when you press
`Ctrl+C`.
...
The script sets `DYNAMO_USE_FLEXKV=1` and selects `FlexKVConnectorV1`. For SSD and distributed-cache
configuration, see the [FlexKV documentation](https://github.com/taco-project/FlexKV/tree/main/docs).
