...
endpoint. Use [Mocker Live Simulation](../dynosim/mocker-local.md) for a live GPU-free deployment,
[DynoSim Replay](../dynosim/runs.md) for request-level offline simulation, and
[Benchmarking with AIPerf](aiperf.mdx) for measurements against the running local endpoint.
...
for [vLLM](../backends/vllm/vllm-examples.mdx),
[SGLang](../backends/sglang/sglang-examples.mdx), and
[TensorRT-LLM](../backends/trtllm/trtllm-examples.mdx).

If one worker requires more GPUs than a node provides, follow
[Multi-Node Deployment](multinode-deployment.mdx) rather than treating the GPU budget as one flat
local device list.

## Validate the recommendation

AIConfigurator narrows the configurations to test; it does not replace measurement. Launch the
recommended local deployment and use [Benchmarking with AIPerf](aiperf.mdx) to validate latency and
...
options, see the [AIConfigurator Reference](../features/disaggregated-serving/aiconfigurator.md).
