...
"""Tests for docs/fern/scripts/sync_deps.py transforms.
...
class RenderMdxTests(unittest.TestCase):
    ...
    def test_generated_marker_comment_is_present(self):
        ...
        self.assertIn("docs/fern/scripts/sync_deps.py", mdx)
        self.assertIn(
...
class SlugValidationTests(unittest.TestCase):
    ...
    An unsafe value can escape docs/fern/pages/proposals/_generated/. load_manifest
    ...
    def _load(self, output_value, tmp_path: Path) -> None:
        manifest = tmp_path / "docs" / "fern" / "scripts" / "deps.json"
...
class ExtractStatusFromMdxTests(unittest.TestCase):
    ...
    For hand-authored DEPs (docs/fern/pages/proposals/*.mdx) that means the
...
class DiscoverHandAuthoredStatusesTests(unittest.TestCase):
    """docs/fern/pages/proposals/*.mdx is scanned for hand-authored DEPs. README + TEMPLATE
    are meta pages (not DEPs) and MUST be excluded. Files without a
    `<DepMetadata status="..." />` prop are skipped. Slug = filename stem
    (matches the slug convention used in docs/fern/index.yml)."""
    ...
    def test_discovers_hand_authored_deps_excludes_readme_and_template(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self._write(
                root,
                "docs/fern/pages/proposals/0000-example-dep.mdx",
                '<DepMetadata dep="0000" status="Draft" />',
            )
            self._write(
                root,
                "docs/fern/pages/proposals/0001-dep-process.mdx",
                '<DepMetadata dep="0001" status="Under Review" />',
            )
            # Meta pages: no DEP status in the sidebar.
            self._write(
                root,
                "docs/fern/pages/proposals/README.mdx",
                "# Overview",
            )
            self._write(
                root,
                "docs/fern/pages/proposals/TEMPLATE.mdx",
    ...
    def test_skips_generated_dir(self):
        # docs/fern/pages/proposals/_generated/*.mdx are synced files — they are
        # covered by the manifest path, NOT by the hand-authored scan.
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self._write(
                root,
                "docs/fern/pages/proposals/_generated/dep-nova-synced.mdx",
    ...
    def test_skips_files_without_status(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self._write(
                root,
                "docs/fern/pages/proposals/only-prose.mdx",
                "# Not a DEP\n\nNo DepMetadata here.",
            )
            got = sync_deps.discover_hand_authored_statuses(root)
        self.assertEqual(got, {})
...
class RenderStatusDataJsTests(unittest.TestCase):
    ...
    Keys are DEP slugs (matching docs/fern/index.yml), values are lifecycle labels
    that docs/fern/js/dep-status-pills.js maps to a colour variant via
...
class WriteStatusDataJsTests(unittest.TestCase):
    """End-to-end: writing the JS data file to docs/fern/js/dep-status-data.js."""

    def test_writes_file_under_fern_js(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / "docs" / "fern" / "js").mkdir(parents=True)
...
class MainWriteGateTests(unittest.TestCase):
    """main() may write docs/fern/js/dep-status-data.js ONLY on a full run.
    ...
    def _setup_root(self, td: str) -> Path:
        root = Path(td)
        # Minimal repo skeleton main() needs: docs/fern/docs.yml (the root marker),
        # the manifest, and the dirs the writers touch.
        (root / "docs" / "fern").mkdir(parents=True)
        (root / "docs" / "fern" / "docs.yml").write_text("x\n", encoding="utf-8")
        (root / "docs" / "fern" / "js").mkdir()
        (root / "docs" / "fern" / "scripts").mkdir()
        (root / "docs" / "fern" / "pages" / "proposals").mkdir(parents=True)
        manifest = {
            "deps": [
                {
                    "output": "dep-nova-synced",
        ...
        (root / "docs" / "fern" / "scripts" / "deps.json").write_text(
    ...
    def test_full_run_writes_status_data(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            data = root / "docs" / "fern" / "js" / "dep-status-data.js"
...
class DiscoverHandAuthoredDepsTests(unittest.TestCase):
    """docs/fern/pages/proposals/*.mdx -> full index records. Same exclusions as the
    ...
    def test_builds_records_with_fields(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self._write(
                root,
                "docs/fern/pages/proposals/0003-router.mdx",
                '---\ntitle: "DEP-0003: Router v2"\n---\n'
                '<DepMetadata\n  dep="0003"\n  status="Under Review"\n'
                '  owningSig="SIG-Router"\n  category="Architecture"\n'
                '  authors="[@tanmayv25](https://github.com/tanmayv25)"\n/>',
            )
            self._write(root, "docs/fern/pages/proposals/README.mdx", "# Overview")
            self._write(
                root,
                "docs/fern/pages/proposals/TEMPLATE.mdx",
    ...
    def test_skips_generated_and_non_dep(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self._write(root, "docs/fern/pages/proposals/only-prose.mdx", "# Not a DEP")
            got = sync_deps.discover_hand_authored_deps(root)
        self.assertEqual(got, [])
...
class MainIndexWriteGateTests(unittest.TestCase):
    """main() writes docs/fern/js/dep-index-data.js on a full run only; a --dry-run
    ...
    def test_full_run_writes_index_data(self):
        with tempfile.TemporaryDirectory() as td:
            root = self._setup_root(td)
            index = root / "docs" / "fern" / "js" / "dep-index-data.js"
            with mock.patch.object(sync_deps, "fetch_source", return_value=NOVA_LIKE):
                rc = sync_deps.main(["--root", str(root)])
            self.assertEqual(rc, 0)
