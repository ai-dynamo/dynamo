...
under `docs/fern/pages/proposals/_generated/`. The generated dir is gitignored — the
...
runs this script first so the nav entries in `docs/fern/index.yml` that point at
`_generated/*.mdx` resolve.

Manifest: `docs/fern/scripts/deps.json` — list of DEPs to sync. Each entry names
...
so a partial/dry run never truncates the committed snapshot):
  ...
  * `docs/fern/js/dep-status-data.js`  — `window.__DEP_STATUS` slug->status map for
                                    the sidebar status pills.
  * `docs/fern/js/dep-index-data.js`   — `window.__DEP_INDEX` list of per-DEP records
                                    for the Proposals index registry grid.

Both draw from the same two sources: the synced upstream markdown and the
hand-authored `<DepMetadata>` cards under docs/fern/pages/proposals/.

Transforms applied to each source markdown (the generated output lands under
`docs/fern/pages/proposals/_generated/`, e.g. `dep-nova-synced.mdx` for the Nova demo):
  ...
  5. Wrap the transformed body with imports + `<DepMetadata>` + a
     ...
     enhancements repo (the runtime in `docs/fern/js/dep-pr-comments.js` reads
     those props at load time).

Usage:
  python3 docs/fern/scripts/sync_deps.py                 # sync all manifest entries
  python3 docs/fern/scripts/sync_deps.py --root ./       # explicit repo root
  python3 docs/fern/scripts/sync_deps.py --dry-run       # print what would change
  python3 docs/fern/scripts/sync_deps.py --only nova     # sync one entry by slug
"""
...
# docs/fern/pages/proposals/_generated/ via `..`, `/`, or leading `.` (dotfiles).
...
def render_mdx(*, entry: dict[str, Any], parsed: ParsedDep) -> str:
    ...
    fm_lines: list[str] = [
        ...
        f"# Synced by docs/fern/scripts/sync_deps.py from {source_url}",
    ...
    marker_body = f"""
        {{/*
          ...
          `docs/fern/scripts/sync_deps.py`, which pulls the DEP body from the
          ...
          Manifest entry: docs/fern/scripts/deps.json
          Slug:           {slug}
        */}}
        """
    ...
    parts = [
        ...
        "    Runtime: docs/fern/js/dep-pr-comments.js reads the data-* attributes.",
        "*/}",
        "",
        pr_mount,
        "",
...
# DEP link. The pill is populated at runtime by docs/fern/js/dep-status-pills.js
...
# docs/fern/js/dep-status-data.js. Two sources feed it:
#
#   1. Synced DEPs      — the parsed `**Status**:` field of the source
#                         markdown, keyed by manifest `output` slug (same
#                         slug used in docs/fern/index.yml).
#   2. Hand-authored    — the `status="..."` prop on <DepMetadata /> in
#      DEPs (proposals/*.mdx) hand-authored proposals under docs/fern/pages/proposals/,
#                         keyed by filename stem (matches docs/fern/index.yml).
#
...
# Hand-authored DEPs live directly under docs/fern/pages/proposals/ as *.mdx files with
...
def discover_hand_authored_statuses(root: Path) -> dict[str, str]:
    """Walk docs/fern/pages/proposals/ for hand-authored DEPs and return {slug: status}.

    Slug = filename stem (the same convention docs/fern/index.yml uses for
    `slug:` entries). Files under docs/fern/pages/proposals/_generated/ are the
    ...
    fresh sync).
    """
    proposals = root / "docs" / "fern" / "pages" / "proposals"
...
def render_status_data_js(status_map: dict[str, str]) -> str:
    """Render the JS data file that docs/fern/js/dep-status-pills.js consumes.
    ...
    header = (
        ...
        "docs/fern/scripts/sync_deps.py.\n"
        " * Consumed by docs/fern/js/dep-status-pills.js to render the "
        "right-aligned\n"
        ...
        "markdown\n"
        " *                        listed in docs/fern/scripts/deps.json.\n"
        ' *   - hand-authored DEPs `status="..."` on <DepMetadata /> in\n'
        " *                        docs/fern/pages/proposals/*.mdx (excluding "
        "README + TEMPLATE).\n"
        " */\n"
    )
    if not status_map:
...
def write_status_data_js(root: Path, status_map: dict[str, str]) -> Path:
    """Write the rendered status data JS to docs/fern/js/dep-status-data.js.

    Creates parent directories as needed. Returns the output path.
    """
    out_path = root / "docs" / "fern" / "js" / "dep-status-data.js"
...
# --------------------------------------------------------------------------- #
#
# The Proposals registry landing page (docs/fern/pages/proposals/registry.mdx -> slug
...
# docs/fern/js/dep-index-data.js and consumed by docs/fern/js/dep-index.js. Same two
...
# parseLinkedItems() in docs/fern/components/DepMetadata.tsx.
...
def discover_hand_authored_deps(root: Path) -> list[dict[str, Any]]:
    """Return index records for hand-authored DEPs under docs/fern/pages/proposals/.
    ...
    index grid renders. Slug = filename stem (matches docs/fern/index.yml).
    """
    proposals = root / "docs" / "fern" / "pages" / "proposals"
    if not proposals.is_dir():
        return []
...
def render_index_data_js(records: list[dict[str, Any]]) -> str:
    """Render the JS data file that docs/fern/js/dep-index.js consumes.
    ...
    header = (
        ...
        "(docs/fern/pages/proposals/registry.mdx).\n"
        " *\n"
...
def write_index_data_js(root: Path, records: list[dict[str, Any]]) -> Path:
    """Write the rendered index data JS to docs/fern/js/dep-index-data.js."""
    out_path = root / "docs" / "fern" / "js" / "dep-index-data.js"
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def load_manifest(root: Path) -> list[dict[str, Any]]:
    manifest_path = root / "docs" / "fern" / "scripts" / "deps.json"
    ...
    for entry in entries:
        ...
        if not isinstance(output, str) or not _SLUG_RE.match(output):
            raise SystemExit(
                ...
                f"becomes docs/fern/pages/proposals/_generated/<output>.mdx and must "
...
def sync_entry(
    ...
    *,
    ...
    The status feeds the sidebar status map (docs/fern/js/dep-status-data.js) and
    the parsed body feeds the index dataset (docs/fern/js/dep-index-data.js), so
    ...
    out_dir = root / "docs" / "fern" / "pages" / "proposals" / "_generated"
...
def _resolve_root(cli_root: str | None) -> Path:
    """Locate the repo root by walking up until we find docs/fern/docs.yml."""
    if cli_root:
        return Path(cli_root).resolve()
    here = Path(__file__).resolve()
    for parent in (here.parent, *here.parents):
        if (parent / "docs" / "fern" / "docs.yml").exists():
            return parent
    raise SystemExit("[sync-deps] could not locate repo root (no docs/fern/docs.yml)")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Sync DEP bodies from ai-dynamo/enhancements into "
        "docs/fern/pages/proposals/_generated/ (see docs/fern/scripts/deps.json).",
    )
    parser.add_argument(
        "--root",
    ...
    # docs/fern/js/dep-status-data.js or docs/fern/js/dep-index-data.js. (Both files are
    ...
    if args.dry_run or args.only:
        ...
        print(
            f"[sync-deps] {reason}: NOT writing docs/fern/js/dep-status-data.js or "
            f"docs/fern/js/dep-index-data.js — a partial or dry run must not clobber "
