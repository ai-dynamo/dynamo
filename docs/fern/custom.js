...
(() => {
  ...
  const USER_GUIDE_PATH = /^(.*)\/(kubernetes|cli)(?:\/|$)/;
  // Font Awesome Free 7.3.1 icons (CC BY 4.0), Copyright Fonticons, Inc.
  ...
  const variants = [
    ...
    },
    {
      ...
      icon: `
        <svg viewBox="0 0 512 512" aria-hidden="true" focusable="false">
          <path fill="currentColor" d="M9.4 118.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l160 160c12.5 12.5 12.5 32.8 0 45.3l-160 160c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L146.7 256 9.4 118.6zM224 384l256 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-256 0c-17.7 0-32-14.3-32-32s14.3-32 32-32z"></path>
        </svg>
      `,
    },
  ];
  ...
  function updateSelector(selector, prefix, activeVariant) {
    setSelectedVariant(selector, activeVariant);
  ...
  function buildSelector(prefix, activeVariant) {
    const selector = document.createElement("nav");
    selector.className = CUSTOM_SELECTOR_CLASS;
    selector.setAttribute("aria-label", "Deployment environment");
    ...
    variants.forEach((variant) => {
      ...
      option.addEventListener("click", (event) => {
        ...
        // Let the highlight visibly slide before loading the selected variant.
        event.preventDefault();
        setSelectedVariant(selector, variant.id);
        window.setTimeout(() => {
  ...
  function enhanceVariantSelector() {
    const match = window.location.pathname.match(USER_GUIDE_PATH);
    ...
    const [, prefix, activeVariant] = match;
