...
Use an immutable commit hash for `MODEL_REVISION`, not a mutable branch such as
`main`. After the download completes, keep the snapshot directory mounted
read-only in serving pods and restrict write access to the PVC. The snapshot path
records the resolved commit, but its contents are trustworthy only while the
backing storage is protected from untrusted writers. See
[Secure model and image provenance](../introduction.mdx#secure-model-and-image-provenance).
...
- [LoRA Adapters](../../operations/lora-adapters.md) — dynamic adapter loading (separate from base model caching)
