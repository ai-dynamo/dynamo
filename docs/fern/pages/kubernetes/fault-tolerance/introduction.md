...
- **Shadow Engine Failover** — Runs an active/passive engine pair on the same node so a shadow engine can take over locally after an engine failure. It does not preserve in-flight requests or KV cache state.
