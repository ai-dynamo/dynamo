...
Without RDMA, disaggregated inference falls back to TCP with severe performance degradation (~98s TTFT vs ~200-500ms with RDMA). See [RDMA Setup](overview.md) for the transport overview.
...
<Step title="Apply the NicClusterPolicy" id="step-2-apply-the-nicclusterpolicy">
...
<Step title="Install the GPU Operator (RDMA-Enabled)" id="step-5-install-the-gpu-operator-rdma-enabled">
