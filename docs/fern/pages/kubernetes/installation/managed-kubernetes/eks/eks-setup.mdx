...
Before deploying an inference graph, download the model weights onto the shared EFS file system. The maintained [Kubernetes examples](https://github.com/ai-dynamo/dynamo/tree/main/examples/deployments/kubernetes) include a `model-download.yaml` Job manifest that downloads the model from Hugging Face.

From a Dynamo checkout, copy the chosen example's download manifest into the local Kustomize directory and apply it:

```bash
# Example: cache the Qwen3-32B-FP8 model used by the vLLM disaggregated example
cp examples/deployments/kubernetes/llm/vllm/disaggregated-qwen3-32b-fp8/model-download.yaml manifests/model-download/model-download.yaml
...
The example manifest sets a resource request for the download container. The `kustomization.yaml` in `manifests/model-download/` can add or raise a memory request when the selected model needs more headroom.
