...
The frontend uses HTTP probes on the `http` port, which defaults to `8000`:
...
The frontend startup probe allows five minutes for the sequential NATS and etcd connection waits and initialization. Older runtimes and images whose runtime version cannot be resolved keep the defaults without a startup probe. If you raise `NATS_STARTUP_CONNECT_TIMEOUT_SECONDS` or `ETCD_STARTUP_CONNECT_TIMEOUT_SECONDS`, raise the startup probe allowance to cover the combined connection windows and initialization. See [Runtime Configuration](../../reference/components/runtime-configuration.mdx) for these timeouts.

Workers use the system port, which defaults to `9090`:
