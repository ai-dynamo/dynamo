...
default exporter, and its Prometheus exporter does not default to `19090` — that port is Dynamo's
convention. The operator presets both variables on worker, prefill, and decode pods, so on an
operator-managed deployment the snippet above is enough. Set all three explicitly anywhere else.
