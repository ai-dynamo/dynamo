...
`NIXL_TELEMETRY_EXPORTER=prometheus` and an explicit `NIXL_TELEMETRY_PROMETHEUS_PORT`: NIXL has no
default exporter, and its Prometheus exporter defaults to `9090`, not to Dynamo's `19090`
convention. Leaving the port unset therefore lands the exporter on the same `9090` the
`DYN_SYSTEM_PORT` preset already uses. The operator presets both variables on worker, prefill, and
decode pods, so on an operator-managed deployment the snippet above is enough. Set all three
