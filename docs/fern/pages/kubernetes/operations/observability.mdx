...
`NIXL_TELEMETRY_PROMETHEUS_PORT` is optional and chooses the port: unset, the exporter falls back to
`9090` rather than to Dynamo's `19090` convention, which is the same `9090` the `DYN_SYSTEM_PORT`
preset already uses — so set it anyway to keep the two apart. The operator presets both variables on
worker, prefill, and decode pods, so on an operator-managed deployment the snippet above is enough.
Set all three explicitly anywhere else.
...
declares each one as a separate container port (`nixl`, `nixl-1`, …).
...
(`dynamo.metrics.podMonitors.nixlTelemetry`), because every worker pod declares the `nixl`
