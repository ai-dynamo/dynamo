...
- **An unachievable SLA is handled per search strategy, and SLA targets are never relaxed.** With `searchStrategy: rapid`, profiling fails and no DGD is generated when no AIC experiment returns an SLA-feasible configuration. With `searchStrategy: thorough`, the profiler selects the closest measured configuration. When the selected configuration misses a target, the profiler logs a warning naming the best achievable latency, but the generated DGD and planner config always carry the SLA values you submitted. See [SLA Cannot Be Met](../../developer-guide/knowledge-base/modular-components/profiler/profiler-guide.md#sla-cannot-be-met) in the Profiler guide.
