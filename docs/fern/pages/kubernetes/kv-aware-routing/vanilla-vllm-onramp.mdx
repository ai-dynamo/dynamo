...
| `DYN_EPP_SESSION_AFFINITY_TTL_SECS` | No | Pin each `x-dynamo-session-id` to the worker that served it for this many seconds after its last request; the binding replicates to peer EPPs over the replica mesh. Unset disables session affinity. |
