...
optional replay. If serving siblings exist but every dump attempt fails, startup fails closed and
the pod exits — Kubernetes restarts it for another attempt rather than ever serving with stale
state.
