...
- Two standalone EPP replicas behind a shared gRPC and peer-discovery Service. Replica traffic uses
  the sibling Pod IPs discovered through that Service's `EndpointSlice` resources.
        ...
        - name: epp
          ...
          env:
            ...
            - name: DYN_EPP_PEER_SERVICE
              value: dynamo-epp
            - name: DYN_EPP_SELECTION_HTTP_PORT
              value: "9093"
            - name: POD_IP
              valueFrom:
                fieldRef:
...
configuration. `DYN_EPP_SELECTION_HTTP_PORT` enables startup KV-index recovery from a serving
sibling; omit it only while rolling out an image that does not yet expose the dump endpoint.
...
events so active-load accounting converges across replicas. Setting `DYN_EPP_SELECTION_HTTP_PORT`
also exposes `GET /dump` on that Pod port and enables startup KV-index recovery.
...
flowchart LR
    ...
    ReplicaA <-->|"startup GET /dump :9093"| ReplicaB
```

After worker discovery starts, a joining or restarted EPP first waits for each worker KV-event
listener to subscribe and buffer. It then fetches a serving sibling's `GET /dump`, applies that
snapshot, drains the buffered events, and only then becomes ready. This preserves the baseline plus
all later worker changes without requiring a second streaming protocol. EndpointSlice updates keep
the active recovery request alive while updating the remaining candidates.

If no serving sibling exists, the EPP bootstraps an empty index and continues from worker events and
optional replay. If serving siblings exist but their dump endpoints are temporarily unreachable,
the EPP remains `NOT_SERVING` and retries with bounded backoff rather than serving with stale state.

For an upgrade from an image without `GET /dump`, first roll out the new image with
`DYN_EPP_SELECTION_HTTP_PORT` unset. Replica lifecycle synchronization remains active and recovery is
skipped. After every replica runs the new image, set the port and perform the normal rolling update.
For consistency details, see
[Standalone Selection Service](../../developer-guide/knowledge-base/modular-components/router/standalone-selection.md).

For a single EPP replica, set `spec.replicas: 1` and remove `DYN_EPP_PEER_SERVICE`,
`DYN_EPP_SELECTION_HTTP_PORT`, and `POD_IP`.
...
| `DYN_EPP_DATA_PARALLEL_SIZE` | No | Data-parallel ranks per pod; defaults to `1`, and `1` is the only accepted value because the EPP cannot convey a selected rank to the pod. |
| `DYN_EPP_KV_EVENT_PORT_STRIDE` | No | Port distance between consecutive ranks' KV event ports; defaults to `1`. Unused while `DYN_EPP_DATA_PARALLEL_SIZE` is fixed at `1`. |
...
| `DYN_EPP_SESSION_AFFINITY_TTL_SECS` | No | Pin each `x-dynamo-session-id` to the worker that served it for this many seconds (1 to 31,536,000) after its last request; the binding replicates to peer EPPs over the replica mesh. Unset disables session affinity. |
...
| `DYN_EPP_SELECTION_HTTP_PORT` | No | Pod HTTP port used to serve and fetch `GET /dump`. When unset, replica sync remains enabled but startup KV-index recovery is skipped. |
| `DYN_KV_RECOVERY_HTTP_TIMEOUT_MS` | No | Timeout for one peer `/dump` fetch; defaults to `30000` ms. |
| `DYN_KV_RECOVERY_MAX_DUMP_BYTES` | No | Maximum accepted `/dump` body; defaults to `536870912` bytes (512 MiB). |
...
| EPP remains `NOT_SERVING` during recovery | Confirm `DYN_EPP_SELECTION_HTTP_PORT` matches every replica, inspect serving endpoints in the peer Service's `EndpointSlice` resources, and verify Pod-to-Pod access to that port. |
