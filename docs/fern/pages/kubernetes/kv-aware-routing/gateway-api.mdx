...
<Warning>
**Go EPP deprecation.** The `eppConfig` block below configures the Go-based EPP, which Dynamo no
longer ships as the default. New EPP components should omit `eppConfig` and use the native Rust EPP
(bundled in the FrontEnd image, first available at `dynamo-frontend:1.5.0`) instead — see
[Migrate from the Go EPP to the Rust EPP](#migrate-from-the-go-epp-to-the-rust-epp) for the Rust EPP
equivalent of this example. Existing DGDs that already set `eppConfig` keep the legacy Go EPP Pod
contract across operator upgrades until you migrate explicitly; that section also covers the
supported blue/green and in-place migration paths, including readiness and rollback checkpoints.

**Availability.** The native Rust EPP path (an EPP component with no `eppConfig`) requires the
Dynamo Operator release that ships runtime-version-aware EPP validation, targeted alongside the
`dynamo-frontend:1.5.0` image. On Operator releases before that, every EPP component — regardless
of image tag — must set `eppConfig`; the validating webhook rejects an EPP component that omits it
with `spec.eppConfig: Required value: is required for EPP components`. Confirm your installed
Operator supports the Rust EPP path before following the migration steps below.
</Warning>
...
sidecars, and workers on the same Dynamo release line. A legacy Go EPP deployment is the exception:
keep its EPP image pinned to the 1.4 release line for as long as `eppConfig` remains set. The EPP
block size must match the backend block size.
...
<Warning>
**Requires an Operator that supports the native Rust EPP path.** Upgrading only the Dynamo
Operator does not require this migration: the Operator supports existing and newly created legacy
DGDs for as long as `eppConfig` remains set and the EPP image stays pinned to the 1.4 release line.
Do not update only one side of that pair — removing `eppConfig` and switching the image to 1.5 or
later must be one intentional DGD update, and admission rejects either mixed combination. Before
following the steps below, confirm your installed Operator actually ships this behavior; on
Operator releases that only support the legacy Go EPP contract, every EPP component must set
`eppConfig` regardless of image tag, and step 3's "delete the `eppConfig` block" is rejected by
admission.
</Warning>
...
<Warning>
If any copied component sets `globalDynamoNamespace: true`, remove that field (or set it to
`false`) on every component in `$NEW_DGD_MANIFEST`. `globalDynamoNamespace: true` places the
component in the shared global Dynamo namespace instead of the namespace derived from the DGD
name, so leaving it set makes `$NEW_DGD_NAME`'s workers join the *same* worker set as
`$OLD_DGD_NAME` instead of an isolated blue/green pool — the new EPP would then select from a pool
that also contains old-DGD workers. If your deployment relies on `globalDynamoNamespace: true` for
cross-DGD worker sharing, blue/green is not a safe migration path; use the in-place procedure
instead.
</Warning>
...
2. Apply it. This is the moment the EPP Pod rolls — there is no readiness gate before it, unlike
   ...
   ```bash
   kubectl apply -n "$NAMESPACE" -f "$DGD_MANIFEST"
   kubectl rollout status deployment/${DGD_NAME}-epp -n "$NAMESPACE" --timeout=1800s
   ```
