...
For compatibility with v1.4 policy files, deprecated `worker_selection.default` fills every worker type that has no explicit selection. Do not combine it with `worker_selection.aggregated`. New policy files should name each intended worker type explicitly.
...
Standalone EPP supplies `WorkerType::Aggregated` because it selects from one full-request worker pool. It uses `worker_selection.aggregated` and logs a startup warning when it ignores a prefill, decode, or encode selection.
