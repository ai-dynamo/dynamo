...
With `--router-session-affinity-mode soft`, `affinity_target()` returns the session target that the request host resolved before selection. The value remains fixed for every policy callback in one selection attempt.
...
The default `hard` session-affinity mode follows the same exact-target path and does not invoke policy callbacks for an existing binding.
