...
<Warning>
**Experimental.** A custom worker-selection policy controls how Dynamo filters and scores eligible workers, then selects one. It can also attach a queue-admission policy that defers and wakes host-owned requests. Dynamo still owns discovery, eligibility, queue storage, reservations, accounting, and metrics.
</Warning>
...
### Control Queue Admission

Implement `QueueAdmissionPolicy` when a request must wait for policy state rather than only worker capacity. `admit` returns `Bypass`, `Ready`, or `Defer`. Dynamo retains deferred requests and serially delivers `Dispatched`, `Completed`, `Aborted`, and `Reconcile` events through `on_event`. Append a previously deferred `QueueAdmissionId` to the `ready` vector to make that request runnable.

Attach one admission-policy instance to each factory-created worker-selection policy:
...
The compile-checked [session admission example](https://github.com/ai-dynamo/dynamo/blob/c9051aa8a924acd9ef94f0725a86b69ba8ca4fe2/examples/router/custom-policy-example/simple-filter-score-pick/src/admission.rs) allows one active request per `session_id`, defers later requests from the same session, and wakes the next request after either `Completed` or `Aborted`. Requests without session metadata bypass that example policy. The host still applies worker eligibility, overload checks, built-in queue ordering, and reservation accounting after the policy wakes a request.

Use `POST /select_and_reserve` for standalone selection with queue admission. The two-step `POST /select` then `POST /reservations` flow cannot preserve admission ownership and is rejected when an admission policy is installed. Report successful requests with `DELETE /reservations/{selection_id}` and failed requests with `POST /reservations/{selection_id}/abort`.
...
- Treat every admitted request as a lifecycle: release policy state on both `Completed` and `Aborted`.
- Retain only policy metadata. Dynamo owns queued requests and wakes deferred IDs supplied through `on_event`.
