...
| `priority_jump()` | Scheduler priority boost. The synthetic fallback queue, used when no configured policy profile applies to this model, treats negative values as zero; a configured policy class ignores it |
| `strict_priority()` | Strict integer priority. The synthetic fallback queue, used when no configured policy profile applies to this model, orders larger values first; a configured policy class ignores it |
