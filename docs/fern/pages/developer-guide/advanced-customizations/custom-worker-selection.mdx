...
Standalone EPP supplies `select` as `worker_type` because it selects from one worker pool. It uses `worker_selection.default`; the prefill and decode selections apply only to the embedded frontend. A standalone EPP logs a startup warning when it ignores either stage-specific setting.
