...
<Step title="Implement an optional filter">

Use a filter only for a hard requirement. This example keeps workers with at least the configured number of device-resident overlap blocks:
...
Pass filters to `WorkerSelectionPolicy::new_with_filters`. Use `WorkerSelectionPolicy::new` when the policy has no filters.
...
The [filter-score-pick example](https://github.com/ai-dynamo/dynamo/blob/main/examples/router/custom-policy-example/simple-filter-score-pick/src/lib.rs) uses `input_trigger()` to give tool-result turns a cache-local picker path.
