...
`WorkerCandidate::preferred_taint_multiplier()` and `ScoredWorkerCandidate::preferred_taint_multiplier()` are ordinary candidate metadata: they return the optional cost multiplier from preferred routing constraints and are always available without declaring `WorkerInputs`. Required routing constraints remain host-enforced eligibility rules.
