...
The `type` selects a registered provider. The `name` identifies one configured instance. `worker_selection.aggregated` selects full-request and public-surface encode worker pools. `worker_selection.prefill` and `worker_selection.decode` select the matching disaggregated worker pools. An omitted role uses Dynamo's built-in policy; one role does not fall back to another role's selection.
