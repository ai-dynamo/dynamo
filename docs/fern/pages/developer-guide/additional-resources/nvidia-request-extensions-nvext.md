...
| `extra_fields` | `string[]` | `None` | Response builder | Fields to include in the response `nvext`. Supported: `"worker_id"`, `"timing"`, `"routed_experts"`, `"engine_data"`, `"custom_encoder"`, `"stop_reason"`, `"completion_token_ids"`, `"prompt_logprobs"`. |
...
| `custom_encoder` | `extra_fields: ["custom_encoder"]` | Ordered per-image JSON metadata produced by an in-process [custom vision encoder](../advanced-customizations/custom-vision-encoders.md#return-response-metadata). Dynamo rejects requests that select this field and include tools. |
