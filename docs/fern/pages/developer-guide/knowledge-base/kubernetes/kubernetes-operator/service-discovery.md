...
| **KV Store (etcd)** | No | etcd cluster | Deprecated on Kubernetes; supported local, bare-metal, or legacy deployments |
...
> [!WARNING]
> etcd discovery is deprecated for Kubernetes deployments. Use Kubernetes-native
> discovery for operator-managed workloads. etcd remains available for supported
> local, bare-metal, and legacy deployments.
...
Do not run etcd anonymously on a shared network. Dynamo can authenticate with
either `ETCD_AUTH_USERNAME` and `ETCD_AUTH_PASSWORD`, or with mutual TLS using
`ETCD_AUTH_CA`, `ETCD_AUTH_CLIENT_CERT`, and `ETCD_AUTH_CLIENT_KEY`. Configure
the etcd server to require the selected method, encrypt the connection, and
authorize clients only for the Dynamo key prefixes they need.

For the deployment trust model and hardening checklist, see
