...
## Verifying on Mixed Device Plugin and DRA Clusters
...
temporary workaround is enough to verify the feature: reserve the occupied GPU
indexes with a blocker ResourceClaim before deploying the GMS workload, so the
...
with the CEL expression adjusted to the occupied indexes on your node, then
