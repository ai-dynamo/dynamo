...
scraping a worker never changes how it is scheduled. KV-event publication is
configured independently with `--publish-kv-events`, and the Planner's
forward-pass metrics with `--fpm-trace` or `DYN_FORWARDPASS_METRIC_PORT`, as on
the other backends. Use both publishing flags when a worker must publish both
event and metric telemetry. For backward compatibility, the deprecated
