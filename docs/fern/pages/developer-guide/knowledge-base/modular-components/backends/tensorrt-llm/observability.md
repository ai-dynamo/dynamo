...
- **Engine Configuration**: `LlmArgs.return_perf_metrics` and `enable_iter_perf_stats` default to `False`. `return_perf_metrics` enables TensorRT-LLM's detailed per-step timing collector, whose `time_breakdown_metrics` payload Dynamo never reads and which costs measurable throughput. Set either option explicitly in `--extra-engine-args` or `--override-engine-args` when custom worker instrumentation needs it.
