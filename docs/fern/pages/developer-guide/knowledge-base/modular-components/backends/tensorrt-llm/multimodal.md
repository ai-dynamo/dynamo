...
TRT-LLM supports aggregated and traditional disaggregated patterns. See [Multimodal Model Serving](../../../../../use-cases/multimodal-serving/overview.md) for detailed explanations.
...
- **No video serve-level test coverage** - Video input is implemented and unit-tested,
  but there is no end-to-end serve test for it on this backend (the vLLM backend has
  one). Validate video changes against a real deployment.
