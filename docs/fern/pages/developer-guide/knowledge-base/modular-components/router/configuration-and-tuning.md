...
During a rolling upgrade, update every frontend in the default `hard` mode before enabling `soft`; pre-revision and current frontends do not reliably converge on each other's session migrations.
