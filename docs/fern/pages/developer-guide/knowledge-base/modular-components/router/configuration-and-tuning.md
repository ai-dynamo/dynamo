...
- `--router-prefill-continue`: **Experimental.** Lets a prefill worker generate the whole response instead of handing off, when the decode pool has no room for it. This is the opposite direction to conditional disaggregation: that one moves prefill work onto a decode worker, this one keeps decode work on a prefill worker. Default off. Requires `--router-mode kv` on **both** the prefill and decode sets, because the load signals that trigger it exist only under KV routing. Use `--router-prefill-continue-config` for the settings; `max_concurrent` is required whenever the feature is enabled, because it is the only bound that can see a continuation that is already running.
