...
Every replica in a worker set must advertise the same routing configuration. A worker set is defined by namespace, component, endpoint, model, and worker type. Mixed router settings split the set into conflicting cohorts, so Dynamo admits no instances from that set: the model is withdrawn until the replicas agree again. Dynamo reports the disagreement at `ERROR` on the frontend, naming the model, the worker set, how many cohorts and workers disagree, and a sample of at most three deployment-card checksums with each one's size and a representative instance.

> [!WARNING]
> The model disappears from `/v1/models` for as long as the disagreement lasts, and it disappears on every frontend replica that observes it, not only the one that logged the line. Treat the log line as the signal: restate the configuration so every replica in the set advertises the same settings, then drain the replicas of that set that still disagree. A worker set whose replicas differ only by a supported release version does not split — the deployment-card checksum normalizes the router configuration a frontend actually serves with, so a rolling upgrade within the supported window keeps the model registered.
