...
Every replica in a worker set must advertise the same routing configuration. A worker set is defined by namespace, component, endpoint, model, and worker type. Mixed router settings split the set into cohorts that disagree about the model. Dynamo admits one cohort so the model stays served. The largest cohort wins, and equal-sized cohorts are settled by a stable tie-break on the deployment-card checksum. The winner depends only on the workers a frontend observes, never on what that frontend served earlier, so every frontend replica watching the same set admits the same cohort. The refused cohorts are logged at `ERROR` with their sizes and a sample of their checksums, and their instances receive no traffic until the disagreement is resolved.

> [!WARNING]
> A refused cohort is idle, not failed, so the worker set serves at reduced capacity with no drop in health. Election does not fail over: if the elected cohort cannot be materialized, the frontend retries it under backoff rather than promoting a refused cohort, so the model can stay unserved while other cohorts are healthy. Treat the refusal log line as the signal: restate the configuration so every replica in the set advertises the same settings, then drain the replicas of that set that still disagree.
