...
[Routing Concepts](routing-concepts.md). For guidance on choosing a named
configuration, see [Router Presets](router-presets.md).
...
- `--load-aware`: Preset for load-aware KV routing without cache-reuse signals. On the frontend, it implies `--router-mode kv`. It sets `overlap_score_credit=0`, disables KV events and KV reuse assumptions, enables active-block and prefill-token load tracking, disables remote/shared cache indexers, and preserves `--router-prefill-load-scale`, `--router-host-cache-hit-weight`, and `--router-disk-cache-hit-weight`. See [Router Presets](router-presets.md#load-aware).
- `--agentic`: **Experimental.** Preset optimized for long-running aggregate agentic workflows where repeated prefixes make cache colocation preferable. On the frontend, it implies `--router-mode kv`. It sets `overlap_score_credit=2`, `prefill_load_scale=4`, and `decode_active_request_weight=64`. See [Router Presets](router-presets.md#agentic).
...
Use `--agentic` for long-running aggregate agentic workflows where repeated prefixes make cache colocation preferable. It strengthens the cache-overlap credit while increasing the cost of uncached prefill work and active decode requests. The preset was validated with 16-token router blocks; benchmark representative traffic before using it with another block size, model, runtime, hardware, or topology. See [Router Presets](router-presets.md#agentic) for its exact settings and scope.
