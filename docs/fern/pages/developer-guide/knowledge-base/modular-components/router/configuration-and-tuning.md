...
Every replica in a worker set must advertise the same routing configuration. A worker set is defined by namespace, component, endpoint, model, and worker type. Mixed router settings split the set into cohorts that disagree about the model. Dynamo admits one cohort so the model stays served. A cohort that already serves the model keeps it, even when a larger cohort joins later. Otherwise the largest cohort wins, and equal-sized cohorts are settled by a stable tie-break that every frontend replica resolves the same way. The refused cohorts and their instances are logged at `ERROR`, and those instances receive no traffic until the disagreement is resolved.

> [!WARNING]
> A refused cohort is idle, not failed, so the worker set serves at reduced capacity with no drop in health. Treat the refusal log line as the signal: restate the configuration so every replica in the set advertises the same settings, then drain the replicas that still disagree.
