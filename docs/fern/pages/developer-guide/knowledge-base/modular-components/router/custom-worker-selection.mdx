...
flowchart LR
    ...
    Filters --> Scorers["WorkerScorer.score<br/>all surviving workers, one cost per worker"]
...
An external policy owns its filters, scorers, and picker. Dynamo's default scorer and picker implement the same public traits as external policies.
...
Dynamo calls `score` once per scorer after filtering. The `WorkerCandidates` view borrows a fixed request snapshot; its order is unspecified and it cannot be retained. Use `iter()` to visit workers, `get(row)` to read one worker, and `len()` or `is_empty()` to inspect the batch. Each returned `WorkerCandidate` borrows the stored row and exposes only that scorer's declared inputs. Creating the view does not copy the worker table. The output slice has the same length and starts with NaN in every slot. Write every cost; an unwritten slot or a non-finite contribution or total stops selection before picking or booking. Empty sets skip scoring. Any preparation helper belongs to the scorer implementation and runs inside its `score` call.
...
Rust embedders using `dynamo-llm` start with the host's `router_plugin_registry()`, which includes Dynamo's default policy. Register the custom catalog before resolving the configuration. Plugin authors and frontend embedders do not need a direct dependency on the default policy's implementation crate:
...
Worker identity and ordinary request metadata are always available. Request-level cache context requires `WorkerInputs::CACHE`. Dynamo calculates optional per-worker signals only for groups that a filter, scorer, or picker requests.

The combined `WorkerInputs` declarations also control cache setup. Without `CACHE`, the router creates no routing indexer, KV event subscription, or shared-cache client. With `CACHE`, cache settings choose event-backed, approximate, or remote indexing. Worker role, conditional disaggregation, and scoring weights do not enable or disable this setup. `serve_indexer` and `enable_session_prefix_index` require a policy that declares `CACHE`; incompatible configurations fail before cache resources start.
...
| `cache()` | With `CACHE`, returns a borrowed `RequestCacheInput`; otherwise returns `None` |
...
Each filter, scorer, and picker must declare `WorkerInputs::CACHE` in its own `required_worker_inputs()` to access `context.cache()`. Another component requesting cache does not enable it. The returned view exposes:

| Accessor | Meaning |
|---|---|
| `has_tier_matches()` | Whether the lookup contains any GPU, CPU, or disk matches before filtering; `false` means only accounting estimates or no cache data were supplied |
| `shared_hits()` | Optional unweighted shared-cache ranges from the current lookup, with `total_hits` and `hits_beyond(prefix)` |

A declared cache view exists even when no shared-cache result was supplied; in that case, `shared_hits()` returns `None`. The host borrows the existing lookup snapshot. Reading the view adds no cache lookup, range copy, or allocation.

Calculate shared-cache credit inside the policy from `shared_hits()` and the worker's cache data. For example, call `hits_beyond(device_prefix)` to count shared blocks beyond the GPU prefix. The default policy uses its accounting estimate as the prefix when tier matches are absent. It performs this calculation only when shared-cache credit is enabled.
...
| `CACHE` | `accounting_cache_estimate()` | Host accounting estimate as weighted KV blocks and rounded tokens; not clamped to prompt length |
...
A filter or scorer reads requested groups through `WorkerCandidate::cache()` or `load()`. A picker reads index-aligned arrays through `WorkerInputView`. Each component must request every optional group that it reads. Candidate and context views enforce that declaration independently for every filter, scorer, and picker, including preferred-taint access. The host materializes the combined requirements once and shares borrowed views; it does not copy or rewrite every row for each scorer.
...
Routing hosts install Dynamo's default policy automatically, even without a custom catalog. An omitted policy or the reserved `default` selection uses that policy. Optional catalogs register named providers without replacing the default or existing classifiers.

Applications that construct the low-level `SelectionServiceBuilder` directly must supply a registry with a default factory or an explicit policy factory. A missing factory fails construction before workers or synchronization resources start. Each routing partition receives its own policy instance.

Resolving `RouterPlugins` from the host registry includes the default factory even when no named policy is selected. `RouterPlugins::default()` remains an empty builder input. Use `has_custom_worker_selection()` to detect an explicit named or programmatic policy, and `has_custom_plugins()` to detect that policy or a classifier. Factory presence alone does not imply a custom selection. These checks preserve stock frontend modes and avoid waiting for a typed model card solely because the builtin factory is present. A classifier-only configuration still uses the builtin default for worker selection.
