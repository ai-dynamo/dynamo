...
The same catalog can install one process-wide request classifier. A classifier observes each request before queue admission and may override its policy class, due time, scheduling cost, or worker-selection target; Dynamo keeps final selection authority. Implement `RequestClassifier` in the policy crate, expose a provider that validates its parameters and returns a `RequestClassifierFactory`, and register it from the catalog's `register_request_classifiers`:
...
The type `default` is reserved: omit `request_classifier` to keep Dynamo's pass-through behavior. The key is honored by the processes that construct the standard KV chooser — the Python frontend with HTTP input and the direct Python `KvRouter` — and is ignored by the standalone EPP, which only selects workers.
