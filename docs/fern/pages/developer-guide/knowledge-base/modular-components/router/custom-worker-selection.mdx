...
pub fn register_request_classifiers(
    ...
    registry.register("acme-classifier", Arc::new(provider))
}
```

The provider receives the YAML `parameters` mapping and returns a factory; the factory is invoked once per routed model with a `RequestClassifierContext` carrying the router block size and a non-blocking snapshot of live workers with their published KV-block capacity. Select the classifier in the same YAML file:
...
The type `default` is reserved: omit `request_classifier` to keep Dynamo's pass-through behavior. Dynamo calls `register_request_classifiers` on every catalog build, so an existing custom catalog must export the function even when its registry stays empty.
