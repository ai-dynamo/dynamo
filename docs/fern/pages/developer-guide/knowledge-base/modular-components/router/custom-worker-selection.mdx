...
| `affinity_target()` | Optional worker and data-parallel rank from soft session affinity |
...
To retain an eligible target without duplicating affinity logic in the picker, opt into scheduler
candidate narrowing when you build the policy:
...
.with_exclusive_affinity_target(true)
```

The default is `false`. When enabled, Dynamo checks discovery, availability, overload, request
allowlists, routing constraints, and the target's data-parallel rank before narrowing. An ineligible
target leaves the normal candidate set intact. Policy filters still run after narrowing, so rejecting
the sole narrowed candidate returns the policy's usual all-filtered error.
