...
An explicit non-default `worker_selection.encode` selection also configures the surface-less multimodal encoder hop. Dynamo constructs its configured scheduler lazily when the encode target becomes active. Without an encode override, that hop keeps its legacy round-robin behavior.
...
| `exact_target()` | Explicit request target when present, otherwise the bound affinity target. A worker-only target keeps `dp_rank` unset |
| `device_aware()` | Request-level required cache-hit count and non-CPU-to-CPU ratio when the picker requests `DEVICE_AWARE` |
...
| `OCCUPANCY` | `active_requests()` | Atomically booked routing occupancy for the worker-rank row. Complete device-cache hits can be excluded by the picker admission result |
| `DEVICE_AWARE` | `device()` | CPU or accelerator device class reported by discovery |
| `DEVICE_AWARE` | `cache_hits()` | This worker's hits for the request's multimodal embedding-cache keys |
...
`OCCUPANCY` is intentionally distinct from `LOAD.active_requests()`: it is the reservation signal
used by the non-KV P2C and least-loaded algorithms. The scheduler serializes selection and booking,
then releases the booking through its existing response-stream lifecycle. `DEVICE_AWARE` is
capability-gated; Dynamo builds request cache keys and per-worker device/cache data only when the
selected policy requests it. A device-aware picker can override `occupancy_admission()` to exclude
a complete cache hit while preserving the ordinary request lifecycle.

Pickers that implement exact routing can use `exact_target()` and opt into
`uses_exclusive_affinity_target()` plus `requires_exact_target()`. The host then rejects a missing
target before queueing. A picker that intentionally resolves a worker-only exact target to one of
its data-parallel ranks can also opt into `resolves_worker_only_target()`; existing policies keep
the host's explicit-rank requirement by default. `supports_lora()` defaults to `true` for existing
custom policies; return `false` when the policy cannot preserve the configured KV scheduler's LoRA
filtering contract.
