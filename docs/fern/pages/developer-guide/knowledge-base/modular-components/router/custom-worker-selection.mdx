...
Rust embedders can use the same setup. From the embedding application's directory, add the builtin policy crate from the same Dynamo checkout:

```bash
cargo add --path "$DYNAMO_DIR/lib/router-plugins/builtin" dynamo-custom-policy-builtin
```

Start with `default_registry()` so omitted worker roles use the builtin policy, then register the custom catalog:

```rust
use dynamo_custom_policy_builtin::default_registry;
...
The default worker selector lives in `dynamo-custom-policy-builtin`, beside the two-tier policy. Routing hosts install its default factory once, even without a custom catalog. An omitted policy or the reserved `default` selection uses that factory. Optional catalogs register named providers without replacing the default or existing classifiers.

A Rust embedding that constructs `SelectionServiceBuilder` must supply a registry with a default factory, such as `default_registry()`, or an explicit policy factory. A missing factory fails construction before workers or synchronization resources start. Each routing partition receives its own policy instance.
