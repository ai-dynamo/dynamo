...
Call this function from the catalog. Every catalog must also export `register_request_classifiers`, even when it registers none — Dynamo calls both functions at startup:
...
The same catalog can install request classifiers. Like worker selection, the classifier is selected once at frontend startup from the frontend's policy YAML; the factory then creates one classifier instance per routed model. A classifier observes each request before the decode or aggregated router's queue admission and may override its policy class, due time, scheduling cost, or worker-selection target; Dynamo keeps final selection authority, a pinned request ignores the target, and the prefill router in a disaggregated deployment is not classified. Implement `RequestClassifier` in the policy crate, expose a provider that validates its parameters and returns a `RequestClassifierFactory`, and register it from the catalog's `register_request_classifiers`:

```rust
use std::sync::Arc;

use dynamo_kv_router::scheduling::{
    RequestClassifier, RequestClassifierFactory, RequestClassifierParameters,
    RequestClassifierProviderError, RequestClassifierRegistry, RequestClassifierRegistryError,
};

#[derive(serde::Deserialize)]
#[serde(deny_unknown_fields)]
struct AcmeParameters {
    pause_threshold: f64,
}

fn acme_classifier_provider(
    parameters: &RequestClassifierParameters,
) -> Result<RequestClassifierFactory, RequestClassifierProviderError> {
    let parameters: AcmeParameters = parameters.deserialize()?;
    Ok(Arc::new(move |context| {
        Box::new(AcmeClassifier::new(parameters.pause_threshold, context))
    }))
}
...
pub fn register_request_classifiers(
    ...
    registry.register("acme-classifier", Arc::new(acme_classifier_provider))
}
```

The provider receives the YAML `parameters` mapping; the factory is invoked once per routed model with a `RequestClassifierContext` carrying the router block size and a non-blocking snapshot of live workers with their published KV-block capacity. Select the classifier in the same YAML file:

```yaml
request_classifier:
  type: acme-classifier
  parameters:
    pause_threshold: 0.9
```

The type `default` is reserved: omit `request_classifier` to keep Dynamo's pass-through behavior. The key is honored by the processes that construct the standard KV chooser — the Python frontend with HTTP input and the direct Python `KvRouter` — and is ignored by the standalone EPP, which only selects workers. Every policy class the classifier names must exist in each routed model's effective queue profile: a worker-advertised `router_policy_config` replaces the frontend's queue profile for that model, and a classifier that names a missing class fails those requests at admission.
