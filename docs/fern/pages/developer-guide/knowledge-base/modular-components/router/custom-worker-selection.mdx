...
Sequence hashes retain the host's tracking configuration. When KV reuse is disabled, they are randomized identities rather than reusable content hashes. Public and keyed hashes must stay within their respective tracking domains and configurations; matching values do not establish physical KV residency or a safe shared-budget discount. This accessor adds no hashing or scheduling overrides. Preparing its snapshot copies the supplied sequence once per classification, so memory and copy cost scale with the number of tracked blocks. Requests without a classifier do not take this copy.
