...
Dynamo calls `score` once per scorer after filtering. The candidate slice borrows a fixed request snapshot; its order is unspecified and it cannot be retained. The output slice has the same length and starts with NaN in every slot. Write every cost; an unwritten slot or a non-finite contribution or total stops selection before picking or booking. Empty sets skip scoring. Any preparation helper belongs to the scorer implementation and runs inside its `score` call.
...
| `has_tier_matches()` | With `CACHE`, returns `Some(true)` if the lookup contains any tier-specific matches before filtering, or `Some(false)` otherwise; without `CACHE`, returns `None` |
| `shared_cache_hits()` | With `CACHE`, returns the optional unweighted shared-cache ranges from the current lookup; without `CACHE`, returns `None` |
...
Each filter, scorer, and picker must declare `WorkerInputs::CACHE` in its own `required_worker_inputs()` to read the two cache context accessors. Another component requesting cache does not enable them. The host borrows the existing lookup snapshot; reading these accessors adds no cache lookup or range copy.
