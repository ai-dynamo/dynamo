...
description: "How Dynamo combines load estimation, deterministic placement, request filtering, and min-cost flow to reduce LoRA routing churn and improve adapter-cache hit rate."
...
In a corrected simulation of 100 LoRAs competing for 32 resident slots, the full MCF configuration reduced routing-target churn by **98.1% to 98.9%** and modeled adapter-cache churn by **34.7% to 43.0%** compared with a stateless random baseline across four traffic patterns. Adapter hit rate increased by **17.5 to 23.8 percentage points**. The distinction between routing churn and physical adapter churn matters: MCF produced much more stable desired placements than HRW, but the additional cache-level gain over HRW was smaller.
...
Three related states appear in a multi-LoRA deployment:
...
Static pinning prevents scatter but cannot respond when one training run becomes hot. The controller computes each active LoRA's share of recent demand and turns that share into a replica count. It caps the count at the worker count and normalizes all replica counts to the cluster's reported slot budget with the largest remainder method.
...
3. Each worker has a capacity-bounded least recently used (LRU) adapter cache. A request to a worker without the adapter records a load; exceeding four resident adapters records an eviction.
...
These are deterministic, single-seed simulation results from commit `fa604bbe`. They validate relative software behavior under a controlled residency model. They do not measure GPU load latency, Time To First Token (TTFT), throughput, or end-to-end training time.
...
The following table averages the four workload totals. Percentages are cumulative unless the cell explicitly names the preceding row.

| Configuration | Mean Route Churn | Mean Adapter Churn | Mean Hit Rate | Improvement Stack |
|---|---:|---:|---:|---|
...
| Workload | Route Churn Reduction | Adapter Churn Reduction | Hit-Rate Increase | MCF Route Reduction vs HRW | MCF Adapter Reduction vs HRW |
|---|---:|---:|---:|---:|---:|
| Zipf and Poisson | 98.8% | 37.9% | +19.7 points | 68.8% | 3.1% |
| Daily | 98.6% | 38.9% | +20.8 points | 66.6% | 3.6% |
| Traffic spikes | 98.9% | 34.7% | +17.5 points | 71.3% | 1.4% |
| MMPP | 98.1% | 43.0% | +23.8 points | 49.1% | 2.3% |
...
Random and HRW share the same load estimator, demand-derived replica budget, production request filter, and synthetic cache model in this experiment. The measured delta therefore isolates stable, capacity-aware HRW placement; it does not independently quantify load estimation or filtering. A hardware benchmark still needs to translate the modeled cache reduction into TTFT and throughput.
...
Mean occupancy stayed between 98.8% and 99.8%. Every workload asked a 32-slot cluster to serve a 100-adapter catalog, and the MCF solver reported soft overflow throughout the runs. Under that pressure, every algorithm recorded adapter movement on nearly every tick.
...
HRW is the default allocation algorithm when LoRA support and allocation are enabled. Select MCF through the frontend environment when global churn minimization is worth the additional controller work:

```bash
export DYN_LORA_ENABLED=true
export DYN_LORA_ALLOCATION_ENABLED=true
...
Monitor `dynamo_frontend_lora_replica_factor`, `dynamo_frontend_lora_estimated_load`, `dynamo_frontend_lora_active_requests`, and the LoRA churn and overflow metrics. Use the [LoRA adapter operations guide](../../kubernetes/operations/lora-adapters.md) for lifecycle configuration and the [router operations guide](../../developer-guide/knowledge-base/modular-components/router/router-operations.md) for frontend replica synchronization behavior.
