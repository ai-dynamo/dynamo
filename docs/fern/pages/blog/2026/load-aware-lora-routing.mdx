...
These are deterministic, single-seed simulation results from [`fa604bbe`](https://github.com/ai-dynamo/dynamo/commit/fa604bbe099d3c685db7d7c9c836db3c08e369df). Reproduce them with `cargo test -p dynamo-llm --test lora_simulation -- test_export_csv --ignored --nocapture`, then inspect `target/lora_sim_csv/v2/summary.csv`. They validate relative software behavior under a controlled residency model. They do not measure GPU load latency, Time To First Token (TTFT), throughput, or end-to-end training time.
