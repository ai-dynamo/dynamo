...
before implementation when a change is over 100 core lines, spans multiple areas, changes a public
API, or adds a dependency. Core lines means changed lines of code, excluding tests, documentation,
and generated files. Wait for the `approved-for-pr` label before investing in a substantial
implementation.

Trusted Contributors are exempt from the size trigger. A Contribution Request is still required for
the structural ones: multiple areas, a public API, or a new dependency.

Architectural changes additionally require a
[Dynamo Enhancement Proposal (DEP)](enhancement-proposals.md), which agrees the design before the
code is written. That guide lists what counts as architectural.
...
<Steps toc={true}>
  ...
  <Step title="Complete automated review and CI">
    ...
    For external contributions, a maintainer approves CI for the pull request's current head, either
    by commenting `/ok to test COMMIT-ID` or by updating the branch. Each new push creates a new head
    and needs approval again, so wait for a maintainer rather than repeatedly pushing unchanged
    commits. Trusted Contributors receive automatic CI approval on every head.

    <Info>
    You are responsible for understanding and validating all submitted code, including code produced
    with AI assistance. Disclose substantial AI assistance in the pull request description; fully
    automated submissions are not accepted.
    </Info>
  </Step>

  <Step title="Request and address review">
    GitHub auto-requests the [CODEOWNERS](https://github.com/ai-dynamo/dynamo/blob/main/CODEOWNERS)
    team that owns each file you touched. Add the maintainer who approved your Contribution Request
    if they were not auto-requested.

    Every pull request needs approval from at least two maintainers other than the author. These are
    human reviews: AI-assisted review is a supplemental signal and does not count toward the
    threshold.
