...
<ParamField path="X-Dynamo-Parent-Session-ID" type="string">
  Parent session when using subagents. Values up to 256 bytes map to `agent_context.parent_session_id` when they differ from the session ID.
</ParamField>
...
<Tabs>
  ...
  <Tab title="Codex">
    ...
    When session affinity is enabled, each thread has its own affinity key. KV-aware routing can still select workers with shared prefix blocks.
