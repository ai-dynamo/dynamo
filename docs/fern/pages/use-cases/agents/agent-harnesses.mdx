...
<Tabs>
  ...
  <Tab title="Omnigent">
    ...
    Omnigent is an open-source meta-harness from the Databricks AI team and Neon. The experimental pinned path wraps Codex, configures a generic gateway provider with Dynamo's `/v1` Responses endpoint, and isolates Omnigent, Codex, and uv state from the user's normal configuration:
    ...
    The helper requires Omnigent commit `733234c303af7254597f99b14bda058878d3e8ca` and a clean checkout. Codex sends streaming `/v1/responses` requests with `thread-id`; Dynamo maps each value to normalized `agent_context.session_id`. Omnigent can also launch a background title turn with its own thread, which is an independent routable session.

    This path is compatible with stock Dynamo routing but is not ThunderAgent lifecycle-qualified. The audited Omnigent shutdown stops its local host, runner, tmux, and Codex processes but does not send `x-dynamo-session-final`. See the repository skill's `OMNIGENT_COMPATIBILITY.md` for the evidence and ownership boundary.
