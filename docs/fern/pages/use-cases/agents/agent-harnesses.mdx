...
<Tabs>
  ...
  <Tab title="DeepSeek Harness">
    ...
    DeepSeek Harness (DSH) uses Chat Completions. Its shipped headless profile accepts the Dynamo `/v1` endpoint through `DEEPSEEK_BASE_URL`, while its model selection lives in the DSH settings profile. The repository's lifecycle relay pins the package, creates that isolated profile, passes a minimal environment, captures redacted request evidence with exclusive creation, and runs one task. It reads `DYNAMO_API_KEY` only and ignores an ambient `DEEPSEEK_API_KEY`:

    ```bash
    node .agents/skills/dynamo-agent-harness/scripts/drive_deepseek_harness.mjs \
      --base-url http://localhost:8000 \
      --model Qwen/Qwen3-0.6B \
      --task 'Use tools to inspect this workspace and report one verified fact.' \
      --capture dsh-request-trace.jsonl
    ```

    Dynamo maps DSH's `x-deepseek-harness-session-id` to `session_id` and exact `x-deepseek-harness-compact: 1` to compaction metadata when DSH identity wins header precedence. The recipe uses only the published DSH package; it does not claim immediate parent lineage.

    Add `--canonicalize-dynamo-headers` only for an older server that predates native DSH normalization. The relay preserves native headers and duplicates session identity into the canonical Dynamo header; the option does not backport compaction normalization.
...
Use the repository's `.agents/skills/dynamo-agent-harness` skill when an agent needs to keep a Claude Code, Codex, or OpenCode session open across multiple prompts, or run a one-shot DSH headless task, while Dynamo serves the model. The persistent drivers use Agent Client Protocol (ACP). The DSH driver uses its native Chat Completions transport through a loopback capture relay and an isolated generated profile. DSH's headless command intentionally accepts one top-level task and exits after flushing it; a future persistent DSH path should host the separately published `@deepseek-ai/dsh-acp` server and keep its stdio connection alive for repeated prompts.
