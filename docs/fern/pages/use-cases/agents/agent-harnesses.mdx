...
| Harness | Signal recognized by Dynamo | Request-trace field |
| --- | --- | --- |
| Codex | `x-codex-turn-metadata` with `request_kind` set to `compaction` | `request_end.agent_context.compaction` |
...
Compaction stays in frontend pipeline context for request tracing. It is not serialized in `PreprocessedRequest.agent_context` or sent to workers.
