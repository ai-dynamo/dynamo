...
Use the repository's [`dynamo-agent-harness` skill](https://github.com/ai-dynamo/dynamo/tree/main/.agents/skills/dynamo-agent-harness) when an agent needs to keep a Claude Code, Codex, or OpenCode session open across multiple prompts while Dynamo serves the model. The skill uses Agent Client Protocol (ACP) and injects the provider configuration through the child process environment instead of adding harness configuration files to the delegated worktree.
...
After the command emits a `ready` JSON record, write one prompt object per line to the same process. Send `{"close":true}` to stop the session.
