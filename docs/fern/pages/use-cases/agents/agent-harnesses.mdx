...
<Tabs>
  ...
  <Tab title="DeepSeek Harness">
    ...
    Dynamo maps DSH's `x-deepseek-harness-session-id` to `session_id`, `x-deepseek-harness-parent-session-id` to `parent_session_id`, and exact `x-deepseek-harness-compact: 1` to compaction metadata when DSH identity wins header precedence. The published basic package carries session and compaction identity; use the separately pinned full recipe under `examples/agent_harnesses/deepseek_harness` when child lineage is required.

    Add `--canonicalize-dynamo-headers` only for an older server that predates native DSH normalization. The relay preserves native headers and duplicates session and parent identity into canonical Dynamo headers; the option does not backport compaction normalization.
