...
<Tabs>
  ...
  <Tab title="DeepSeek Harness">
    ...
    DeepSeek Harness (DSH) uses Chat Completions. Its shipped headless profile accepts the Dynamo `/v1` endpoint through `DEEPSEEK_BASE_URL`, while its model selection lives in the DSH settings profile. The repository's lifecycle relay pins the package, creates that isolated profile, captures redacted request evidence, and runs one task:
    ...
    Dynamo maps DSH's `x-deepseek-harness-session-id` to `session_id`, `x-deepseek-harness-parent-session-id` to `parent_session_id`, and exact `x-deepseek-harness-compact: 1` to compaction metadata. The published basic package carries session and compaction identity; use the separately pinned full recipe under `examples/agent_harnesses/deepseek_harness` when child lineage is required.

    Add `--session-final` only when the target is the native ThunderAgent frontend. The relay waits for DSH to flush, then sends `x-dynamo-session-final: true` after normal or interrupted shutdown. Stock KV has no program lifecycle to close, so leave the option off there.
...
| DeepSeek Harness | Exact `x-deepseek-harness-compact: 1` | `agent_context.compaction` |
