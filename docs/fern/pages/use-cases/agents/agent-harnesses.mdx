...
<Tabs>
  <Tab title="Codex">
    ...
    For ThunderAgent qualification, use the explicitly lifecycle-enabled command:
    ...
    After the ACP process closes, the runner attempts exactly one terminal request for the created Codex thread on normal completion, turn failure, or interruption. It skips finalization when no nonempty thread ID was created, and a terminal failure cannot silently pass the run. See the [Codex well-lit path](https://github.com/ai-dynamo/dynamo/blob/main/examples/agent_harnesses/codex/README.md) for the frozen dependency tuple, verification procedure, evidence contract, and cleanup steps.
