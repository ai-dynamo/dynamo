...
| Large SGLang `meta_info` | `nvext.metadata_upload` on `/v1/chat/completions`, `/v1/completions`, `/v1/responses`, or native `/generate` | Uploaded out of band by an RL-enabled SGLang worker. Set required `url` and optional `fallback_url`; the destination is trusted control input. |
