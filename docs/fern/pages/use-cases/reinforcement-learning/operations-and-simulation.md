...
Do not infer the join from request order, model name, prompt tokens, or wall-clock proximity. [NeMo RL PR #3856 at `2af6e1b`](https://github.com/NVIDIA-NeMo/RL/pull/3856) proposes forwarding stable direct-rollout and NeMo Gym session IDs to Dynamo, but it remains an open draft with focused CPU evidence and no functional or GPU run recorded in the PR. It is not part of the [`6ae03578` contract](https://github.com/NVIDIA-NeMo/RL/tree/6ae035784fe40fd9c9e31d27fffa4a403243a0bd), and forwarding a session ID would provide correlation context without enabling affinity unless the frontend is separately configured for it.
