...
Configure the frontend and aggregated vLLM worker with both feature switches. Configure the plugin and its scheme policy on the worker only:

```bash
export DYN_LORA_ENABLED=true
export DYN_LORA_RUNTIME_LOAD_ENABLED=true
export DYN_LORA_DOWNLOADER_PLUGIN=my_package.wandb_resolver:resolver
export DYN_LORA_ALLOWED_SCHEMES=wandb-artifact
python3 -m dynamo.vllm --model Qwen/Qwen3-0.6B --enable-lora
```

Scope `DYN_LORA_RUNTIME_LOAD_ENABLED` to the frontend and eligible aggregated vLLM worker processes. Do not place it in an environment block shared with SGLang, TensorRT-LLM, or disaggregated workers; those backends do not advertise protocol version 2 and continue to support only their existing explicit LoRA lifecycle.
...
Set proxy and client timeouts longer than `DYN_LORA_RESOLVE_TIMEOUT_SECONDS` plus the backend adapter-load and first-generation-event budgets. Otherwise, an intermediary can return an untyped timeout before Dynamo returns the resolver's structured error.
