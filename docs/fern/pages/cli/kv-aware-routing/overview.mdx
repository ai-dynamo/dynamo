...
`respect_soft_affinity: true` makes the two-tier picker retain a soft session-affinity target when
that target is among its eligible candidates. The default is `false`, which lets cache and load
choose among the full eligible set. An absent target falls back to normal two-tier selection; hard
affinity and explicit worker targets remain exact.
