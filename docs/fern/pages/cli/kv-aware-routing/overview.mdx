...
The YAML changes the worker-choice rule inside the configured `kv` routing path; it does not change
the top-level router mode. Aggregated, prefill, decode, and encode stages can select different
instances. An omitted stage uses Dynamo's default selector.

For the available policy types and their exact signals, affinity, occupancy, and data-parallel rank
behavior, see
