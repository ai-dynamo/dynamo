...
`respect_soft_affinity: true` keeps an eligible soft session-affinity target as the exclusive
candidate. The default is `false`, which leaves the target advisory. Ineligible soft targets fall
back to normal two-tier selection; hard affinity and explicit worker targets remain exact.
