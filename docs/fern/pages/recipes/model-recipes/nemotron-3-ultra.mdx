...
subtitle: "Serve nvidia/NVIDIA-Nemotron-3-Ultra-550B-A55B-NVFP4 with Dynamo and vLLM, including Day-0 and Refresh profiles."
---
...
The target picker covers the original Day-0 vLLM deployments of Nemotron-3-Ultra — NVIDIA's ~550B hybrid Mamba/Attention/MoE model (~55B active). The same recipe also includes Refresh profiles for B200, GB200, and H200 at 256K and 1M context lengths.
...
Refresh profiles include two-worker aggregated and 1P1D disaggregated agentic deployments with KV-aware routing, prefix caching, asynchronous scheduling, FP8 KV cache, BF16 Mamba state, decoupled weights, and Expert Parallelism disabled. Aggregated 256K profiles use five-token MTP; aggregated 1M and all 1P1D profiles run without MTP.

<Info>
The Refresh manifests use the pinned `vllm-runtime:1.4.0-rc.4` image digest shown in the source recipe. Review the image and cluster-specific placement before deployment.
</Info>

| GPU | Context | Topology | Worker shape | Reference concurrency | Manifest |
|---|---:|---|---:|---:|---|
| B200 | 256K | Aggregated | 2 × TP4 | 94 | `vllm/agg-b200-agentic-256k-2w-kv/deploy.yaml` |
| B200 | 1M | Aggregated | 2 × TP4 | 46 | `vllm/agg-b200-agentic-1m-2w-kv/deploy.yaml` |
| GB200 | 256K | Aggregated | 2 × TP4 | 96 | `vllm/agg-gb200-agentic-256k-2w-kv/deploy.yaml` |
| GB200 | 1M | Aggregated | 2 × TP4 | 48 | `vllm/agg-gb200-agentic-1m-2w-kv/deploy.yaml` |
| H200 | 256K | Aggregated | 2 × TP8 | 64 | `vllm/agg-h200-agentic-256k-2w-kv/deploy.yaml` |
| H200 | 1M | Aggregated | 2 × TP8 | 32 | `vllm/agg-h200-agentic-1m-2w-kv/deploy.yaml` |
| B200 | 256K | Disaggregated | 1P TP4 + 1D TP4 | 72 | `vllm/disagg-b200-agentic-256k-1p1d/deploy.yaml` |
| B200 | 1M | Disaggregated | 1P TP4 + 1D TP4 | 32 | `vllm/disagg-b200-agentic-1m-1p1d/deploy.yaml` |
| GB200 | 256K | Disaggregated | 1P TP4 + 1D TP4 | 64 | `vllm/disagg-gb200-agentic-256k-1p1d/deploy.yaml` |
| GB200 | 1M | Disaggregated | 1P TP4 + 1D TP4 | 28 | `vllm/disagg-gb200-agentic-1m-1p1d/deploy.yaml` |
| H200 | 256K | Disaggregated | 1P TP8 + 1D TP8 | 36 | `vllm/disagg-h200-agentic-256k-1p1d/deploy.yaml` |
| H200 | 1M | Disaggregated | 1P TP8 + 1D TP8 | 32 | `vllm/disagg-h200-agentic-1m-1p1d/deploy.yaml` |

The 256K 1P1D rows and the H200/B200 1M 1P1D rows are closed concurrency boundaries. GB200 1M concurrency 28 is the current complete SLA-passing anchor; the next boundary point remains to be rerun after an infrastructure interruption.

The B200 and GB200 256K 1P1D profiles use FlashInfer attention; the H200 256K profile uses FlashAttention. B200 and H200 1P1D profiles also require a scheduler-accessible GPU node pool that advertises one `rdma/ib` resource per GPU. If those pods remain pending even though suitable nodes exist, correct the namespace or queue placement instead of removing the qualified RDMA transport.

Deploy a Refresh profile by selecting the GPU and context length:
...
For a disaggregated profile, select the same GPU and context length:
...
All Refresh profiles share one benchmark Job and runner:

```bash
# Edit the env block for the selected target first.
kubectl apply \
  -f recipes/nemotron-3-ultra/perf/refresh-runner.configmap.yaml \
  -n ${NAMESPACE}
kubectl apply \
  -f recipes/nemotron-3-ultra/perf/refresh-perf.yaml \
  -n ${NAMESPACE}
```

See the [benchmark workflow](https://github.com/ai-dynamo/dynamo/blob/main/recipes/nemotron-3-ultra/perf/README.md) for the per-profile environment values and artifact layout.
...
- A `shared-model-cache` PVC containing the tokenizer-patched Ultra model view, or permission to create and populate it with the manifests in `model-cache/` (~1200 Gi). Weights are decoupled from the runtime image: they are downloaded and validated onto this PVC independently, not baked into the container.
...
```bash
kubectl apply -f recipes/nemotron-3-ultra/vllm/agg-b200-chat-256k-mtp/deploy.yaml -n ${NAMESPACE}
...
```bash
kubectl apply -f recipes/nemotron-3-ultra/vllm/agg-b200-agentic-256k-mtp/deploy.yaml -n ${NAMESPACE}
...
```bash
kubectl apply -f recipes/nemotron-3-ultra/vllm/disagg-b200-agentic-256k/deploy.yaml -n ${NAMESPACE}
...
```bash
kubectl apply -f recipes/nemotron-3-ultra/vllm/agg-h200-chat-256k-mtp/deploy.yaml -n ${NAMESPACE}
...
```bash
kubectl apply -f recipes/nemotron-3-ultra/vllm/agg-h200-agentic-256k-mtp/deploy.yaml -n ${NAMESPACE}
...
<tbody>
<tr data-sku="b200" data-usecase="chat"><td><code>vllm/agg-b200-chat-256k-mtp/deploy.yaml</code></td><td>B200</td><td>AGG</td><td>chat</td><td>yes</td><td>18</td><td>52.0</td><td>201.4</td></tr>
<tr data-sku="b200" data-usecase="chat"><td><code>vllm/agg-b200-chat-256k-nomtp/deploy.yaml</code></td><td>B200</td><td>AGG</td><td>chat</td><td>no</td><td>16</td><td>51.0</td><td>181.3</td></tr>
<tr data-sku="b200" data-usecase="agentic"><td><code>vllm/agg-b200-agentic-256k-mtp/deploy.yaml</code></td><td>B200</td><td>AGG</td><td>agentic</td><td>yes</td><td>20</td><td>80.6</td><td>310.8</td></tr>
<tr data-sku="b200" data-usecase="agentic"><td><code>vllm/agg-b200-agentic-256k-nomtp/deploy.yaml</code></td><td>B200</td><td>AGG</td><td>agentic</td><td>no</td><td>8</td><td>99.5</td><td>175.9</td></tr>
<tr data-sku="b200" data-usecase="agentic"><td><code>vllm/disagg-b200-agentic-256k/deploy.yaml</code></td><td>B200</td><td>1P1D</td><td>agentic</td><td>no</td><td>32</td><td>61.6</td><td>231.1</td></tr>
<tr data-sku="h200" data-usecase="chat"><td><code>vllm/agg-h200-chat-256k-mtp/deploy.yaml</code></td><td>H200</td><td>AGG</td><td>chat</td><td>yes</td><td>10</td><td>58.7</td><td>46.8</td></tr>
<tr data-sku="h200" data-usecase="chat"><td><code>vllm/agg-h200-chat-256k-nomtp/deploy.yaml</code></td><td>H200</td><td>AGG</td><td>chat</td><td>no</td><td>8</td><td>54.2</td><td>43.0</td></tr>
<tr data-sku="h200" data-usecase="agentic"><td><code>vllm/agg-h200-agentic-256k-mtp/deploy.yaml</code></td><td>H200</td><td>AGG</td><td>agentic</td><td>yes</td><td>8</td><td>53.2</td><td>27.4</td></tr>
<tr data-sku="h200" data-usecase="agentic"><td><code>vllm/agg-h200-agentic-256k-nomtp/deploy.yaml</code></td><td>H200</td><td>AGG</td><td>agentic</td><td>no</td><td>8</td><td>52.3</td><td>26.5</td></tr>
</tbody>
</table>
</div>
...
- No-MTP fallback manifests are included for every Day-0 aggregated target at `vllm/agg-<sku>-<usecase>-256k-nomtp/deploy.yaml`; their DGD names carry the `-nomtp` suffix, and their measured rows appear in the performance table above.
...
- For deterministic structured-output and required or named tool-call smoke tests, disable thinking with the nested `chat_template_kwargs` control. Strict JSON and tool-call envelopes are not yet qualified with MTP speculative decoding plus prefix caching in the pinned runtime; use a no-MTP Refresh profile for contract validation until that runtime compatibility issue is resolved.
...
- B200 chat + MTP: [vllm/agg-b200-chat-256k-mtp/deploy.yaml](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/vllm)
- B200 agentic + MTP: [vllm/agg-b200-agentic-256k-mtp/deploy.yaml](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/vllm)
- H200 chat + MTP: [vllm/agg-h200-chat-256k-mtp/deploy.yaml](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/vllm)
- H200 agentic + MTP: [vllm/agg-h200-agentic-256k-mtp/deploy.yaml](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/vllm)
- B200 disaggregated agentic: [vllm/disagg-b200-agentic-256k/deploy.yaml](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/vllm)
- Refresh profiles: [vllm/](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/vllm) (`agg-<sku>-agentic-<context>-2w-kv/deploy.yaml` and `disagg-<sku>-agentic-<context>-1p1d/deploy.yaml`)
- Refresh benchmark: [refresh-perf.yaml](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/perf) and [refresh-runner.configmap.yaml](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/perf)
