...
Refresh profiles include two-worker aggregated and selected 1P1D or 1P2D disaggregated agentic deployments with KV-aware routing, prefix caching, asynchronous scheduling, FP8 KV cache, BF16 Mamba state, decoupled weights, and Expert Parallelism disabled. Aggregated 256K profiles use five-token MTP; aggregated 1M and all disaggregated profiles run without MTP.

<Info>
The Refresh manifests use the released `vllm-runtime:1.4.0` image pinned by digest in the source recipe. Review the image and cluster-specific placement before deployment.
</Info>
...
| B200 | 256K | Disaggregated | 1P TP4 + 2D TP4 | 144 | `vllm/disagg-b200-agentic-256k-1p2d/deploy.yaml` |
| B200 | 1M | Disaggregated | 1P TP4 + 1D TP4 | 38 | `vllm/disagg-b200-agentic-1m-1p1d/deploy.yaml` |
| GB200 | 256K | Disaggregated | 1P TP4 + 2D TP4 | 140 | `vllm/disagg-gb200-agentic-256k-1p2d/deploy.yaml` |
| GB200 | 1M | Disaggregated | 1P TP4 + 1D TP4 | 48 | `vllm/disagg-gb200-agentic-1m-1p1d/deploy.yaml` |
| H200 | 256K | Disaggregated | 1P TP8 + 2D TP8 | 72 | `vllm/disagg-h200-agentic-256k-1p2d/deploy.yaml` |
| H200 | 1M | Disaggregated | 1P TP8 + 1D TP8 | 30 | `vllm/disagg-h200-agentic-1m-1p1d/deploy.yaml` |

The numeric disaggregated rows passed the complete exact-count trace and all three SLA gates. H200 1M selects CC30; CC32 is its first valid TTFT failure.

The B200 and GB200 256K 1P2D profiles use FlashInfer attention; the H200 256K 1P2D profile uses FlashAttention. GB200 disaggregated profiles use GKE multi-network MRDMA.
...
For a disaggregated profile, select the GPU, context length, and qualified topology:
...
- Refresh profiles: [vllm/](https://github.com/ai-dynamo/dynamo/tree/main/recipes/nemotron-3-ultra/vllm) (`agg-<sku>-agentic-<context>-2w-kv/deploy.yaml` and the selected `disagg-<sku>-agentic-<context>-1p1d|1p2d/deploy.yaml`)
