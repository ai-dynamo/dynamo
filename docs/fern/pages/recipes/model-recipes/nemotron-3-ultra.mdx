...
`System output tok/s/GPU` is aggregate output TPS divided by GPU count. All twelve profiles below
met their TTFT and tok/s/user SLA.
...
Disaggregation trades TTFT for throughput and tail latency headroom differently by SKU and context:
256K disaggregated profiles trail their aggregated counterpart on system output tok/s/GPU (prefill
and decode run on separate workers instead of sharing GPUs), while 1M disaggregated profiles are
roughly on par with or ahead of aggregated. Pick a topology based on which metric your workload
needs to hold, not on tok/s/GPU alone.
