...
<span><b>GPUs</b> 1x H200 per replica, `replicas: 2`</span>
...
Deploy `replicas: 2` or more so the KV-aware router has replicas to route shared prefixes across. A full 8x H200 node runs `replicas: 8`.
...
|--------|------|-------------|-------------------|------------|-------------------------|
| Aggregated, 2x TP1 + MTP | 2 | 8 | 136.3 | 760 ms | 381.3 |
...
|--------|----------|------|-------------|----------------|
| `agg-h200-agentic` | Aggregated | 2 | MTP, 3 tokens | c8, TTFT-bound |
| `disagg-h200-agentic` | Disaggregated 1P2D | 3 | None | c18, at the 50 tok/s/user floor |

Aggregated and disaggregated deliver near-identical total throughput (762 vs 769 output tok/s); disaggregated spends a third GPU on a dedicated prefill worker that emits no output tokens, which is why its per-GPU figure is lower. What disaggregation buys is capacity at the SLA — 18 concurrent users at the 50 tok/s/user floor, against roughly 8 for aggregated.
...
Aggregated cannot reach the 50 tok/s/user target on H200. FP8 weights leave roughly 20 GB for paged KV, capping the running batch near 8 requests, so added concurrency becomes queue delay rather than slower decode: 136 tok/s/user at c8 (TTFT 0.8 s), 112 at c14 (TTFT 9.2 s). Use disaggregated for that operating point.
