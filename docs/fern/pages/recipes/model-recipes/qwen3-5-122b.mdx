...
All four targets below are Dynamo + vLLM deployments of Alibaba's Qwen3.5-122B-A10B — a 122B total / 10B active hybrid MoE combining Gated DeltaNet linear attention with full attention every fourth layer — tuned for an agentic workload (64K median ISL / 400 median OSL, 90% KV cache hit) with KV-aware routing. B200 runs the NVFP4 checkpoint at TP1; H200 runs FP8, where the weights leave less room for paged KV, so the aggregated profile shards to TP2. Both aggregated profiles add MTP speculative decoding. Aggregated is the recommended path on both SKUs. Pick your target; every command on this page updates to match.
...
First cold starts take roughly 30 minutes while the runtime loads the weights and captures CUDA graphs.
...
|--------|------|-------------|-------------------|------------|-------------------------|
| B200 aggregated, 2x TP1 + MTP | 2 | 80 | 50.4 | 335 ms | 1806.4 |
...
|--------|-----|----------|------|-------------|----------------|
| `agg-b200-agentic` | B200 | Aggregated | 2 | MTP, 3 tokens | c80, at the 50 tok/s/user floor |
...
Aggregated leads on both SKUs — 1806.4 against 916.6 output tok/s per GPU on B200, and 720.3 against 256.5 on H200. Disaggregated remains supported for deployments that scale prefill and decode independently.
...
- Benchmark with the shipped `speculative-config` ConfigMap key. MTP measures an acceptance length of about 3.2 on this trace, so the `speculative-config-synthetic` override is not needed.
