...
See [fp8/perf/README.md](https://github.com/ai-dynamo/dynamo/blob/main/recipes/qwen3.5-122b/fp8/perf/README.md) for the full workflow.
...
Aggregated leads on both SKUs at the same operating point — 1173.2 against 916.6 output tok/s per GPU on B200, and 720.3 against 256.5 on H200. Disaggregated remains supported for deployments that scale prefill and decode independently.
...
TP2 is an H200 FP8 result. FP8 weights leave roughly 20 GB per card for paged KV at TP1, which caps the running batch near 8 requests; sharding to TP2 removes that cap. B200 NVFP4 is never KV-starved, so it ships at TP1.
