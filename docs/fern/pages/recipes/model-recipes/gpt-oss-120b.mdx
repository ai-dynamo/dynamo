...
| `CONCURRENCY` | `384` | Starting point only — use the measured per-variant value from the table above (B200 agg `512`, H200 agg `256`, both disagg `256`) to reproduce the published results. See the concurrency sweep below |
