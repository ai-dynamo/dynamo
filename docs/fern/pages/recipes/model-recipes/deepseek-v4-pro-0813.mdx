...
Partial run (93% of the trace) and **below the 50 tok/s/user gate**. The trace's unique KV working set is ~57.5M tokens against ~3.12M of H200 GPU KV — 18.4x oversubscribed — so measured prompt-cache read is 42.5% versus the ~94% the trace could offer. On a synthetic 64K / 400 profile whose working set fits in GPU KV, the same recipe reaches 25.0 tok/s/GPU at user output 53.0 (concurrency 4) and clears the gate.
...
| `agg-h200-agentic` | 8x H200 | Aggregated | 1,048,576 | Trace (partial) + synthetic |
