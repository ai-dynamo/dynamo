...
Not yet run on the shared trace. On a synthetic 64K / 400 / 90%-reuse profile the SLA crossing sits near concurrency 6, at 18.3–19.9 system tok/s/GPU and user output ~50 tok/s. Expect the same KV-oversubscription penalty as the aggregated H200 target when run on the trace.
...
| `disagg-h200-agentic` | 16x H200 | 1P1D | 1,048,576 | Synthetic only |
