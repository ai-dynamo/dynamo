...
- **8x B200** on a single node. Prefill and decode are co-located by a required pod affinity so the KV hop stays on one fabric.
...
```bash
kubectl apply -f recipes/k-exaone-2.0/vllm/agg-b200-chat/deploy.yaml -n ${NAMESPACE}
...
```bash
kubectl apply -f recipes/k-exaone-2.0/vllm/disagg-b200-chat/deploy.yaml -n ${NAMESPACE}
...
Mooncake chat trace replay, 1,805 requests. Concurrency 14 is this target's operating point — the highest concurrency meeting both legs of the gate. The concurrency 7 row is included only because it is where the aggregated target runs, so the two can be read side by side.

| Concurrency | tok/s/GPU | E2E tok/s/user | TTFT p50 | ITL |
| ----------- | --------- | -------------- | -------- | --- |
| **14**      | **85**    | 54.9           | 2,230 ms | 15.98 ms |
| 7           | 55        | 70.6           | 1,018 ms | 13.15 ms |

At its own operating point disaggregation is level with aggregation — 85 against 87 tok/s/GPU under the same gate — so it is an SLA and scaling choice for this model rather than a throughput win or loss. It buys per-token latency (ITL 15.98 ms against the aggregated recipe's 19.15 ms) and independent prefill/decode scaling, for twice the GPUs. At concurrency 7 the deployment is simply under-loaded, which is what the 55 reflects.
...
- [Aggregated manifest](https://github.com/ai-dynamo/dynamo/blob/main/recipes/k-exaone-2.0/vllm/agg-b200-chat/deploy.yaml)
- [Disaggregated manifest](https://github.com/ai-dynamo/dynamo/blob/main/recipes/k-exaone-2.0/vllm/disagg-b200-chat/deploy.yaml)
