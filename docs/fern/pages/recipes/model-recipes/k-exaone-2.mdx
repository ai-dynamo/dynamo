...
| Expert parallelism | ➖ | measured within noise at TP4; not enabled |
...
- **The fabric settings are pre-injected, and they are cluster-specific.** Disaggregation needs InfiniBand, so the published manifest is `deploy-ib.yaml` rather than `deploy-generic.yaml`: the fabric configuration is a Kustomize component (`kustomize/components/ib`) layered onto a base that carries none, so it is visible in one place instead of scattered through the manifest. Two values need checking against your cluster. The RDMA extended resource is requested as `rdma/shared_ib`; other clusters expose `rdma/ib` or `rdma/rdma_shared_device_a`, and a shared flavour is preferable — with an exclusive-mode resource, two co-located workers each claiming HCAs can deadlock NCCL bootstrap. `UCX_TLS` is set to `rc_x,rc,cuda_copy,cuda_ipc`, which are the InfiniBand verbs transports and are wrong on a RoCE or EFA fabric; `tcp` is omitted deliberately so a misconfigured fabric fails loudly instead of silently falling back to a slow transport. Verify the transport engaged before trusting any measurement:
...
- **KV-aware routing does nothing until you scale out — but scaling out is all it takes.** The frontend ships `--router-mode kv --router-kv-events` and the worker (prefill, in the disaggregated recipe) ships `--kv-events-config`, so the block index is published and consumed. Both halves are inert at `replicas: 1`, because one worker is one destination. Raising `replicas` is sufficient; no flag edit is needed. Measured on a 2 x TP4 deployment this workload sees no throughput gain from it — a 9.494% prefix hit rate against round-robin's 9.458% — because achieved reuse is capacity-capped, so it is shipped for correct scale-out behaviour rather than for throughput.
- **There is no cache-clearing endpoint.** `dynamo.vllm` runs the Dynamo worker rather than vLLM's OpenAI server, so `POST /reset_prefix_cache` is not served anywhere and `VLLM_SERVER_DEV_MODE` has no effect. To isolate benchmark points, use a fresh `cache_salt` per run — the shipped `perf.yaml` does — or restart the deployment between measurements.
