...
<span><b>KV transfer</b> Mooncake over the NVLink fabric</span>
<span><b>Context</b> Up to 1,048,576 tokens</span>
</div>
</div>
...
DeepSeek-V4.1-Flash is a Mixture-of-Experts model. SGLang resolves its attention, MoE, and GEMM backends from the checkpoint. Do not set those backend flags. A hand-set backend flag selects a slower fallback.

The recipe sets these values, and SGLang resolves the rest:

| Setting | Aggregated | Disaggregated |
|---|---|---|
| **Workers** | 2 | 1 prefill + 1 decode |
| **Parallelism** | TP4, EP4 | TP4, EP4 per role |
| **Static memory fraction** | 0.8 | 0.8 |
| **Maximum running requests** | 256 | 256 |
| **Page size** | 256, set | 256, resolved |
| **Speculation** | DSpark, block size 5 | None |
| **Decode CUDA graph batch** | 64 | 512, resolved |
| **KV transfer** | None | Mooncake over MNNVL |
| **Routing** | KV-aware | KV-aware |

SGLang resolves `attention_backend` to `dsv4`, `moe_runner_backend` to `flashinfer_mxfp4`, `kv_cache_dtype` to `fp8_e4m3`, and `grammar_backend` to `xgrammar`.
...
- Each worker needs TP4 and a GB200 node holds 4 GPUs. So prefill and decode always sit on different nodes, and KV crosses nodes. Mooncake carries KV over the NVLink fabric with an NVLINK memory pool and an IMEX channel. Without the ComputeDomain, KV falls back to TCP and runs about 80 times slower.
