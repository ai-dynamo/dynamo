...
- Each worker needs TP4 and a GB200 node holds 4 GPUs. So prefill and decode always sit on different nodes, and KV crosses nodes. Mooncake picks the transport. It uses TCP by default, and RDMA where the cluster attaches RDMA devices, as the GKE variant does.
- The NVLink fabric is the faster KV path, and this recipe does not take it. To opt in, set `SGLANG_MOONCAKE_CUSTOM_MEM_POOL=NVLINK` and `MC_FORCE_MNNVL=true` on both workers. KV then moved about 80 times faster in our measurement. Read the warning that follows first.
