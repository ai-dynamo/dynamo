...
# OR AWS EFA on p6-b200.48xlarge (1 EFA per GPU: 4 per decode worker, 2 per prefill worker)
...
Only recipe contributors update the checked-in derived artifacts. The source of truth is the matrix, `kustomize/base/`, the recipe-local Components, the shared template sources, and any referenced shared Components under `recipes/kustomize/components/`; the public overlay `kustomization.yaml` files, the generated template Components, and the `deploy-*.yaml` manifests are generated — commit them for review, but do not edit them by hand. After editing the matrix, base, Components, or templates, regenerate from the repository root:
