...
<ParamField path="--connector" type="string" default="null">
  KV cache connector for the TensorRT-LLM engine. Accepts at most one value: `kvbm` enables the [KVBM](../components/kvbm-configuration.mdx) integration, `none` disables it. When unset, no connector is configured. This flag is specific to TensorRT-LLM; other backends configure KV transfer through their own engine options.
  ...
  > [!WARNING]
  > KVBM is deprecated as of v1.5.0, with removal targeted for v1.6.0.
