...
<ParamField path="--publish-metrics" type="boolean" default="false">
  Expose TensorRT-LLM iteration and request metrics on this worker's Prometheus endpoint. This flag does not publish KV cache events. It enables `enable_iter_perf_stats` and the per-request `SamplingParams.return_perf_metrics`, but keeps the engine's `return_perf_metrics` disabled. The same iteration statistics carry the per-rank used-block count read by the frontend's optional busy-rejection thresholds; the KV router's worker selection does not depend on it.
