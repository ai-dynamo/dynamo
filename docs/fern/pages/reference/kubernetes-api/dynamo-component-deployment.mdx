...
<ParamField path="lpx" type="LPXConfig">
  ...
  Supported modes are LPU, hybrid, and LPU draft with LPU target. Every LPX graph requires exactly one explicit `conductor` role and template. A single LPX component declares both `conductor` and `agent` roles. In speculative decoding, the draft has only an `agent` role; the target declares both roles. Component names are arbitrary. All LPX components in a DGD share one LPX deployment and PodCliqueSet, separate from ordinary components.
</ParamField>
...
<ParamField path="roles" type="[]ComponentRoleSpec">
  ...
  LPX components use `agent` for Agent Pods and `conductor` for the serving runtime. Exactly one LPX component in the graph must declare a conductor role with an explicit template. In a speculative pair, the draft has only an agent role and the target declares both roles. A hybrid engine's GPU runtime uses the conductor template.
...
<Indent>
  ...
  <ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
    Native Pod template for this role. LPX-only; required for every declared role. Each template requires a `main` container with an explicit image. Agent and conductor templates are independent.

    For LP20 and LP30, the template owns runtime commands, static arguments and environment settings, init containers, health probes, and startup storage. It may use the image's entrypoint or supply `command`. The operator retains device-access security requirements and supplies generated configuration, model and hardware bindings, and `LPX_ALLOCATION` for the LPU conductor. A Nova template consumes that value through its authored `--allocation` argument. See [Deploy with DGD](../../kubernetes/model-deployment/deploy-with-dgd.md) for security-context overrides, role structure, and runtime examples.
