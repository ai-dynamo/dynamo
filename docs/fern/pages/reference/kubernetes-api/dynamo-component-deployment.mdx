...
DGD admission requires `runtimeVersionOverride` when the `main` image has no parseable semantic-version tag, including digest-only references and tags such as `latest` or a commit SHA. Also set it when a parseable tag represents another version, such as the inference engine's version. The override must describe the Dynamo runtime actually contained in the image.

Standalone non-EPP DCDs may omit the override for an unversioned image. EPP components require a resolvable runtime version even in a standalone DCD, so admission can enforce the version-dependent `eppConfig` rules. Existing components with an unresolved version can retain an unchanged image and override during unrelated updates; changing either revalidates the requirement.

Setting or changing an override that resolves to Dynamo 1.5.0 or later may trigger a rollout. The override does not select, replace, or upgrade the image.
