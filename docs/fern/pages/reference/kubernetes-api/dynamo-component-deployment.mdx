...
<ParamField path="lpx" type="LPXConfig">
  ...
  Supported modes are LPU, hybrid, and LPU draft with LPU target. Declare one complete component, or separate speculative draft and target components. The draft has only an `agent` role; the target has `conductor` and `agent` roles. Component names are arbitrary. All LPX components in a DGD share one LPX deployment and PodCliqueSet, separate from ordinary components.
</ParamField>
...
<ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
  ...
  For `type: lpx`, omit this field and use `roles[].podTemplate`. Each supplied LPX template requires an explicit `main` container image; see `roles` below for conductor-template fallback.
...
<ParamField path="roles" type="[]ComponentRoleSpec">
  ...
  LPX components use `agent` for Agent Pods and `conductor` for the conductor. With one LPX component, the conductor role is optional. In a speculative pair, the draft has only an agent role and the target declares both roles. Hybrid execution may also create Nova Pods from Agent configuration.
...
<Indent>
  ...
  <ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
    Native Pod template for this role. LPX-only; required for the agent. Each supplied template requires a `main` container with an explicit image. An omitted conductor template reuses the same component's agent template; a single LPX component also uses this fallback when its conductor role is omitted. A speculative pair uses the target's conductor. An explicit conductor template merges with shared defaults independently, without inheriting the agent template.
  ...
  <ParamField path="replicas" type="integer">
    ...
    For LPX, omitted conductor replicas use `1`, subject to runtime-specific limits. Agent replicas are derived from the model build; an explicit count must match the build.
  </ParamField>
