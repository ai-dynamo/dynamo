...
<ParamField path="lpx" type="LPXConfig">
  DGD-only compiled-model and scheduling configuration. Set `buildId`; placement comes from the build manifest. Runtime options belong in role pod templates. See [LPXConfig](full-api-reference.mdx#lpxconfig).
