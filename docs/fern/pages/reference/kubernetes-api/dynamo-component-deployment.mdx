...
<Indent>
  ...
  <ParamField path="identity" type="DynamoCheckpointIdentity" deprecated={true}>
    Deprecated: omit for DGD-managed checkpoints; no action is needed. Use `checkpointRef` to restore an existing PodSnapshot. Legacy checkpoint identity duplicated from `v1alpha1`; `DynamoCheckpoint` itself remains `v1alpha1`.
  </ParamField>
  <Indent>
