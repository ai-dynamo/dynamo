...
<ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
  Complete Pod template shared by all roles of the component. It is mutually exclusive with `roles[*].podTemplate`. The operator injects its defaults (image, command, env, ports, probes, resources, volume mounts) into the container named `main`, merging your overrides by name. Every other container is treated as a user-managed sidecar and receives no injected defaults — sidecars must specify their own required fields such as `image`. Replaces the ten separate per-component fields (`resources`, `envs`, `livenessProbe`, and so on) that existed in `v1alpha1`.
...
<ParamField path="roles" type="[]ComponentRoleSpec">
  Optional explicit structure of a compound component. In this release, roles are supported for multinode components and must contain exactly one `leader` and one `worker`. Their cardinality remains derived from `multinode.nodeCount`. Omit the field to retain the implicit leader/worker layout. If any role sets `podTemplate`, every role must set one and the component-level `podTemplate` must be absent.
</ParamField>
...
<Indent>
  ...
  <ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
    Complete Pod template for this role. Role templates do not inherit from each other or from the component-level template. The component remains the lifecycle, rollout, scaling, service, and status boundary.
...
<ParamField path="flagsInjection" type="FlagsInjectionMode" default="Automatic">
  Controls backend-specific multinode launch generation independently from Pod-template selection. `Automatic` preserves the existing generated commands and topology flags. `Manual` preserves the authored role commands and arguments, except that Dynamo still owns vLLM MP port `29500`, worker probe handling, and required Pod/provider wiring. Manual mode is valid only for multinode engine components and cannot be combined with GMS or failover.

  <span className="enum-values"><span className="enum-label">Allowed values:</span> <Badge intent="note" minimal>Automatic</Badge> <Badge intent="note" minimal>Manual</Badge></span>
</ParamField>
