...
<Indent>
  ...
  <ParamField path="replicas" type="integer">
    Optional number of Pods of this role in one complete component instance. For multinode, admission defaults an omitted value to `leader: 1` or `worker: multinode.nodeCount - 1`; an explicit value must match that fixed shape.
  </ParamField>
