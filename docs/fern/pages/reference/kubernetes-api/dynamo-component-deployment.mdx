...
<ParamField path="dynamoSidecar" type="string">
  **Experimental.** Names a container in `podTemplate.spec.initContainers` with `restartPolicy: Always` that runs the Dynamo runtime. Supported for `worker`, `prefill`, and `decode` components. The engine remains in `containers[name=main]`, which must specify an image.

  The operator injects runtime environment variables, discovery settings, system port and probes, namespace suffix, and topology configuration into the selected sidecar. The engine retains its launch command, GPU resources, shared memory, and compilation cache. Component overrides take precedence over defaults; graph-level environment variables apply to both containers.

  Multinode, enabled checkpointing, GPU memory service, and failover are unsupported in this mode. A regular `frontendSidecar` can coexist with the native sidecar. This field is available only in `v1beta1`; conversion preserves it through `v1alpha1` round trips.

  The runtime version is resolved from the selected sidecar image; use `runtimeVersionOverride` for a tag without a semantic version. The current vLLM sidecar requires temporary startup probe overrides until independent lifecycle support is available. See the [vLLM native-sidecar examples](https://github.com/ai-dynamo/dynamo/tree/main/lib/sidecar/vllm#deploy-on-kubernetes-quick-start).
</ParamField>
