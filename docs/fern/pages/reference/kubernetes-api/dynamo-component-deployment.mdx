...
<ParamField path="lpx" type="LPXConfig">
  DGD-only compiled-model configuration. Set `buildId`; see [LPXConfig](full-api-reference.mdx#lpxconfig) for the field definitions. LPU-only Single and SpecDecode accept only boolean placement `settings`: `prop_sync`, plus XT-only `cpu_embeddings` and `prop_sync_multiple_load_sets`. Runtime options belong in the conductor pod template.
</ParamField>
...
<Indent>
  ...
  <ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
    ...
    The template owns runtime startup and configuration for LP20 and LP30. See [Deploy with DGD](../../kubernetes/model-deployment/deploy-with-dgd.md) for Nova inputs and the operator's model-path, partition, and allocation bindings.
