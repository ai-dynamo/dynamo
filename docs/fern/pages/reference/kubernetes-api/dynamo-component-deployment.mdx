...
<ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
  ...
  For runtime 1.6.0 and later, frontend containers receive a startup probe that allows five minutes for NATS and etcd connections and initialization. The same default applies to a designated frontend sidecar, using the parent component's resolved runtime version. Older or unknown runtime versions do not receive this default. A user-specified startup probe replaces the default. A frontend sidecar startup probe must include exactly one handler (`exec`, `httpGet`, `tcpSocket`, or `grpc`). The operator rejects overrides that only change timing or define multiple handlers. See [Health Probes](../../kubernetes/operations/observability.mdx#check-and-customize-health-probes) for probe settings and timeout adjustments.
