...
<Indent>
  ...
  <ParamField path="replicas" type="integer">
    Optional assertion of the role's logical cardinality in one complete component instance. For multinode, an omitted value is derived as `leader: 1` or `worker: multinode.nodeCount - 1` without being persisted; an explicit value must match that fixed shape.
  </ParamField>
