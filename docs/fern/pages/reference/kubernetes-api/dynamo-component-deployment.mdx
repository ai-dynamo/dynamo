...
<ParamField path="lpx" type="LPXConfig">
  DGD-only compiled-model configuration. Set `buildId`; placement comes from the build manifest. Runtime options belong in role pod templates. See [LPXConfig](full-api-reference.mdx#lpxconfig).
</ParamField>
