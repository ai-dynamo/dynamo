...
<ParamField path="type" type="string">
  Role of the component within the graph. Drives port mapping, frontend detection, planner RBAC, and the pod label `nvidia.com/dynamo-component-type`. `prefill` and `decode` are first-class values for disaggregated serving and can be set directly. At most one component per graph may be `epp`. Standalone DCDs cannot use `lpx`.
...
<ParamField path="lpx" type="LPXConfig">
  DGD-only configuration for a `type: lpx` component; omitted from the standalone DCD schema. Set the required `buildId` and optional build `settings`. See [LPXConfig](full-api-reference.mdx#lpxconfig) for the full field set.

  Supported modes are LPU, hybrid, and LPU draft with LPU target. Declare one complete component, or separate speculative draft and target components. The draft has only a `worker` role; the target has `leader` and `worker` roles. Component names are arbitrary. All LPX components in a DGD share one LPX deployment and PodCliqueSet, separate from ordinary components.
</ParamField>
...
<ParamField path="replicas" type="integer">
  ...
  For a single `type: lpx` component in a DGD, this counts complete engine replicas, not individual role Pods. Hybrid engines support `1–9` fixed replicas; LPU-only requires `1`. In a speculative pair, draft replicas count model instances (`1–8`), and target replicas must be `1`. LPX does not support scaling adapters or scale-to-zero.
</ParamField>
...
<ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
  ...
  For `type: lpx`, omit this field and use `roles[].podTemplate`. Each supplied LPX template requires an explicit `main` container image; see `roles` below for leader-template fallback.
...
<ParamField path="roles" type="[]ComponentRoleSpec">
  Optional explicit structure of a compound component. Multinode components must contain exactly one `leader` and one `worker`. Omit the field to retain the implicit leader/worker layout.

  LPX components use `worker` for Agent Pods and `leader` for the conductor. With one LPX component, the leader role is optional. In a speculative pair, the draft has only a worker role and the target declares both roles. Hybrid execution may also create Nova Pods from Agent configuration.

  Standalone DCD roles accept `name` and `replicas`. See [ComponentRoleSpec](full-api-reference.mdx#componentrolespec).
</ParamField>

<Indent>
  <ParamField path="name" type="string" required={true}>
    Semantic role name, either `leader` or `worker`. Names must be unique within the component.
  </ParamField>
  <ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
    Native Pod template for this role. LPX-only; required for the worker. Each supplied template requires a `main` container with an explicit image. An omitted leader template reuses the same component's worker template; a single LPX component also uses this fallback when its leader role is omitted. A speculative pair uses the target's leader. An explicit leader template merges with shared defaults independently, without inheriting the worker template.
  ...
  <ParamField path="replicas" type="integer">
    ...
    For LPX, omitted leader replicas use `1`, subject to runtime-specific limits. Worker replicas are derived from the model build; an explicit count must match the build.
  </ParamField>
  <ParamField path="providerOverride" type="ProviderOverride">
    Provider configuration for the workload unit generated for this role. Available only on ordinary multinode components embedded in a DGD. LPX roles do not support it; standalone DCD OpenAPI omits this field.
  </ParamField>
</Indent>
