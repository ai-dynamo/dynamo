...
<Indent>
  ...
  <ParamField path="replicas" type="integer">
    Optional number of Pods of this role in one complete component instance. For multinode, omission derives `leader: 1` and `worker: multinode.nodeCount - 1`; an explicit value must match that fixed shape.
  </ParamField>
