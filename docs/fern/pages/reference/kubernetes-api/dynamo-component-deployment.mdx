...
<Indent>
  ...
  <ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
    ...
    For LP20 and LP30, the template owns runtime commands, static arguments and environment settings, init containers, health probes, startup storage, and Pod and container security contexts. It may use the image's entrypoint or supply `command`. The operator supplies generated configuration, model and hardware bindings, and `LPX_ALLOCATION` for the LPU conductor. A Nova template consumes that value through its authored `--allocation` argument. See [Deploy with DGD](../../kubernetes/model-deployment/deploy-with-dgd.md) for role structure and runtime examples.
