...
<Indent>
  ...
  <ParamField path="deletionPolicy" type="CheckpointDeletionPolicy" default="Delete">
    Whether a DGD-managed automatic checkpoint CR is deleted or retained when the owning DGD is deleted. Checkpoint artifact data remains for both policies. Explicit `checkpointRef` checkpoints are never owned or deleted by the DGD.
