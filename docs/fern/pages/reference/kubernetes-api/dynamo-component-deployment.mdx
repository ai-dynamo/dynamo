...
<ParamField path="podTemplate" type="core/v1.PodTemplateSpec">
  Pod template for the component's pods. Every component must include a container named `main` with a non-empty `image`. In standard mode the operator injects command, environment, port, probe, resource, and volume-mount defaults into `main`, merging your overrides by name. In Dynamo sidecar mode (`dynamoSidecar` is set), `main` is the engine container and is fully user-managed — the operator injects no Dynamo defaults into it. Existing components created without a main image may retain that omission on unrelated updates. For DGD components whose image has no parseable semantic-version tag, set `runtimeVersionOverride`. Every other container is treated as a user-managed sidecar and receives no injected defaults, unless it is the init container named by `dynamoSidecar`, which receives Dynamo runtime defaults. User-managed sidecars must specify their own required fields such as `image`. Replaces the ten separate per-component fields (`resources`, `envs`, `livenessProbe`, and so on) that existed in `v1alpha1`.
...
<ParamField path="dynamoSidecar" type="string">
  Designates a restartable init container (`restartPolicy: Always`) in `podTemplate.spec.initContainers` as the Dynamo runtime container, activating Dynamo sidecar mode. The value must match a container `name` in that list. In Dynamo sidecar mode the operator injects Dynamo defaults (env, identity, transport TLS, ports, startup, liveness, and readiness probes) into the named init container instead of `main`; `main` becomes the engine container and receives no Dynamo defaults. Dynamo sidecar mode is not compatible with multinode, checkpoint/restore, GPU memory service, or failover. The validation webhook rejects a value that matches no init container name in `podTemplate.spec.initContainers`.
</ParamField>
...
DGD admission requires `runtimeVersionOverride` when the runtime image has no parseable semantic-version tag. The runtime image is the init container named by `dynamoSidecar` in Dynamo sidecar mode, or `main` otherwise. Also set it when a parseable tag does not represent the Dynamo runtime version in the image. The override must describe the Dynamo runtime actually contained in the image.
