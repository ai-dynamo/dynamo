...
<Indent>
  ...
  <ParamField path="replicas" type="integer" required={true}>
    Pods of this role in one complete component instance. For multinode, `leader` must be `1`, `worker` must be `multinode.nodeCount - 1`, and the sum must equal `multinode.nodeCount`.
  </ParamField>
