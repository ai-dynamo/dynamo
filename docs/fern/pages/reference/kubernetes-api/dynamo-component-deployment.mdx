...
<Indent>
  <ParamField path="packDomain" type="string" required={true}>
    Topology domain to pack pods within. Must match a domain defined in the referenced `ClusterTopology`. When the parent DGD also sets `spec.topologyConstraint.packDomain`, this value must be narrower than or equal to it. The operator renders this as the Grove `topologyConstraint.pack.required` domain, which Grove documents as applying to each replica of the resource it is set on. Whether replicas added by a later replica-count change land in the domain the existing replicas already occupy is determined by Grove and the backend scheduler, not by this operator.
  </ParamField>
</Indent>
