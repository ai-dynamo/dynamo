...
<ParamField path="replicas" type="integer">
  ...
  When LPX serving-component `replicas` is omitted, the controller follows the live Grove `PodCliqueScalingGroup` (PCSG) replica count without writing its Scale subresource. This externally managed count is lost when the `PodCliqueSet` (PCS) or PCSG is replaced, including PCS replacement for immutable workload or compiler-provenance changes. The new group starts from the template's `minAvailable` seed (default `1`); the external scaler must reapply its desired capacity. Explicit DGD `replicas` are reapplied by the operator after replacement.
</ParamField>
