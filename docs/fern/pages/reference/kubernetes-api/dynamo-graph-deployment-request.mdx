...
For a specific DGDR, read the authoritative names from the selected configuration:
...
Set `spec.autoApply: false` when you want to inspect these names before the operator creates the
DGD. Names can differ in older Dynamo releases, so use `selectedConfig` when updating an existing
deployment.
