...
<ParamField path="min_endpoint" type="integer" default="1">
  Legacy shared minimum number of engine endpoints. Component-specific minimums inherit this value when unset. Aggregated mode always uses this field. Must be nonnegative; use `0` for scale-to-zero compatibility.
</ParamField>

<ParamField path="prefill_min_endpoint" type="integer | null" default="null">
  Minimum prefill endpoints. When unset, inherits `min_endpoint`. Supported in disaggregated and prefill-only modes. Must be at least `1` when set.
</ParamField>

<ParamField path="decode_min_endpoint" type="integer | null" default="null">
  Minimum decode endpoints. When unset, inherits `min_endpoint`. Supported in disaggregated and decode-only modes. Aggregated mode uses `min_endpoint`. Must be at least `1` when set.
</ParamField>
