...
<ParamField path="ais_perf_model" type="AISPerfModelSpec" default="null">
  ...
  interpolation sweep. The old `aic_perf_model` name and `hf_id` / pick shape are
  compatibility inputs; serialized output uses `ais_perf_model` only.
</ParamField>
...
<Indent>
  ...
  <ParamField path="roles.&lt;role&gt;.fallback_policy" type="string" default="deny">
    `deny` rejects failure of an explicitly selected mode. `allow` permits the
    remaining lower-priority modes; `regression` permits a regression fallback.
...
A parallelism pick emitted by AIConfigurator. Referenced by [AICInterpolationSpec](#aicinterpolationspec) and accepted when migrating legacy `features.planner.aic_perf_model` inputs.
