...
| `gpusPerReplica` _integer_ | GPUsPerReplica is the unique GPU allocation added when this service scales<br />by one replica, across all nodes, runtime containers, and provider-owned Pods.<br />Shared DRA claims are counted once. A present zero records a successful<br />non-GPU resolution; omission means no current shape is available. |  | Minimum: 0 <br />Optional: \{\} <br /> |
...
| `gpusPerReplica` _integer_ | gpusPerReplica is the unique GPU allocation added when this component<br />scales by one replica, across all nodes, runtime containers, and<br />provider-owned Pods. Shared DRA claims are counted once. A present zero<br />records a successful non-GPU resolution; omission means no current shape<br />is available. |  | Minimum: 0 <br />Optional: \{\} <br /> |
