...
<ParamField path="scheduling" type="SchedulingSpec">
  Configures scheduling attempts for `type: lpx` components. Omit this object for attempts without a deadline.
</ParamField>

<Indent>
  <ParamField path="attemptDeadlineSeconds" type="int64">
    Wall-clock limit for each pending LPU pipeline request, measured from the scheduler-published start of its current scheduling cycle until it reports `Bound`, `NoFit`, or `Unsupported`. The deadline is not armed until the scheduler publishes `status.schedulingStartedAt`, and a later scheduling cycle resets that timestamp. Valid values are `1` through `9,223,372,036`; omission means unlimited.
    Expiry reports `LPXSchedulingDeadlineExceeded` and deletes the expired request. When `replicas` is explicit, it also lowers the Grove replica count to the surviving prefix; externally managed capacity is left unchanged. A later LPX input revision authorizes another publication. This field is not a solver budget.
  </ParamField>
</Indent>
...
<ParamField path="status.placement" type="PlacementStatus">
  Public deployment-level placement summary. For LPX workloads, this field mirrors `status.lpx.placement` for compatibility with existing clients.
</ParamField>

<ParamField path="status.lpx" type="DynamoGraphDeploymentLPXStatus">
  LPX download and placement observations for the current configuration. Omitted when no current LPX result exists, including ordinary-only deployments. LPX component entries and aggregate readiness remain under `status.components`, `status.state`, and `status.conditions`.
</ParamField>

<Indent>
  <ParamField path="modelDownload" type="ModelDownloadStatus">
    Downloaded builds and the time they were last checked.
  </ParamField>
  <ParamField path="placement" type="PlacementStatus">
    Placement progress and scheduling-attempt details. The attempt's `observedGeneration` refers to this DGD's generation.
  </ParamField>
</Indent>
