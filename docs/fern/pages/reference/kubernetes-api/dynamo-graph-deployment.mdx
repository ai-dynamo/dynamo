...
<Indent>
  ...
  <ParamField path="packDomain" type="string">
    Default topology domain to pack pods within. Optional at this level; omit when only components carry constraints. The operator renders this as the Grove `topologyConstraint.pack.required` domain, which Grove documents as applying to each replica of the resource it is set on. Whether replicas added by a later replica-count change land in the domain the existing replicas already occupy is determined by Grove and the backend scheduler, not by this operator.
  </ParamField>
</Indent>
