...
<Indent>
  <ParamField path="attemptDeadlineSeconds" type="int64">
    Wall-clock lifetime of the aggregate LPX attempt, from the first request's API-server creation time through `Bound`, `NoFit`, or `Unsupported`. Valid values are `1` through `9,223,372,036`; omission means unlimited.
    Dynamo durably records the absolute deadline. Resumed scheduling keeps it; late terminal status loses to expiry. Expiry reports `LPXSchedulingDeadlineExceeded` and retires the exact attempt through LPX cleanup. It is neither a solver budget nor retry authorization.
  </ParamField>
</Indent>
