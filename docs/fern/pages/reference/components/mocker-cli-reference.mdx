...
<ParamField path="--num-gpu-blocks-override" type="integer" default="auto">
  Usable KV cache blocks per data-parallel rank. Non-AIS timing defaults to 16,384 blocks; AIS
...
Opt-in flags for the AISonfigurator (AIS) latency model. Live Mocker workers do not use AIS by
default.
...
<ParamField path="--ais-perf-model" type="boolean" default="false">
...
<ParamField path="--ais-system" type="string" default="h200_sxm when AIS is enabled">
...
<ParamField path="--ais-backend" type="string" default="--engine-type">
  Backend used for AIS performance-data lookup. Set it only to model timing for a different backend
  than the simulated scheduler.

  <span className="enum-values"><span className="enum-label">Allowed values:</span> <Badge intent="note" minimal>vllm</Badge> <Badge intent="note" minimal>sglang</Badge> <Badge intent="note" minimal>trtllm</Badge></span>
</ParamField>

<ParamField path="--ais-backend-version" type="string" default="auto">
  AIS performance-database version. Use `current`, `previous`, or `next` when the slot is available
...
<ParamField path="--ais-tp-size" type="integer" default="1">
  Tensor-parallel size for AIS latency prediction. Affects only AIS performance-model lookups, not
  mocker scheduling.
</ParamField>
...
<ParamField path="--ais-nextn" type="integer" default="null">
  Experimental Multi-Token Prediction (MTP) draft length, from 1 through 5.
</ParamField>

<ParamField path="--ais-nextn-accept-rates" type="string" default="null">
  Experimental comma-separated conditional MTP acceptance rates.
</ParamField>

<ParamField path="--ais-mtp-seed" type="integer" default="42">
