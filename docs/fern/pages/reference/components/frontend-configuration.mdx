...
The per-model concurrency gate covers HTTP inference request/response endpoints, including
`/inference/v1/generate`. It does not count `/v1/realtime` WebSocket sessions because their model
...
These gates are HTTP-only. KServe gRPC inference does not enforce them; gate settings are ignored
in KServe gRPC mode and produce a startup warning.
