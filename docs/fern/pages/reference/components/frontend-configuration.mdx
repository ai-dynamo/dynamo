...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/tokenize` | Token ids for a prompt or a chat conversation, without running inference |
| `POST` | `/detokenize` | Text for a list of token ids |

`/tokenize` takes either `prompt` or `messages`; the chat form renders the model's chat
template first. Both run the same preprocessor the generate path runs, so the reported
count is the count the model is sent. A field the frontend cannot apply returns 501 rather
than a count for a prompt it did not build.
...
| `DYN_HTTP_SVC_TOKENIZE_PATH` | `/tokenize` |
| `DYN_HTTP_SVC_DETOKENIZE_PATH` | `/detokenize` |
