...
<ParamField path="--load-aware / --no-load-aware" type="boolean" default="false">
  Preset for KV load-aware routing without cache-reuse signals; implies `--router-mode kv`. Incompatible with `--shared-cache-type mooncake-store`, which requires local GPU event learning.
