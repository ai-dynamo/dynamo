...
<ParamField path="--dyn-preprocess-workers" type="integer" default="auto for SGLang; 0 otherwise">
  ...
  where available CPUs account for process affinity and cgroup CPU quotas. Each model gets its own
  preprocessing worker pool. Set `0` to run preprocessing on the main event loop. Nonzero values are
  supported only with the SGLang processor.
