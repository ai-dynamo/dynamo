...
<ParamField path="--tokenizer-fallback" type="boolean" default="true">
  Fall back to HuggingFace when `fastokens` or `basetenkenizer` cannot load the model tokenizer. Set
  this option to `false` to fail during startup instead.
