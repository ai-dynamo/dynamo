...
<ParamField path="--router-decode-policy" type="string" default="null">
  Frontend-only worker-selection instance for decode workers and the single worker pool in an
  aggregated deployment. This option overrides `worker_selection.decode` from
  `--router-policy-config`. Set it to `default` to use Dynamo's built-in worker selector.
