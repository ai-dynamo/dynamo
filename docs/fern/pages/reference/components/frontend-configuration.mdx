...
The HTTP server checks its certificate and private key files for changes every
30 seconds and reloads valid updates automatically for subsequent TLS handshakes.
Existing connections remain open. The trusted client CA certificates are loaded
at startup; restart the frontend after changing the client CA file.
