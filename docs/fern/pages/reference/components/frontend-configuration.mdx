...
Most frontend latency and size histograms use log-spaced buckets, rounded to two significant figures, always starting with a `0` edge. Each histogram is tuned by three environment variables that share a prefix: `_MIN` and `_MAX` bound the range, and `_COUNT` is the total number of bucket edges **including** that leading `0`, so `COUNT - 1` edges are spread across the range. Every edge, including the top one, is rounded to two significant figures, so the highest exported edge is `MAX` rounded rather than `MAX` itself. These variables have no CLI equivalent, and the frontend reads them once while building its metrics, so changing one requires a restart.
...
| `dynamo_frontend_inter_token_latency_seconds` | `DYN_METRICS_ITL` | See below | See below | See below | `8` |
...
The inter-token latency histogram defaults to the same explicit boundaries as SGLang:

```text
0.002,0.004,0.006,0.008,0.010,0.015,0.020,0.025,0.030,0.035,0.040,0.060,0.080,0.100,0.200,0.400,0.600,0.800,1.000,2.000,4.000,6.000,8.000
```

Set `DYN_METRICS_ITL_BUCKETS` to a comma-separated, strictly increasing list of finite, non-negative boundaries. This explicit list takes precedence over `DYN_METRICS_ITL_MIN`, `DYN_METRICS_ITL_MAX`, and `DYN_METRICS_ITL_COUNT`. If the explicit list is invalid, the frontend warns and uses the default list. When `DYN_METRICS_ITL_BUCKETS` is unset, setting any of the three older variables continues to generate log-spaced buckets for compatibility.
