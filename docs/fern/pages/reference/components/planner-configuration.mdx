...
<ParamField path="min_endpoint" type="integer" default="1">
  Minimum engine endpoints (replicas) for aggregated deployments. In disaggregated deployments, this value applies to both prefill and decode unless a role-specific value is set. Must be nonnegative; use `0` for scale-to-zero compatibility.
</ParamField>
