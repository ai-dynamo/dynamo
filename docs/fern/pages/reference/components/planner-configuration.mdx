...
<ParamField path="min_endpoint" type="integer" default="1">
  Legacy shared minimum number of engine endpoints (replicas). In disaggregated and single-component modes, an unset component-specific minimum inherits this value. Aggregated mode always uses this field. Must be nonnegative; use `0` for scale-to-zero compatibility.
</ParamField>
