...
<ParamField path="min_endpoint" type="integer" default="1">
  Legacy shared minimum number of engine endpoints (replicas). In disaggregated and single-component modes, an unset component-specific minimum inherits this value. Aggregated mode always uses this field. Must be at least `1`.
</ParamField>

<ParamField path="prefill_min_endpoint" type="integer" default="null">
  Minimum prefill endpoints. When `null`, inherits `min_endpoint`. Supported in `disagg` and `prefill` modes. Must be at least `1` when set.
</ParamField>

<ParamField path="decode_min_endpoint" type="integer" default="null">
  Minimum decode endpoints. When `null`, inherits `min_endpoint`. Supported in `disagg` and `decode` modes. Must be at least `1` when set.
</ParamField>
...
PATCH is atomic. Malformed JSON, unknown fields, and empty patches return HTTP `400`. `null` or below-`1` values, fields that are inactive for the current mode, and minimums that exceed the configured GPU or power budget return HTTP `422` without changing the active configuration.
