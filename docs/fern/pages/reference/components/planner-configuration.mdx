...
A parallelism pick emitted by AIConfigurator. Referenced by `AICInterpolationSpec` and accepted when migrating legacy `aic_perf_model` inputs.
...
- When `ais_perf_model` is set, its `roles` must cover the deployment: `prefill` and `decode` for `disagg`, the matching role for `prefill`/`decode`, or `aggregated` for `agg`. Role keys must agree with `worker_type`. Unknown canonical fields and simultaneous `ais_perf_model` / `aic_perf_model` inputs are rejected.
