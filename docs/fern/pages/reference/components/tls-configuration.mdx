...
<Note>
  This page covers Dynamo's internal TCP request plane and NATS connections.
  Frontend HTTPS uses the separate `--tls-cert-path` and `--tls-key-path`
  options documented in [Frontend Configuration](frontend-configuration.mdx#client-authentication-and-exposure).
</Note>
...
(vLLM, SGLang, TensorRT-LLM, and TokenSpeed) through `DynamoRuntimeArgGroup`:
...
NATS carries event-plane messages such as KV events and router replica sync,
JetStream indexer recovery/replay, the audit sink, and—when the request plane is
set to NATS (`--request-plane nats`)—inference request distribution. TLS is
configured separately from TCP:
...
TLS encrypts the connection but does not by itself authorize NATS subjects.
Configure a NATS authentication method and server-side permissions as described
in [event-plane security](../../developer-guide/knowledge-base/concepts/system-architecture/architecture.md#event-plane-security).

## Trust Domain and Peer Verification

Production TLS clients must verify both parts of the server identity:

- The certificate chains to a trust root whose scope matches the intended
  service peers. Prefer a dedicated internal CA over a broad shared trust root.
- The certificate's DNS or IP Subject Alternative Name (SAN) matches the
  expected server identity. TCP clients verify the discovered host or IP by
  default. Set `DYN_TCP_TLS_SERVER_NAME` only when the certificate uses a
  different DNS SAN. NATS clients verify the hostname in `NATS_SERVER`.

For mTLS, configure `DYN_TCP_TLS_CLIENT_CA_CERT_PATH` with a CA restricted to the
intended client workloads. Trusting a broad client CA can admit an unintended
workload with a certificate from that CA. Certificate validation establishes
cryptographic identity; it does not replace application authorization or network
isolation.

Do not set `DYN_TCP_TLS_INSECURE` or `NATS_TLS_INSECURE` in production. These
options bypass certificate and server-identity verification.
...
<Note>
  Dynamo cannot detect whether the NATS broker actually requests a client
  certificate. If you set `NATS_TLS_CLIENT_CERT_PATH` and
  `NATS_TLS_CLIENT_KEY_PATH` but the broker does not verify client certificates,
  the connection provides only one-way TLS. Enable client-certificate
  verification on the NATS server to make NATS mTLS effective.
</Note>
...
- [Secure Deployment Guidelines](../../developer-guide/security/secure-deployment-guidelines.md) — Deployment trust model and checklist
- [Request Plane](../../developer-guide/knowledge-base/concepts/system-architecture/architecture.md#request-plane) — Request transport behavior
- [Event Plane](../../developer-guide/knowledge-base/concepts/system-architecture/architecture.md#event-plane) — Event transport and NATS authentication
