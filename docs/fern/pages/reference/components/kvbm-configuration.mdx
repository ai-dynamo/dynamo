...
| `KVBM_MESSENGER_DISCOVERY_` | Peer discovery | `TYPE` (required), `CLUSTER_ID`, plus etcd/p2p/filesystem fields |
| `KVBM_NIXL_` | NixL backends | `BACKENDS` (map, for example `{UCX={},POSIX={}}`) |
...
- **Object storage** (`object.client`): `s3` or `nixl` client; S3 fields `bucket` (required), `region` (`us-east-1`), `endpoint_url`, `force_path_style`, `max_concurrent_requests` (16).
