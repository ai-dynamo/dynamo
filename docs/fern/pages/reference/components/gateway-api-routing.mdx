...
<ParamField path="spec.components[].replicas" type="integer" default="1">
  ...
  EPP pod. Above `1`, the operator defaults `DYN_ROUTER_REPLICA_SYNC` to `true` so replicas share
  active-load state, and the routing accuracy of the fleet becomes approximate. Read
...
<ParamField path="DYN_ROUTER_REPLICA_SYNC" type="boolean" default="false">
  ...
  plane. The operator defaults it to `true` when `replicas` is above `1`, and a value set in
  `podTemplate` takes precedence. It has no peer to synchronize with at one replica. See
...
- **A retry can land on a different EPP.** Envoy load-balances `ext_proc` streams across EPP pods with
  no request affinity. If a stream fails and Envoy retries, the replica that made the original booking
  frees it when its stream ends, while the backend may still be generating. That worker's load is then
  understated on the replica that placed it and, once the retry is booked elsewhere, double-counted
  across the fleet until one of the two bookings is freed.
- **Backend work can outlive the stream.** An `ext_proc` disconnect does not cancel work already
  dispatched to the worker. The EPP stops accounting for a request that is still consuming KV blocks.
