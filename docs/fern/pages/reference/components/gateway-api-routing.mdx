...
[Route Requests with Gateway API](../../kubernetes/kv-aware-routing/gateway-api.mdx). For architecture, see
...
<ParamField path="spec.components[].eppConfig" type="object">
  Deprecated Go-EPP configuration. Omit for the native Rust EPP. When present, the operator keeps
  the legacy Go EPP Pod contract (CLI flags and config mount) so existing DGDs survive operator
  upgrades until you migrate by clearing this field and updating the image. See
