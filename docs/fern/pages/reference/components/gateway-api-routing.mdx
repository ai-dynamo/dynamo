...
The gateway dispatches, so the EPP's booking is tied to the `ext_proc` stream, and the EPP frees it
when that stream ends rather than when the worker stops generating.

Those coincide in the normal case, and they also coincide when the stream ends early, because the
gateway tears the upstream down with it. agentgateway drops fail-open once request headers are sent
and has no response-side fail-open, so an `ext_proc` failure mid-response ends the exchange, which
closes the stream to the worker. The worker's Frontend sidecar sees the disconnect and cancels
generation. Freeing the booking is therefore correct: nothing keeps running that the EPP has stopped
accounting for.
