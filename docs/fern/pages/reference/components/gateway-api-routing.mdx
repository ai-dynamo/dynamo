...
<ParamField path="DYN_ROUTER_REPLICA_SYNC" type="boolean" default="false">
  ...
  replicated at all. Set this field to `true` when enabling multiple EPP replicas. See
...
Set `DYN_ROUTER_REPLICA_SYNC` to `true` when running more than one EPP replica. Without it, each
replica sees only the load it routed itself, and load balancing across workers suffers.

The EPP embeds the same KV router as the Frontend and reads the same `DYN_ROUTER_*` settings, so
replicated EPPs keep prefix-cache and active-load state the same way replicated Frontends do. See
[Router Operations](../../developer-guide/knowledge-base/modular-components/router/router-operations.md)
for how that state behaves across replicas and what `DYN_ROUTER_REPLICA_SYNC` synchronizes.

One difference is specific to the EPP. The gateway, not the EPP, sends the request to the worker, so
the EPP frees a booking when the gateway's `ext_proc` stream for that request ends. If the gateway
keeps the request running after that stream ends, the EPP no longer counts that request's load.
