...
<ParamField path="DYN_ROUTER_DECISION_LOG_SAMPLE_RATE" type="number" default="0.0">
  Samples payload-free diagnostics for decisions that selected less effective KV overlap than another
  eligible worker. Accepts a fraction from `0` through `1`; `0` disables the diagnostic. Sampling is
  deterministic by request ID and applies only to the built-in worker-selection policy.
</ParamField>
