...
are interpreted: `auto` uses AISimulate's heuristic, `absolute` uses timestamps directly,
and `relative` adds the parent marker's timestamp. Explicit overrides take precedence over
the heuristic. For example, a marker at 1 second with a child timestamp of 2 seconds places
the child at 3 seconds with `relative`, and at 2 seconds with `absolute`.
