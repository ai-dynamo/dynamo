...
identity by normalized source-relative path, and preserves public AgentX semantics. Sequential
turns wait for completion. Overlapping subagents depend on parent dispatch, post-completion
subagents depend on parent completion, blocking results join the one owning parent stream, and
`async_launched` work remains background. Timestamp-inferred cross-stream ordering is represented
as `replay_barrier`, not as a causal join. Ambiguous ownership, unsupported terminal states,
blocking empty subagents, symlinks, mixed block sizes, and mixed models are rejected.
