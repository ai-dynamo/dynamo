...
<ParamField path="DYN_TCP_RESPONSE_STREAM_HOST" type="string" default="auto-detected local address">
  IPv4 address or network interface name used to bind and advertise the TCP response stream server.
  An interface name resolves to its lowest usable IPv4 address. Dynamo rejects explicit IPv6 and
  unspecified addresses such as `0.0.0.0`.
</ParamField>

<ParamField path="DYN_TCP_RESPONSE_STREAM_PORT" type="integer" default="0">
  Port used by the TCP response stream server. When unset or set to `0`, the operating system assigns
  a free port. Values must be between `0` and `65535`.
</ParamField>
...
<ParamField path="DYN_EVENT_PLANE_HOST" type="string" default="auto-detected local IPv4 address">
  IPv4 address or network interface name advertised by direct ZMQ event publishers. An interface
  name resolves to its lowest usable IPv4 address. Dynamo rejects IPv6 and unspecified addresses
  such as `0.0.0.0`. This variable does not affect NATS or ZMQ broker mode.
</ParamField>
