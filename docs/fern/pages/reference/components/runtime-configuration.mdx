...
<ParamField path="DYN_TCP_RPC_HOST" type="string" default="auto-detected">
  ...
  addresses are accepted. Unspecified, multicast, broadcast, and unscoped IPv6 link-local
  advertisement addresses are rejected.
</ParamField>
...
Automatic local-address selection caches only successful non-loopback results. Dynamo retries after
interface-enumeration errors and loopback fallbacks.
