...
<ParamField path="DYN_EVENT_PLANE_HOST" type="string" default="auto-detected local IP">
  IPv4 or IPv6 address advertised by direct ZMQ event publishers. Set this variable to an address
  on the network interface that subscribers use to reach the publisher. Dynamo rejects hostnames.
  This variable does not affect NATS or ZMQ broker mode.
</ParamField>
