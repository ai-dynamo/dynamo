...
<ParamField path="DYN_DISCOVERY_GROUP_REMOVAL_GRACE_MS" type="integer" default="2000">
  Number of milliseconds that the frontend keeps serving state for a model whose last worker has
  gone away, before withdrawing the model from the catalog. A rolling update withdraws the outgoing
  worker's model card before its replacement publishes one, and this window spans that gap: the
  model stays listed on `/v1/models` and requests get a retryable `503` instead of a `404`. A
  replacement worker joining inside the window takes the model over in place. Raise it if your
  workers take longer than the default to publish a card after startup; set it to `0` to withdraw
  the model as soon as its last worker leaves. The process reads this value once at startup;
