...
<ParamField path="DYN_TCP_RPC_HOST" type="string" default="auto-detected">
  Host for the TCP request-plane server. Accepts IPv4 and IPv6 literals, bracketed IPv6 literals,
  wildcards, and interface names such as `eth0:1`. If unset, Dynamo selects the first usable
  non-loopback IPv4 address, then IPv6, then IPv4 loopback, then IPv6 loopback.

  A wildcard advertises a reachable address in its requested family. If only the other family has a
  usable non-loopback address, Dynamo switches the bind wildcard to that family. IPv4 link-local
  addresses are accepted. Multicast, broadcast, and IPv6 link-local addresses are rejected.
  Wildcards are accepted as bind addresses and are never advertised.
</ParamField>

<ParamField path="DYN_TCP_RESPONSE_STREAM_HOST" type="string" default="auto-detected">
  Host for the TCP response-stream server. It accepts the same values and uses the same address
  selection and wildcard-family rules as `DYN_TCP_RPC_HOST`. The QUIC response listener uses the
  selected concrete address.
</ParamField>

Automatic selection for advertised URLs caches only successful non-loopback results and resolves
again after interface-enumeration errors and loopback fallbacks. The TCP request-plane and
response-stream servers resolve once at startup. A loopback fallback persists until restart, and an
interface-enumeration error fails server startup.
...
<ParamField path="DYN_EVENT_PLANE_HOST" type="string" default="auto-detected local address">
  ...
  invalid. When unset, automatic advertisement stays in the IPv4 bind family.
