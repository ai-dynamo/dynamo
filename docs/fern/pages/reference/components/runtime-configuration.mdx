...
<ParamField path="DYN_TCP_RPC_HOST" type="string" default="auto-detected local address">
  ...
  loopback. Dynamo does not trim whitespace; a value containing only whitespace fails server startup.
  ...
  interface with only loopback addresses uses IPv4 loopback, then IPv6 loopback.

  A wildcard selects a usable non-loopback address in its requested family for advertisement. If
  only the other family has one, Dynamo switches the bind wildcard to that family. If neither family
  has one, Dynamo prefers loopback in the requested family before trying the other family. If no
...
<ParamField path="DYN_EVENT_PLANE_HOST" type="string" default="auto-detected local address">
  ...
  invalid. When unset, automatic advertisement stays in the IPv4 bind family. If the host has no
  usable non-loopback IPv4 address, automatic selection advertises IPv4 loopback (`127.0.0.1` when
  no IPv4 loopback address is found), and remote subscribers cannot reach the publisher.
