...
These series record, per routing attempt, how much of the prompt prefix the router expected to find cached and how much the worker actually reused. Each is a histogram labelled by `phase` (`aggregated`, `prefill`, or `decode` — the same label the frontend exports on `stage_duration_seconds`) and `model`: `_sum` carries the token total, `_count` the number of attempts observed, and the buckets the per-attempt distribution.

| Stage | Metric | Source |
| --- | --- | --- |
| Prompt tokens received | `dynamo_component_router_input_sequence_tokens` (existing) | router, at selection |
| Cached prefix on the best eligible worker | `dynamo_component_router_kv_best_eligible_cached_prefix_tokens{phase,model}` | router estimate, at selection |
| Cached prefix on the selected worker | `dynamo_component_router_kv_selected_cached_prefix_tokens{phase,model}` | router estimate, at selection |
| Prefix found by the worker's cache lookup | `dynamo_component_router_kv_worker_lookup_tokens{phase,model}` | worker report, at completion |
| Prefix reused by the worker | `dynamo_component_router_kv_worker_reused_tokens{phase,model}` | worker report, at completion |
| Worker report per attempt | `dynamo_component_router_kv_worker_outcomes_total{phase,model,result}` | `complete` or `incomplete` |

The best-eligible and selected estimates are raw contiguous prefix overlap in tokens across device, host-pinned, and disk/external tiers; the router's tier weights affect worker selection only. The lookup and reused tokens are recorded only when the stream completes with a complete worker report whose prompt length matches the routed prompt; every other attempt increments `kv_worker_outcomes_total{result="incomplete"}` and contributes no tokens. Lookup is GPU hits plus external lookup tokens; reused is GPU hits plus successful external hits. Values are uncapped.

**Pools and phases.** Inside one router process the prefill and decode attempts of a disaggregated request share a `router_id`; compare stages within one `phase` and do not sum across phases. **Retries.** Migration retries are additional attempts in the same series and are not labelled; on a retry, the received-token count includes the tokens already generated. **Query-only lookups** are excluded from all of these series — including `router_input_sequence_tokens`, which is a change to that existing metric. **`router_kv_hit_rate`** is the tier-weighted selection score as a block ratio; the selected-worker estimate here is the raw resident prefix in tokens, so the two do not agree numerically. **Backend coverage.** The vLLM worker reports lookup and reused tokens; SGLang and TRT-LLM attempts record the router estimates and count as `incomplete`. On stock vLLM without `num_external_lookup_tokens`, lookup is a lower bound and equals reused. **Multimodal** requests use routing tokens for the estimates and input token IDs for the received count, so stage differences are not necessarily cache misses.
