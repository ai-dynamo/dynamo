...
Emitted by the HTTP frontend at `/metrics` on port 8000 by default. Most carry a `model` label. That label always holds the primary served model name — the first `--served-model-name` value — so a request sent to an alias is counted under the primary. A name the frontend has not registered is counted under `unknown_model`.
