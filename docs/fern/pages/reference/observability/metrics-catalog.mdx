...
The cache-reuse funnel records token totals per routing attempt. F2 through F5 are always enabled, with a separate counter for each measurement. F0 reuses the existing input-token sum. Query-only worker lookups are excluded because they do not produce worker stages. Compare stages from the same router component or pool; summing prefill and decode pools counts separate routing attempts.

| Stage | Metric |
| --- | --- |
| F0: Tokens Received | `dynamo_component_router_input_sequence_tokens_sum` |
| F2: Tokens Resident on Some Worker | `dynamo_component_router_cache_loss_max_eligible_cached_prefix_tokens_total` |
| F3: Tokens Resident on Chosen Worker | `dynamo_component_router_cache_loss_selected_cached_prefix_tokens_total` |
| F4: Tokens Found by Worker | `dynamo_component_router_cache_loss_worker_lookup_tokens_total` |
| F5: Tokens Actually Used From Cache by Worker | `dynamo_component_router_cache_loss_worker_reused_tokens_total` |
