...
<ParamField path="dynamo_backend_admission_active_requests" type="gauge">
  Requests currently holding an admitted backend slot.
</ParamField>

<ParamField path="dynamo_backend_admission_queue_depth" type="gauge">
  Requests currently waiting in the admission queue.
</ParamField>

<ParamField path="dynamo_backend_admission_concurrency_limit" type="gauge">
  Effective concurrent-request limit. Changes if a model card reports a capacity hint after startup.
</ParamField>

<ParamField path="dynamo_backend_admission_queue_capacity" type="gauge">
  Configured admission-queue length. Static for the life of the process.
</ParamField>

<ParamField path="dynamo_backend_admission_rejections_total" type="counter">
  Cumulative requests the gate refused, labelled `reason`: `queue_full` when the limit was busy and the queue was full, `queue_timeout` when the request waited past the queue delay. A request cancelled while queued is not a rejection and is not counted under either reason.
</ParamField>
