...
The following worker-side gauges and counter track the TCP request-plane worker pool, sized by `DYN_TCP_WORKER_POOL_SIZE` and `DYN_TCP_WORK_QUEUE_SIZE`. They do not track the backend admission gate, which has no metrics of its own; see [Worker-Side Request Admission](../../developer-guide/knowledge-base/concepts/fault-tolerance/request-rejection-architecture.md#worker-side-request-admission).

<ParamField path="dynamo_rejection_request_total" type="counter">
  Cumulative requests rejected because the TCP worker pool and its work queue were both full.
</ParamField>

<ParamField path="dynamo_engine_request" type="gauge">
  Current requests dispatched to a TCP worker-pool task.
</ParamField>

<ParamField path="dynamo_request_queue" type="gauge">
  Current requests sitting in the TCP work queue, not yet dispatched.
</ParamField>
