...
The optional cache-reuse funnel records prompt-token totals at each handoff from router selection through worker cache lookup. Comparing the stages shows where potential prefix reuse was lost.

Enable it on the router/frontend process:

```bash
DYN_CACHE_REUSE_FUNNEL_F2_ONWARD_ENABLED=true
```
