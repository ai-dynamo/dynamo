...
<ParamField path="dynamo_component_router_input_f0_total" type="counter">
  Input tokens observed across routing decisions. This is the denominator for `dynamo_component_router_input_f1_total`. It does not match the sum of the `dynamo_component_router_input_sequence_tokens` histogram, which is recorded when a request is dispatched rather than when it is routed. Labeled by `worker_type`.
</ParamField>

<ParamField path="dynamo_component_router_input_f1_total" type="counter">
  Input tokens already cached on the eligible instance with the most KV cache overlap, counted whether or not the router selected that instance. `f1 / f0` is therefore the prefix cache hit rate the router could have reached, not the one it did reach — compare it against `dynamo_component_router_kv_hit_rate` to see how much of the ceiling routing gives away. Labeled by `worker_type`.
</ParamField>
...
| `dynamo_component_router_decisions_total`, `..._decision_kv_optimal_total`, `..._input_f0_total`, `..._input_f1_total` | Registered for both `worker_type` values; only the routed one is populated | Registered and populated for `worker_type` = `prefill`/`decode` | Registered, **always zero** | Populated (on `DYN_SYSTEM_PORT`) |
