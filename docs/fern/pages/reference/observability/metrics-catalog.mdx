...
`dynamo_frontend_cached_tokens` reports reuse per logical frontend request, not per routing attempt. The Generate path records zero when matching backend usage is unavailable and increments `dynamo_frontend_cached_tokens_missing_total`, rather than substituting the router estimate. An explicit worker-reported zero is not missing usage. Removing this fallback does not make the frontend metric interchangeable with F5 during migration or disaggregated routing.
