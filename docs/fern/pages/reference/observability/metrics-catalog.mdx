...
<ParamField path="dynamo_frontend_model_migration_success_total" type="counter">
  Total migration events that recovered on another worker. Labeled by `model` and [`migration_type`](metric-labels.mdx#metric-specific-labels).
</ParamField>

<ParamField path="dynamo_frontend_model_migration_failure_total" type="counter">
  Total migration events that ended without recovery. Labeled by `model` and [`migration_type`](metric-labels.mdx#metric-specific-labels).
</ParamField>

<ParamField path="dynamo_frontend_model_migration_duration_seconds" type="histogram">
  Time from detecting a migratable worker failure until the replacement response stream is established or migration ends without recovery. Labeled by `model`, [`migration_type`](metric-labels.mdx#metric-specific-labels), and [`outcome`](metric-labels.mdx#metric-specific-labels). It excludes work completed before the failure and generation after the replacement stream is established.
</ParamField>
