...
<ParamField path="dynamo_frontend_worker_remote_kv_waiting_requests" type="gauge">
  Requests whose external KV transfer has not completed, per worker. This is transfer pressure, not prefill compute.
</ParamField>

<ParamField path="dynamo_frontend_worker_remote_kv_waiting_tokens" type="gauge">
  KV context tokens belonging to requests whose external KV transfer has not completed, per worker.
</ParamField>
