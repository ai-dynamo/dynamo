...
These frontend-only metrics describe the frontend's view of normalized base model-card discovery and router admission. They do not report Kubernetes Pod existence, and LoRA overlays do not count as additional workers. Both metrics carry `model`, `target_namespace`, `target_component`, `target_endpoint`, and `worker_type` labels. The target labels identify the discovered serving endpoint.
...
Each previously observed group emits all four counts, including explicit zeros after its last worker disappears, for the lifetime of the frontend's model manager. Groups never observed by that frontend have no series. There are no retained per-worker samples for removed IDs.
...
| `config_conflict` | Model Deployment Card fingerprints conflict |
...
For example, if a conflicting Model Deployment Card (MCD) causes a group of two discovered workers to be rejected, its counts are `discovered=2`, `available=0`, `pending=0`, and `excluded=2`. The per-worker samples show `state="excluded",reason="config_conflict"`.
...
Scope these queries to the target frontend and deployment. Evaluate each frontend separately because their discovery and admission views can differ. A missing series does not mean zero workers; check Prometheus `up` separately for scrape health. These metrics reflect updates after the frontend processes discovery and routing state changes, not at the instant a worker process terminates.
