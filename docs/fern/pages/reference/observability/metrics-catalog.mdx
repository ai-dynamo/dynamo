...
The following vLLM decode metrics appear only when `--decode-max-remote-prefill-inflight` is positive and vLLM statistics are enabled. They carry `model`, `dynamo_component`, and `dp_rank` labels.

<ParamField path="dynamo_component_decode_remote_prefill_admission_limit" type="gauge">
  Configured remote-prefill admission limit for each local data-parallel rank.
</ParamField>

<ParamField path="dynamo_component_decode_remote_prefill_admission_active" type="gauge">
  Remote-prefill decode requests admitted before first output.
</ParamField>

<ParamField path="dynamo_component_decode_remote_prefill_admission_waiting" type="gauge">
  Remote-prefill decode requests waiting for admission.
</ParamField>

<ParamField path="dynamo_component_decode_remote_prefill_admission_wait_seconds" type="histogram">
  Time spent waiting for remote-prefill decode admission.
</ParamField>

<ParamField path="dynamo_component_decode_remote_prefill_admission_limit_hits_total" type="counter">
  Remote-prefill requests that found their local data-parallel limit full.
</ParamField>

<ParamField path="dynamo_component_decode_remote_prefill_admission_cancelled_waiters_total" type="counter">
  Remote-prefill requests cancelled while waiting for admission.
</ParamField>

<ParamField path="dynamo_component_decode_remote_prefill_admission_releases_total" type="counter">
  Admission permits released by terminal reason. This metric also carries the `reason` label.
</ParamField>
