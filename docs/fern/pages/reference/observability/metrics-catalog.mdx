...
|---|---|---|---|---|---|
| `dynamo_component_router_*` | Registered and populated | Registered and populated | Registered, **always zero** | Populated (on `DYN_SYSTEM_PORT`) | Only requests started and input/output sequence lengths are registered and populated (on `DYN_EPP_METRICS_PORT`) |
...
The standalone EPP router registers the three existing `dynamo_component_router_*` families it can observe on this endpoint. Its `model` and `inference_pool` labels come from startup configuration. Successful selections increment `dynamo_component_router_requests_started_total` and observe `dynamo_component_router_input_sequence_tokens`. Terminal responses with `usage.completion_tokens` observe `dynamo_component_router_output_sequence_tokens`, including a reported zero.

The other router families are absent from the EPP endpoint until equivalent scheduler or token-level signals are available. Response byte chunks do not provide TTFT or inter-token latency.
