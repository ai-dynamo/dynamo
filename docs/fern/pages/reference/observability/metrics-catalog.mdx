...
| `dynamo_component_*` | Backend workers, standalone router, standalone EPP router | `/metrics` on `DYN_SYSTEM_PORT` (must be set), or EPP `/metrics` on `DYN_EPP_METRICS_PORT` (default 9090) |
...
| Metric group | Frontend + KV (agg) | Frontend + KV (disagg) | Frontend + non-KV | Standalone router | Standalone EPP |
|---|---|---|---|---|---|
| `dynamo_component_router_*` | Registered and populated | Registered and populated | Registered, **always zero** | Populated (on `DYN_SYSTEM_PORT`) | Registered; only requests started and input/output sequence lengths are populated (on `DYN_EPP_METRICS_PORT`) |
| `dynamo_router_overhead_*` | Registered and populated | Registered and populated | **Not registered** | **Not created** | **Not created** |
| `dynamo_frontend_router_queue_*` | Registered; populated when a CLI or policy-class queue threshold is set | Registered; populated when a CLI or policy-class queue threshold is set | **Not registered** | **Not created** | **Not created** |
| `dynamo_component_kv_cache_events_applied` | Populated when KV events received | Populated when KV events received | **Not registered** | Populated when KV events received | **Not created** |
| `dynamo_frontend_worker_*` | Registered and populated | Registered and populated (`worker_type` = `prefill`/`decode`) | Registered and populated (`worker_type` = `decode`) | **Not created** | **Not created** |
...
The standalone EPP router also registers the existing `dynamo_component_router_*` families on this endpoint. Its `model` and `inference_pool` labels come from startup configuration. Successful selections increment `dynamo_component_router_requests_started_total` and observe `dynamo_component_router_input_sequence_tokens`. Terminal responses with `usage.completion_tokens` observe `dynamo_component_router_output_sequence_tokens`, including a reported zero.

The other router families retain their names and buckets but receive no EPP observations until equivalent scheduler or token-level signals are available. Some emit preinitialized zero series; those zeros do not mean the EPP measured zero latency or zero requests. Response byte chunks do not provide TTFT or inter-token latency.
