...
### KV-cache reuse telemetry (F0–F5)

KV-cache reuse telemetry records one observation per routing attempt. F2 through F5 are always enabled, each as a histogram whose `_sum` carries the token total and `_count` the number of observations. Every series carries a `phase` label (`aggregated`, `prefill`, or `decode`, the same vocabulary as `stage_duration_seconds{phase}`): inside one router process the prefill and decode attempts of a disaggregated request are recorded on the same `router_id`, so compare stages within one `phase` rather than summing across phases. Migration retries are additional attempts in the same series and are not labelled. F0 reuses the existing input-token sum. Query-only worker lookups are excluded because they do not produce worker stages. Compare stages from the same router component or pool; summing prefill and decode pools counts separate routing attempts.

| Stage | Metric |
| --- | --- |
| F0: Tokens Received | `dynamo_component_router_input_sequence_tokens_sum` |
| F2: Tokens Resident on Some Worker | `dynamo_component_router_kv_best_eligible_cached_prefix_tokens_sum` |
| F3: Tokens Resident on Chosen Worker | `dynamo_component_router_kv_selected_cached_prefix_tokens_sum` |
| F4: Tokens Found by Worker | `dynamo_component_router_kv_worker_lookup_tokens_sum` |
| F5: Tokens Actually Used From Cache by Worker | `dynamo_component_router_kv_worker_reused_tokens_sum` |
...
F2 and F3 are recorded at selection time. F4 and F5 require a complete worker outcome with a matching prompt length and a successfully completed stream. Missing or invalid outcomes increment `router_kv_worker_outcomes_total{result="incomplete"}` and contribute no tokens to F4/F5. F4 is GPU hits plus CPU/external lookup tokens; F5 is GPU hits plus successful CPU/external hits. Values are uncapped.
...
The vLLM worker includes cache-reuse metadata in its final response without an enable flag. Older workers without this metadata contribute incomplete observations rather than estimated F4/F5 values.
