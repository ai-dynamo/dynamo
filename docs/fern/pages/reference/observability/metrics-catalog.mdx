...
Available when request tracing is enabled. This counter is registered on the frontend's `/metrics`
endpoint and tracks source-level decisions before a record fans out to configured sinks.

<ParamField path="dynamo_frontend_request_trace_sampling_records_total" type="counter">
  Request-trace records evaluated by sampling. Labeled by `decision` (`retained` or `dropped`) and
  `event_type` (`request_end`, `request_payload`, `tool_start`, `tool_end`, or `tool_error`). A
  retained record is offered to every configured sink; this counter does not report broadcast queue
  pressure or file, NATS, OTLP, or S3 delivery outcomes.
</ParamField>
