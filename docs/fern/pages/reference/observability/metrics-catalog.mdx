...
<Note>
A frontend can serve one model name from several namespaces at once, so several `dynamo_frontend_*` families carry a `dynamo_namespace` label identifying the deployment a series belongs to. Three do not, by design. See [Frontend metrics that carry a namespace](metric-labels.mdx#frontend-metrics-that-carry-a-namespace) for the split and for how to aggregate each kind.
</Note>
