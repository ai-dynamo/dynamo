...
- **Exemplars are not converted.** No engine Dynamo runs emits them today, so nothing is lost in practice, but a backend cannot yet jump from a histogram bucket to the trace that produced it.
