...
See [Engine Provenance in the Self-Benchmark Artifact](forward-pass-metrics-traces.mdx#engine-provenance-in-the-self-benchmark-artifact) for the optional `engine` block these runs capture, including the difference between a requested and a resolved attention backend.
