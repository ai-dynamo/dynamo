...
<ParamField path="DYN_BENCH_GIANT_KV_THRESHOLD" type="integer" default="1000000">
  Total-KV-read threshold (tokens) above which a decode point is measured with repeated steady steps. Set `0` to apply repeat protection to every point.
</ParamField>

<ParamField path="DYN_BENCH_GIANT_KV_REPEATS" type="integer" default="3">
  Steady-step repeat count for points above the threshold; the recorded latency is the median.
</ParamField>
...
<ParamField path="DYN_BENCH_PREFILL_REAL_SEED" type="string" default="off">
  Real-KV seeding for prefill points that read past KV (`on`/`1`/`true`). The seeded prefix is first computed by a real prefill pass (staging), an untimed same-shape warm shot follows, and only then does the timed request hit the real KV in the prefix cache; the expected hit length is validated before admission and the point is skipped on a miss. Off keeps the synthetic prefix blocks that are registered but never computed. Dense-attention models measure the same either way; sparse-attention (DSA) and hybrid-KV models need it for deep past-KV points.
</ParamField>
...
Every result and skipped-point entry in the artifact carries a `kv_seed_regime` field — `real_kv`, `fake_fallback`, `legacy` (warm-up switched off), `skip:<reason>` (gate rejected the configuration), `unstamped` (decode point that never reached injection); prefill rows that read past KV carry `real_prefix` or `fake_prefix`, other prefill rows `not_applicable`, or `not_applicable` (prefill) — plus `kvwarm` and `synthetic_prompts` metadata blocks, so downstream consumers can filter by seeding provenance.
