...
<ParamField path="NIXL_TELEMETRY_PROMETHEUS_PORT" type="integer">
  ...
  `19090` is Dynamo's convention rather than a NIXL default. With this variable unset, the exporter falls back to its own built-in port, which collides with the `DYN_SYSTEM_PORT` operator preset — so set it explicitly.
