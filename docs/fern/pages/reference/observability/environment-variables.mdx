...
One switch controls the NVTX annotations that appear on an Nsight Systems timeline, across both the Rust runtime and the Python components. It is off by default.

<ParamField path="DYN_NVTX" type="boolean" default="false">
  Enables NVTX annotations in the Rust runtime — frontend HTTP handling, worker ingress and egress, transport, and routing — and in the Python components. Accepts `1`, `true`, `on`, or `yes` to enable and `0`, `false`, `off`, or `no` to disable, trimmed and case-insensitive; any other value raises. The Rust half additionally requires a build with the `nvtx` Cargo feature, which released builds do not have. The Python half requires the `nvtx` package from the `ai-dynamo[profiling]` extra, and raises `ImportError` at startup if it is missing rather than recording nothing.
</ParamField>
