...
<ParamField path="DYN_SYSTEM_HOST" type="string" default="0.0.0.0">
  Bind address for the component system server.
</ParamField>

<ParamField path="DYN_SYSTEM_PORT" type="integer" default="-1 (disabled)">
  Port for the component system server. In addition to `/metrics` and the health endpoints, this server can expose `/metadata`, capability-dependent `/engine/*` management routes, and LoRA management routes. Disabled by default; local examples use `8081`. Must be set explicitly for backend workers and the standalone router to expose metrics.

  **Operator preset:** `9090` on worker, prefill, decode, and planner pods.
</ParamField>

<Warning>
  The system server uses plaintext HTTP and does not authenticate or authorize callers. Keep it private even when health and metrics endpoints must be scraped. Bind it to loopback where possible; otherwise restrict the port with NetworkPolicy or place it behind an authenticated TLS or service-mesh proxy. Treat `/engine/*` and LoRA routes as privileged control surfaces.
</Warning>
