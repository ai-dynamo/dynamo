...
<ParamField path="OTEL_TRACES_SAMPLE_RATIO" type="number">
  Head-sampling ratio in `[0.0, 1.0]` for runtime OpenTelemetry spans. Unset exports every trace. For example, `0.01` retains approximately one percent of traces.
</ParamField>
...
<ParamField path="DYN_REQUEST_TRACE_SAMPLE_RATIO" type="number">
  Deterministic request-trace sampling ratio in `[0.0, 1.0]`. Agent records use the session ID, and other records use the request ID. A selected record is sent to every configured sink. `dynamo_frontend_request_trace_sampling_records_total` reports retained and dropped decisions separately from sink delivery. `DYN_AUDIT_FORCE_LOGGING` selects legacy payload records but does not bypass this ratio.
</ParamField>
