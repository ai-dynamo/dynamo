...
Rows produced by the collector carry a `kv_seed_regime` stamp (`real_kv`, `fake_fallback`, or `skip:<reason>`) plus `kvwarm` and `synthetic_prompts` metadata blocks, so downstream consumers can filter by seeding provenance.
