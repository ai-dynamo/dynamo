...
<ParamField path="DYN_REQUEST_TRACE_SAMPLE_RATIO" type="number">
  Deterministic request-trace sampling ratio in `[0.0, 1.0]`. Agent records use the session ID, and other records use the request ID. A selected record is sent to every configured sink. `DYN_AUDIT_FORCE_LOGGING` selects legacy payload records but does not bypass this ratio.
</ParamField>
