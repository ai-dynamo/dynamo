...
<ParamField path="DYN_NVTX" type="boolean" default="false">
  Enables the Python components' NVTX annotations. Accepts `1`, `true`, or `yes`. Requires the `nvtx` package from the `ai-dynamo[profiling]` extra, and raises `ImportError` at startup if it is missing rather than recording nothing.
</ParamField>
