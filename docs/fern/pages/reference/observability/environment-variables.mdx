...
<ParamField path="OTEL_TRACES_SAMPLE_RATIO" type="number">
  Head-sampling ratio in `[0.0, 1.0]`. Unset exports every trace. For example, `0.01` retains approximately one percent of traces. The same ratio applies to runtime spans, request-trace records before they fan out to file, standard error, NATS, S3, or the OTLP log sink, and forward-pass metric trace files. Legacy payload capture enabled by `DYN_AUDIT_FORCE_LOGGING` remains unsampled.
</ParamField>
