...
<ParamField path="DYN_REQUEST_TRACE_OUTPUT_SEQUENCE_HASHES" type="boolean">
  Opt in to generated-continuation sequence hashes on `request_end` rows. Dynamo retains no output text or token IDs.
</ParamField>
