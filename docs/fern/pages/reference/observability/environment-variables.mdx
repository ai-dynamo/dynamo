...
<ParamField path="NIXL_TELEMETRY_ENABLE" type="string" default="n">
  ...
  This variable alone does not produce a scrapeable endpoint. Set `NIXL_TELEMETRY_EXPORTER` and `NIXL_TELEMETRY_PROMETHEUS_PORT` as well, as described below.
  ...
  NIXL compares the value case-insensitively, so `Y` and `TRUE` are also accepted. Any other value is an error.
...
<ParamField path="NIXL_TELEMETRY_EXPORTER" type="string">
  ...
  Required for a Prometheus endpoint. NIXL has no default exporter: with this variable unset, enabling telemetry collects metrics in-process and exports nothing, so no port is opened.

  **Operator preset:** `prometheus` on worker, prefill, and decode pods.
</ParamField>

<ParamField path="NIXL_TELEMETRY_PROMETHEUS_PORT" type="integer">
  ...
  `19090` is Dynamo's convention rather than a NIXL default. With this variable unset, the exporter falls back to its own built-in `9090`, which collides with the `DYN_SYSTEM_PORT` operator preset — so set it explicitly. A value outside `1`-`65535` is not a fallback either: NIXL then binds no exporter at all.
