...
| `DYN_REQUEST_TRACE_SAMPLE_RATIO` | unset | `[0.0, 1.0]` | Deterministic record ratio applied before sink fan-out. Agent records use the session ID; other records use the request ID. |
...
`DYN_REQUEST_TRACE_SAMPLE_RATIO` applies to request-trace records before fan-out. Dynamo uses the
agent session ID for agent request, payload, and tool rows, and a stable request ID for other
request-end and payload rows. A selected record is written to every configured sink and a rejected
record is written to none. `DYN_AUDIT_FORCE_LOGGING` selects legacy payload records; an unset or
`1.0` ratio retains every selected payload, while an explicit ratio also controls legacy payload
export volume. `dynamo_frontend_request_trace_sampling_records_total` counts these retained and
dropped decisions; it does not indicate whether a sink later accepted a retained record.
