...
| `DYN_REQUEST_TRACE_OUTPUT_SEQUENCE_HASHES` | unset | Truthy value | Adds generated-continuation sequence hashes to `request_end` rows. Requires `request_end`; no output text or token IDs are recorded. |
...
`input_sequence_hashes` are Dynamo's sequence-aware rolling hashes. Set
`DYN_REQUEST_TRACE_OUTPUT_SEQUENCE_HASHES=1` to add `output_sequence_hashes`, which continues the
same chain across generated tokens. When an input ends mid-block, its first output hash replaces the
trailing input hash; otherwise output hashes append to the input chain. DynoSim maps these hashes to
compact internal IDs while loading the original trace; it does not create an intermediate Mooncake
file.
