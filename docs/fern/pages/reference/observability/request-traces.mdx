...
| `DYN_REQUEST_TRACE_HTTP_HEADER_CAPTURE_LIST` | unset | Header names | Comma- or whitespace-separated, case-insensitive allowlist captured only on `request_payload` rows. Unset or empty captures none. |
| `DYN_REQUEST_TRACE_HTTP_HEADER_REDACT_LIST` | Eight names listed below | Header names | Replaces captured values with `<redacted>` by name. Setting this replaces the defaults; empty disables redaction. |

### HTTP Header Redaction

Capture and redaction are independent. Only headers in the capture list appear in payload records.
Of those headers, names in the redact list retain their key with the value `<redacted>`, including
repeated headers. Other repeated values are comma-joined.

The default redact list is `authorization`, `proxy-authorization`, `cookie`, `set-cookie`,
`x-api-key`, `api-key`, `x-auth-token`, and `x-access-token`. Both lists accept comma- or
whitespace-separated names and match case-insensitively. Configure them before starting the process.

An override replaces the entire redact list. To retain the defaults and redact a custom header:

```bash
export DYN_REQUEST_TRACE_HTTP_HEADER_REDACT_LIST='authorization,proxy-authorization,cookie,set-cookie,x-api-key,api-key,x-auth-token,x-access-token,x-custom-credential'
```

Remove a name from the override to capture its value without redaction, or set the variable to an
empty string to disable header redaction. Values are not scanned for Bearer, Basic, or other
credential schemes, so include custom credential header names in the redact list. Wildcard matching
and a separate deny/drop list are not supported by these settings.
