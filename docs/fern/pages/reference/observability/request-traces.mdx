...
With a Python chat processor, routed requests that start a response stream also emit this record.
Inputs that cannot be replayed, including multimodal requests, retain timing and routing metadata
but omit `request.replay`. This path does not yet add session or parsed finish-reason metadata.
...
For Python chat processors, those requests can still produce metadata-only `request_end` rows. They
cannot be loaded as replay requests without `request.replay`.
