...
are redacted by header name according to `DYN_REQUEST_TRACE_HTTP_HEADER_REDACT_LIST`.
Include custom credential header names in the redact list; values are not scanned for credentials.
