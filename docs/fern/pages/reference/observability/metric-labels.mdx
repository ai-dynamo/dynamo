...
<ParamField path="outcome" type="string">
  `[Shared]` On `dynamo_frontend_model_migration_duration_seconds` — whether the migration recovered on another worker or ended without recovery.

  <span className="enum-values"><span className="enum-label">Allowed values:</span> <Badge intent="note" minimal>success</Badge> <Badge intent="note" minimal>failure</Badge></span>
</ParamField>
