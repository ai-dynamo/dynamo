...
<ParamField path="policy_class" type="string">
  `[Shared]` On `dynamo_frontend_router_queue_*` — the scheduler queue the request was placed in. This is the class named by the client, or `default_policy_class` when the client named none. Deployments without policy classes report the synthetic `default` class.
</ParamField>
...
<ParamField path="stage" type="string">
  `[Shared]` On `dynamo_frontend_router_queue_deadline_expired_total` — where the missed class deadline was detected: `admission`, `deferred_wake`, or `dispatch`.
</ParamField>
