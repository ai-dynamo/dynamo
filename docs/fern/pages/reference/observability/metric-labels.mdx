...
- Per-deployment configuration gauges, which also carry `worker_type` because a disaggregated deployment's prefill and decode cards advertise different limits: `model_context_length`, `model_kv_cache_block_size`, `model_migration_limit`, `model_total_kv_blocks`, `model_max_num_seqs`, `model_max_num_batched_tokens`.
...
<Note>
Configuration gauges are per-deployment facts, so they are not additive: aggregate them with `max by (model)`, not `sum`. Counters and additive gauges aggregate with `sum by (model)`; histograms with `histogram_quantile(..., sum by (model, le) (rate(..._bucket[5m])))`.
</Note>
