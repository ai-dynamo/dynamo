...
<Warning>
`dynamo_router_overhead_*` is a fourth case and a known gap: it is registered on the frontend's own registry with only a `router_id` label, so in a multi-namespace frontend the per-phase routing latencies of every deployment merge into one histogram. A slow namespace is not separable there the way it is on `dynamo_component_router_*`.
</Warning>
