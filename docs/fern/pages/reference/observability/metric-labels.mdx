...
| `generate` | Engine setup | The engine's `generate()` returned an error before opening a response stream. |
| `engine_stream` | Streaming | A typed engine error or untyped error envelope arrived after the response stream opened. Counted at most once per request. Excludes cancellation, explicit worker shutdown, and serialization failures; includes engine crashes. |
| `serialization` | Streaming | A response could not be serialized, including failures delivered as fallback error frames. |
...
To measure the fraction of component requests with an engine-stream failure over
five minutes, group both counters by the same endpoint labels:

```promql
sum by (dynamo_namespace, dynamo_component, dynamo_endpoint) (
  rate(dynamo_component_errors_total{error_type="engine_stream"}[5m])
)
/
sum by (dynamo_namespace, dynamo_component, dynamo_endpoint) (
  rate(dynamo_component_requests_total[5m])
)
```

This is a component-request ratio: disaggregated prefill and decode are separate
requests. It excludes setup failures counted under `generate`. The ratio is
undefined when the request rate is zero; choose an alert threshold and minimum
traffic rate appropriate to the endpoint.
