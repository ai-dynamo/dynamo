...
### Frontend metrics that carry a namespace

A frontend can serve one model name from several namespaces at once. These `dynamo_frontend_*` families carry `dynamo_namespace` so their series stay attributable to a single deployment:

- Response metrics, labeled by the namespace that served the request: `output_tokens_total`, `time_to_first_token_seconds`, `inter_token_latency_seconds`, `input_sequence_tokens`, `output_sequence_tokens`, `cached_tokens`, `images_per_request`, `videos_per_request`, `audio_per_request`, `image_tokens_per_request`.
- Per-deployment configuration gauges, labeled by the namespace whose model card supplied the value: `model_context_length`, `model_kv_cache_block_size`, `model_migration_limit`, `model_total_kv_blocks`, `model_max_num_seqs`, `model_max_num_batched_tokens`.
...
<Warning>
`dynamo_frontend_requests_total`, `dynamo_frontend_request_duration_seconds`, and `dynamo_frontend_inflight_requests` deliberately do **not** carry `dynamo_namespace`. They are recorded from a guard created before a worker set is selected, so that errors raised during model resolution are still counted. In a frontend serving several namespaces they aggregate across all of them.
</Warning>
