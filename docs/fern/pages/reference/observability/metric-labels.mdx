...
A label reaches a series through one of five origins:
...
- **Configured deployment-wide** — set by the operator through [`DYN_METRICS_CONST_LABELS`](environment-variables.mdx), attached to every series the process exposes unless the series already carries that label.
