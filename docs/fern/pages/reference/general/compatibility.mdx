...
Every release with recorded CUDA data, grouped by minor line — newest first. Expand a line to see each release's backend pins, CUDA toolkit, and minimum driver, ordered CUDA 12 before CUDA 13. Platform previews, development builds, and model-specific builds are excluded, as are releases predating the point CUDA requirements were tracked per release: v0.6.0, v0.6.1, and the v0.6.1, v0.7.0 and v0.8.1 post-releases have no rows here. The notes below call out the releases whose toolkit support differs, and the [Releases (machine-readable)](releases-machine-readable.mdx) page has the full inventory.
