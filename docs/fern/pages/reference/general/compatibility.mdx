...
For Dynamo 1.5.0 SGLang embedding workers, the Operator supplies an embedding-shaped health-check payload when `--embedding-worker` or a literal `DYN_SGL_EMBEDDING_WORKER` value enables embedding mode. If embedding mode is set through Kubernetes `valueFrom` or `envFrom`, set `DYN_HEALTH_CHECK_PAYLOAD` explicitly before upgrading to an Operator that enables active health checks. Use a JSON payload with the worker's model name and a text input, for example `{"model":"your-model","input":"Test"}`. The Operator also leaves `envFrom` payload sources untouched; if an `envFrom` source might supply `DYN_HEALTH_CHECK_PAYLOAD`, it does not inject a default that could override that source.
