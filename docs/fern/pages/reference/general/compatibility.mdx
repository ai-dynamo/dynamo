...
Every stable release and patch, grouped by minor line — newest first. Expand a line to see each release's backend pins, CUDA toolkit, and minimum driver, ordered CUDA 12 before CUDA 13. Platform previews and model-specific builds are excluded; the notes below call out the ones whose toolkit support differs, and the [Releases (machine-readable)](releases-machine-readable.mdx) page has the full inventory.
...
| KV-Aware Routing | Yes | n/a | Yes | Experimental | No — This feature combination is not supported. (https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/router/overview) | Yes | Yes | No — SGLang does not support LoRA in Dynamo, so every LoRA pairing is unsupported. (https://docs.nvidia.com/dynamo/dev/knowledge-base/modular-components/backends/sg-lang/overview) | Yes | Experimental |
