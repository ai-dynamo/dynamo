...
During a rolling update, worker spec changes create generation-specific Dynamo namespaces. The frontend watches the deployment's base namespace prefix, so it can discover overlapping generations without allowing direct worker-to-worker communication across those generations. In a disaggregated deployment, prefill and decode workers in the same generation share one namespace and remain isolated from other generations.
