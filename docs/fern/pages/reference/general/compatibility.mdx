...
<Note>
See [Release Artifacts](release-artifacts.mdx) for the full artifact inventory — container images, wheels, Helm charts, and crates — [Local Installation](../getting-started/local-installation.mdx) for host OS and architecture requirements, and [Model Early Access Builds](model-early-access-builds.mdx) for per-model early access container builds. For backend-specific runtime workarounds — including the local `docker run --network host` hang first reported on Amazon Linux 2023 — see [TensorRT-LLM Known Issues](../backends/trtllm/trtllm-known-issues.md).
</Note>
...
| **Multimodal** | <Tooltip tip={<>Supports Qwen2-Audio experimentally and video input with frame sampling. <a href="../features/multimodal/multimodal-vllm.md">Source</a></>}><InteractionStatus status="yes" noteLabel="Audio and video support" /></Tooltip> | <Tooltip tip={<>The Rust frontend supports models handled by <code>llm-multimodal</code>; the Python path delegates to vLLM's multimodal processor. <a href="../features/multimodal/multimodal-kv-routing.md">Source</a></>}><InteractionStatus status="yes" noteLabel="Image-aware KV routing" /></Tooltip> | <InteractionStatus status="na" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | | | | | |
...
| **Speculative Decoding** | <InteractionStatus status="yes" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | <Tooltip tip={<>Eagle3 support is documented. <a href="../features/speculative-decoding/speculative-decoding-vllm.md">Source</a></>}><InteractionStatus status="yes" noteLabel="Eagle3 support" /></Tooltip> | <InteractionStatus status="na" /> |
...
| **Multimodal** | <Tooltip tip={<>Supports aggregated EPD, E/PD, and E/P/D patterns. Traditional disaggregated EP/D is not supported. <a href="../features/multimodal/multimodal-sglang.md">Source</a></>}><InteractionStatus status="yes" noteLabel="Supported serving patterns" /></Tooltip> | <Tooltip tip={<>This feature combination is not supported. <a href="../components/router/README.md">Source</a></>}><InteractionStatus status="no" noteLabel="Not supported with KV-aware routing" /></Tooltip> | <InteractionStatus status="na" /> | <InteractionStatus status="wip" /> | <InteractionStatus status="na" /> | | | | | |
...
| **Request Cancellation** | <Tooltip tip={<>Cancellation during remote prefill is not supported in disaggregated mode. <a href="../backends/sglang/README.md">Source</a></>}><InteractionStatus status="wip" noteLabel="Remote-prefill limitation" /></Tooltip> | <InteractionStatus status="yes" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="wip" /> | <InteractionStatus status="wip" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | | | |
...
| **SLA-Based Planner** | <InteractionStatus status="yes" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | | | | | | | |
| **KV Block Manager** | <InteractionStatus status="yes" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | | | | | | |
| **Multimodal** | <Tooltip tip={<>Supports EP/D and E/P/D image flows with image URLs and pre-computed embeddings. <a href="../features/multimodal/multimodal-trtllm.md">Source</a></>}><InteractionStatus status="yes" noteLabel="Disaggregated image flows" /></Tooltip> | <Tooltip tip={<>Workers must publish KV events with block reuse enabled. <a href="../features/multimodal/multimodal-kv-routing.md">Source</a></>}><InteractionStatus status="yes" noteLabel="Image-aware KV routing" /></Tooltip> | <InteractionStatus status="na" /> | <InteractionStatus status="yes" /> | <InteractionStatus status="na" /> | | | | | |
...
Every stable release and patch, grouped by minor line — newest first. Each release breaks the backend pins out by CUDA major so a reader can scan which majors ship and what driver each needs before opening a card. Platform previews and model-specific builds are covered in the notes below; the [Releases (machine-readable)](releases-data.mdx) page has the full inventory.
