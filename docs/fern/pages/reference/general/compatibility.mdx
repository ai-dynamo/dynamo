...
In Kubernetes deployments, worker spec changes during a rolling update create generation-specific Dynamo namespaces. The frontend watches the deployment's base namespace prefix, so it can discover overlapping generations without allowing direct worker-to-worker communication across those generations. In a disaggregated deployment, prefill and decode workers in the same generation share one namespace and remain isolated from other generations.
...
Worker count is a capacity approximation, not a measurement of end-to-end serving capacity. Selection between WorkerSets also does not provide request stickiness. See [Rolling Updates](../../developer-guide/knowledge-base/kubernetes/kubernetes-operator/rolling-update.md) for behavior by workload type.
