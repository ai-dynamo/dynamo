...
When you route through the Gateway API Inference Extension, the endpoint picker (EPP) is scraped by its own `dynamo-epp` PodMonitor on the `metrics` port (9090). Set `DYN_EPP_METRICS_PORT` to move that endpoint, or `0` to disable it.
...
# Prefix-cache hits the model server reported, as observed by the endpoint picker
dynamo_epp_cached_tokens_bucket
```
