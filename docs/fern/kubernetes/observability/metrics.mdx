...
In Gateway API Inference Extension deployments there is no standalone frontend pod. Each worker pod instead runs a frontend sidecar in `--router-mode direct`, which exposes the `dynamo_frontend_*` families (including `dynamo_frontend_cached_tokens`) on its `http` port. The worker PodMonitor scrapes that port when the sidecar is present, so these deployments report the same frontend metrics as a standalone frontend, attributed per worker pod. Plain workers declare no `http` port and are unaffected. See [PodMonitors created by the Dynamo chart](../../observability/README.md#podmonitors-created-by-the-dynamo-chart) for the full endpoint list.
