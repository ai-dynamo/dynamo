...
This is complementary to [Model Caching](model-caching.mdx): caching gets the weights onto a shared PVC once; this page makes each worker *read* them fast. It applies to the **PVC + Download Job** path, not the ModelExpress object-storage path (which streams from S3/GCS/Azure Blob instead — see [ModelExpress](modelexpress.md)).
...
For SGLang and TensorRT-LLM, the striping fix alone still helps; there is no `runai_streamer` equivalent to add. (SGLang's fast-distribution path is ModelExpress `remote_instance` — a P2P pull from an already-loaded worker, not a faster first read from storage. See [Model Caching](model-caching.mdx#option-2-modelexpress-p2p-distribution).)
...
Run the normal [PVC + Download Job](model-caching.mdx#step-2-download-the-model). Because the directory default is now wide, every shard lands striped across all OSTs.
...
for f in /mnt/model-cache/hub/models--<org>--<model>/blobs/*; do
  ...
  [ "$calc" = "$name" ] && echo "OK   $name" || echo "BAD  $name"
done
```
...
- [Model Caching](model-caching.mdx) — get the weights onto a shared PVC once (download Job, mount config, ModelExpress)
- [Azure Lustre CSI Driver](cloud-providers/aks/azure-lustre-csi.md) — provision AMLFS and the striping recommendation for it
- [Storage for Model Caching on AKS](cloud-providers/aks/storage.md) — choosing a storage class per cache type
- [Cold Start and Resiliency](cold-start-and-resiliency.md) — snapshot/restore and warm-start options for repeated restarts
