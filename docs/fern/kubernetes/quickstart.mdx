...
Select **NVIDIA GPU**, **Intel GPU**, or **CPU** based on the hardware in your cluster, and follow
that tab throughout the guide. The Intel GPU path uses the XPU runtime and tooling.
  ...
  <Step title="Check the prerequisites">
    <Tabs>
      ...
      <Tab title="CPU">
        ...
        - A Kubernetes v1.30 or later cluster
        - `kubectl` v1.30 or later configured for the cluster
        - Helm v3 or later
        - Docker and access to load images into or push images to the cluster
        - A local clone of the Dynamo repository
      </Tab>
    </Tabs>
  </Step>
...
```
      ...
      <Tab title="CPU">
        CPU deployments do not require an accelerator operator or device plugin. Verify that the
        cluster is ready from the root of your Dynamo clone:
...
```
      ...
      <Tab title="CPU">
        From the root of your Dynamo clone, build the vLLM CPU runtime image:

```bash
export CPU_IMAGE="dynamo:latest-vllm-cpu-runtime"
...
python3 container/render.py \
  ...
  --target=runtime \
  --output-short-filename
docker build --tag "$CPU_IMAGE" --file container/rendered.Dockerfile .
```

        Load the image into a local cluster or push it to a registry accessible to your cluster.
        For example, with kind:

```bash
kind load docker-image "$CPU_IMAGE"
```
...
export FRONTEND_SERVICE=qwen3-cpu-frontend
```
      </Tab>
    </Tabs>
  </Step>
...
```bash
kubectl delete dgd/qwen3-cpu --namespace "$NAMESPACE" --ignore-not-found
```
  </Tab>
</Tabs>
