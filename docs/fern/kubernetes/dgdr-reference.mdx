...
`spec.features.planner` is a `PlannerConfig` object, passed through to the Planner service without field-level validation; the Planner validates it at startup. Setting it (non-null) enables the planner in the generated DGD. The same object is documented standalone at [Planner Configuration](../components/planner/planner-config-reference.mdx); its fields are reproduced here. Nested list-and-object types are expanded under [Additional Types](#additional-types). All fields are optional.
...
<Note>
  `spec.overrides.dgd` is not required to enable the Planner. Use `spec.features.planner` for Planner enablement and configuration; use `spec.overrides.dgd` only to customize the generated DGD after DGDR assembles it. For the deployment workflow, scaling-mode concepts, and cross-field validation rules, see the [Planner Configuration reference](../components/planner/planner-config-reference.mdx) and [Planner Guide](../components/planner/planner-guide.md).
</Note>

## Generated DGD overrides

`spec.overrides.dgd` is a partial `nvidia.com/v1alpha1` DGD object merged into the profiler-generated deployment after Dynamo selects a configuration. Use it when the generated DGD needs a field that DGDR does not expose directly — including router mode, KV-event publication, and other frontend or worker settings, since DGDR does not yet expose first-class router or EPP features. `overrides.dgd` only patches components that already exist in the generated DGD; it cannot add a missing component (such as an `Epp`). Component names depend on the selected backend and topology, so inspect the generated DGD first, especially when `autoApply: false`.
...
For worked examples — per-component env overrides, enabling KV-aware routing on the generated frontend, worker KV-event publication, and EPP/gateway topologies — see [DGDR Examples](dgdr-examples.md), [Router Guide](../components/router/router-guide.md), and [Gateway API Inference Extension](inference-gateway.mdx).
