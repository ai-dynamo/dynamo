...
For the task-oriented walkthrough — authoring a request step by step — see [Auto Deploy with DGDR](dgdr-guide.md). For ready-to-copy manifests, see [DGDR Templates](../templates/dgdr.mdx).
...
<Indent>
  <ParamField path="auth" type="AuthConfig">
    Registry authentication. In the Planner process, an empty `trusted_sources` activates a legacy development-mode unauthenticated fallback and logs a warning. Production deployments should explicitly configure `static_secret`.
  </ParamField>
  <Indent>
...
For worked examples, see [DGDR Templates](../templates/dgdr.mdx). For router configuration and EPP
topologies, see the [Router Guide](../components/router/router-guide.md) and
[Gateway API Inference Extension](inference-gateway.mdx).
...
<CardGroup cols={2}>
  ...
  <Card title="DGDR Templates" icon="file-code" href="../templates/dgdr.mdx">
