...
<Tabs>
  <Tab title="Kubernetes">
    Set the object under `spec.features.planner` in a [DynamoGraphDeploymentRequest](../../kubernetes/dgdr-reference.mdx) (DGDR). DGDR passes it through without field-level validation, mounts it as a `planner-config-*` ConfigMap, and launches the Planner with `--config` pointing at the mounted file. The Planner validates it at startup. Every field below can be set here.
