...
<ParamField path="--tokenizer" type="string" default="default">
  Tokenizer implementation. `default` uses HuggingFace, `fastokens` uses the high-performance hybrid encoder, and `basetenkenizer` uses Baseten Tokenizer for native encoding and decoding. See [Tokenizer](Tokenizer.md).
