 ...
 *
 * Tests for the pure-JS parsing helper in docs/fern/components/DepMetadata.tsx.
 *
 ...
 * `node docs/fern/components/test_dep_metadata.mjs` without a TSX toolchain.
 *
 ...
 * matches the extraction pattern used in docs/fern/js/test_dep_pr_comments.mjs.
 */
...
console.log("");
console.log(
  "statusVariant() — maps a lifecycle status to a pill color; MUST mirror variant() in docs/fern/js/dep-status-pills.js",
);
...
test("Implemented → accepted (green, via /implement/)", () => {
  ...
  // stay identical to variant() in docs/fern/js/dep-status-pills.js.
