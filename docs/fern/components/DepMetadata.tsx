 ...
 * so the companion test file at docs/fern/components/test_dep_metadata.mjs can
 ...
 * to variant() in docs/fern/js/dep-status-pills.js so the on-page card pill and the
 ...
 * Values match ACCENT in docs/fern/js/dep-index.js. */
const STATUS_ACCENT = {
  draft: "#e0a800",
  proposed: "#5b8def",
  accepted: "#76b900",
 ...
 * docs/fern/components/test_dep_metadata.mjs can regex-extract and eval it, matching
 ...
 * `docs/fern/scripts/sync_deps.py` when it pulls DEP bodies from
 ...
 * companion test file at `docs/fern/components/test_dep_metadata.mjs` can regex-
 * extract and eval it under node, matching the extraction pattern used by
 * `docs/fern/js/test_dep_pr_comments.mjs`.
 *
 ...
 * docs/fern/main.css). Every background is paired with an explicit foreground so the
