...
Start the frontend and a vLLM worker with its system status server on port `8081`:
...
<Step title="Query the health endpoints" id="query-the-health-endpoints">

Check the frontend and worker health status:
...
<Steps>
<Step title="Detect an idle endpoint" id="detect-an-idle-endpoint">

After no activity on an endpoint for a configurable wait time (10 seconds by default), Dynamo
triggers a canary health check.

</Step>
<Step title="Send a health check request" id="send-a-health-check-request">

Dynamo sends a lightweight request with a minimal payload that generates one token.

</Step>
<Step title="Reset the timer after activity" id="reset-the-timer-after-activity">

If a normal request arrives, Dynamo resets the canary timer and skips the health check.

</Step>
<Step title="Handle a timeout" id="handle-a-timeout">

If the health check does not respond within the timeout (three seconds by default), Dynamo marks
