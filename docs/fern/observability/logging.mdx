...
<Info>
Set `OTEL_EXPORT_ENABLED=true` on every Dynamo process. Without it, logs never leave the process,
and Loki remains empty regardless of `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`.
</Info>
...
The [local observability stack](../cli/observability.mdx) includes an OpenTelemetry Collector that
receives OTLP on `localhost:4317` and routes traces to Tempo and logs to Loki. In Grafana, the Loki
data source includes a derived field that links `trace_id` labels to Tempo.
...
To collect and visualize logs with Grafana Loki, or view trace context alongside Grafana Tempo,
start the stack as described in [Install Observability](../cli/observability.mdx). The stack includes
Loki, an OpenTelemetry Collector, and Tempo.

</Step>
<Step title="Enable structured logging" id="enable-structured-logging">
...
<Note>
This workflow displays trace context in logs. For persistent trace analysis and visualization in
Grafana Tempo, see [Tracing](tracing.mdx).
</Note>

<Steps toc={true}>
<Step title="Configure trace context in logs" id="configuration-for-logging">
...
<Step title="Send a request" id="example-request">
...
<Step title="Inspect the logs" id="inspect-the-logs">
...
- [Distributed Tracing with Tempo](tracing.mdx)
