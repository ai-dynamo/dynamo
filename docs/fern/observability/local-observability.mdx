...
Select the tab for your accelerator:
...
<Tabs>
  ...
  For a two-GPU prefill/decode deployment, use `./disagg_tracing.sh` instead.
...
The scripts enable JSONL logging and OTLP export over gRPC and assign service names to the Dynamo
processes.
