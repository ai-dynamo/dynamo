...
Create the shared model cache, then download the checkpoint:
...
> [!NOTE]
> The GB200 manifests mount weights from a node-local `hostPath` (a GKE pattern) rather than the `model-cache` PVC. Edit the `models` volume in the deploy manifest to match your cluster, or point it at the PVC.
