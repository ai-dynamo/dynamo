...
<Tabs>
  <Tab title="Kubernetes">
    Set flags in the worker container's `args` and environment variables in its `env`, under the vLLM worker service of a [DynamoGraphDeployment](../../kubernetes/dgd-reference.mdx) (DGD). The operator passes both through to the process without validation.
...
<ParamField path="--multimodal-decode-worker" type="boolean" default="false" deprecated={true}>
...
<CardGroup cols={2}>
  <Card title="vLLM Reference Guide" href="vllm-reference-guide.md" icon="book">
