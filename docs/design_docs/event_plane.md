...
```bash
# vLLM backend
python -m dynamo.vllm --event-plane zmq --model Qwen/Qwen3-0.6B

# SGLang backend
python -m dynamo.sglang --event-plane zmq --model Qwen/Qwen3-0.6B
```
...
- [Discovery](discovery.md) -- Service discovery and coordination (etcd, Kubernetes)
