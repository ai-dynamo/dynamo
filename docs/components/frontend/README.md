...
| `--router-mode` | `round-robin` | Routing strategy: `round-robin`, `random`, `kv`, `token-dp-balance`, `direct`, `least-loaded`, `device-aware-weighted` (`power-of-two` and `least-loaded` use synchronous prefill fallback in disaggregated prefill mode) |
