...
| `GET` | `/workers` | List worker serving state, including overloaded and draining workers (gated by `DYN_ENABLE_FRONTEND_ADMIN_API`, see below) |
| `POST` | `/workers/{worker_id}/drain` | Stop routing new requests to a worker (gated by `DYN_ENABLE_FRONTEND_ADMIN_API`, see below) |
| `POST` | `/workers/{worker_id}/resume` | Resume routing new requests to a drained worker (gated by `DYN_ENABLE_FRONTEND_ADMIN_API`, see below) |
...
| `DYN_ENABLE_FRONTEND_ADMIN_API` | `true` | Frontend admin routes such as `GET /busy_threshold`, `POST /busy_threshold`, `GET /workers`, `POST /workers/{worker_id}/drain`, and `POST /workers/{worker_id}/resume` are not registered (404 instead of 503). Inference, metrics, models, health, and liveness routes are unaffected. |
