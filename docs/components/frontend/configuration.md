...
| `--enable-cache-control` / `--no-enable-cache-control` | `DYN_ENABLE_CACHE_CONTROL` | `false` | Enable experimental cache-control plumbing in the KV router (requires `--router-mode=kv`) |
