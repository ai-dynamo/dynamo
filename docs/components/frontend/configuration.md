...
| `--router-session-affinity-ttl-secs` | `DYN_ROUTER_SESSION_AFFINITY_TTL_SECS` | unset | Enables session affinity with this router-local idle TTL |
| `--router-session-affinity-replica-sync` / `--no-router-session-affinity-replica-sync` | `DYN_ROUTER_SESSION_AFFINITY_REPLICA_SYNC` | `false` | Best-effort sync of session-affinity bindings; requires `--router-session-affinity-ttl-secs` |
