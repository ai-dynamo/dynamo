...
| `--router-decode-active-request-weight` | `DYN_ROUTER_DECODE_ACTIVE_REQUEST_WEIGHT` | `0.0` | Experimental finite, nonnegative block-equivalent decode cost added for each potential active request on a candidate worker, including the incoming request |
