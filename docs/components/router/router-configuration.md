...
Use `--router-prefill-load-scale` when prompt-side load should count more or less than decode-side block load after cache-hit credits are applied. The final score is `prefill_load_scale * adjusted_prefill_blocks + decode_blocks + decode_active_request_weight * active_requests`.

Use `--router-decode-active-request-weight` when decode forward-pass time depends more on the number of active requests than on their resident KV footprint. The value is measured in block-equivalent cost per active request. For example, a weight of `32` makes four active requests contribute the same routing cost as 128 decode blocks. The router captures active-request count with the same worker-load snapshot used for prefill tokens and decode blocks, so enabling the term does not add a second slot-tracker lookup.
