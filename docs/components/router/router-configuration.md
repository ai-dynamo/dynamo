...
- `DYN_ROUTER_REPLICA_SYNC_ORPHAN_TIMEOUT_SECS`: Defensive timeout for stale active-request records received from another router when `--router-replica-sync` is enabled. Defaults to 900 seconds. It does not expire locally owned requests or limit request processing time.
