...
- `--router-decode-active-request-weight`: Experimental finite, nonnegative block-equivalent decode cost added for each potential active request on a candidate worker, including the incoming request. Defaults to 0.
...
Use `--router-prefill-load-scale` when prompt-side load should count more or less than decode-side block load after cache-hit credits are applied. The final score is `prefill_load_scale * adjusted_prefill_blocks + decode_blocks + decode_active_request_weight * potential_active_requests`.

Use `--router-decode-active-request-weight` when decode forward-pass time depends more on the number of active requests than on their resident KV footprint. The value is measured in block-equivalent cost per potential active request, including the incoming request. For example, a weight of `32` makes a projected batch of four requests contribute the same routing cost as 128 decode blocks. The router captures current request count in the same worker-load snapshot used for prefill tokens and decode blocks, then adds the incoming request without a second slot-tracker read.

This setting is experimental and defaults to `0`, which preserves block-only decode scoring. A positive value trades some KV locality for batch-size balance and can help model, runtime, and hardware combinations near a compute-bound roofline knee, such as decode configurations that combine Multi-head Latent Attention (MLA) and Multi-Token Prediction (MTP). It can regress throughput, Time To First Token (TTFT), and Inter-Token Latency (ITL) when decode remains primarily memory-bound. Benchmark representative traffic and start with a small weight before increasing it.
