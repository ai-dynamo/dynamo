...
- `--router-conditional-disagg`: **Experimental.** Enables conditional disaggregation in frontend-embedded disaggregated serving. The prefill router can bypass the remote prefill worker and route directly to a decode worker when the configured policy says local prefill+decode is preferable. Requires `--router-mode kv` and separate prefill/decode worker pools. See [Conditional Disaggregation](router-disaggregated-serving.md#conditional-disaggregation) for the policy and threshold flags.
