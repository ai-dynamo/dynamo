...
- `--router-conditional-disagg`: **Experimental.** Enables conditional disaggregation in frontend-embedded disaggregated serving. The prefill router can bypass the remote prefill worker and route directly to a decode worker when the configured policy says local prefill+decode is preferable. Requires `--router-mode kv`, separate prefill/decode worker pools, and decode-worker KV event publishing. See [Conditional Disaggregation](router-disaggregated-serving.md#conditional-disaggregation) for backend requirements, policy flags, and threshold flags.
