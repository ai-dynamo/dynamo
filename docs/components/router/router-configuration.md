...
- `--router-predicted-ttl-secs`: TTL in seconds for entries in the side indexer used by `--router-predict-on-route` (default: 5.0). Independent of `--router-ttl-secs`; kept short so decisions the engine never confirms (cancelled requests, prefill failures) age out quickly.
