...
要使用 KV Router 启动 Dynamo frontend：
...
- 不能将 `--static-endpoint` 模式与 KV routing 一起使用（请改用动态发现）
...
**限制：**
- 不支持静态 endpoint - KV router 需要通过 etcd 进行动态模型发现，以跟踪 worker 实例及其 KV cache 状态
...
- **[Disaggregated Serving](router-disaggregated-serving.md)**：Prefill 和 decode 路由设置
