...
The cost function combines three worker-specific terms:
...
- **Active-request cost**: An optional block-equivalent charge for the active batch projected after assigning the incoming request. This term can represent decode regimes where step latency follows batch size more closely than resident KV footprint.
...
active_request_blocks = decode_active_request_weight * potential_active_requests
cost = prefill_load_scale * adjusted_prefill_blocks + decode_blocks + active_request_blocks
```
...
For decode load, the router tracks active KV blocks and active-request count for each worker. For every candidate, it projects both values after assigning the incoming request. It captures the current block and request counts in one worker-load snapshot, then adds the incoming request without a second slot-tracker read. By default, the cost covers the prompt-side blocks that are already assigned to active requests and frees them when each request finishes.
...
When `--router-decode-active-request-weight` is positive, every potential active request, including the incoming request, adds the configured block-equivalent cost. This can improve data-parallel batch balance when decode latency is sensitive to request count, but it can reduce cache locality or over-penalize batches when KV-memory traffic remains the dominant cost. The default is `0`.
