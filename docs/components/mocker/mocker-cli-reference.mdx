...
[Live Simulation with Mocker](../../dynosim/mocker.md); for the engine internals these flags
configure, see [Mocker Engine Architecture](../../design-docs/mocker-architecture.md).

The same engine settings apply to [DynoSim Runs](../../dynosim/runs.md): `python -m dynamo.replay`
...
See [Live Simulation with Mocker](../../dynosim/mocker.md#performance-modeling-setup) for setup.
...
<ParamField path="--durable-kv-events" type="boolean" default="false">
  Deprecated JetStream KV-event mode. Prefer the local indexer / event-plane subscriber path.
...
<CardGroup cols={2}>
  <Card title="Live Simulation with Mocker" icon="server" href="../../dynosim/mocker.md">
    Deploy and run the mocker worker locally or on Kubernetes.
  </Card>
  <Card title="DynoSim Runs" icon="play" href="../../dynosim/runs.md">
    Run one workload through a simulated configuration with `python -m dynamo.replay`.
  </Card>
  <Card title="Mocker Engine Architecture" icon="diagram-project" href="../../design-docs/mocker-architecture.md">
    Scheduler, KV block manager, eviction, and timing internals.
  </Card>
  <Card title="Planner Configuration" icon="sliders" href="../planner/planner-config-reference.mdx">
