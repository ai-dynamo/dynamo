...
| 字段 | 类型 | 默认值 | 说明 |
|-------|------|---------|-------------|
| `scheduling.scale_interval_seconds` | float | legacy loop interval 的最大公约数 | 基础 pipeline cadence。Pipeline 每个 interval 唤醒一次；每个 plugin 的 `execution_interval_seconds` 决定该 plugin 是否在当前 tick 执行。默认值是 `load_adjustment_interval_seconds` 和启用 throughput scaling 时 `throughput_adjustment_interval_seconds` 的最大公约数，以保留旧配置的 fire time。 |
| `scheduling.tick_max_duration_seconds` | float | `30.0` | 包住完整 plugin pipeline 的外层 deadline。超时会中止当前 tick；下一次 tick 从干净状态继续。 |
| `scheduling.use_orchestrator` | bool | `true` | 已废弃的兼容字段。旧 DGDR/YAML payload 即使设置为 `false` 也能解析，但 planner 会将其归一化为 `true`；运行时不再有其他 planner 路径。 |
