...
[engine fallback](chat-processors.mdx#engine-fallback)。
...
> [!IMPORTANT]
> `--dyn-tool-call-parser` 与默认的 `dynamo` chat processor 配对，放在 **worker** 上。裸的 `--tool-call-parser`（无 `--dyn-` 前缀）是另一个 flag——它驱动 [engine fallback](chat-processors.mdx#engine-fallback) 并放在 Frontend 上。不要在默认 processor 下使用裸 flag，也不要把 `--dyn-tool-call-parser` 放在 `vllm`/`sglang` chat processor 上。
...
**Upstream name** 列标出 vLLM 或 SGLang 的解析器名称与 Dynamo 不同之处——在使用 `--dyn-chat-processor vllm` 或 `sglang` 时（参阅 [engine fallback](chat-processors.mdx#engine-fallback)）尤为相关。upstream 列为空表示同名在各处通用。`Dynamo-only` 表示该格式没有对应的上游解析器。
