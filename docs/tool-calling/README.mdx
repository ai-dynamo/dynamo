...
## Release probe snapshot (Dynamo 1.2)

This is a one-time Dynamo 1.2.0 release snapshot from the tool-calling probe
harness, generated on 2026-06-05 at 07:24 UTC. It is **not** a live dashboard.
The probe exercises tool-calling workflows; reasoning appears only where a
model emits tool calls and reasoning in the same native format, or where
final assistant text is misrouted into the reasoning channel by the parser.
...
cases and runtime or endpoint/deployment failures where the request timed out
before a usable OpenAI response was returned. **Other errors** counts
engine/model behavior and mixed/needs-inspection failures. The probe classifier
buckets each failure as:
...
- **Endpoint/deployment**: the request timed out before a usable response.
  Counted as a Dynamo runtime failure in this static table.
...
of treating every row as an exact A/B pass-rate comparison. With this
classification, Dynamo runtime/parser/API failures improve on Kimi K2.6,
GLM 5.1, and Qwen3.6-35B-A3B. MiniMax 2.7 improves in total failures, but its
remaining parser-boundary failure count is unchanged.

Select a model to see its per-scenario results:
