...
| `sidecar` | Unified multi-architecture sidecar image build, publish, and compliance checks for changes under `lib/sidecar/**` (docs excluded) |
...
> **TODO:** The sidecar image also consumes root Cargo files, shared libraries, compliance files, and composite actions. Expanding the filter to cover every build input is deferred until the additional PR CI fan-out is evaluated and agreed.
