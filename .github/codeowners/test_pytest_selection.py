...
def test_backend_text_files_are_not_treated_as_documentation(path: str) -> None:
    spec = yaml.safe_load(
        (REPO_ROOT / ".github/codeowners/areas.yaml").read_text(encoding="utf-8")
    )
    plan = build_plan(compute_resolution(spec), [path])

    assert plan.mode == "markers"
    assert not plan.ignored_paths
    assert all(plan.lanes[lane].mode == "full" for lane in BACKEND_MARKERS)
    assert plan.lanes["generic"].mode == "none"


def test_unowned_text_file_falls_back_to_full() -> None:
    plan = build_plan(_model(), ["notes/release.txt"])
...
def test_repository_selective_cpu_jobs_keep_default_marker_dimensions() -> None:
    ...
    assert len(selective_marker_inputs) == 3
    assert all("and (unit or ({0}))" in line for line in selective_marker_inputs)
    assert all("gpu_0" in line for line in selective_marker_inputs)
    assert all(
    ...
    assert workflow.count("_mode == 'markers'") == 3
    assert workflow.count("_mode == 'none'") == 3
    assert workflow.count("and unit' ||") == 3
    for backend in BACKEND_MARKERS:
        assert (
            f"gpu_test_markers: pre_merge and {backend} and gpu_1 and not sidecar"
            in workflow
        )
        assert f"gpu_test_markers: pre_merge and {backend} and gpu_2" in workflow
    assert "cpu_e2e_test_markers: pre_merge and vllm and gpu_0 and e2e" in workflow
    assert (
