...
| **`/diagnose`** | AI-powered CI failure analysis with root cause and fix suggestions. |
| **Auto-classifier** | Reusable workflow for CI pipelines to post error analysis on failure. |

## Slash Commands

| Command | Access | Description |
|---------|--------|-------------|
| `/diagnose` | Any contributor (not first-time) | Analyze CI failures with LLM |
...
| `lgtm-bot-diagnose.yml` | `/diagnose` comment | AI diagnostics |
| `classify-workflow-errors-reusable.yml` | Called by CI pipelines | Auto-classify on failure |
...
.github/workflows/lgtm-bot-diagnose.yml   → /diagnose trigger
.github/workflows/classify-*-reusable.yml  → reusable for CI pipelines
.github/scripts/lgtm_bot_diagnose.py      → diagnose logic (gh CLI)
.github/scripts/classify_workflow_errors.py → classifier entry point
error_classification/llm_client.py         → LLM API client
error_classification/config.py             → env config
```

All scripts use `gh api` for GitHub API calls — no raw HTTP.
