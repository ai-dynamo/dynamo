...
def test_claude_connection():
    ...
    # Try to create a PR comment if in PR context
    github_token = os.getenv("GITHUB_TOKEN")
    if github_token:
        print("-" * 70)
        print("💬 Testing PR comment creation...")
        print("-" * 70)
        print()

        try:
            annotator = GitHubAnnotator()
            pr_number = annotator._get_pr_number()
            ...
            if pr_number:
                ...
                # Create mock classifications from test results
                mock_classifications = [
                    MockClassification(
                        error_id=f"test-{i}",
                        job_name="test-claude-connection",
                        step_name="Test Claude API",
                        primary_category=r["actual"],
                        confidence_score=r["confidence"],
                        root_cause_summary=f"Test classification for {r['expected']} error"
                    )
                    for i, r in enumerate(results)
                ]

                success = annotator.create_pr_comment(mock_classifications, {})

                if success:
                    print("   ✅ PR comment created successfully")
                else:
                    print("   ⚠️  PR comment creation failed (may not be PR context)")
            else:
                print("   ℹ️  Not in PR context, skipping PR comment")
        except Exception as e:
            print(f"   ⚠️  Error testing PR comment: {e}")
