#!/bin/bash
#SBATCH --job-name=core_dlfw_ci-benchx.4ctx4gen.dynamo.kvrouter.newcfg
#SBATCH --nodes=2
#SBATCH --partition=gb200
#SBATCH --account=core_dlfw_ci
#SBATCH --time=04:00:00
#SBATCH --output=bench/logs/run_benchx_4ctx4gen_dynamo_kvrouter_new_config_%j.log
#SBATCH --error=bench/logs/run_benchx_4ctx4gen_dynamo_kvrouter_new_config_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 4 ctx (TP=1) + 4 gen (TP=1 each) with
# ConversationRouter, driven by dynamo.trtllm (etcd + nats + dynamo frontend)
# instead of trtllm-serve disaggregated.
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 4 ctx workers (GPUs 0-3)
#   NODE1 — 4 gen workers, one per GPU (GPUs 0-3)
#
...
#
# Submit:
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_4ctx4gen_dynamo_kvrouter_new_config.sh
# =============================================================================
...
EXP_NAME="run_benchx_4ctx4gen_dynamo_kvrouter_new_config_${HCTAG}_c${CONCURRENCY}"
...
# --- 4 gen workers on $NODE1 GPUs 0-3 (decode, TP=1 each) ---
GEN_PIDS=()
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:${DYN_SYS_PORT_CTX_0},${NODE0}:${DYN_SYS_PORT_CTX_1},${NODE0}:${DYN_SYS_PORT_CTX_2},${NODE0}:${DYN_SYS_PORT_CTX_3},${NODE1}:${DYN_SYS_PORT_GEN_0},${NODE1}:${DYN_SYS_PORT_GEN_1},${NODE1}:${DYN_SYS_PORT_GEN_2},${NODE1}:${DYN_SYS_PORT_GEN_3}" \
