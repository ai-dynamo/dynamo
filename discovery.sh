...
# Delete all DynamoWorkerMetadata CRs
delete_all_metadata() {
    log_info "Deleting all DynamoWorkerMetadata CRs in namespace '${NAMESPACE}'..."
    
    local count=$(kubectl get dynamoworkermetadatas -n "${NAMESPACE}" --no-headers 2>/dev/null | wc -l)
    
    if [[ "$count" -eq 0 ]]; then
        log_info "No DynamoWorkerMetadata CRs found"
        return
    fi
    
    kubectl delete dynamoworkermetadatas --all -n "${NAMESPACE}"
    log_success "Deleted ${count} DynamoWorkerMetadata CR(s)"
}

# List all DynamoWorkerMetadata CRs
list_metadata() {
    log_info "DynamoWorkerMetadata CRs in namespace '${NAMESPACE}':"
    echo ""
...
# List all resources (EndpointSlices and DynamoWorkerMetadata CRs)
list_all() {
    list_endpoint_slices
    echo ""
    list_metadata
...
Commands:
  ...
  delete-all-metadata
      Delete all DynamoWorkerMetadata CRs in the namespace.
  ...
  list-all
      List both EndpointSlices and DynamoWorkerMetadata CRs.
