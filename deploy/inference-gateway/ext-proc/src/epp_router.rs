...
impl TokenizeError {
    ...
    fn should_degrade(&self) -> bool {
        match self {
            ...
            | TokenizeError::Render(VllmRenderError::Timeout { .. }) => true,
            TokenizeError::Render(VllmRenderError::UpstreamStatus { status, .. }) => {
            ...
            TokenizeError::Render(VllmRenderError::InvalidResponse { .. })
            | TokenizeError::Render(VllmRenderError::ResponseTooLarge { .. }) => false,
        }
    }
        ...
        match self {
            ...
            TokenizeError::Render(e) => {
                ...
                match e {
                    ...
                    // 4xx, and 5xx are not client payload errors. The transient
                    // subset is handled before this mapper by `should_degrade`.
...
mod tests {
    ...
    async fn transient_renderer_failure_degrades_and_keeps_priority() {
        ...
        let render_error = renderer
            ...
            .unwrap_err();
        server.await.unwrap();
        assert!(matches!(&render_error, VllmRenderError::Unavailable { .. }));
        ...
        server.abort();
        let _ = server.await;
        assert!(matches!(&timeout_error, VllmRenderError::Timeout { .. }));
        let degraded = tokenized_or_load_only(
            Err(TokenizeFailure {
    ...
    fn tokenization_failure_policy_keeps_bad_input_visible() {
        ...
        assert!(matches!(malformed, Err(PickError::TokenizationFailed(_))));
        ...
        for status in [StatusCode::BAD_REQUEST, StatusCode::UNPROCESSABLE_ENTITY] {
            ...
            assert!(
                matches!(result, Err(PickError::TokenizationFailed(_))),
        ...
        let contract_error = tokenized_or_load_only(
            Err(TokenizeFailure {
                priority_jump: Some(1.0),
                strict_priority: Some(2),
                error: TokenizeError::Render(VllmRenderError::InvalidResponse {
