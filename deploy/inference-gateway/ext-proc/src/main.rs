...
use anyhow::Result;
use dynamo_ext_proc::{ExtProcServer, Router, SelectorConfig};
...
async fn main() -> Result<()> {
    ...
    let router_only = SelectorConfig::is_enabled();
    ...
    if router_only {
        let selector_cfg = SelectorConfig::from_env()?;
        tracing::info!(
        ...
        let router = dynamo_ext_proc::SelectorRouter::from_selector(selector_cfg).await?;
