...
pub struct SelectorConfig {
    ...
    /// [`SelectorMode::Http`]; unused for [`SelectorMode::Embedded`].
    pub selector_urls: Vec<String>,
    /// KV indexer thread-pool size for [`SelectorMode::Embedded`].
...
impl SelectorConfig {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        let mode = match env_trimmed("DYN_EPP_SELECTOR_MODE").as_deref() {
            None => default_mode(),
            Some("http") => SelectorMode::Http,
            Some("embedded") => SelectorMode::Embedded,
        ...
        let selector_urls = env_trimmed("DYN_EPP_SELECTOR_URLS")
            ...
            .unwrap_or_default();
        if matches!(mode, SelectorMode::Http) && selector_urls.is_empty() {
            anyhow::bail!(
...
mod tests {
    ...
    fn mode_detection() {
        ...
        clear_all();
        assert!(!SelectorConfig::is_enabled());
    ...
    fn parses_required_and_defaults() {
        ...
        let cfg = SelectorConfig::from_env().expect("config should parse");
    ...
    fn embedded_mode_does_not_require_urls() {
        ...
        let cfg = SelectorConfig::from_env().expect("embedded config should parse without urls");
        assert_eq!(cfg.mode, SelectorMode::Embedded);
    ...
    fn invalid_selector_mode_fails() {
        ...
        assert!(SelectorConfig::from_env().is_err());
    }
