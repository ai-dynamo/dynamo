...
pub struct RawWorker {
    ...
    /// Restart-stable routing identity. Decoupled from `worker_id` so sticky
    /// routing can survive a pod restart even though the pod name (and thus
    /// `worker_id`) changes.
...
impl PodDiscovery {
    ...
    /// is still `Ready` and pool-selected.
    pub fn resolve_endpoint(&self, worker_id: u64) -> Option<String> {
        self.ready_workers()
            .into_iter()
            .find(|w| w.worker_id == worker_id)
            .map(|w| strip_scheme(&w.http_endpoint).to_string())
    }
