...
///
/// Only DP rank 0 is supported today. Multi-DP routing requires per-rank
/// inference HTTP ports in addition to per-rank KV-event ports; that design
...
fn raw_worker_from_pod(
    ...
    Some(RawWorker {
        ...
        kv_events_endpoints: HashMap::from([(
            0u32,
            format!("tcp://{}", SocketAddr::new(ip, kv_event_port)),
        )]),
...
mod tests {
    ...
    fn terminating_pod_is_skipped() {
        ...
        assert!(raw_worker_from_pod(&p, &pool(), 5557u16, None).is_none());
    }
