...
//!   for routing only),
//! - a [`SelectorReflector`] that discovers Ready raw vLLM pods from Kubernetes,
...
impl SelectorRouter {
    ...
    pub async fn from_selector(cfg: SelectorConfig) -> Result<Self> {
...
#[tonic::async_trait]
impl EndpointPicker for SelectorRouter {
    async fn pick(
        &self,
        req: &RequestInfo,
        _endpoints: &[Endpoint],
