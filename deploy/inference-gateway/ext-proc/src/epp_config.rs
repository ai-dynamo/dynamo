...
//! `router-only` (fronts raw `vllm serve` pods with no Dynamo runtime and runs an
//! in-process, runtime-free KV-aware selector). The pod selector and target port
//! come from the `InferencePool`; the rest of the router-only contract comes from
//! the environment and is parsed here.
...
impl EppConfig {
    ...
    fn parse(get: &EnvGet) -> anyhow::Result<Self> {
        ...
        let peer_sync_port =
            opt_parse::<u16>(get, "DYN_EPP_PEER_SYNC_PORT")?.unwrap_or(DEFAULT_PEER_SYNC_PORT);
