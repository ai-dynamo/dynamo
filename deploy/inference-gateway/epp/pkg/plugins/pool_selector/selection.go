...
// PriorityRetryOrder returns the selected pool followed by faster pools (lower
// priority number = faster), slowest-to-fastest, for fallback retries. Mirrors
// the Global Router's get_priority_retry_order. When enable is false, only the
// selected pool is returned.
func PriorityRetryOrder(selectedPool int, poolPriorities []int, enable bool) ([]int, error) {
	if !enable {
		return []int{selectedPool}, nil
	}
	if selectedPool < 0 || selectedPool >= len(poolPriorities) {
		return nil, fmt.Errorf("selectedPool %d out of range for %d pool priorities", selectedPool, len(poolPriorities))
	}
	selectedPriority := poolPriorities[selectedPool]

	// Unique faster priorities, sorted descending (slowest faster → fastest).
	fasterSet := map[int]struct{}{}
	for _, p := range poolPriorities {
		if p < selectedPriority {
			fasterSet[p] = struct{}{}
		}
	}
	faster := make([]int, 0, len(fasterSet))
	for p := range fasterSet {
		faster = append(faster, p)
	}
	// descending sort
	for i := 0; i < len(faster); i++ {
		for j := i + 1; j < len(faster); j++ {
			if faster[j] > faster[i] {
				faster[i], faster[j] = faster[j], faster[i]
			}
		}
	}

	order := []int{selectedPool}
	for _, prio := range faster {
		for poolIdx, poolPrio := range poolPriorities {
			if poolPrio == prio {
				order = append(order, poolIdx)
			}
		}
	}
	return order, nil
