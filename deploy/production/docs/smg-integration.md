...
|---|---|---|
| **SMG** (`addons/smg/`) | Client-facing OpenAI-compatible HTTP surface, model selection (`/v1/models`), retry, circuit-breaker, request timeouts, server-sent observability (Prometheus + tracing), optional history/audit storage, MCP tooling pass-through | Prefix routing, KV state, GPU orchestration, kernel selection |
...
| **SGLang prefill / decode workers** | Kernel-level inference: forward pass, KV cache management, speculative decoding (SMC-SD), HiSparse top-k selection (decode only), IndexCache / TurboQuant for the DSA indexer and dense KV | Routing decisions across workers (Dynamo owns that) |
...
## What did *not* change in this addon

- 4-GPU prefill + 4-GPU decode in `examples/deepseek-v32-reap-sglang.yaml`: zero changes. SMG runs CPU-only (no `nvidia.com/gpu` requests); the disaggregated split is preserved as-is on the single B200 A4 node.
...
The 8 B200 GPUs remain split 4+4 prefill/decode. SMG adds at most a 4-CPU / 4-GiB footprint on the same node. No GPU contention with the SGLang workers.
