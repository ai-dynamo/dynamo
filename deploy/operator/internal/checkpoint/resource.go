...
func CreateOrGetAutoCheckpoint(
	...
	ckpt := &nvidiacomv1alpha1.DynamoCheckpoint{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				commonconsts.CheckpointDeletionPolicyAnnotation:      string(DeletionPolicy(deletionPolicy)),
			},
		},
	...
	if UsesRetainDeletionPolicy(deletionPolicy) {
	...
	if err := c.Create(ctx, ckpt); err != nil {
		...
		desiredDeletionPolicy := string(DeletionPolicy(deletionPolicy))
