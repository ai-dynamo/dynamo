...
const (
	...
	// SnapshotControlDirEnv is the canonical environment variable exposing
	// the control mount path to the workload.
	SnapshotControlDirEnv = "SNAPSHOT_CONTROL_DIR"

	// LegacySnapshotControlDirEnv is retained while Dynamo workloads migrate
	// to SnapshotControlDirEnv.
...
// each see an isolated view), and sets SNAPSHOT_CONTROL_DIR plus the deprecated
// DYN_SNAPSHOT_CONTROL_DIR alias on the container's env. Idempotent — safe to
// call from multiple code paths
