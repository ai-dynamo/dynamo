...
func TestEffectiveComponentForRoleAutomaticWorkerOverrides(t *testing.T) {
	...
	equivalent := component.DeepCopy()
	baseMain := GetMainContainer(equivalent)
	equivalent.Multinode.Worker.PodTemplateOverrides = &v1beta1.MultinodePodTemplateOverrides{
		Metadata: &v1beta1.MultinodePodTemplateMetadataOverrides{
			Labels:      ptr.To(maps.Clone(equivalent.PodTemplate.Labels)),
			Annotations: ptr.To(maps.Clone(equivalent.PodTemplate.Annotations)),
		},
		Spec: &v1beta1.MultinodePodSpecOverrides{
			NodeSelector:     ptr.To(maps.Clone(equivalent.PodTemplate.Spec.NodeSelector)),
			Tolerations:      ptr.To(equivalent.PodTemplate.Spec.Tolerations),
			ResourceClaims:   ptr.To(equivalent.PodTemplate.Spec.ResourceClaims),
			ImagePullSecrets: ptr.To(equivalent.PodTemplate.Spec.ImagePullSecrets),
			Containers: []v1beta1.MultinodeContainerOverride{{
				Name:      commonconsts.MainContainerName,
				Image:     ptr.To(baseMain.Image),
				Command:   ptr.To(baseMain.Command),
				Args:      ptr.To(baseMain.Args),
				Env:       ptr.To(baseMain.Env),
				Resources: &v1beta1.MultinodeContainerResourceOverrides{Claims: ptr.To(baseMain.Resources.Claims)},
			}},
		},
	}
	equivalentWorker, err := EffectiveComponentForRole(equivalent, RoleWorker)
	if err != nil {
		t.Fatalf("resolve worker with equivalent overrides: %v", err)
	}
	if !reflect.DeepEqual(equivalentWorker.PodTemplate, equivalent.PodTemplate) {
		t.Fatalf("equivalent worker overrides changed the effective pod template: %#v", equivalentWorker.PodTemplate)
	}
}
