...
func TestWorkerHealthCheckDefaultsByBackendMode(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "vLLM native decode worker enables canary",
			backend:   &VLLMBackend{},
		...
		{
			name:      "vLLM legacy decode flag enables canary",
			backend:   &VLLMBackend{},
			component: workerHealthComponent(v1beta1.ComponentTypeWorker),
		...
		{
			name:      "vLLM disaggregation decode mode enables canary",
			backend:   &VLLMBackend{},
			component: workerHealthComponent(v1beta1.ComponentTypeWorker),
		...
		{
			name:      "TRT-LLM decode mode enables canary",
			backend:   &TRTLLMBackend{},
			component: workerHealthComponent(v1beta1.ComponentTypeWorker),
			container: workerHealthContainerWithArgs("--disaggregation-mode decode"),
			want:      envValueTrue,
		...
		{
			name:      "SGLang tokenizer flag enables canary",
			backend:   &SGLangBackend{},
			component: workerHealthComponent(v1beta1.ComponentTypeWorker),
		...
		{
			name:      "SGLang tokenizer env enables canary",
			backend:   &SGLangBackend{},
			component: workerHealthComponent(v1beta1.ComponentTypeWorker),
