...
func TestAggregatePlacementScore_CurrentBehavior(t *testing.T) {
	tests := []struct {
		name      string
		pcs       *grovev1alpha1.PodCliqueSet
		wantScore *float64
		wantState v1beta1.PlacementScoreState
	}{
		{
			name:      "nil pod clique set",
			pcs:       nil,
			wantScore: nil,
			wantState: v1beta1.PlacementScoreStateUnsupported,
		},
		{
			name: "no pod gangs reported",
			pcs: &grovev1alpha1.PodCliqueSet{
		...
		},
		{
			...
			pcs: &grovev1alpha1.PodCliqueSet{
				Status: grovev1alpha1.PodCliqueSetStatus{
					PodGangStatutes: []grovev1alpha1.PodGangStatus{{
						Name:  "pg-0",
						Phase: grovev1alpha1.PodGangRunning,
					}},
				},
			},
			wantScore: nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			score, state := AggregatePlacementScore(tt.pcs)
