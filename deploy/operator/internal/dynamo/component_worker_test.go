...
func TestWorkerDefaultsCanaryHealthCheckVersionGate(t *testing.T) {
	tests := []struct {
		...
		{
			name:                     "older runtime",
			runtimeVersion:           runtimeversion.Version{Major: 1, Minor: 3, Patch: 0},
		...
		},
		{
			...
			runtimeVersion:           runtimeversion.Version{Major: 1, Minor: 4, Patch: 0},
			runtimeVersionKnown:      true,
			wantEnabled:              "true",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			container, err := NewWorkerDefaults().GetBaseContainer(ComponentContext{
				...
				RuntimeVersionKnown: tt.runtimeVersionKnown,
			})
