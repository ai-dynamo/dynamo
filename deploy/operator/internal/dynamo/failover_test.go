...
func TestBuildFailoverPod_SGLangDisablesPiecewiseCudaGraph(t *testing.T) {
	...
	shellWrapped := intraPodFailoverPodSpec()
	shellWrapped.Containers[0].Command = []string{"sh", "-c"}
	shellWrapped.Containers[0].Args = []string{"exec python3 -m dynamo.sglang --model-path Qwen/Qwen3-0.6B"}
	err = buildFailoverPod(&shellWrapped, 1, BackendFrameworkSGLang)
	require.NoError(t, err)
	for i := range 2 {
		assert.True(t, containerCommandLineHasFlag(&shellWrapped.Containers[i], sglangDisablePiecewiseCudaGraphFlag), "engine-%d", i)
		assert.Equal(t, 0, countArg(shellWrapped.Containers[i].Args, sglangDisablePiecewiseCudaGraphFlag), "engine-%d", i)
		assert.Contains(t, shellWrapped.Containers[i].Args[0], "dynamo.sglang --disable-piecewise-cuda-graph")
	}
}
