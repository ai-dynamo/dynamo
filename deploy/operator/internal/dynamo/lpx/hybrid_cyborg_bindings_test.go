...
func TestRenderSelectedCyborgConfigMapServerNames(t *testing.T) {
	...
	for _, replicas := range []int32{1, 3, 9} {
		t.Run(strconv.Itoa(int(replicas)), func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Len(t, configMap.Data, int(replicas)+1)
			for replica := int32(0); replica < replicas; replica++ {
				key := "lpu_servers"
				if replicas > 1 {
					key += "-" + strconv.Itoa(int(replica))
				}
				prefix := plan.LPXScalingGroupTemplate + "-" + strconv.Itoa(int(replica)) + "-"
				require.Equal(t, prefix+"lpu-wkr-m-0-0\n"+prefix+"lpu-wkr-m-0-2", configMap.Data[key])
			}
	...
	_, err = renderSelectedCyborgConfigMap("test-namespace", "test-dgd", plan, "/models", &projection.configuredBuild)
