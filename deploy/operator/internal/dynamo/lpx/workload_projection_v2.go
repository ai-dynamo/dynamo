...
func appendV2ModelProjections(dst []*ModelProjection, intent ModelProjectionInput, modelSettings map[string]any) ([]*ModelProjection, error) {
	...
	// Hybrid embeddings always come from Cyborg, after validating authored settings.
	if !usesResolvedRuntime {
		if _, err := boolSetting(modelSettings, "cpu_embeddings"); err != nil {
			return nil, fmt.Errorf("model settings.%w", err)
		}
		configured.omitStandaloneEmbeddingPartition()
	}
	...
	if intent.Pipeline == PipelineLPX && len(configured.SelectedPropSyncChains) != 0 {
		omittedSourceZero := len(partitions) < len(connectorBuild.Partitions)
		...
		for _, chain := range connectorBuild.SelectedPropSyncChains {
			runtimeChain := chain
			if omittedSourceZero && runtimeChain[0] == 0 {
				runtimeChain = runtimeChain[1:]
			}
			if len(runtimeChain) >= 2 {
				runtimeChainByRoot[runtimeChain[0]] = runtimeChain
			}
		}
