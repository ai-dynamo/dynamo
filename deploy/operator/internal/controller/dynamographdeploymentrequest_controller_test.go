...
// Helper function to create v1beta1 DGDR with Legacy fields for testing
// This wraps v1alpha1-style config into v1beta1 Legacy struct
func createTestDGDRv1beta1(name, namespace, model, backend, profilerImage string, config *apiextensionsv1.JSON) *nvidiacomv1beta1.DynamoGraphDeploymentRequest {
	return &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
		...
		Spec: nvidiacomv1beta1.DynamoGraphDeploymentRequestSpec{
			Model:   model,
			Backend: nvidiacomv1beta1.BackendType(backend),
			Image:   profilerImage, // v1beta1 uses spec.image for profiler
			Legacy: &nvidiacomv1beta1.LegacySpec{
				Backend: backend,
				ProfilingConfig: &nvidiacomv1beta1.LegacyProfilingConfigSpec{
...
var _ = Describe("DynamoGraphDeploymentRequest Controller", func() {
	...
	Context("When reconciling initial DGDR", func() {
		It("Should validate spec and transition to Pending", func() {
			...
			dgdr := createTestDGDRv1beta1(
				dgdrName,
				namespace,
				"test-model",
				"vllm",
				"test-profiler:latest",
				createTestConfig(map[string]interface{}{
			...
			// Check status (v1beta1 uses Legacy.State)
			Eventually(func() string {
				...
				if updated.Status.Legacy != nil {
					return updated.Status.Legacy.State
				}
				return ""
			}, timeout, interval).Should(Equal(string(nvidiacomv1alpha1.DGDRStatePending)))
	...
	Context("When creating profiling job", func() {
		It("Should create online profiling job", func() {
			...
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				...
				Spec: nvidiacomv1beta1.DynamoGraphDeploymentRequestSpec{
					Model:   "test-model",
					Backend: nvidiacomv1beta1.BackendType("vllm"),
					...
					Legacy: &nvidiacomv1beta1.LegacySpec{
						...
						ProfilingConfig: &nvidiacomv1beta1.LegacyProfilingConfigSpec{
							...
							ConfigMapRef: &nvidiacomv1beta1.LegacyConfigMapKeySelector{
								Name: "test-config",
								Key:  "disagg.yaml",
							},
						},
		...
		It("Should create offline (AIC) profiling job", func() {
			...
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				...
				Spec: nvidiacomv1beta1.DynamoGraphDeploymentRequestSpec{
					Model:   "test-model",
					Backend: nvidiacomv1beta1.BackendType("trtllm"),
					Image:   "test-profiler:latest",
					Legacy: &nvidiacomv1beta1.LegacySpec{
						Backend: "trtllm",
						ProfilingConfig: &nvidiacomv1beta1.LegacyProfilingConfigSpec{
	...
	Context("When profiling completes", func() {
		It("Should generate DGD spec from ConfigMap", func() {
			...
			dgdr.Status.Legacy.State = string(nvidiacomv1alpha1.DGDRStateProfiling)
  ...
  services:
    Frontend:
      replicas: 1`
			...
			Expect(updated.Status.Legacy).NotTo(BeNil())
			Expect(updated.Status.Legacy.GeneratedDeployment).NotTo(BeNil())

			// Verify state transitioned to Ready (since autoApply is false by default)
			Expect(updated.Status.Legacy.State).Should(Equal(string(nvidiacomv1alpha1.DGDRStateReady)))
		})
	})
  ...
  services:
    Frontend:
      replicas: 1`
			...
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: dgdrName, Namespace: namespace}, &updated)).Should(Succeed())
Expect(updated.Status.Legacy).NotTo(BeNil())
				Expect(updated.Status.Legacy.State).Should(Equal(string(nvidiacomv1alpha1.DGDRStateDeploying)))
			...
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: dgdrName, Namespace: namespace}, &updated)).Should(Succeed())
			Expect(updated.Status.Legacy).NotTo(BeNil())
			Expect(updated.Status.Legacy.Deployment).NotTo(BeNil())
			Expect(updated.Status.Legacy.Deployment.Created).Should(BeTrue())
			Expect(updated.Status.Legacy.Deployment.Name).Should(Equal("test-dgd-auto"))
	...
	Context("When enforcing spec immutability", func() {
		It("Should reject spec changes after profiling starts", func() {
			...
			if current.Status.Legacy == nil {
				current.Status.Legacy = &nvidiacomv1beta1.LegacyStatus{}
			}
			current.Status.Legacy.State = string(nvidiacomv1alpha1.DGDRStateProfiling)
			...
			Expect(yaml.Unmarshal(current.Spec.Legacy.ProfilingConfig.Config.Raw, &config)).Should(Succeed())
			config["sla"].(map[string]interface{})["ttft"] = 200.0
			current.Spec.Legacy.ProfilingConfig.Config = createTestConfig(config)
...
Expect(current.Status.Legacy).NotTo(BeNil())
				Expect(current.Status.Legacy.State).Should(Equal(string(nvidiacomv1alpha1.DGDRStateProfiling))) // State unchanged
	...
	Context("When handling DGD deletion", func() {
		It("Should transition to DeploymentDeleted state", func() {
			...
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: dgdrName, Namespace: namespace}, &updated)).Should(Succeed())
			Expect(updated.Status.Legacy).NotTo(BeNil())
			Expect(updated.Status.Legacy.State).Should(Equal(string(nvidiacomv1alpha1.DGDRStateDeploymentDeleted)))
		})
	})
})
...
var _ = Describe("DGDR Error Handling", func() {
	...
	Context("When profiling job fails", func() {
		It("Should capture detailed error from pod termination state", func() {
			...
			Expect(k8sClient.Get(ctx, types.NamespacedName{Name: dgdrName, Namespace: namespace}, &updated)).Should(Succeed())
Expect(updated.Status.Legacy).NotTo(BeNil())
				Expect(updated.Status.Legacy.State).Should(Equal(string(nvidiacomv1alpha1.DGDRStateFailed)))
  ...
  services: {}`
	...
	Context("GPU Discovery Integration Tests", func() {
		It("Should use GPU discovery when nodes have GPU labels", func() {
			...
			_ = k8sClient.Get(ctx, types.NamespacedName{Name: dgdrName, Namespace: namespace}, &updated)
			Expect(updated.Status.Legacy).NotTo(BeNil())
			Expect(updated.Status.Legacy.State).Should(Equal(string(nvidiacomv1alpha1.DGDRStatePending)))
		})
