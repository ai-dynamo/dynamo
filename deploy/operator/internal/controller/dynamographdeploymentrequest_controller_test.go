...
var _ = Describe("DGDR setSucceededConditionWithDetails", func() {
	Context("When called with Failed phase", func() {
		It("Should set Succeeded condition to False with custom reason", func() {
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				ObjectMeta: metav1.ObjectMeta{
					Name:       "test-dgdr",
					Generation: 1,
			...
			setSucceededConditionWithDetails(dgdr, nvidiacomv1beta1.DGDRPhaseFailed,
				"SweepingDecodeFailed", "OOM during decode sweep")

			cond := meta.FindStatusCondition(dgdr.Status.Conditions, nvidiacomv1beta1.ConditionTypeSucceeded)
			Expect(cond).NotTo(BeNil())
			Expect(cond.Status).Should(Equal(metav1.ConditionFalse))
			Expect(cond.Reason).Should(Equal("SweepingDecodeFailed"))
			Expect(cond.Message).Should(Equal("OOM during decode sweep"))
		})
	})

	Context("When called with Ready phase", func() {
		It("Should set Succeeded condition to True", func() {
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				ObjectMeta: metav1.ObjectMeta{
					Name:       "test-dgdr",
					Generation: 1,
			...
			setSucceededConditionWithDetails(dgdr, nvidiacomv1beta1.DGDRPhaseReady,
				"SpecGenerated", "Spec available")

			cond := meta.FindStatusCondition(dgdr.Status.Conditions, nvidiacomv1beta1.ConditionTypeSucceeded)
			Expect(cond).NotTo(BeNil())
			Expect(cond.Status).Should(Equal(metav1.ConditionTrue))
			Expect(cond.Reason).Should(Equal("SpecGenerated"))
		})
	})

	Context("When called with Deployed phase", func() {
		It("Should set Succeeded condition to True", func() {
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				ObjectMeta: metav1.ObjectMeta{
					Name:       "test-dgdr",
			...
			setSucceededConditionWithDetails(dgdr, nvidiacomv1beta1.DGDRPhaseDeployed,
				"Deployed", "All replicas healthy")
	...
	Context("When called with Profiling phase", func() {
		It("Should set Succeeded condition to False", func() {
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				ObjectMeta: metav1.ObjectMeta{
					Name:       "test-dgdr",
					Generation: 1,
			...
			setSucceededConditionWithDetails(dgdr, nvidiacomv1beta1.DGDRPhaseProfiling,
				"SweepingDecode", "Sweeping decode strategies")

			cond := meta.FindStatusCondition(dgdr.Status.Conditions, nvidiacomv1beta1.ConditionTypeSucceeded)
			Expect(cond).NotTo(BeNil())
			Expect(cond.Status).Should(Equal(metav1.ConditionFalse))
			Expect(cond.Reason).Should(Equal("SweepingDecode"))
		})
	})
})
...
var _ = Describe("DGDR Profiling Failure Attribution", func() {
	...
	Context("getFailedProfilingPhase", func() {
		It("Should return profilingPhase from status when set", func() {
			ctx := context.Background()
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-dgdr-fail-phase",
					Namespace: defaultNamespace,
				},
			}
			dgdr.Status.ProfilingPhase = nvidiacomv1beta1.ProfilingPhaseSweepingDecode

			phase := reconciler.getFailedProfilingPhase(ctx, dgdr)
			Expect(phase).Should(Equal(nvidiacomv1beta1.ProfilingPhaseSweepingDecode))
		})

		It("Should fall back to progress ConfigMap when profilingPhase is empty", func() {
			ctx := context.Background()
			dgdrName := "test-dgdr-fallback-cm"
			namespace := defaultNamespace

			// Create the progress ConfigMap
			cm := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      ProgressConfigMapPrefix + dgdrName,
					Namespace: namespace,
				},
				Data: map[string]string{
			...
			// profilingPhase is empty
			dgdr.Status.ProfilingPhase = ""

			phase := reconciler.getFailedProfilingPhase(ctx, dgdr)
			Expect(phase).Should(Equal(nvidiacomv1beta1.ProfilingPhase("SweepingPrefill")))
		})

		It("Should return empty when neither status nor ConfigMap has phase", func() {
			ctx := context.Background()
			dgdr := &nvidiacomv1beta1.DynamoGraphDeploymentRequest{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-dgdr-no-phase",
					Namespace: defaultNamespace,
				},
			}
			dgdr.Status.ProfilingPhase = ""
			...
			phase := reconciler.getFailedProfilingPhase(ctx, dgdr)
			Expect(phase).Should(Equal(nvidiacomv1beta1.ProfilingPhase("")))
		})
	})
