...
func TestLPXRenderingChecksFinalPodCliqueSetSize(t *testing.T) {
	...
	require.NoError(t, err)
	plan, err := selected.PlanNodeLocalMaterialization(dynamo.PCSNameForLPX(source))
	require.NoError(t, err)
...
func TestLPXRenderingPreservesInputs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			equivalentPlan, err := equivalentSelected.PlanNodeLocalMaterialization(dynamo.PCSNameForLPX(equivalent))
			require.NoError(t, err)
...
func TestLPXSpecDecodeConductorTemplate(t *testing.T) {
	t.Log("Create immutable native builds for the explicit and inherited conductor cases")
	registry := newTestDataModelRegistry(t, t.TempDir())

	for _, explicit := range []bool{false, true} {
		t.Run(fmt.Sprintf("explicit=%t", explicit), func(t *testing.T) {
			t.Log("Give the speculative components distinct templates and select the target conductor")
			...
			pcs, _, err := renderPodCliqueSet(t.Context(), source,
				&configv1alpha1.OperatorConfiguration{MPI: configv1alpha1.MPIConfiguration{SSHSecretName: "ssh-secret"}},
				&controller_common.RuntimeConfig{}, newTestLPXClient(t), nil, selected, plan, newLPXRenderDeployment(t, source))
			require.NoError(t, err)

			t.Log("Preserve independent role fields while retaining the Nova launch")
...
func mustPlanSelectedLPX(t *testing.T, dgd *v1beta1.DynamoGraphDeployment, selected *lpx.SelectedWorkload) *lpx.MaterializationPlan {
	t.Helper()
	plan, err := selected.PlanNodeLocalMaterialization(dynamo.PCSNameForLPX(dgd))
	require.NoError(t, err)
	return plan
}
...
func TestSingleV2ManifestDefaultsDriveConfigAndHash(t *testing.T) {
	...
	for _, settingsJSON := range []string{"", `{"batch_size":1,"swa":{"chunked":false}}`, `{"sequence_length":65536}`} {
		payload, err := os.ReadFile("../../dynamo/testdata/from_dgd_yaml/single_v2_manifest_defaults.input.yaml")
		require.NoError(t, err)
		...
		podCliqueSet, resources, err := renderPodCliqueSet(
			...
			newLPXRenderDeployment(t, &deployment),
		)
		require.NoError(t, err)

		configMap := getResource[*corev1.ConfigMap](t, resources, "dgd-single-v2-manifest-defaults-lpu")
		var hash string
...
func TestGenerateGrovePodCliqueSet_FromDGDYaml(t *testing.T) {
	...
	for _, name := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, err)
			plan, err := selected.PlanNodeLocalMaterialization(dynamo.PCSNameForLPX(&dynamoDeployment))
			require.NoError(t, err)
...
func TestGenerateGrovePodCliqueSet_ImplicitV2HybridPreservesAgentRuntime(t *testing.T) {
	t.Log("Load the pre-scheduler V2 hybrid DGD shape without scheduler intent or an image-owned command")
	...
	customConfigVolume := corev1.Volume{
		...
		VolumeSource: corev1.VolumeSource{ConfigMap: &corev1.ConfigMapVolumeSource{
			LocalObjectReference: corev1.LocalObjectReference{Name: dgd.Name + "-lpu"},
	...
	t.Log("Verify LPX places the worker while the generated backend owns the complete process contract")
	...
	require.True(t, main.Stdin)
	require.True(t, main.TTY)
	...
	require.Equal(t, dgd.Name+"-lpu", gasDirSource.ConfigMapKeyRef.Name)
	...
	require.Equal(t, dgd.Name+"-decode", configVolume.ConfigMap.Name)
	...
	require.Equal(t, dgd.Name+"-decode", tokenizerEnv.ValueFrom.ConfigMapKeyRef.Name)
	...
	lpuConfig := getResource[*corev1.ConfigMap](t, extraResources, dgd.Name+"-lpu")
	...
	decodeConfig := getResource[*corev1.ConfigMap](t, extraResources, dgd.Name+"-decode")
	require.Len(t, extraResources, 2)
	require.Equal(t, dgd.Name+"-decode", extraResources[0].GetName())
	require.Equal(t, dgd.Name+"-lpu", extraResources[1].GetName())
...
func TestGenerateGrovePodCliqueSet_V2NodeLocalPreservesImageEntrypoint(t *testing.T) {
	...
	modes := []struct {
		...
		{name: "Single V2", file: "single_v2.input.yaml", single: true},
		{name: "V2 LPU-only", file: "node-local-v2-lpu-only.input.yaml", single: true},
		{name: "V2 hybrid", file: "node-local-v2-hybrid.input.yaml", hybrid: true},
	}
	intents := []struct {
		name          string
		command       []string
