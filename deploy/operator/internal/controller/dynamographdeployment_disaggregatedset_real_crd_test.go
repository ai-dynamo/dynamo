...
func TestDisaggregatedSetRealLWSCRDValidationAndConvergence(t *testing.T) {
	...
	t.Log("generate and create a two-role DisaggregatedSet through the real API schema")
	...
	desired, err := reconciler.generateDisaggregatedSet(t.Context(), dgd, dcds, selection)
	require.NoError(t, err)
	modified, _, err := reconciler.syncDisaggregatedSet(t.Context(), dgd, desired)
	require.NoError(t, err)
	require.True(t, modified)

	t.Log("reconcile identical desired state after API defaulting and verify convergence")
	desired, err = reconciler.generateDisaggregatedSet(t.Context(), dgd, dcds, selection)
	require.NoError(t, err)
	modified, _, err = reconciler.syncDisaggregatedSet(t.Context(), dgd, desired)
	require.NoError(t, err)
