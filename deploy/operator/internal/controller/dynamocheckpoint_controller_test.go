...
func TestBuildCheckpointJob(t *testing.T) {
	...
	// Multi-GPU checkpoint jobs keep the target container entrypoint
	// unchanged; restore-time rediscovery is handled by the NCCL KVS endpoint
	// restore hook.
...
func TestBuildCheckpointJobPreservesPreparedSharedMemoryWhenUnset(t *testing.T) {
	s := checkpointTestScheme()
	ckpt := makeTestCheckpoint(nvidiacomv1alpha1.DynamoCheckpointPhasePending)
	preparedShmSize := resource.MustParse("64Gi")
	ckpt.Spec.Job.PodTemplateSpec.Spec.Volumes = append(
		ckpt.Spec.Job.PodTemplateSpec.Spec.Volumes,
		corev1.Volume{
			Name: consts.KubeValueNameSharedMemory,
			VolumeSource: corev1.VolumeSource{
				EmptyDir: &corev1.EmptyDirVolumeSource{
					Medium:    corev1.StorageMediumMemory,
					SizeLimit: &preparedShmSize,
				},
			},
		},
	)
	ckpt.Spec.Job.PodTemplateSpec.Spec.Containers[0].VolumeMounts = append(
		...
		corev1.VolumeMount{
			Name:      consts.KubeValueNameSharedMemory,
			MountPath: consts.DefaultSharedMemoryMountPath,
		},
	)
	...
	foundPreparedShmVolume := false
	for _, volume := range job.Spec.Template.Spec.Volumes {
		if volume.Name != consts.KubeValueNameSharedMemory {
			continue
		}
		foundPreparedShmVolume = true
		require.NotNil(t, volume.EmptyDir)
		require.NotNil(t, volume.EmptyDir.SizeLimit)
		assert.Equal(t, preparedShmSize, *volume.EmptyDir.SizeLimit)
	}
	require.True(t, foundPreparedShmVolume, "shared-memory volume not found: "+consts.KubeValueNameSharedMemory)
