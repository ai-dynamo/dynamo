...
// +kubebuilder:validation:XValidation:rule="!has(self.restart) || !has(self.services) || !self.services.exists(name, has(self.services[name].componentType) && self.services[name].componentType == 'lpx') || (has(self.restart.strategy) && has(self.restart.strategy.type) && self.restart.strategy.type == 'Parallel')",message="spec.restart.strategy.type must be Parallel when the graph contains LPX components"
