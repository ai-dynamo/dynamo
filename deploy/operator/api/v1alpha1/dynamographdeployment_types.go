...
type ComponentGroupSpec struct {
	...
	// ScalingAdapter configures whether this component group uses the
	// DynamoGraphDeploymentScalingAdapter. When enabled, Replicas is managed
	// through the Scale subresource. When disabled, Replicas can be modified
	// directly.
	// +optional
...
type ComponentGroupComponentSpec struct {
	// Name is the DGD service included in this group.
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:MinLength=1
	Name string `json:"name"`
}
