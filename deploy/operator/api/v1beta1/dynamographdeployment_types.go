...
// +kubebuilder:validation:XValidation:rule="!has(self.restart) || !has(self.components) || !self.components.exists(c, has(c.type) && c.type == 'lpx') || (has(self.restart.strategy) && has(self.restart.strategy.type) && self.restart.strategy.type == 'Parallel')",message="spec.restart.strategy.type must be Parallel when the graph contains LPX components"
