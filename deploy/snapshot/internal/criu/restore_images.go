...
func prepareRestoreImageDirForSocketScope(checkpointPath string, restoreSocketScope uint64) (string, func(), error) {
	...
	if tcpRewrite != nil {
		...
		if err != nil {
			// Port deconfliction is best effort. Leave the reciprocal tuple
			// unchanged and let CRIU's normal restore behavior decide.
			tcpRewrite = nil
		}
	}
	...
	for _, entry := range image.Entries {
		...
		if rewriteCUDAUVMFDListenerSocketName(fileEntry.Usk, restoreSocketScope) {
			rewritten = true
		}
...
func rewriteCUDAUVMFDListenerSocketName(entry *sk_unix.UnixSkEntry, restoreSocketScope uint64) bool {
	if entry == nil ||
		entry.Type == nil ||
		entry.State == nil ||
		entry.Peer == nil ||
	...
	nameEnd := len(entry.Name)
	for nameEnd > len(cudaUVMFDSocketNamePrefix) && entry.Name[nameEnd-1] == 0 {
		nameEnd--
	}
	rewritten := make([]byte, 0, len(entry.Name)+1+20)
	rewritten = append(rewritten, entry.Name[:nameEnd]...)
