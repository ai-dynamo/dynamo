...
func TestRewriteCUDAUVMFDListenerSocketName(t *testing.T) {
	tests := []struct {
		name       string
		socketName []byte
		socketType uint32
		...
		{
			name:       "abstract listener with trailing null",
			socketName: []byte("\x00cuda-uvmfd-4026544015-1\x00"),
			socketType: uint32(unix.SOCK_SEQPACKET),
			state:      linuxUnixSocketStateListen,
			want:       []byte("\x00cuda-uvmfd-4026544015-1-987654321\x00"),
		},
		{
			name:       "abstract listener without trailing null",
			socketName: []byte("\x00cuda-uvmfd-4026544015-904"),
			socketType: uint32(unix.SOCK_SEQPACKET),
			state:      linuxUnixSocketStateListen,
			want:       []byte("\x00cuda-uvmfd-4026544015-904-987654321"),
		},
		{
		...
		},
		{
			...
			socketName: []byte("\x00cuda-uvmfd-4026544015-1\x00"),
			socketType: uint32(unix.SOCK_STREAM),
			state:      linuxUnixSocketStateListen,
		},
		{
			name:       "non-listening socket",
			socketName: []byte("\x00cuda-uvmfd-4026544015-1\x00"),
			socketType: uint32(unix.SOCK_SEQPACKET),
			state:      linuxUnixSocketStateClose,
		},
		{
			name:       "pathname listener",
			socketName: []byte("/tmp/cuda-uvmfd-4026544015-1"),
			socketType: uint32(unix.SOCK_SEQPACKET),
			state:      linuxUnixSocketStateListen,
		},
		{
			name:       "other abstract listener",
			socketName: []byte("\x00other-4026544015-1"),
			socketType: uint32(unix.SOCK_SEQPACKET),
			state:      linuxUnixSocketStateListen,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			rewritten := rewriteCUDAUVMFDListenerSocketName(entry, 987654321)
			if test.want == nil {
				if rewritten {
					t.Fatalf("socket name unexpectedly rewritten to %q", entry.Name)
				}
				if !bytes.Equal(entry.Name, test.socketName) {
					t.Fatalf("socket name = %q, want unchanged %q", entry.Name, test.socketName)
				}
				return
			}
			if !rewritten {
				t.Fatal("socket name was not rewritten")
			}
			if !bytes.Equal(entry.Name, test.want) {
				t.Fatalf("socket name = %q, want %q", entry.Name, test.want)
			}
		})
	}
}
...
func TestPrepareRestoreImageDir(t *testing.T) {
	...
	entries := []*fdinfo.FileEntry{
		...
		newAutobindSocketEntry(8, []byte("\x00cuda-uvmfd-4026544015-904\x00"), 808, uint32(unix.SOCK_SEQPACKET), linuxUnixSocketStateListen, 0),
	}
	...
	originalEntries[7].Usk.Name = []byte("\x00cuda-uvmfd-4026544015-904-987654321\x00")
...
func TestPrepareRestoreImageDirWithoutSupportedSocketsUsesCheckpoint(t *testing.T) {
	checkpointPath := t.TempDir()
...
func TestPrepareRestoreImageDirConcurrentSocketScopesAreIndependent(t *testing.T) {
	...
	canonicalFilesImage := writeFilesImage(t, checkpointPath, []*fdinfo.FileEntry{
		newAutobindSocketEntry(1, []byte("\x000a1b2"), 101, uint32(unix.SOCK_DGRAM), linuxUnixSocketStateClose, 0),
		newAutobindSocketEntry(2, []byte("\x00cuda-uvmfd-4026544015-904\x00"), 202, uint32(unix.SOCK_SEQPACKET), linuxUnixSocketStateListen, 0),
	})
	...
	for result := range results {
		...
		scope := strconv.FormatUint(result.socketScope, 10)
		wantAutobind := []byte("\x000a1b2-" + scope)
		if got := entries[0].Usk.Name; !bytes.Equal(got, wantAutobind) {
			t.Fatalf("autobind socket name for scope %d = %q, want %q", result.socketScope, got, wantAutobind)
		}
		wantCUDA := []byte("\x00cuda-uvmfd-4026544015-904-" + scope + "\x00")
		if got := entries[1].Usk.Name; !bytes.Equal(got, wantCUDA) {
			t.Fatalf("CUDA socket name for scope %d = %q, want %q", result.socketScope, got, wantCUDA)
		}
	}
	if paths[0] == paths[1] {
...
func TestPrepareRestoreImageDirLeavesUnsupportedTCPGraphsUnchanged(t *testing.T) {
