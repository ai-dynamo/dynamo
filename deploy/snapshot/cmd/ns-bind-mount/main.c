...
struct ns_open_how { uint64_t flags, mode, resolve; };
#ifndef MOUNT_ATTR_RDONLY
#define MOUNT_ATTR_RDONLY 0x00000001
#define MOUNT_ATTR_NOSUID 0x00000002
#define MOUNT_ATTR_NODEV 0x00000004
#define MOUNT_ATTR_NOEXEC 0x00000008
struct mount_attr { uint64_t attr_set, attr_clr, propagation, userns_fd; };
#endif
...
static int sys_openat2(int dfd, const char *path, uint64_t flags, uint64_t resolve) {
...
static int set_mount_attrs(int tree_fd, uint64_t attrs) {
...
static int ensure_empty_destination(const char *dst, int *created) {
  *created = 0;
  ...
  int parent_fd = sys_openat2(AT_FDCWD, parent, O_PATH | O_DIRECTORY | O_CLOEXEC,
                              NS_RESOLVE_NO_SYMLINKS | NS_RESOLVE_NO_MAGICLINKS);
  if (parent_fd < 0) {
    fprintf(stderr, "open %s in target namespace: %s"
  ...
  int fd = sys_openat2(parent_fd, leaf, O_RDONLY | O_DIRECTORY | O_CLOEXEC,
                       NS_RESOLVE_BENEATH | NS_RESOLVE_NO_SYMLINKS |
                       NS_RESOLVE_NO_MAGICLINKS | NS_RESOLVE_NO_XDEV);
  close(parent_fd);
  if (fd < 0) {
  ...
  if (!*created) {
    ...
    while ((entry = readdir(dir))) {
      if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
        fprintf(stderr, "destination %s is not empty (contains %s);"
...
static int mount_tree(int ns_fd, const char *src, const char *dst, uint64_t attrs) {
  ...
  int src_fd = sys_openat2(AT_FDCWD, src, O_PATH | O_DIRECTORY | O_CLOEXEC,
                           NS_RESOLVE_NO_SYMLINKS | NS_RESOLVE_NO_MAGICLINKS);
  if (src_fd < 0) {
  ...
  if (set_mount_attrs(tree_fd, attrs) < 0) { close(tree_fd); return 1; }
  ...
  if (dst_fd < 0) { close(tree_fd); return 1; }
  ...
  fflush(stdout);
  int rc = 0;
  if (syscall(__NR_move_mount, tree_fd, "", dst_fd, "",
...
static int unmount_tree(int ns_fd, const char *dst, int strict, int created) {
  if (setns(ns_fd, CLONE_NEWNS) < 0) {
    fprintf(stderr, "setns: %s\n", strerror(errno));
    return 1;
  }
  ...
  if (umount2(dst, (strict ? 0 : MNT_DETACH) | UMOUNT_NOFOLLOW) < 0 &&
...
static uint64_t parse_attr(const char *token) {
  if (!strcmp(token, "ro")) return MOUNT_ATTR_RDONLY;
  if (!strcmp(token, "nosuid")) return MOUNT_ATTR_NOSUID;
  if (!strcmp(token, "nodev")) return MOUNT_ATTR_NODEV;
  if (!strcmp(token, "noexec")) return MOUNT_ATTR_NOEXEC;
  return 0;
}
...
int main(int argc, char **argv) {
  ...
  if (!strcmp(argv[1], "umount-fd")) {
    ...
    for (int i = 4; i < argc; i++) {
      if (!strcmp(argv[i], "created")) created = 1;
