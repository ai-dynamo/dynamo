...
async def create_deployment(deployment: CreateDeploymentSchema):
    ...
    try:
        ...
        deployment_name = (
            deployment.name
            or f"dep-{dynamo_nim_name}-{dynamo_nim_version}--{uuid.uuid4().hex}"
        )
        ...
        dynamo_deployment = DynamoDeployment(
            type_meta=type_meta, object_meta=object_meta, spec=spec
        )
        ...
        created_crd = api.create_namespaced_custom_object(
            ...
            body=dynamo_deployment.dict(
                by_alias=True
            ),  # Use by_alias to maintain camelCase
        )
        ...
        creator = UserSchema(
            uid=ownership["user_id"], name="default-user", email="default@example.com"
        )

        # TODO: Replace with actual cluster info
        cluster = ClusterSchema(
            uid="default-cluster", name="default", description="Default cluster"
        )
