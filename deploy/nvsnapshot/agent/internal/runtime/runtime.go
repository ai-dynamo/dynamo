...
// empty socket to use the per-type default. ctx is used for backend dial
// timeouts and tracing.
func New(ctx context.Context, runtimeType, socket string) (Runtime, error) {
	if socket == "" {
	...
	case RuntimeCRIO:
		return NewCRIORuntime(ctx, socket)
	default:
