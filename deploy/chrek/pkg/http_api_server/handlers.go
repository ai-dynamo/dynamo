...
func (h *Handlers) HandleCheckpoint(w http.ResponseWriter, r *http.Request) {
	...
	params := checkpoint.CheckpointParams{
		...
		CheckpointDir: config.CheckpointBasePath,
		NodeName:      h.cfg.Agent.NodeName,
		PodName:       req.PodName,
...
func (h *Handlers) HandleListCheckpoints(w http.ResponseWriter, r *http.Request) {
	...
	checkpointIDs, err := config.ListCheckpoints(config.CheckpointBasePath)
	if err != nil {
	...
	for _, id := range checkpointIDs {
		meta, err := config.GetCheckpointInfo(config.CheckpointBasePath, id)
		if err != nil {
			continue
		}
