...
from dynamo.sdk import api, endpoint, service
from dynamo.sdk.cli.serve_dynamo import add_fastapi_routes
...
from dynamo.sdk.core.runner.bentoml import BentoEndpoint
...
def test_dynamo_endpoint_http_method():
    ...
    endpoint = DynamoEndpoint(lambda: None, http_method="GET")
    ...
    endpoint = DynamoEndpoint(lambda: None, http_method="post")
    assert endpoint.http_method == "POST"

    # Test mixed case methods
    endpoint = DynamoEndpoint(lambda: None, http_method="DeLeTe")
...
def test_invalid_http_methods():
    ...
    with pytest.raises(ValueError, match="Invalid HTTP method"):
        DynamoEndpoint(lambda: None, http_method="INVALID")

    # Test with empty HTTP method
    with pytest.raises(ValueError, match="HTTP method cannot be empty"):
        DynamoEndpoint(lambda: None, http_method="")

    # Test with non-string HTTP method
    with pytest.raises(TypeError, match="HTTP method must be a string"):
        DynamoEndpoint(lambda: None, http_method=123)
...
def test_bento_endpoint():
    """Test BentoEndpoint HTTP method handling."""

    # Create a DynamoEndpoint to wrap
    async def test_func(request: dict) -> dict:
        return {}

    dyn_endpoint = DynamoEndpoint(test_func, http_method="GET")

    # Test method propagation
    bento_endpoint = BentoEndpoint(dyn_endpoint, "test", [DynamoTransport.HTTP])
    assert bento_endpoint.http_method == "GET"

    # Test default behavior
    dyn_endpoint_no_method = DynamoEndpoint(test_func)
    bento_endpoint = BentoEndpoint(
        dyn_endpoint_no_method, "test", [DynamoTransport.HTTP]
    )
    assert bento_endpoint.http_method is None
