...
def endpoint(
    ...
    **kwargs,
    ...
    def decorator(func: Callable) -> DynamoEndpoint:
        actual_transports = transports or [DynamoTransport.DEFAULT]
        if http_method is not None and DynamoTransport.HTTP not in actual_transports:
            raise ValueError("http_method can only be specified for endpoints with HTTP transport")
        return DynamoEndpoint(func, name, actual_transports, http_method, **kwargs)
