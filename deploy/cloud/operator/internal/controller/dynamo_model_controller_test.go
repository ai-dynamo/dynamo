...
var _ = Describe("DynamoModel Controller", func() {
	...
	Context("When endpoint readiness changes", func() {
		It("Should update condition from NotReady to AllEndpointsReady for LoRA model", func() {
			ctx := context.Background()
			namespace := defaultNamespace
			modelName := "test-lora-readiness-change"
			baseModelName := "base-model-readiness"
			...
			// Create EndpointSlice with one ready, one not ready
			trueVal := true
			falseVal := false
			modelHash := dynamo.HashModelName(baseModelName)
			endpointSlice := &discoveryv1.EndpointSlice{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-readiness-endpoints",
					Namespace: namespace,
					Labels: map[string]string{
						consts.KubeLabelDynamoBaseModelHash: modelHash,
					},
				...
				Endpoints: []discoveryv1.Endpoint{
					...
					{
						Addresses: []string{"10.0.1.12"},
						Conditions: discoveryv1.EndpointConditions{
							Ready: &falseVal, // Not ready
						},
						TargetRef: &corev1.ObjectReference{
							Kind: "Pod",
							Name: "worker-1",
			...
			// Reconcile - should only find 1 ready endpoint
			_, err := reconciler.Reconcile(ctx, reconcile.Request{
				NamespacedName: types.NamespacedName{
					Name:      modelName,
					Namespace: namespace,
			...
			// Should have 1 endpoint (only the ready one)
			Eventually(func() int {
				var updated v1alpha1.DynamoModel
				_ = k8sClient.Get(ctx, types.NamespacedName{Name: modelName, Namespace: namespace}, &updated)
				return updated.Status.TotalEndpoints
