...
func TestGenerateBasePodSpec_ValidateExtraPodSpecMainContainer(t *testing.T) {
	secretsRetriever := &mockSecretsRetriever{}
	controllerConfig := controller_common.Config{}

	// Define a startup probe for testing
	startupProbe := &corev1.Probe{
		ProbeHandler: corev1.ProbeHandler{
			HTTPGet: &corev1.HTTPGetAction{
				Path: "/startup",
	...
	tests := []struct {
		...
		{
			name: "startup probe should be swapped exactly",
			component: &v1alpha1.DynamoComponentDeploymentOverridesSpec{
				DynamoComponentDeploymentSharedSpec: v1alpha1.DynamoComponentDeploymentSharedSpec{
					ComponentType: commonconsts.ComponentTypeWorker,
					ExtraPodSpec: &common.ExtraPodSpec{
			...
			expectError: false,
			validateFunc: func(t *testing.T, podSpec corev1.PodSpec) {
				if diff := cmp.Diff(startupProbe, podSpec.Containers[0].StartupProbe); diff != "" {
					t.Errorf("StartupProbe mismatch (-want +got):\n%s", diff)
				}
			},
		},
		{
			name: "should block resources in main container",
			component: &v1alpha1.DynamoComponentDeploymentOverridesSpec{
				DynamoComponentDeploymentSharedSpec: v1alpha1.DynamoComponentDeploymentSharedSpec{
					ComponentType: commonconsts.ComponentTypeWorker,
					ExtraPodSpec: &common.ExtraPodSpec{
			...
			expectError:   true,
			errorContains: "resources.requests is not supported in main container",
		},
		{
			name: "should block readiness probe in main container",
			component: &v1alpha1.DynamoComponentDeploymentOverridesSpec{
				DynamoComponentDeploymentSharedSpec: v1alpha1.DynamoComponentDeploymentSharedSpec{
					ComponentType: commonconsts.ComponentTypeWorker,
					ExtraPodSpec: &common.ExtraPodSpec{
			...
			expectError:   true,
			errorContains: "readiness probe is not supported in main container",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				if err == nil {
					t.Error("GenerateBasePodSpec() expected error but got none")
					return
				}
				if !strings.Contains(err.Error(), tt.errorContains) {
					t.Errorf("GenerateBasePodSpec() error = %v, want error containing %v", err, tt.errorContains)
				}
				return
			}
