...
func TestSGLangBackend_DirectFlagInjection(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			multinodeDeploymentType: consts.MultinodeDeploymentTypeGrove,
			initialArgs:             []string{"python -m dynamo.sglang.worker"},
			expectedArgs:            []string{"python -m dynamo.sglang.worker --dist-init-addr ${GROVE_HEADLESS_SERVICE}:29500 --nnodes 2 --node-rank 0"},
		...
		},
		{
			...
			expectedArgs:            []string{"echo blah | wc -l && python -m dynamo.sglang.worker --dist-init-addr ${GROVE_HEADLESS_SERVICE}:29500 --nnodes 2 --node-rank 0 && ls -al"},
		...
		},
		{
			...
			multinodeDeploymentType: consts.MultinodeDeploymentTypeGrove,
			initialArgs:             []string{"python -m dynamo.sglang.worker"},
			expectedArgs:            []string{"python -m dynamo.sglang.worker --dist-init-addr ${GROVE_HEADLESS_SERVICE}:29500 --nnodes 3 --node-rank $((GROVE_PCLQ_POD_INDEX + 1))"},
		...
		},
		{
			...
			expectedArgs:            []string{"python -m dynamo.sglang.worker --dist-init-addr ${GROVE_HEADLESS_SERVICE}:29500 --nnodes 2 --node-rank 0 | tee /tmp/log"},
		...
		},
		{
			...
			expectedArgs:            []string{"echo start python -m dynamo.sglang.worker --dist-init-addr ${GROVE_HEADLESS_SERVICE}:29500 --nnodes 2 --node-rank 0 echo done"},
