...
: "${NAMESPACE:=cai-system}"  # Default namespace
: "${NGC_TOKEN:=<your-ngc-token>}"  # Default NGC token
: "${CI_COMMIT_SHA:=latest}"  # Default commit SHA
: "${RELEASE_NAME:=compoundai-platform}"  # Default commit SHA
...
envsubst '${NAMESPACE} ${NGC_TOKEN} ${CI_COMMIT_SHA} ${RELEASE_NAME}' < compoundai-platform-values.yaml > generated-values.yaml
