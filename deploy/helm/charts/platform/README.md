...
| grove.config.server.healthProbes.enable | bool | `true` | Enable Grove's health and readiness probes on port 9444. Grove's /readyz gates on certificate readiness and its webhook listener, so PodCliqueSet admission is not attempted before Grove can serve it. |
