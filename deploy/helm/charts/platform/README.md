...
| dynamo-operator.checkpoint.storage.signalHostPath | string | `"/var/lib/dynamo-checkpoint/signals"` | Host path for signal files used for communication between checkpoint job pods and the DaemonSet. Both components mount this path to coordinate checkpoint completion. |
