...
| dynamo-operator.webhook.podCheckpointRestoreFailurePolicy | string | `"Fail"` | Failure policy for the Pod CREATE checkpoint-restore mutating webhook. Keep Fail so bound restore Pods cannot bypass checkpoint revalidation. Ignore is an emergency-only bypass and also affects ordinary Pod creates. |
