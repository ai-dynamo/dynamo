...
| dynamo-operator.upgradeCRD | bool | `true` | Whether the cluster-wide operator applies CRDs from its image through the crd-apply init container. This must be false for namespace-restricted installations, which use the CRDs managed by the cluster-wide operator. |
