...
### Basic Installation (PVC Storage)
...
### With S3 Storage (Planned - Not Yet Implemented)

> ⚠️ **Note:** S3 storage backend is defined in the API but not yet fully implemented. Currently, only PVC storage is supported.
...
helm install chrek nvidia/chrek \
  --namespace my-team \
  --set storage.type=s3 \
  --set storage.s3.uri=s3://my-bucket/checkpoints \
