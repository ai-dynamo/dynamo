#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.1ctx2gen_tp4.dynamo.kvrouter
#SBATCH --nodes=2
...
#SBATCH --output=bench/logs/run_benchx_1ctx2gen_tp4_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_1ctx2gen_tp4_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 1 ctx TP1 + 2 gen TP4 with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 1 ctx worker(s) (GPUs 0-0) TP1
#   NODE0 — gen worker 0 GPUs 1,2,3,4 (TP4)
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_1ctx2gen_tp4_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_1ctx2gen_tp4_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_1ctx2gen_tp4_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
GEN_PIDS=()
# --- decode worker 0 on $NODE0 GPUs 1,2,3,4 (TP4) ---
GEN_PORT=8085
echo "[$(date +%H:%M:%S)] Starting gen worker 0 on $NODE0 GPUs 1,2,3,4 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=4 --ntasks-per-node=4 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
  --output="$RESULTS_DIR/gen_worker_g0.log" \
  --container-image="$CONTAINER_IMAGE" --container-mounts="$CONTAINER_MOUNTS" \
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=1,2,3,4 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
...
# ==============================================================================
echo "[$(date +%H:%M:%S)] Waiting for servers (1 prefill + 2 decode)..."
if ! wait_for_dynamo_workers "${NODE0}" "${FRONTEND_PORT}" 1 2 2700 60; then
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8085,${NODE1}:8086" \
    --labels "ctx_g0,gen_g0,gen_g1" \
