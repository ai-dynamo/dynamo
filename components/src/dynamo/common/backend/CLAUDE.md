...
`output_options`.  `GenerateChunk` has `token_ids` (required), optional
`index` for multi-choice responses, plus optional `finish_reason` and
`completion_usage` (both required on the final chunk).  Engines may read
