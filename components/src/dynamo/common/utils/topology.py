...
_VALID_KV_TRANSFER_ENFORCEMENTS = {"required", "preferred"}
...
def _read_topology_domains(mount_path: Path) -> dict[str, str]:
    ...
    for topology_file in topology_files:
        file_name = topology_file.name.strip()
        ...
        if value:
            topology_domains[file_name.lower()] = value

    return topology_domains


def _optional_env(var_name: str) -> str | None:
    return os.environ.get(var_name, "").strip().lower() or None
...
def _read_kv_transfer_policy() -> tuple[str, str, float | None]:
    ...
    if kv_transfer_enforcement is None:
        kv_transfer_enforcement = _DEFAULT_KV_TRANSFER_ENFORCEMENT

    if kv_transfer_enforcement not in _VALID_KV_TRANSFER_ENFORCEMENTS:
        logger.error(
            "%s must be one of %s, got %r. Exiting.",
            _KV_TRANSFER_ENFORCEMENT_VAR,
            sorted(_VALID_KV_TRANSFER_ENFORCEMENTS),
            kv_transfer_enforcement,
        )
        sys.exit(1)

    if not raw_preferred_weight:
        if kv_transfer_enforcement == "preferred":
            logger.error(
                "%s=preferred requires %s. Exiting.",
                _KV_TRANSFER_ENFORCEMENT_VAR,
                _KV_TRANSFER_PREFERRED_WEIGHT_VAR,
            )
            sys.exit(1)
        return kv_transfer_domain, kv_transfer_enforcement, None

    if kv_transfer_enforcement != "preferred":
        logger.error(
            "%s is only valid when %s=preferred. Exiting.",
            _KV_TRANSFER_PREFERRED_WEIGHT_VAR,
            _KV_TRANSFER_ENFORCEMENT_VAR,
        )
        sys.exit(1)
    ...
    try:
        preferred_weight = float(raw_preferred_weight)
    except ValueError:
        logger.error(
            "%s must be a float between 0.0 and 1.0, got %r. Exiting.",
            _KV_TRANSFER_PREFERRED_WEIGHT_VAR,
            raw_preferred_weight,
        )
        sys.exit(1)

    if not math.isfinite(preferred_weight) or not 0.0 <= preferred_weight <= 1.0:
        logger.error(
            "%s must be finite and between 0.0 and 1.0, got %r. Exiting.",
            _KV_TRANSFER_PREFERRED_WEIGHT_VAR,
            raw_preferred_weight,
        )
        sys.exit(1)
...
def read_topology_config(
    ...
    Raises:
        SystemExit: If DYN_TOPOLOGY_ENABLED=true but DYN_KV_TRANSFER_DOMAIN is
            ...
            empty after the timeout, or the policy env vars are invalid.
    """
