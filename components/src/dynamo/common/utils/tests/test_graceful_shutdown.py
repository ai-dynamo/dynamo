...
def test_normal_shutdown_does_not_apply_failover_unregister_timeout(monkeypatch):
    ...
    async def slow_unregister(_endpoints):
        await asyncio.sleep(0.01)
        call_order.append("unregister")

    async def _run():
        monkeypatch.delenv("DYN_GMS_FAILOVER_FAST_EXIT_ON_SIGTERM", raising=False)
        monkeypatch.setenv("DYN_GMS_FAILOVER_UNREGISTER_TIMEOUT_SECS", "0")
        monkeypatch.setattr(_gs, "_unregister_endpoints", slow_unregister)
        await graceful_shutdown_with_discovery(
            runtime=mock_runtime,
            endpoints=[],
            grace_period_s=0,
    ...
    asyncio.run(_run())
    assert call_order == ["unregister", "shutdown"]


def test_fast_failover_timeout_still_releases_ownership(monkeypatch):
    class FastExit(Exception):
        pass
    ...
    async def stuck_unregister(_endpoints):
    ...
    async def _run():
        ...
        monkeypatch.setattr(_gs, "_unregister_endpoints", stuck_unregister)
        monkeypatch.setattr(_gs, "_fast_exit_after_failover_unregister", fake_fast_exit)
        await graceful_shutdown_with_discovery(
            runtime=mock_runtime,
            endpoints=[],
