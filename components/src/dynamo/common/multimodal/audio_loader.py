...
class AudioLoader:
    ...
    async def load_audio(self, audio_url: str) -> tuple[np.ndarray, float]:
        ...
        try:
            ...
            # A deliberate client-error verdict. Must precede the generic
            # handler, since UrlValidationError subclasses ValueError and
            # would otherwise be flattened. Mirrors
            # ImageLoader._fetch_and_process.
    ...
    async def load_audio_batch(
        ...
        for media_item, result in zip(audio_mm_items, results, strict=True):
            if isinstance(result, BaseException):
                ...
                # Keep the first typed client error. gather() has already
                # awaited everything, so deferring the raise costs nothing and
                # lets a typed error win over a generic one that happened to
                # come first in the batch. Folding these into the bare
                # Exception below would discard the status they carry.
