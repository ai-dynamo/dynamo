...
def _reclaim_foreign_kv_leases_after_fence(backend_name: str, role: str) -> None:
    ...
    backend_env = f"GMS_{engine.upper()}_KV_LEASES"
    if backend_env in os.environ:
        if not _truthy_env(backend_env):
            return
    elif not _truthy_env("GMS_KV_LEASES"):
        return
    try:
...
async def prepare_gms_failover(
    ...
    *,
    ...
    resumed_after_acquire = False
    try:
        await run_gms_failover_post_lock_fence(backend_name=backend_name, role=role)

        await controller.resume(tag_list)
        resumed_after_acquire = True
        ...
        if resumed_after_acquire:
            try:
                await controller.quiesce(tag_list)
            except Exception:
                logger.exception(
                    "[GMS failover] %s %s failed to re-quiesce after activation failure",
                    backend_name,
                    role,
                )
        try:
            ...
            logger.exception(
                "[GMS failover] %s %s failed to release lock after activation failure",
                backend_name,
                role,
            )
        raise
