...
def _install_fake_kv_lease_client(monkeypatch):
    calls = []

    def resolve_lease_device(env_name):
        calls.append(("resolve_device", env_name))
        return 0

    def reclaim_foreign_kv_leases_in_shm_dir(engine, device, *, max_blocks_per_file):
        calls.append(("reclaim", engine, device, max_blocks_per_file))
        return types.SimpleNamespace(files=1, reclaimed_blocks=1, errors=0)

    module = types.ModuleType("gpu_memory_service.integrations.common.kv_lease_client")
    module.resolve_lease_device = resolve_lease_device
    module.reclaim_foreign_kv_leases_in_shm_dir = reclaim_foreign_kv_leases_in_shm_dir
    ...
    monkeypatch.setitem(
        sys.modules,
        "gpu_memory_service.integrations.common.kv_lease_client",
        module,
    )
    return calls
...
@pytest.mark.asyncio
async def test_gms_failover_shadow_activation_releases_lock_when_resume_fails(
    monkeypatch,
):
    monkeypatch.setenv("DYN_GMS_FAILOVER_SHADOW_MODE", "true")
    monkeypatch.setenv("ENGINE_ID", "1")
    lock = _BusyOnTryLock("/locks/failover.lock")

    class _FailingResumeController(_Controller):
        async def resume(self, tags):
            await super().resume(tags)
            raise RuntimeError("resume failed")

    class _FailingOwner:
        def __init__(self):
            self._quiesce_controller = _FailingResumeController()
...
@pytest.mark.asyncio
async def test_gms_failover_shadow_activation_requiesces_and_releases_lock_on_warmup_failure(
    monkeypatch,
):
    monkeypatch.setenv("DYN_GMS_FAILOVER_SHADOW_MODE", "true")
    monkeypatch.setenv("ENGINE_ID", "1")
...
def test_gms_failover_lease_reclaim_honors_backend_specific_disable(monkeypatch):
    from dynamo.common import gms_failover

    monkeypatch.setenv("GMS_KV_LEASES", "1")
    monkeypatch.setenv("GMS_VLLM_KV_LEASES", "0")
    calls = _install_fake_kv_lease_client(monkeypatch)

    gms_failover._reclaim_foreign_kv_leases_after_fence("vllm", "shadow")

    assert calls == []


def test_gms_failover_lease_reclaim_backend_specific_enable_overrides_global_disable(
    monkeypatch,
):
    from dynamo.common import gms_failover
    ...
    gms_failover._reclaim_foreign_kv_leases_after_fence("trt-llm", "shadow")

    assert calls == [
        ("resolve_device", "GMS_TRTLLM_KV_LEASE_DEVICE"),
        ("reclaim", "trtllm", 0, 0),
    ]
