...
def test_host_cache_hit_weight_cli_uses_kv_router_config_field() -> None:
    parser = argparse.ArgumentParser()
    KvRouterArgGroup().add_arguments(parser)

    args = parser.parse_args(["--router-host-cache-hit-weight", "0.5"])

    assert args.host_cache_hit_weight == 0.5
    assert not hasattr(args, "router_host_cache_hit_weight")


def test_host_cache_hit_weight_env_uses_kv_router_config_field(monkeypatch) -> None:
    monkeypatch.setenv("DYN_ROUTER_HOST_CACHE_HIT_WEIGHT", "0.9")
    parser = argparse.ArgumentParser()
    KvRouterArgGroup().add_arguments(parser)

    args = parser.parse_args([])

    assert args.host_cache_hit_weight == 0.9


def test_host_cache_hit_weight_default_matches_kv_router_config_binding() -> None:
    parser = argparse.ArgumentParser()
    KvRouterArgGroup().add_arguments(parser)

    args = parser.parse_args([])

    assert args.host_cache_hit_weight == 0.75


def test_disk_cache_hit_weight_cli_uses_kv_router_config_field() -> None:
    parser = argparse.ArgumentParser()
    KvRouterArgGroup().add_arguments(parser)

    args = parser.parse_args(["--router-disk-cache-hit-weight", "0.1"])

    assert args.disk_cache_hit_weight == 0.1
    assert not hasattr(args, "router_disk_cache_hit_weight")


def test_disk_cache_hit_weight_env_uses_kv_router_config_field(monkeypatch) -> None:
    monkeypatch.setenv("DYN_ROUTER_DISK_CACHE_HIT_WEIGHT", "0.4")
    parser = argparse.ArgumentParser()
    KvRouterArgGroup().add_arguments(parser)

    args = parser.parse_args([])

    assert args.disk_cache_hit_weight == 0.4


def test_disk_cache_hit_weight_default_matches_kv_router_config_binding() -> None:
    parser = argparse.ArgumentParser()
    KvRouterArgGroup().add_arguments(parser)

    args = parser.parse_args([])

    assert args.disk_cache_hit_weight == 0.25


def test_cache_hit_weights_flow_through_kv_router_kwargs() -> None:
    parser = argparse.ArgumentParser()
    KvRouterArgGroup().add_arguments(parser)
