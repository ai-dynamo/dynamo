...
def test_session_affinity_header_key_cli_and_environment(monkeypatch) -> None:
    ...
    monkeypatch.setenv("DYN_ROUTER_SESSION_HEADER_KEY", "X-Session-ID")
    parser = argparse.ArgumentParser()
    FrontendArgGroup().add_arguments(parser)
    config = FrontendConfig.from_cli_args(parser.parse_args([]))
    config.validate()
    assert config.session_affinity_header_key == "X-Session-ID"
    assert config.router_kwargs()["session_affinity_header_key"] == "X-Session-ID"
...
def test_session_affinity_header_key_rejects_invalid_http_header_name() -> None:
    ...
    with pytest.raises(SystemExit):
        parser.parse_args(["--router-session-header-key", "x session id"])
