...
async def worker():
    ...
    try:
        if args.trace_file is not None:
            output_file = args.output_file or default_replay_output_path(args.trace_file)
            report = run_mocker_trace_replay(
                trace_file=args.trace_file,
                extra_engine_args=extra_engine_args_path,
            )
            write_replay_report(report, output_file)
            print_replay_summary(report, output_file)
            return
