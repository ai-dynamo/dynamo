...
from dynamo.experimental.workflow.remote import (
...
class _RemoteEcho:
    ...
    async def run(self, inputs, context):
        self.invocations.append(context.invocation_id)
        if inputs["text"] == "fail":
            raise RuntimeError("requested failure")
        return {"text": inputs["text"]}


async def test_handler_branches_loops_and_recovers_across_remote_calls() -> None:
    workflow = Workflow("remote-handler")
    ...
    async def run(inputs, context):
        ...
        if request["fallback"]:
            try:
                ...
                fallback = await context.call(echo, text="fallback")
                return {"result": fallback["text"]}

        result = {"text": ""}
        for index in range(request["count"]):
            result = await context.call(echo, text=f"step-{index}")
        return {"result": result["text"]}

    endpoint_id = "workflows.echo.generate"
    runner = _RemoteEcho()
    runtime = _Runtime(
        {endpoint_id: _LoopbackClient(RemoteStageServer("echo", runner))}
    )
    plan = compile_workflow(workflow, DeploymentSpec.remote(echo=endpoint_id))
    executor = await WorkflowExecutor.bind(plan, runtime=runtime)
...
def test_handler_rejects_rich_remote_stage_ports_at_compile_time(spec) -> None:
    workflow = Workflow("unsupported-remote-handler")
    contract = StageContract(
        id="rich-stage",
        inputs={"value": spec},
        outputs={"result": ValueSpec(type="json")},
    )
    workflow.use("rich", contract)
    ...
    with pytest.raises(
        WorkflowValidationError,
        match="imperative remote stages support only json, text, and bytes",
    ):
        compile_workflow(
            workflow,
            DeploymentSpec.remote(rich="workflows.rich.generate"),
        )
