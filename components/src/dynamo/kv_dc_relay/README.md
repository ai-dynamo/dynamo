...
- The first `SubscribeKvPool` client initializes that pool's publication hub. The hub captures one
  ...
  subscriber reconnects for a fresh snapshot. Idle hubs can be evicted when another pool needs
  admission; a later subscription rebuilds the mirror from the current actor state.
...
The gRPC server is included in the standard build. Set `--bind` to start a plaintext listener;
without `--bind`, the Relay runs locally without a gRPC listener:
...
authentication, or client authentication. Mutual TLS (mTLS) is an optional deployment concern,
implemented entirely in an external sidecar; Relay has no TLS flags or certificate configuration.
A sidecar is not required to start Relay on a trusted, isolated network. To protect traffic across
a trust boundary:

1. Bind Relay to `127.0.0.1` or `::1`.
2. Run a gRPC-capable proxy or sidecar on the same host or in the same pod.
3. Configure the sidecar's external listener for mTLS and require trusted client certificates.
...
to 64; the initialized-hub bound limits resident per-pool CKF mirrors, with idle hubs eligible
for eviction). A client must drain snapshot frames within
