...
def test_wan_tuning_requires_bind(key: str, capsys: pytest.CaptureFixture[str]) -> None:
    """Reject every WAN-only environment setting before constructing the Relay."""
    name = f"DYN_RELAY_{key.upper()}"
    with pytest.raises(SystemExit) as error:
        parse_args(["--dc-id", "dc-a"], {name: "1"})
    assert error.value.code == 2
    assert f"{name} requires --bind or DYN_RELAY_BIND" in capsys.readouterr().err


def test_producer_tuning_does_not_require_bind() -> None:
    """Keep local publication tuning available without a WAN listener."""
    config = parse_args(
        ["--dc-id", "dc-a"],
        {
            "DYN_RELAY_PUBLICATION_THRESHOLD": "7",
