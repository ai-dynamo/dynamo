...
| `args.py` | `Config` (extends `DynamoRuntimeConfig`), `ExampleBackendConfig`, and CLI argument parsing |
...
2. `ExampleBackend.create_handler()` creates an `ExampleHandler` with `config.backend.token_delay`.
