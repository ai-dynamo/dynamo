...
class DiffusionFormatter:
    ...
    async def _encode_video(
        ...
        try:
            ...
            # Encode through the shared LGPL-safe encoder: the in-tree ffmpeg's
            # h264_nvenc (NVIDIA HW) encoder, not the GPL libx264 that
            # diffusers.export_to_video defaults to for MP4 (libx264 is purged
            # from the runtime image — see vllm_runtime.Dockerfile codec purge).
