...
class AudioFormatter:
    ...
    async def format(
        ...
        try:
            ...
            return self._error_response(request_id, str(e), request_type=request_type)
    ...
    def _chat_audio_chunk(
        ...
        *,
        ...
        content = text or _extract_text(stage_output)
        if audio_data_obj.b64_json:
            content = audio_data_obj.b64_json
    ...
    def _error_response(
        self, request_id: str, error: str, *, request_type: Any = None
    ) -> Dict[str, Any]:
        if request_type == RequestType.CHAT_COMPLETION:
            return _error_chunk(request_id, self._model_name, error)
        return NvAudioSpeechResponse(
            id=request_id,
            model=self._model_name,
            status="failed",
