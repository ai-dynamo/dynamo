...
class AudioGenerationHandler:
    ...
    async def build_engine_inputs(
        ...
        * **Generic audio path** (MiMo-Audio, etc.): plain text prompt.
        ...
        if audex_model_type is not None:
            return self._engine_inputs_audex(req, request_id, audex_model_type)
    ...
    def _engine_inputs_audex(
        ...
        """Wrap the Audex adapter's prepared request in ``EngineInputs``.

        Everything model-specific (validation, ChatML prompt, CFG/RVQ
        contracts) belongs to ``omni.audex.AudexRequestAdapter``; this only adds
        the response-delivery fields, which are the same for every audio model.
        ``model_type`` is the resolution the caller already made to get here, so
        the adapter does not repeat the stage lookup.

        ``stream_audio`` is left off: per-payload chunk streaming has not been
        validated for the Audex pipeline, so its requests take the aggregate
        path and the response carries one complete waveform. Aggregation itself
        follows the resolved output kind, not the model (see
        ``utils.audio_output_is_cumulative``).
        """
        from dynamo.vllm.omni.omni_handler import EngineInputs

        prepared = self.audex.prepare(req, request_id, model_type)
        return EngineInputs(
            prompt=OmniTextPrompt(prompt=prepared.prompt),
