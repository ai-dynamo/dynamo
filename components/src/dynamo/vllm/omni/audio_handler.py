...
class AudioGenerationHandler:
    ...
    async def build_engine_inputs(
        ...
        * **Generic audio path** (MiMo-Audio, etc.): plain text prompt.
        ...
        # Audex's decoder re-emits the whole waveform on every yield, so its
        # requests are always aggregated (see emits_cumulative_waveforms) and
        # never take the chunk-streaming path below.
