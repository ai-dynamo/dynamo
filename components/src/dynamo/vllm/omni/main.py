...
from dynamo.vllm.main import is_object_storage_path, setup_metrics_collection
...
async def worker():
    ...
    if not os.path.exists(config.model) and not is_object_storage_path(config.model):
