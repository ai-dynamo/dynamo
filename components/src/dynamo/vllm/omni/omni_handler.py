...
class OmniHandler(BaseOmniHandler):
    ...
    async def load_lora(self, request=None):
        ...
        async with lock:
            ...
            try:
                ...
                # Register LoRA in discovery so frontend can resolve it to the correct endpoint
                if self.generate_endpoint is not None:
                    try:
                        ...
                        logger.info(
                            "Registered LoRA '%s' on endpoint %s", lora_name, self.generate_endpoint
                        )
    ...
    async def unload_lora(self, request=None):
        lora_name = (request or {}).get("lora_name")
        if not lora_name:
            yield {"status": "error", "message": "'lora_name' is required in request"}
            return
