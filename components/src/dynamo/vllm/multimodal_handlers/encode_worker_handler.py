...
class EncodeWorkerHandler:
    ...
    async def _generate_custom_encoder(
        ...
        prepared_prompt = self._custom_encoder_adapter.prepare_prompt(
            token_ids,
            encodings,
        )
        handoff, transfer_future = await stage_linear_embeds_prompt(
            ...
            image_token_id=self._custom_encoder.get_image_placeholder_token_id(),
            model_config=self._custom_encoder_model_config,
        )
        self.send_complete_queue.put_nowait(
            (transfer_future, prepared_prompt["prompt_embeds"])
        )
        self._processed_requests += 1
        logger.debug(
