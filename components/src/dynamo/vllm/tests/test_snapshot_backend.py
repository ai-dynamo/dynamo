...
import dynamo.vllm.snapshot_backend as snapshot_backend
...
def test_registration_is_idempotent_without_requiring_overlay(monkeypatch):
    registry = {}
    factory = SimpleNamespace(_registry=registry)

    def register_backend(name, module_name, class_name):
        registry[name] = (module_name, class_name)

    factory.register_backend = MagicMock(side_effect=register_backend)
    monkeypatch.setattr(snapshot_backend, "_SLEEP_MODE_IMPORT_ERROR", None)
    monkeypatch.setattr(snapshot_backend, "SleepModeBackendFactory", factory)

    register_dynamo_gms_snapshot_backend()
    register_dynamo_gms_snapshot_backend()

    factory.register_backend.assert_called_once_with(
        GMS_BACKEND_NAME,
...
def test_selection_requires_gms_load_format(monkeypatch):
    register_backend = MagicMock()
    monkeypatch.setattr(
        snapshot_backend,
        "register_dynamo_gms_snapshot_backend",
        register_backend,
