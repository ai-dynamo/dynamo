...
def test_text_decoder_selects_linear_adapter_and_builds_final_prompt():
    ...
    assert prepared.embedding_sequence_length == 4
...
def test_qwen_adapter_builds_final_tokens_prompt_in_image_order(monkeypatch):
    ...
    prepared = adapter.prepare_prompt(token_ids, [first, second])

    assert prepared.embedding_sequence_length is None
    assert prepared.prompt["prompt_token_ids"] == token_ids
    image = prepared.prompt["multi_modal_data"]["image"]
    ...
    assert set(prepared.prompt["multi_modal_data"]) == {"image"}
