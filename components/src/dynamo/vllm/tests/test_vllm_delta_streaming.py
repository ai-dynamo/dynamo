...
class _FailingRecordArtifactSession(_RecordingArtifactSession):
    contents = frozenset({"selected_logprobs"})

    def record_chunk(self, **kwargs):
        super().record_chunk(**kwargs)
        if len(self.records) > 1:
            raise ValueError("capture detail must not escape")


@pytest.mark.parametrize(
    ("backend_request", "artifact_contents", "expected"),
    [
        ({}, None, True),
        ({}, {"selected_logprobs"}, True),
        ({}, {"moe_routes"}, False),
        ({"nvext": {"extra_fields": ["engine_data"]}}, {"moe_routes"}, True),
        ({"nvext": {"extra_fields": ["routed_experts"]}}, {"moe_routes"}, True),
    ],
)
def test_routed_experts_response_selection_preserves_non_artifact_behavior(
    ...
    artifact_session = (
        SimpleNamespace(contents=frozenset(artifact_contents))
...
@pytest.mark.asyncio
async def test_artifact_capture_failure_does_not_interrupt_generation() -> None:
    session = _FailingRecordArtifactSession()
    responses = [
        _request_output([_output([7])], prompt_token_ids=[101]),
        _request_output(
            [_output([8], finish_reason="length")], prompt_token_ids=[101]
        ),
    ]

    chunks, _ = await _collect_handler_chunks(responses, session)

    assert [chunk["token_ids"] for chunk in chunks] == [[7], [8]]
    assert chunks[-1]["finish_reason"] == "length"
    assert chunks[-1]["engine_data"]["generation_artifact"] == {
        "format": "generation_artifact_v1",
        "contents": ["selected_logprobs"],
        "state": "failed",
        "error_code": "artifact_delivery_failed",
