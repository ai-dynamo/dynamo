...
def test_moe_routes_round_trip_with_nonzero_token_start() -> None:
    ...
    encoded = encode_generation_artifact(view, codec=CODEC_NONE)
    decoded = decode_generation_artifact(encoded.data)

    component = decoded.manifest["choices"][0]["components"][0]
    assert component["token_start"] == 2
    assert decoded.choices[0].routed_experts_token_start == 2
    np.testing.assert_array_equal(
        decoded.choices[0].routed_experts,
        np.array([[[1, 2]]], dtype=np.int32),
    )


def test_choice_rejects_oversized_input_before_copy() -> None:
    values = np.array([101, 102], dtype=np.int64)

    with (
        patch("dynamo.vllm.generation_artifact_format._MAX_PAYLOAD_BYTES", 8),
        patch(
            "dynamo.vllm.generation_artifact_format.np.ascontiguousarray"
        ) as contiguous,
        pytest.raises(GenerationArtifactFormatError, match="payload limit"),
    ):
        GenerationArtifactChoice(
            choice_index=0,
            prompt_token_count=1,
            sequence_token_ids=values,
        )
