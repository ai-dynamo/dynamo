...
class TestDecodeWorkerMultimodalBranching:
    ...
    async def test_decode_only_with_mm_data_no_prefill_result_errors(self):
        """Decode-only worker receiving mm request without prefill_result -> error."""
    ...
    async def test_decode_only_non_qwen_missing_embedding_params_errors(self):
        """Decode-only non-Qwen with prefill_result but no embedding_params -> error."""
        handler = _make_decode_handler(
...
class TestBuildEmbeddingParams:
    ...
    def test_incomplete_dict_raises_error(self):
        """Dict-style image data missing image_embeds -> ValueError."""
        handler = _make_prefill_handler(model="Qwen/Qwen3-VL-2B-Instruct")
        mm_data = {
            "image": {
                "image_grid_thw": torch.tensor([[1, 16, 16]]),
                # image_embeds missing
            }
        }
        with pytest.raises(ValueError, match="incomplete Qwen VL embedding metadata"):
            handler._build_embedding_params(mm_data)

    def test_empty_grid_thw_raises_error(self):
        """Dict-style image data with empty image_grid_thw -> ValueError."""
        handler = _make_prefill_handler(model="Qwen/Qwen3-VL-2B-Instruct")
        mm_data = {
            "image": {
                "image_grid_thw": [],
    ...
    def test_pil_image_qwen_computes_grid(self):
        ...
        from dynamo.vllm.multimodal_utils.model import QwenGridParams
    ...
    def test_pil_image_qwen_params_unavailable_raises_error(self):
        """Qwen VL with no grid params -> raises ValueError."""
        from PIL import Image
        ...
        img = Image.new("RGB", (640, 480))
        with pytest.raises(ValueError, match="PIL images without embedding"):
            handler._build_embedding_params({"image": img})
