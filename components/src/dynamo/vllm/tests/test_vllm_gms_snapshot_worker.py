...
def test_snapshot_suspend_failure_terminates_worker(monkeypatch):
    ...
    with (
        patch(
            "gpu_memory_service.integrations.vllm.worker.get_mx_load_context",
            return_value=None,
        ),
        patch.object(
            Worker,
...
async def test_pending_nixl_registration_blocks_non_kv_wake(monkeypatch):
    ...
    with (
        ...
        patch(
            "gpu_memory_service.integrations.vllm.worker.get_mx_load_context",
            return_value=mx_context,
        ),
        patch(
            "gpu_memory_service.integrations.vllm.worker.resume_serving",
            resume_serving,
            create=True,
        ),
    ):
