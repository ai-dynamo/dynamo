...
@pytest.mark.parametrize("load_format_args", [[], ["--load-format", "auto"]])
def test_gms_shadow_mode_accepts_v1_load_profile(load_format_args):
    config = parse_args([*_BASE_V1_ARGS, *load_format_args, "--gms-shadow-mode"])
...
@pytest.mark.asyncio
async def test_shadow_election_rejects_resumed_engine():
    factory = WorkerFactory(Mock(), Mock(), AsyncMock(), Mock(), Mock())

    with pytest.raises(RuntimeError, match="shadow engine must be paused"):
