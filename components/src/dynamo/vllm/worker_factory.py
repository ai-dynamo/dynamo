...
class WorkerFactory:
    ...
    async def _wake_with_failover_lock(
        ...
        """Elect the paused active shadow and wake it while retaining the lock."""
        if config.gms_shadow_mode is not True:
            return False, None

        if not pause_controller.is_paused:
            raise RuntimeError("shadow engine must be paused before failover election")
    ...
    async def _run_decode_worker(
        ...
        if config.gms_shadow_mode is True and snapshot_controller is not None:
