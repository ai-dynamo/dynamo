...
class BaseWorkerHandler(ABC, Generic[RequestT, ResponseT]):
    ...
    **Subclass contract for LoRA support**: Subclasses that support LoRA must
    ...
    async def sleep(self, body: dict) -> dict:
        ...
        async with self._pause_lock:
            ...
            try:
                ...
                if lock_released:
                    # This process deliberately handed shared-KV writer
                    # ownership to its successor. The ordinary admin wake API
                    # cannot reacquire/fence that ownership, so never let the
                    # old primary resume or re-register through it.
                    self._gms_failover_lock_relinquished = True
                return {
                    "status": "ok",
    ...
    async def wake_up(self, body: dict) -> dict:
        ...
        tags = body.get("tags")
        async with self._pause_lock:
            if getattr(self, "_gms_failover_lock_relinquished", False):
                return {
                    "status": "error",
                    "message": (
                        "Engine handed off GMS KV writer ownership and cannot be "
                        "woken through the admin API; restart it through the "
                        "failover activation path"
                    ),
                }
