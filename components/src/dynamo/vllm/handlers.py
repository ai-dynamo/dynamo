...
from .lora_handler import LoRAHandlerMixin
...
class BaseWorkerHandler(LoRAHandlerMixin, ABC, Generic[RequestT, ResponseT]):
    """
    ...
    **Subclass contract for LoRA support**: Subclasses that support LoRA must
    ...
    def __init__(
        ...
        # Sets _served_model_name/_served_model_aliases used by
        # _resolve_lora_request(); specialized handlers can override those
        # fields after super().__init__.
        self.init_lora_state(config, generate_endpoint)
...
class EmbeddingWorkerHandler(LoRAHandlerMixin):
    ...
    LoRA comes from ``LoRAHandlerMixin`` instead, which both worker families
    share. Adapters only apply to architectures vLLM declares ``SupportsLoRA``:
    decoder-backed pooling models such as Qwen3-Embedding do, while
    encoder-backed ones (BERT, RoBERTa, ModernBERT) do not and make vLLM raise
    at engine start.
    """
    ...
    def _lora_model_type(self) -> ModelType:
        """Model type stamped on this role's adapter cards."""
        return ModelType.Embedding

    async def _register_lora_discovery(self, lora_name: str, lora_id: int) -> None:
        """Publish a loaded adapter as a pooling-family model card.

        Pooling roles are always aggregated and have no KV cache, so this
        carries none of the generation card's prefill/decode wiring, token
        budget, or parser fields.
        """
        if self.generate_endpoint is None:
            logger.debug(
                "Cannot publish LoRA '%s': no generate_endpoint on this worker",
                lora_name,
            )
            return
