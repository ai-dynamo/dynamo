...
2. **Sidecar**: NEW. A separate Dynamo worker talks to a native vLLM engine
   process over the engine RPC v1 gRPC contract. This worker is implemented in
...
| Engine | `vllm.AsyncLLM` in-process | native vLLM engine over engine RPC gRPC |
| Imports vllm? | yes | **no** — tonic engine RPC client only |
...
from the engine RPC API instead of in-process vLLM objects.
