...
2. **Sidecar**: NEW. A separate Dynamo worker talks to a native vLLM engine
   process over the vLLM gRPC contract. This worker is implemented in
...
| Engine | `vllm.AsyncLLM` in-process | native vLLM engine over vLLM gRPC |
| Imports vllm? | yes | **no** — tonic vLLM gRPC client only |
...
from the vLLM gRPC API instead of in-process vLLM objects.
