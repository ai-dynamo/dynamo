...
class VideoLoader:
    """
    Video loader that fetches, decodes, and samples frames from video URLs.

    Ported from examples/multimodal/utils/video_utils.py. Wraps the free
    functions above into a stateful class with a BytesIO content cache
    (matching the original Queue-based eviction pattern).
    """
    ...
    def __init__(
        ...
        self._cache_queue: Queue[str] = Queue(maxsize=cache_size)

    async def load_video(self, video_url: str) -> np.ndarray:
        """
        Load video from URL, sample frames, and return as np.ndarray (F, H, W, C).

        This follows the same pipeline as the old video_encode_worker:
        load_video_content -> open_video_container -> get_video_metadata
        -> calculate_frame_sampling_indices -> read_video_pyav
        """
        ...
        logger.debug(
            f"Successfully extracted {clip_np.shape[0]} frames from {video_url} "
            f"with shape {clip_np.shape}"
        )
