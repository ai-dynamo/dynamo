...
def _immutable_array(value: Any) -> np.ndarray:
    ...
    if array.nbytes > _MAX_PAYLOAD_BYTES:
        raise GenerationArtifactFormatError("array exceeds configured payload limit")
...
class GenerationArtifactChoice:
    ...
    routed_experts_token_start: int = 0
...
def _integer_array(
    value: Any, field: str, max_bytes: int = _MAX_PAYLOAD_BYTES
) -> np.ndarray:
    array = np.asarray(value)
    ...
    if maximum <= np.iinfo(np.uint8).max:
        ...
        dtype = np.dtype("<i8")
    if max_bytes < 0 or array.size > max_bytes // dtype.itemsize:
        raise GenerationArtifactFormatError("artifact payload exceeds configured limit")
    return np.ascontiguousarray(array, dtype=dtype)


def _token_array(value: Any, max_bytes: int = _MAX_PAYLOAD_BYTES) -> np.ndarray:
    raw = np.asarray(value)
    dtype = np.dtype("<i8")
    if max_bytes < 0 or raw.size > max_bytes // dtype.itemsize:
        raise GenerationArtifactFormatError("artifact payload exceeds configured limit")
    array = _integer_array(value, "sequence_token_ids", max_bytes)
    if array.ndim != 1:
        raise GenerationArtifactFormatError("sequence_token_ids must have rank 1")
    return np.ascontiguousarray(array, dtype=dtype)


def _logprob_array(value: Any, max_bytes: int = _MAX_PAYLOAD_BYTES) -> np.ndarray:
    array = np.asarray(value)
...
def _validate_choice(
    ...
    sequence = _token_array(choice.sequence_token_ids, max_payload_bytes)
    remaining_payload_bytes = max_payload_bytes - sequence.nbytes
    ...
    if choice.routed_experts is not None:
        route_start = choice.routed_experts_token_start
        if (
            isinstance(route_start, bool)
            or not isinstance(route_start, int)
            or not 0 <= route_start < sequence_length
        ):
            raise GenerationArtifactFormatError(
                "moe route token_start must index the sequence"
            )
        routes = _integer_array(
            choice.routed_experts, "moe_routes.expert_ids", remaining_payload_bytes
        )
        remaining_payload_bytes -= routes.nbytes
        if routes.ndim != 3:
        ...
        expected_route_rows = sequence_length - route_start - 1
        if routes.shape[0] != expected_route_rows:
            raise GenerationArtifactFormatError(
                "route token count does not match token_start and sequence length"
            )
    ...
    if choice.selected_logprobs is not None:
        selected = _logprob_array(choice.selected_logprobs, remaining_payload_bytes)
...
def encode_generation_artifact(
    ...
    def append_tensor(array: np.ndarray) -> dict[str, Any]:
        nonlocal payload_offset
        if array.nbytes > _MAX_PAYLOAD_BYTES - payload_offset:
            raise GenerationArtifactFormatError(
                "artifact payload exceeds configured limit"
            )
        reference = _tensor_ref(array, payload_offset)
        payload_parts.append(array.tobytes(order="C"))
        payload_offset += array.nbytes
        return reference

    for choice in view.choices:
        sequence, routes, selected = _validate_choice(
            choice, _MAX_PAYLOAD_BYTES - payload_offset
        )
        sequence_ref = append_tensor(sequence)
        components: list[dict[str, Any]] = []

        if routes is not None:
            route_ref = append_tensor(routes)
            components.append(
                {
                    "kind": "moe_routes",
                    "token_start": choice.routed_experts_token_start,
        ...
        if selected is not None:
            selected_ref = append_tensor(selected)
            components.append(
                {
    ...
    for raw_choice in raw_choices:
        ...
        for component in components:
            ...
            if kind == "moe_routes":
                ...
                route_start = component["token_start"]
                if (
                    isinstance(route_start, bool)
                    or not isinstance(route_start, int)
                    or route_start < 0
                    ...
                    raise GenerationArtifactFormatError(
                        "moe route token_start must be a non-negative integer"
                    )
        ...
        choice = GenerationArtifactChoice(
            ...
            routed_experts_token_start=route_start,
