...
class VllmHybridCacheSimulator:
    ...
    def _cpu_lookup(self, request: HybridCacheRequest, gpu_tokens: int) -> int:
        max_hit = request.input_length - 1
        start_block = gpu_tokens // self._full_group.block_size
        end_block = max_hit // self._full_group.block_size
        full_hits = 0
        ...
        if full_hits == 0:
            return 0
        candidate = min(max_hit, gpu_tokens + full_hits * self._full_group.block_size)
        sliding_groups = sorted(
            ...
            key=lambda group: self._sliding_need(group, gpu=False),
            reverse=True,
        )
        for group in sliding_groups:
            start_block = gpu_tokens // group.block_size
            end_block = min(
                (candidate + group.block_size - 1) // group.block_size,
                request.input_length // group.block_size,
            )
            needed = self._sliding_need(group, gpu=False)
            consecutive = 0
            end_of_run = 0
            for block_index in range(end_block - 1, start_block - 1, -1):
                ...
                if consecutive == needed:
                    end_of_run = block_index + needed
                    break
            if end_of_run == 0:
                return 0
            candidate = min(candidate, end_of_run * group.block_size)
        external_tokens = max(0, candidate - gpu_tokens)
        if external_tokens:
            for group in self.config.groups:
                if not group.offload:
                    continue
                first = gpu_tokens // group.block_size
                last = (
                    gpu_tokens + external_tokens + group.block_size - 1
                ) // group.block_size
                block_indices = list(range(first, last))
                if group.sliding_window is not None:
                    block_indices = block_indices[
                        -self._sliding_need(group, gpu=False) :
                    ]
    ...
    def _store_offer_keys(
        ...
        for group in self.config.groups:
            if not group.offload:
                continue
            block_count = request.input_length // group.block_size
            first = (
                combined_hit_tokens // group.block_size if external_hit_tokens else 0
            )
            if group.sliding_window is None:
                block_indices = range(first, block_count)
            else:
                needed = self._sliding_need(group, gpu=False)
                blocks_per_alignment = (
                    self.config.scheduler_block_size // group.block_size
                )
                block_indices = (
                    block_index
                    for block_index in range(first, block_count)
                    if block_index % blocks_per_alignment
                    >= blocks_per_alignment - needed
                    and (
                        block_index * group.block_size >= gpu_hit_tokens
                        or (
                            block_index >= gpu_hit_tokens // group.block_size - needed
                            and self._prompt_key(request, group, block_index)
                            in self._gpu_by_group[group.group_index]
                        )
                    )
                )
            for block_index in block_indices:
                offered.append(self._prompt_key(request, group, block_index))
        return offered
