...
def _deploy_snapshot_chart(
    ...
    # Annotate the PVC so helm uninstall does not delete it.  The checkpoint
    # data itself must survive cleanup between benchmark runs.
    _kubectl(
        "annotate",
        "pvc",
        "snapshot-pvc",
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Worker image (used for spec.image/profiler, Planner, and workers). "
        "When using a placeholder image, pass it here and also set --frontend-image.",
    )
    p.add_argument(
        "--frontend-image",
        default=os.getenv("FRONTEND_IMAGE"),
        help="Frontend service image. Defaults to deriving dynamo-frontend:<tag> from --image. "
        "Set explicitly when --image is a placeholder or non-standard image.",
    )
...
def main():
    ...
    cfg = {
        "image": args.image,
        "frontend_image": args.frontend_image or None,
        "namespace": ns,
        "model": args.model,
