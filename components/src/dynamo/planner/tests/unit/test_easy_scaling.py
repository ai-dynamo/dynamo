...
class TestEasyDiagnostics:
    def test_prefill_queue_ratio_populated(self):
        core = _make_core(mode="prefill", optimization_target="throughput")
        fpm = _make_fpm(queued_prefill_tokens=CONTEXT_LENGTH // 2)
        tick = TickInput(
            now_s=5.0,
        ...
        effects = core.on_tick(_tick_for(tick), tick)
        assert effects.diagnostics.easy_prefill_queue_ratio is not None
        assert effects.diagnostics.easy_prefill_queue_ratio == pytest.approx(0.5, abs=0.01)

    def test_decode_kv_util_populated(self):
        core = _make_core(
            mode="decode", optimization_target="throughput", caps=_decode_caps()
        )
        fpm = _make_fpm(sum_decode_kv_tokens=70000, queued_decode_kv_tokens=0)
        tick = TickInput(
            now_s=5.0,
            fpm_observations=FpmObservations(decode={("w1", 0): fpm}),
            worker_counts=WorkerCounts(ready_num_decode=1),
        ...
        effects = core.on_tick(_tick_for(tick), tick)
        assert effects.diagnostics.easy_decode_kv_utilization is not None
        assert effects.diagnostics.easy_decode_kv_utilization == pytest.approx(
            0.7, abs=0.01
        )
    ...
    def test_diagnostics_reset_each_tick(self):
        ...
        assert effects1.diagnostics.easy_prefill_queue_ratio is not None

        tick2 = TickInput(now_s=10.0, worker_counts=WorkerCounts(ready_num_prefill=1))
        st2 = ScheduledTick(
            at_s=10.0,
        ...
        assert effects2.diagnostics.easy_prefill_queue_ratio is None
