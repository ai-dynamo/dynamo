...
class TestRoutedEnginePath:
    @pytest.mark.parametrize("extra_field", ["routed_experts", "engine_data"])
    def test_generation_artifact_preserves_worker_extra_fields(
        self, vllm_processor_module, extra_field
    ):
        request = {
            "nvext": {
                "extra_fields": [extra_field],
                "generation_artifact": {"format": "generation_artifact_v1"},
            }
        }

        assert vllm_processor_module._generation_artifact_worker_nvext(request) == {
            "extra_fields": [extra_field],
            "generation_artifact": {"format": "generation_artifact_v1"},
        }
    ...
    @pytest.mark.parametrize("extra_field", ["routed_experts", "engine_data"])
    @pytest.mark.asyncio
    async def test_generation_artifact_projects_requested_coexistence_field(
        self, vllm_processor_module, extra_field
    ):
        receipt = {
        ...
        routed_experts = {"start": 0, "data": "opaque-routes"}
        worker_engine_data = {
            "generation_artifact": receipt,
            "routed_experts": routed_experts,
            "backend_private": "explicit-engine-data-only",
        }
            ...
            [
                {
                    ...
                    "index": 0,
                    "finish_reason": "stop",
                    "engine_data": worker_engine_data,
                }
            ]
        )
        ...
        nvext = chunks[-1]["data"]["nvext"]
        assert nvext["generation_artifact"] == receipt
        if extra_field == "engine_data":
            assert nvext["engine_data"] == worker_engine_data
            assert "routed_experts" not in nvext
        else:
            assert nvext["routed_experts"] == routed_experts
            assert "engine_data" not in nvext
            assert "backend_private" not in json.dumps(nvext)
