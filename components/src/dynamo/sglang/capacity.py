...
def publishes_kv_events(server_args: Any) -> bool:
    ...
    node, so every node of a multinode gang advertises the identical
    ``(leader_worker_id, 0)`` source. The frontend marks a key advertised by more
    than one publisher ``Ambiguous``; ``active_source()`` then returns ``None``,
    no source is ever ready, the direct-ZMQ ingress never leaves ``Preconnected``,
    and the indexer applies zero events -- KV routing silently degrades to
    round-robin with no error anywhere.

    Only the leader owns that single logical rank; in TP-only mode SGLang emits
    the radix-cache events from the rank-0 scheduler anyway, so the non-leader
    socket has nothing distinct to contribute.
    """
    dp_size = getattr(server_args, "dp_size", 1) or 1
    enable_dp_attention = getattr(server_args, "enable_dp_attention", False)
    nnodes = getattr(server_args, "nnodes", 1) or 1
