...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    ("processor_name", "bootstrap_room", "expected_room"),
    [
        ("_process_token_stream", 4242, "4242"),
        ("_process_token_stream", None, "None"),
        ("_process_text_stream", 4242, "4242"),
    ],
)
async def test_first_engine_response_logs_the_engine_id_map(
    caplog, processor_name, bootstrap_room, expected_room
):
    handler = _new_decode_handler(
        use_sglang_tokenizer=processor_name == "_process_text_stream"
    )
    context = _Context()
    context.id = lambda: "11111111-2222-3333-4444-555555555555"
    chunks = [
        {"index": 0, "output_ids": [7], "text": "a", "meta_info": {"id": "sgl-rid-1"}},
        {
            "index": 0,
            "output_ids": [8],
            "text": "b",
            "meta_info": {"id": "sgl-rid-1", "finish_reason": {"type": "stop"}},
        },
    ]
    kwargs = {"bootstrap_room": bootstrap_room}
    if processor_name == "_process_text_stream":
        kwargs["request"] = {}
    with caplog.at_level("INFO"):
        await _collect(
            getattr(handler, processor_name)(_stream(chunks), context, **kwargs)
        )
    ...
    assert maps == [
        "Engine ID map: request_id=11111111-2222-3333-4444-555555555555 "
        f"sglang_rid=sgl-rid-1 bootstrap_room={expected_room}"
    ]


@pytest.mark.asyncio
async def test_engine_id_map_covers_every_choice_of_a_fanned_out_request(caplog):
    handler = _new_decode_handler()
    context = _Context()
    context.id = lambda: "req-uuid"
    chunks = [
        {"index": 0, "output_ids": [1], "meta_info": {"id": "rid-a"}},
        {"index": 1, "output_ids": [2], "meta_info": {"id": "rid-b"}},
        {"index": 0, "output_ids": [3], "meta_info": {"id": "rid-a"}},
        {
            "index": 0,
            "output_ids": [],
            "meta_info": {"id": "rid-a", "finish_reason": {"type": "stop"}},
        },
        {
            "index": 1,
            "output_ids": [],
            "meta_info": {"id": "rid-b", "finish_reason": {"type": "stop"}},
        },
    ]
    with caplog.at_level("INFO"):
        await _collect(
            handler._process_token_stream(_stream(chunks), context, bootstrap_room=None)
        )
    ...
    assert maps == [
        "Engine ID map: request_id=req-uuid sglang_rid=rid-a bootstrap_room=None",
        "Engine ID map: request_id=req-uuid sglang_rid=rid-b bootstrap_room=None",
    ]
