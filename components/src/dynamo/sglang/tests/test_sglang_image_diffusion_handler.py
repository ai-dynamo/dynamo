...
def mock_config():
    ...
    config.dynamo_args.media_fs_url = "file:///tmp/images"
    config.dynamo_args.media_base_url = "file:///tmp/images"
    return config
...
class TestImageDiffusionWorkerHandler:
    ...
    def test_initialization_with_url_base(
        ...
        config = MagicMock()
        config.dynamo_args = MagicMock()
        config.dynamo_args.media_fs_url = "s3://my-bucket/images"
        config.dynamo_args.media_base_url = "http://localhost:8008/images"
