...
`0 <= logprob_start_len < prompt length` -- the default `-1` scores output tokens only),
the chunk carries a non-null `finish_reason`, and `input_token_logprobs` is absent or
empty; aggregated workers and the prefill worker's responses are untouched.
`DecodeWorkerHandler._process_native_generate_stream` and the Rust sidecar's
`native_http::output` both emit it (via
