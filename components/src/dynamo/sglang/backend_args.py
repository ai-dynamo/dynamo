...
class DynamoSGLangArgGroup(ArgGroup):
    ...
    def add_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        add_argument(
            g,
            flag_name="--max-output-tokens",
            env_var="DYN_SGL_MAX_OUTPUT_TOKENS",
            default=None,
            arg_type=int,
            help=(
                "Hard upper limit for tokens generated by one request. Requests "
                "with a larger or omitted max_tokens value are capped at this value."
            ),
        )
        add_argument(
            g,
