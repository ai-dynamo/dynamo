...
def test_required_interpolation_failure_is_fatal(failure):
    ...
    with pytest.raises(
        RuntimeError,
        match=(
            "Thorough mode requires real-GPU interpolation data. For GPU-free "
            "profiling, use searchStrategy='rapid' for mocker-only deployments "
            "or pre_deployment_sweeping_mode='rapid' when Planner is configured."
        ),
    ) as exc_info:
        asyncio.run(
            _wait_for_required_interpolation_deployment(
