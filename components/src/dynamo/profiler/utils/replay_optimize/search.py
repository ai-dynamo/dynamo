...
def _normalize_router_mode(router_mode: str) -> Literal["kv_router", "round_robin"]:
    if router_mode not in {"kv_router", "round_robin"}:
        raise ValueError(
            "router_mode must be one of ['kv_router', 'round_robin'], "
            f"got {router_mode!r}"
        )
    return router_mode
...
def optimize_dense_disagg_with_replay(
    *,
    ...
    router_mode: Literal["kv_router", "round_robin"] = "kv_router",
