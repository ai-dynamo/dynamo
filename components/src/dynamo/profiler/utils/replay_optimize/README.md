...
- `SLASpec` — latency bounds (`ttft`, `itl`, `e2eLatency`, plus p95 variants); each is independent
  and optional
- `RouterSpec` — router mode, overlap-score-weight sweep, prefill-load-scale sweep, KV-router base config
...
- change `RouterSpec.overlapWeights` within the valid 0.0 to 1.0 range
...
- routing: `router_mode`, `overlap_score_weight`, `prefill_load_scale`
...
- `prefill_tp=4`, `decode_tp=1`, `prefill_workers=3`, `decode_workers=4`, `overlap_score_weight=0.5`, `prefill_load_scale=1.0`
