...
class VllmV1ConfigModifier(BaseConfigModifier):
    ...
    def set_device_type(cls, config: dict, device_type: str) -> dict:
        ...
        #    each worker pod to use it.  We use one claim template named
        #    "gpu-template" (matching the example YAMLs) and reference it from
        #    each worker via resourceClaims / claims.
        if worker_names:
            ...
            for name in worker_names:
                ...
                existing_claim_names = [
                    rc.get("name") for rc in pod_extra["resourceClaims"]
