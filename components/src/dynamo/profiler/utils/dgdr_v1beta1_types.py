...
class ThoroughSpec(BaseModel):
    ...
    prefilterTopN: Optional[int] = Field(default=None, description="PrefilterTopN, when set, uses AIC to score enumerated candidates offline and benchmarks only the top-N per side (prefill and decode). Omit or set to null to benchmark every candidate (unchanged behavior).")
...
class DynamoGraphDeploymentRequestSpec(BaseModel):
    ...
    thorough: Optional[ThoroughSpec] = Field(default=None, description="Thorough configures options for the THOROUGH search strategy. Only applies when SearchStrategy is \"thorough\".")
