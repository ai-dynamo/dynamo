...
def generate_config_data(
    ...
    data[PlotType.PREFILL]["chart"]["axes"]["y"]["title"] = f"Prefill Throughput per {device_label} (tokens/s/{device_label})"
    data[PlotType.DECODE]["chart"]["axes"]["y"]["title"] = f"Decode Throughput per {device_label} (tokens/s/{device_label})"
    data[PlotType.COST]["chart"]["axes"]["y"]["title"] = f"{device_label} Hours"
    data[PlotType.PREFILL]["table"]["columns"] = [f"{device_label}s", "TTFT (ms)", f"Throughput (tokens/s/{device_label})", "Action"]
    data[PlotType.DECODE]["table"]["columns"] = [f"{device_label}s", "ITL (ms)", f"Throughput (tokens/s/{device_label})", "Action"]
    data[PlotType.COST]["table"]["columns"] = ["TTFT (ms)", f"Prefill Thpt (tokens/s/{device_label})", "ITL (ms)", f"Decode Thpt (tokens/s/{device_label})", "Tokens/User", f"{device_label} Hours", "Action"]
...
def populate_prefill_data(data: dict, prefill_data: Any, args: Any) -> None:
    ...
    data[PlotType.PREFILL]["chart"]["labels"] = [f"{gpu} {device_label}s" for gpu in unique_gpus]
