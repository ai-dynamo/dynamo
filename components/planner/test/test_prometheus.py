...
from dynamo.planner.utils.prometheus import FrontendMetric, FrontendMetricContainer, PrometheusAPIClient
...
def test_frontend_metric_container_with_nan_value():
    ...
    assert math.isnan(container.value[1]) # becomes special float value that can't be asserted to itself
...
def test_get_average_metric_none_result():
    ...
    with patch.object(client.prom, 'custom_query') as mock_query:
        ...
        result = client._get_average_metric(
            ...
            dynamo_namespace="test_namespace"
        )
