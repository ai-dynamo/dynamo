...
mod nixl_tests {
    ...
    // Device tests (when testing-nixl is paired with testing-cuda or testing-xpu-sycl).
    // `test_device_ctx()` is only defined under those feature combos, so the
    // mod must be skipped if testing-nixl is enabled in isolation.
    #[cfg(any(feature = "testing-cuda", feature = "testing-xpu-sycl"))]
