...
//! `zeMemAllocDevice` / `zeMemFree`, mirroring the [`super::cuda::CudaMemPool`]
//! API surface for Intel GPUs.
//!
...
//!   On a miss it falls back to `zeMemAllocDevice`.
...
//! is achieved by the caller via [`syclrc::level_zero::ze::safe::ZeImmediateCmdList`].

use anyhow::{Result, anyhow};
use syclrc::level_zero::ze::safe::ZeDevice;
use syclrc::level_zero::ze::result as ze_result;
...
#[derive(Debug)]
struct FreeBlock {
    /// Device pointer returned by `zeMemAllocDevice`.
    ptr: u64,
...
impl PoolInner {
    ...
    /// Returns the drained blocks (caller must `zeMemFree` them).
...
/// use syclrc::level_zero::ze::safe::ZeDevice;
///
/// let dev = ZeDevice::new(0).unwrap();
/// let pool = ZeMemPoolBuilder::new(dev.clone(), 64 * 1024 * 1024) // 64 MiB reserve
///     .release_threshold(32 * 1024 * 1024) // 32 MiB release threshold
///     .build()
///     .unwrap();
/// ```
pub struct ZeMemPoolBuilder {
    /// Level Zero device (owns driver + device + context handles).
    device: Arc<ZeDevice>,
...
impl ZeMemPoolBuilder {
    ...
    ///
    /// # Arguments
    /// * `device` - Level Zero device (wraps driver + device + context)
    /// * `reserve_size` - Number of bytes to pre-allocate to warm the pool
    pub fn new(device: Arc<ZeDevice>, reserve_size: usize) -> Self {
        Self {
            device,
            reserve_size,
...
/// Free-list memory pool for Intel GPUs via Level Zero.
///
...
/// are returned to the driver via `zeMemFree`.
///
/// # Thread Safety
///
...
pub struct ZeMemPool {
    /// The Level Zero device this pool allocates on.
    device: Arc<ZeDevice>,
    /// Mutex protecting the free-list.
    inner: Mutex<PoolInner>,
    /// Cached bytes above this threshold trigger actual `zeMemFree` calls.
    release_threshold: u64,
}

// SAFETY: ZeMemPool is Send because the Mutex serializes all host-side access
// to the free-list, and `zeMemAllocDevice` / `zeMemFree` are thread-safe when
// properly serialized.
unsafe impl Send for ZeMemPool {}

// SAFETY: ZeMemPool is Sync because all access to the inner state goes through
// the Mutex.
...
impl ZeMemPool {
    ...
    /// * `device` - Level Zero device (wraps driver + device + context)
    /// * `reserve_size` - Number of bytes to pre-allocate to warm the pool
    pub fn builder(device: Arc<ZeDevice>, reserve_size: usize) -> ZeMemPoolBuilder {
        ZeMemPoolBuilder::new(device, reserve_size)
    }

    /// Allocate memory from the pool.
    ///
    /// Tries the free-list first (best-fit); falls back to `zeMemAllocDevice`.
    ///
    /// Unlike [`super::cuda::CudaMemPool::alloc_async`] this does **not** take a
    /// stream parameter because Level Zero device allocation is CPU-synchronous.
    ///
    /// # Host Serialization
    ///
    ...
    /// freed via `zeMemFree` until the cache is within budget.
    ///
    /// Unlike [`super::cuda::CudaMemPool::free_async`] this does **not** take a
    /// stream parameter because Level Zero memory free is CPU-synchronous.
    ///
    /// # Arguments
    ...
    pub fn device(&self) -> &Arc<ZeDevice> {
        &self.device
    }
    ...
    /// Raw allocation via `zeMemAllocDevice`.
    fn raw_alloc(&self, size: usize) -> Result<u64> {
        let ptr = unsafe {
            ze_result::memory::alloc_device(
                self.device.ze_context(),
                size,
                0, // alignment — 0 lets the driver choose
                self.device.ze_device(),
            )
        }
    ...
    /// Raw free via `zeMemFree`.
    fn raw_free(&self, ptr: u64) -> Result<()> {
        unsafe {
            ze_result::memory::free(
                self.device.ze_context(),
                ptr as *mut std::ffi::c_void,
            )
        }
        .map_err(|e| anyhow!("zeMemFree failed: {}", e))?;
...
impl Drop for ZeMemPool {
    fn drop(&mut self) {
        ...
        // Drain every block from the free-list and return to the driver.
        let all_blocks = inner.drain_to(0);
        for block in all_blocks {
            if let Err(e) = unsafe {
                ze_result::memory::free(
                    self.device.ze_context(),
                    block.ptr as *mut std::ffi::c_void,
                )
            } {
                tracing::warn!("zeMemFree failed during ZeMemPool drop: {e}");
            }
        }
    }
}
...
mod tests {
    ...
    fn test_pool_creation_with_builder() {
        let dev = match ZeDevice::new(0) {
            Ok(d) => d,
            Err(e) => {
                eprintln!("Skipping test - no Level Zero device: {e}");
                return;
            }
        };

        let pool = ZeMemPool::builder(dev, 1024 * 1024) // 1 MiB reserve
    ...
    fn test_pool_alloc_free_reuse() {
        ...
        let pool = ZeMemPool::builder(dev, 0).build().expect("pool build");

        // First allocation — cache miss, goes to driver.
    ...
    fn test_pool_release_threshold() {
        ...
        // Threshold of 8 KiB.
        let pool = ZeMemPool::builder(dev, 0)
