...
/// let pool = ZeMemPoolBuilder::new(queue, 64 * 1024 * 1024) // 64 MiB reserve
...
impl ZeMemPool {
    ...
    /// * `queue` - SYCL queue (wraps device + context + queue)
    /// * `reserve_size` - Number of bytes to pre-allocate to warm the pool
    pub fn builder(queue: Arc<SyclQueue>, reserve_size: usize) -> ZeMemPoolBuilder {
        ZeMemPoolBuilder::new(queue, reserve_size)
    }
    ...
    fn drop(&mut self) {
        ...
        for block in all_blocks {
            unsafe {
                ...
                .result()
                {
                    tracing::warn!("sycl_rs_free failed during ZeMemPool drop: {e}");
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_pool_creation_with_builder() {
        ...
        let pool = ZeMemPool::builder(queue, 1024 * 1024) // 1 MiB reserve
    ...
    fn test_pool_alloc_free_reuse() {
        ...
        let pool = ZeMemPool::builder(queue, 0).build().expect("pool build");
    ...
    fn test_pool_release_threshold() {
        ...
        // Threshold of 8 KiB.
        let pool = ZeMemPool::builder(queue, 0)
