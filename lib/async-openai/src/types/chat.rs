...
/// Deserialize `arguments` from either a JSON string or a JSON object/array.
///
/// Some agent frameworks (e.g. LangChain, custom harnesses) pass tool-call
/// arguments as a pre-parsed dict rather than a raw JSON string.  This helper
/// normalises both representations to the canonical string form so downstream
/// code never needs to branch on the wire format.
    ...
    D: serde::Deserializer<'de>,
{
    ...
    match value {
        ...
        other => Err(D::Error::custom(format!(
            "expected string or object for `arguments`, got {}",
            other
        ))),
    }
}

/// Deserialize an optional `arguments` field from either a JSON string or a
/// JSON object/array (see [`deserialize_arguments`]).
...
pub struct FunctionCall {
    ...
    ///
    /// Accepts either a JSON string (`"{\"key\":\"value\"}"`) or a JSON object
    /// (`{"key": "value"}`); both are normalised to a JSON string on
    /// deserialisation so that callers always receive the canonical form.
...
pub struct FunctionCallStream {
    ...
    /// Validate the arguments in your code before calling your function.
    ///
    /// Accepts either a JSON string or a JSON object; both are normalised to a
    /// JSON string on deserialisation (see [`deserialize_arguments_opt`]).
    #[serde(
        default,
