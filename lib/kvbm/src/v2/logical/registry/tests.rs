...
use crate::v2::logical::blocks::{
    Block, BlockDuplicationPolicy, BlockMetadata, CompleteBlock, PrimaryBlock,
    state::{Complete, Registered, Reset},
};
...
fn test_concurrent_try_get_block_and_drop() {
    ...
    let pool_return_fn = Arc::new(
        move |block: Arc<Block<TestMetadata, Registered>>| {
            barrier1_clone.wait();
            barrier2_clone.wait();
        ...
        as Arc<
            dyn Fn(Arc<Block<TestMetadata, Registered>>)
                + Send
                + Sync,
        >;

    let complete_block = Block::<TestMetadata, _>::new(0, 4)
