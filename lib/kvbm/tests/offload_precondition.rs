...
async fn test_offload_precondition_single_worker() -> Result<()> {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("warn,dynamo_nova=error,dynamo_kvbm=info")),
        )
        .with_test_writer()
        .try_init();
    ...
    let metadata = instance.leader.process_scheduler_output(scheduler_output)?;
...
async fn test_offload_precondition_cached_request() -> Result<()> {
    ...
    tracing::info!(
        "Processing cached request with {} new blocks",
        new_block_ids.len()
    );

    // 6. Process cached request
    let metadata2 = instance.leader.process_scheduler_output(cached_output)?;
