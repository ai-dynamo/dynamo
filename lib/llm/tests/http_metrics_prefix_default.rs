...
#[tokio::test]
async fn metrics_prefix_default() {
    env::remove_var(metrics::METRICS_PREFIX_ENV);
    let svc = HttpService::builder().port(9101).build().unwrap();
    let token = CancellationToken::new();
    let _handle = svc.spawn(token.clone()).await;
    tokio::time::sleep(Duration::from_millis(100)).await;

    // Force-create a labeled metric without needing a model/engine
    let state = svc.state_clone();
    {
        let _guard = state
            .metrics_clone()
            .create_inflight_guard("test-model", Endpoint::ChatCompletions, false);
        // guard drops here -> counter/gauge updated
    }

    let body = reqwest::get("http://localhost:9101/metrics")
        .await.unwrap()
        .text().await.unwrap();
