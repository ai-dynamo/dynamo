...
fn request_body(stream: bool, logprobs: bool) -> Value {
    let mut body = json!({
        "model": MODEL,
        ...
        .map(|(index, &(token, token_id, logprob))| {
            generator
                .choice_from_postprocessor(BackendOutput {
                    ...
                    cum_log_probs: None,
                    // Supply chosen logprobs even for the disabled control so
                    // that the converter's request gate is exercised as well.
...
async fn assert_http_response(stream: bool, logprobs: bool) {
    let body = request_body(stream, logprobs);
    ...
    println!("stream={stream}, logprobs={logprobs}, HTTP response:\n{raw}");
    ...
    assert_eq!(requests[0].inner.logprobs, Some(logprobs));
    assert_eq!(requests[0].inner.top_logprobs, logprobs.then_some(0));
    assert_eq!(requests[0].inner.stream, Some(stream));
    assert_eq!(svc.engine.remaining_scripts().await, 0);
    svc.shutdown().await;
}

async fn run_case(stream: bool, logprobs: bool) {
    temp_env::async_with_vars(
        [(DYN_HTTP_GRACEFUL_SHUTDOWN_TIMEOUT_SECS, Some("0"))],
        async {
            tokio::time::timeout(
                Duration::from_secs(15),
                assert_http_response(stream, logprobs),
            )
            .await
...
async fn streaming_top_zero_preserves_chosen_logprobs() {
    run_case(true, true).await;
}

#[tokio::test]
#[serial]
async fn disabled_logprobs_stay_absent_in_both_http_modes() {
