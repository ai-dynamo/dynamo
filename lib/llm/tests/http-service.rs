...
// =============================================================================
// Videos route error surfacing: worker material-fetch 403 vs genuine internal.
// The non-streaming /v1/videos path folds the annotated stream through
// from_anyhow, the same mapping images/audio already use.
// =============================================================================
...
async fn post_videos_generation(port: u16) -> reqwest::Response {
    reqwest::Client::new()
...
/// A backend material HTTP 403 (the urllib HTTPError envelope produced by
/// py_err_to_dynamo) must land as HTTP 403 and metrics validation, not 500/internal.
#[tokio::test]
async fn test_videos_material_http_403_is_4xx_not_internal() {
    let error = DynamoError::builder()
        ...
        .message(
            serde_json::json!({"message": "HTTP Error 403: Forbidden", "code": 403}).to_string(),
        )
        .build();
    ...
    let response = post_videos_generation(port).await;
    let status = response.status();
    let body = response.text().await.unwrap();
    assert!(
        status.is_client_error(),
        "material HTTP 403 must be HTTP 4xx, not {status}; body: {body}"
    );
    assert_eq!(
        status,
        StatusCode::FORBIDDEN,
        "expected HTTP 403; got {status}, body: {body}"
    );
    assert!(
...
/// A genuine internal worker failure on /v1/videos must stay HTTP 500/internal.
#[tokio::test]
