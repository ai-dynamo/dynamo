...
async fn test_multimodal_count_metrics_exposed() {
    ...
    temp_env::async_with_vars([(METRICS_PREFIX_ENV, None::<&str>)], async {
        ...
        assert!(
            metrics_body
                .contains("dynamo_frontend_images_per_request_count{dynamo_namespace=\"__local_chat_mmmodel\",model=\"mmmodel\"} 1\n"),
            "images_per_request_count should be 1; got:\n{metrics_body}"
        );
        assert!(
            metrics_body.contains("dynamo_frontend_images_per_request_sum{dynamo_namespace=\"__local_chat_mmmodel\",model=\"mmmodel\"} 2\n"),
            "images_per_request_sum should be 2; got:\n{metrics_body}"
        );
        assert!(
            metrics_body.contains("dynamo_frontend_videos_per_request_sum{dynamo_namespace=\"__local_chat_mmmodel\",model=\"mmmodel\"} 1\n"),
            "videos_per_request_sum should be 1; got:\n{metrics_body}"
        );
        assert!(
            metrics_body.contains("dynamo_frontend_audio_per_request_sum{dynamo_namespace=\"__local_chat_mmmodel\",model=\"mmmodel\"} 0\n"),
            "audio_per_request_sum should be 0; got:\n{metrics_body}"
        );
        assert!(
            metrics_body
                .contains("dynamo_frontend_image_tokens_per_request_count{dynamo_namespace=\"__local_chat_mmmodel\",model=\"mmmodel\"} 1\n"),
            "image_tokens_per_request_count should be 1; got:\n{metrics_body}"
        );
        assert!(
            metrics_body
                .contains("dynamo_frontend_image_tokens_per_request_sum{dynamo_namespace=\"__local_chat_mmmodel\",model=\"mmmodel\"} 1290\n"),
