...
mod declared_image_placeholder_token {
    use dynamo_llm::model_card::ModelDeploymentCard;
    use dynamo_llm::preprocessor::prompt::prompt_formatter_from_mdc;
    use dynamo_llm::protocols::openai::chat_completions::NvCreateChatCompletionRequest;
    use dynamo_renderer::PromptFormatter;

    // mock-llama renders through the Jinja chat_template in its
    // tokenizer_config.json — i.e. the path that must ignore the declaration.
    const MODEL_PATH: &str = "tests/data/sample-models/mock-llama-3.1-8b-instruct";

    // What SGLang's Qwen-VL processor declares. Under the old design SGLang
    // set a mode flag and the preprocessor rewrote segments; now it declares a
    // literal token for every multimodal model it serves, so a Jinja family
    // seeing one is the case that has to stay inert.
    const QWEN_VL_TOKEN: &str = "<|vision_start|><|image_pad|><|vision_end|>";

    fn render_with(image_placeholder_token: Option<&str>) -> String {
        let mut mdc = ModelDeploymentCard::load_from_disk(MODEL_PATH, None).unwrap();
        mdc.runtime_config.image_placeholder_token = image_placeholder_token.map(str::to_string);

        let PromptFormatter::OAI(formatter) = prompt_formatter_from_mdc(&mdc).unwrap();

        let messages: Vec<dynamo_protocols::types::ChatCompletionRequestMessage> =
            serde_json::from_str(r#"[{"role": "user", "content": "What is deep learning?"}]"#)
                .unwrap();
        let inner = dynamo_protocols::types::CreateChatCompletionRequestArgs::default()
            .model("test-model")
            .messages(messages)
    ...
    /// The containment guarantee: only native formatters that emit per-image
    /// segments take the declared token as a constructor argument, so a
    /// declaration made for some other family cannot reach a Jinja model.
    #[test]
    fn declared_token_does_not_change_a_jinja_rendered_prompt() {
        assert_eq!(render_with(None), render_with(Some(QWEN_VL_TOKEN)));
    }
}
