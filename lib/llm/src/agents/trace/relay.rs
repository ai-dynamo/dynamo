...
impl AgentToolEventRelay {
    ...
    ///
    /// - `component`: Dynamo component providing runtime/discovery context.
    /// - `zmq_endpoint`: local ZMQ PUB endpoint to subscribe to.
    /// - `zmq_topic`: optional ZMQ topic filter; `None` subscribes to all topics.
    /// - `event_namespace`: optional Dynamo namespace override. Defaults to the
    ///   component namespace.
    /// - `event_topic`: optional Dynamo event-plane topic override. Defaults to
    ...
    /// Create and start a relay from async Rust code.
    pub async fn start(
        component: Component,
        zmq_endpoint: String,
        zmq_topic: Option<String>,
