...
mod tests {
    ...
    #[tokio::test]
    async fn test_retry_manager_error_response_marks_stream_output() {
        let context_id = uuid::Uuid::new_v4().to_string();
        let request = create_mock_request(1);
        ...
        retry_manager.track_response(&error_response);

        assert!(retry_manager.current_stream_has_output);
        assert_eq!(retry_manager.request.stop_conditions.max_tokens, Some(1));
    }
