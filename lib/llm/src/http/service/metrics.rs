...
//! LLM-specific adapters for the plane-independent HTTP metrics core.
...
use dynamo_http_server::metrics::{
    ANNOTATION_AUDIT_USAGE, ANNOTATION_LLM_METRICS, HttpQueueGuard, Metrics, Registry,
    ResponseMetricCollector, encode_registry, observe_response_metrics,
};
...
/// Observe annotation-encoded metrics for response types without a typed payload.
pub fn process_response_and_observe_metrics<T>(
    ...
    observe_response_metrics(annotated, None, response_collector, http_queue_guard);
}

/// Observe the typed chat metric payload, with annotation-encoded fallback.
pub fn process_chat_response_and_observe_metrics(
    ...
    observe_response_metrics(annotated, typed, response_collector, http_queue_guard);
}
...
pub fn process_response_using_event_converter_and_observe_metrics<T: Serialize>(
    ...
    let annotation_metrics =
        observe_response_metrics(&annotated, None, response_collector, http_queue_guard);
    if annotation_metrics && annotated.event.as_deref() == Some(ANNOTATION_LLM_METRICS) {
        annotated.event = None;
        annotated.comment = None;
    }
    if annotated.event.as_deref() == Some(ANNOTATION_AUDIT_USAGE) {
        annotated.event = None;
        annotated.comment = None;
        annotated.data = None;
    }
...
/// Process chat streaming responses with the typed metrics fast path before SSE conversion.
pub fn process_chat_response_using_event_converter_and_observe_metrics(
    ...
    let observed_metrics =
        observe_response_metrics(&annotated, typed, response_collector, http_queue_guard);
    let annotation_metrics = typed.is_none() && observed_metrics;
    if annotation_metrics && annotated.event.as_deref() == Some(ANNOTATION_LLM_METRICS) {
        annotated.event = None;
        annotated.comment = None;
    }
...
async fn handler_metrics(State(state): State<Arc<MetricsHandlerState>>) -> impl IntoResponse {
    let mut metrics = match encode_registry(&state.registry) {
        Ok(metrics) => metrics,
        Err(_) => {
            return (
...
mod tests {
    ...
    async fn metrics_annotations_stripped_from_client_sse() {
        ...
        use dynamo_http_server::metrics::{ANNOTATION_AUDIT_USAGE, LLMMetricAnnotation};
    ...
    fn test_chat_typed_metrics_fast_path_observes_without_annotation() {
        use dynamo_http_server::metrics::{
            LLMMetricAnnotation, WORKER_TYPE_DECODE, WORKER_TYPE_PREFILL,
        };
