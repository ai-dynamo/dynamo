...
mod tests {
    ...
    #[tokio::test]
    async fn test_check_for_backend_error_typed_invalid_argument_preserves_explicit_status() {
        use crate::types::openai::chat_completions::NvCreateChatCompletionStreamResponse;
        use dynamo_runtime::error::{BackendError, DynamoError, ErrorType};
        use futures::stream;
        ...
        let error_event = Annotated::<NvCreateChatCompletionStreamResponse> {
            ...
            error: Some(
                DynamoError::builder()
                    .error_type(ErrorType::Backend(BackendError::InvalidArgument))
                    .message(r#"{"code":415}"#)
                    .cause(DynamoError::msg("transport context"))
                    .build(),
            ),
        };
        ...
        let error_response = match result {
            Err(error_response) => error_response,
            Ok(_) => panic!("explicit backend HTTP error must fail"),
        };
        assert_eq!(error_response.0, StatusCode::UNSUPPORTED_MEDIA_TYPE);
        assert_eq!(error_response.1.code, 415);
        assert_eq!(error_response.1.message, r#"{"code":415}"#);
        assert!(!error_response.1.message.contains("transport context"));
    }

    #[tokio::test]
    async fn test_check_for_backend_error_does_not_promote_invalid_argument_cause() {
        use crate::types::openai::chat_completions::NvCreateChatCompletionStreamResponse;
        use dynamo_runtime::error::{BackendError, DynamoError, ErrorType};
        use futures::stream;
        ...
        let invalid_argument = DynamoError::builder()
            .error_type(ErrorType::Backend(BackendError::InvalidArgument))
            .message("inner validation detail")
            .build();
        let error_event = Annotated::<NvCreateChatCompletionStreamResponse> {
            data: None,
            id: None,
        ...
        let error_response = match result {
            Err(error_response) => error_response,
            Ok(_) => panic!("backend error must fail"),
        };
        assert_eq!(error_response.0, StatusCode::INTERNAL_SERVER_ERROR);
        assert_eq!(error_response.1.code, 500);
        assert_eq!(error_response.1.message, "Internal server error");
        assert!(!error_response.1.message.contains("inner validation detail"));
    }
