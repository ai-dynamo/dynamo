...
mod tests {
    ...
    #[test]
    fn test_context_metadata_preserves_canonical_priority_over_configured_header() {
        let mut headers = HeaderMap::new();
        headers.insert("x-session-id", "configured".parse().unwrap());
        headers.insert("x-dynamo-session-id", "canonical".parse().unwrap());
        ...
        assert_eq!(affinity.as_str(), "canonical");
    }
