...
async fn perform_allocation_and_build_handler(
    ...
    // host
    let primary_host_blocks = if leader_meta.num_host_blocks > 0 {
    ...
    // disk
    let primary_disk_blocks = if leader_meta.num_disk_blocks > 0 {
        ...
        additional_device_layouts
    {
        ...
        let add_host_blocks = if leader_meta.num_host_blocks > 0 {
            let host_allocator = Arc::new(PinnedAllocator::new(device_id)?);
        ...
        let add_disk_blocks = if leader_meta.num_disk_blocks > 0 {
            let disk_allocator = Arc::new(DiskAllocator);
