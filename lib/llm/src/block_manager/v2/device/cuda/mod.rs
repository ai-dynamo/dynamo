...
use cudarc::driver::sys::{CU_MEMHOSTALLOC_PORTABLE, CUresult};
use std::sync::Arc;
...
impl DeviceContextOps for CudaContext {
    ...
    fn allocate_device(&self, size: usize) -> Result<u64> {
        ...
        let ptr =
            unsafe { cuda_result::malloc_sync(size).context("Failed to allocate device memory")? };
        Ok(ptr)
    }
    ...
    fn free_device(&self, ptr: u64) -> Result<()> {
        self.context.bind_to_thread()?;
        unsafe {
            cuda_result::free_sync(ptr).context("Failed to free device memory")?;
        }
        Ok(())
    }
...
impl DeviceStreamOps for CudaStreamWrapper {
    fn copy_h2d(&self, dst_device_ptr: u64, src_host_data: &[u8]) -> Result<()> {
        unsafe {
            cuda_result::memcpy_htod_async(dst_device_ptr, src_host_data, self.stream.cu_stream())
    ...
    fn copy_d2h(&self, dst_host_data: &mut [u8], src_device_ptr: u64) -> Result<()> {
        unsafe {
            cuda_result::memcpy_dtoh_async(dst_host_data, src_device_ptr, self.stream.cu_stream())
...
pub fn is_available() -> bool {
    ...
    match cuda_result::init() {
        Ok(()) => match cuda_result::device::get_count() {
            Ok(count) => count > 0,
            Err(_) => false,
        },
        Err(_) => false,
