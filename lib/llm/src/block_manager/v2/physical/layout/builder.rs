...
impl PhysicalLayoutBuilder<HasConfig, HasLayout, NoMemory> {
    ...
    /// Allocate Level Zero host-pinned memory (DMA-accessible by XPU device).
    ///
    /// This is the XPU equivalent of `allocate_pinned()` (CUDA).
    /// Uses `zeMemAllocHost` instead of `cuMemHostAlloc`.
