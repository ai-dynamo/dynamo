        ...
        let device_to_host_task = OffloadManager::offload_worker(
            ...
            Arc::new(TransferBatcher::new(
                LocalTransferManager::new(
                    ...
                    config.kvbm_metrics.as_ref().map(|m| m.transfer_d2h.with_label_values(&["local"])),
                    config.kvbm_metrics.as_ref().map(|m| m.queue_d2h.clone()),
                )?,
        ...
        let host_to_disk_task = OffloadManager::offload_worker(
            ...
            Arc::new(TransferBatcher::new(
                LocalTransferManager::new(
                    ...
                    &config.async_rt_handle,
                    config.cancellation_token.clone(),
                    config.kvbm_metrics.as_ref().map(|m| m.transfer_h2d.with_label_values(&["local"])),
                    config.kvbm_metrics.as_ref().map(|m| m.queue_h2d.clone()),
                )?,
                MAX_TRANSFER_BATCH_SIZE,
                &config.async_rt_handle,
                config.cancellation_token.clone(),
        ...
        let host_to_device_task = OffloadManager::onboard_worker(
            ...
            Arc::new(TransferBatcher::new(
                LocalTransferManager::new(
                    ...
                    &config.async_rt_handle,
                    config.cancellation_token.clone(),
                    config.kvbm_metrics.as_ref().map(|m| m.transfer_h2d_onboard.with_label_values(&["local"])),
                    config.kvbm_metrics.as_ref().map(|m| m.queue_h2d_onboard.clone()),
                )?,
                MAX_TRANSFER_BATCH_SIZE,
                &config.async_rt_handle,
                config.cancellation_token.clone(),
        ...
        let disk_to_device_task = OffloadManager::onboard_worker(
            ...
            Arc::new(TransferBatcher::new(
                LocalTransferManager::new(
                    ...
                    &config.async_rt_handle,
                    config.cancellation_token.clone(),
                    config.kvbm_metrics.as_ref().map(|m| m.transfer_d2d_onboard.with_label_values(&["local"])),
                    config.kvbm_metrics.as_ref().map(|m| m.queue_d2d_onboard.clone()),
                )?,
                MAX_TRANSFER_BATCH_SIZE,
                &config.async_rt_handle,
                config.cancellation_token.clone(),
            ...
            let device_to_disk_task = OffloadManager::offload_worker(
                ...
                Arc::new(TransferBatcher::new(
                    LocalTransferManager::new(
                        ...
                        &config.async_rt_handle,
                        config.cancellation_token.clone(),
                        config.kvbm_metrics.as_ref().map(|m| m.transfer_d2d.with_label_values(&["local"])),
                        config.kvbm_metrics.as_ref().map(|m| m.queue_d2d.clone()),
                    )?,
                    MAX_TRANSFER_BATCH_SIZE,
                    &config.async_rt_handle,
                    config.cancellation_token.clone(),
