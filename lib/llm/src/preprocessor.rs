...
use dynamo_prompt::OAIPromptFormatter;
...
use dynamo_prompt::{OAIChatLikeRequest, PromptFormatter, PromptInput, TextInput, TokenInput};
...
impl OpenAIPreprocessor {
    ...
    fn is_reasoning_disabled_by_request(
        ...
        match reasoning_parser {
            ...
            | Some("deepseekv4") => {
                if let Some(enabled) = dynamo_prompt::thinking_bool_from_args(chat_template_args) {
                    return !enabled;
                }
                if let Some(args) = chat_template_args
