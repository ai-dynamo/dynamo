...
impl OpenAIPreprocessor {
    ...
    >(
        ...
        // Check if the chat template injected this parser's reasoning start
        // token at the end of the prompt. If so, the model's completion starts
    ...
    pub fn gather_mm_exact_routing_info(
        ...
        // Effective prompt length before block-padding: the real expanded
        // token count the backend sees (image placeholders expanded to
        // per-image counts). Captured here so consumers (e.g. the
        // omitted-max_tokens cap) aren't inflated by the routing padding added
        // below.
