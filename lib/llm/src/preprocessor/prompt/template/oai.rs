...
fn filter_tools(
    ...
    let Some(ref tc) = tool_choice else {
        return tools;
    };
    ...
    match tc.as_str() {
        ...
        _ => {
            // tool_choice is generated by OAIChatLikeRequest::tool_choice()
            // an illegal tool choice will also be eliminated by frontend
...
fn filter_tools_by_name(origin_tools: Value, name: &str) -> Option<Value> {
    ...
    let tools = serde_json::to_value(origin_tools).ok()?;
...
{%- if add_generation_prompt %}assistant:{%- endif %}"#;
    ...
    #[test]
    fn test_filter_tools_with_tool_choice_named() {
        let json_str = r#"{
            "model": "gpt-4o",
            "messages": [],
            "tools": [
        ...
        let request: NvCreateChatCompletionRequest = serde_json::from_str(json_str).unwrap();
        let result =
            serde_json::to_value(filter_tools(request.tools(), request.tool_choice(), true))
                .unwrap();
        let tools = serde_json::to_value(request.tools()).unwrap();
        let result_string = serde_json::to_string(&result[0]).unwrap();
        let target_string = serde_json::to_string(&tools[0]).unwrap();

        assert_eq!(
            result.as_array().map(|arr| arr.len()).unwrap_or(usize::MAX),
            1
        );
        assert_eq!(result_string, target_string);
    }

    #[test]
    fn test_filter_tools_with_tool_choice_named_mismatch() {
        let json_str = r#"{
            "model": "gpt-4o",
            "messages": [],
            "tools": [
        ...
        let request: NvCreateChatCompletionRequest = serde_json::from_str(json_str).unwrap();
        let result = filter_tools(request.tools(), request.tool_choice(), true);
        assert_eq!(result, None);
    }

    #[test]
    fn test_filter_tools_with_tool_choice_none() {
        let json_str = r#"{
            "model": "gpt-4o",
            "messages": [],
            "tools": [
    ...
    #[test]
    fn test_filter_tools_with_tool_choice_auto() {
        let json_str = r#"{
            "model": "gpt-4o",
            "messages": [],
            "tools": [
        ...
        let target_string = serde_json::to_string(&tools).unwrap();

        assert_eq!(result_string, target_string);
    }

    #[test]
    fn test_filter_tools_with_tool_choice_required() {
        let json_str = r#"{
            "model": "gpt-4o",
            "messages": [],
            "tools": [
    ...
    #[test]
    fn test_filter_tools_with_no_tool_choice() {
        let json_str = r#"{
            "model": "gpt-4o",
            "messages": [],
            "tools": [
