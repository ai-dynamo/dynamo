...
impl PromptFormatter {
    ...
    pub fn from_mdc_with_chat_template(
        ...
        //
        // Skip the native-formatter shortcut when the caller supplied an
        // explicit chat_template_override — they want the override applied
        // through the Jinja path, not bypassed by the native Rust formatter.
        ...
        if chat_template_override.is_none()
            && is_deepseek_v4(&model_type_lower, &display_name_lower)
        {
            tracing::info!(
        ...
        if chat_template_override.is_none()
            && is_deepseek_v3_2_non_exp(&model_type_lower, &display_name_lower)
        {
