...
>
> **Video backends**: the default video backend is `video_rs`. The `ffmpeg` backend uses Dynamo's sampled-frame decode path and has the same FFmpeg runtime requirements. The optional `opencv` backend requires building `dynamo-llm` or `dynamo-py3` with `media-opencv-video` and linking OpenCV. Dynamo container builds install OpenCV from source in `wheel_builder` and copy the runtime libraries into framework images. If `backend: "opencv"` is selected in a build without that feature, Dynamo logs a warning once and falls back to `video_rs`.
...
### Backend
- **backend**: `"video_rs"` (default), `"ffmpeg"`, or `"opencv"`. The `ffmpeg` backend converts only sampled frames to RGB. The `opencv` backend is video-only and requires the `media-opencv-video` build feature plus OpenCV shared libraries at runtime.
