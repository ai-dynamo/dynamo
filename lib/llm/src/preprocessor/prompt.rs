...
pub fn prompt_formatter_from_mdc(mdc: &ModelDeploymentCard) -> Result<PromptFormatter> {
    prompt_formatter_from_mdc_with_chat_template(mdc, None)
}

/// Build a [`PromptFormatter`] from a [`ModelDeploymentCard`], optionally
/// overriding the chat template with a caller-supplied Jinja template.
///
/// When `chat_template_override` is `Some`, the native (non-Jinja) DeepSeek
/// formatter shortcut is skipped and any separately-shipped chat-template file
/// is ignored — the caller's template is applied through the Jinja path.
pub fn prompt_formatter_from_mdc_with_chat_template(
    ...
    // Skip the native-formatter shortcut when the caller supplied an explicit
    // chat_template_override — they want it applied through the Jinja path, not
    // bypassed by the native Rust formatter.
        ...
        .ok_or(anyhow::anyhow!("MDC does not contain a prompt formatter"))?
    {
        PromptFormatterArtifact::HfTokenizerConfigJson(checked_file) => {
            ...
            // separate file. Skip this when the caller supplied an override.
            if chat_template_override.is_none() {
