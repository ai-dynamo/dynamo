...
pub struct RouterRequestMetrics {
    ...
    pub kv_transfer_latency_seconds: prometheus::Histogram,
}
    ...
    pub fn from_component(component: &Component) -> Arc<Self> {
        ROUTER_REQUEST_METRICS
            .get_or_init(|| {
                ...
                let kv_transfer_latency_seconds = metrics
                    .create_histogram(
                        &router_metric(frontend_service::KV_TRANSFER_LATENCY_SECONDS),
                        "Time between prefill completion and decode dispatch in disaggregated serving",
                        extra_labels,
                        Some(generate_log_buckets(0.001, 10.0, 15)),
                    )
                    .expect("failed to create router_kv_transfer_latency_seconds");
                Arc::new(Self {
                    requests_total,
...
";
    ...
    #[test]
    fn test_kv_transfer_latency_metric_pef() {
        ...
        let name = format!(
            ...
            frontend_service::KV_TRANSFER_LATENCY_SECONDS,
        );
        ...
        let output = gather_pef(&registry);
        assert!(
            output.contains("# HELP router_kv_transfer_latency_seconds"),
            "PEF missing HELP line. Got:\n{output}"
        );
        assert!(
            output.contains("# TYPE router_kv_transfer_latency_seconds histogram"),
            "PEF missing TYPE line. Got:\n{output}"
        );
        assert!(
            output.contains("router_kv_transfer_latency_seconds_count 1"),
            "PEF missing observation count. Got:\n{output}"
        );
        assert!(
            output.contains("router_kv_transfer_latency_seconds_sum 0.005"),
