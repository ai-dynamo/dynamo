...
use crate::{
    http::service::metrics::generate_log_buckets,
    kv_router::minimal_cache_loss::cache_loss_funnel_enabled,
};
...
pub struct RouterRequestMetrics {
    ...
    cache_loss: Option<CacheLossMetrics>,
}

struct CacheLossMetrics {
    observation_input_tokens_total: prometheus::IntCounter,
    funnel_tokens_total: IntCounterVec,
    observations_total: IntCounterVec,
    history_block_records: IntGauge,
    history_unique_hashes: IntGauge,
    history_represented_tokens: IntGauge,
    history_estimated_bytes: IntGauge,
    history_capacity_bytes: IntGauge,
    history_capacity_blocks: IntGauge,
}
...
impl RouterRequestMetrics {
    ...
    pub fn from_component(component: &Component) -> Arc<Self> {
        ROUTER_REQUEST_METRICS
            .get_or_init(|| {
                ...
                let cache_loss = cache_loss_funnel_enabled().then(|| {
                ...
                for result in ["complete", "incomplete"] {
                    observations_total.with_label_values(&[result]);
                }
                let history_block_records = metrics
                    .create_intgauge(
                ...
                let history_unique_hashes = metrics
                    .create_intgauge(
                ...
                let history_represented_tokens = metrics
                    .create_intgauge(
                ...
                let history_estimated_bytes = metrics
                    .create_intgauge(
                ...
                let history_capacity_bytes = metrics
                    .create_intgauge(
                ...
                let history_capacity_blocks = metrics
                    .create_intgauge(
                ...
                CacheLossMetrics {
                    ...
                    funnel_tokens_total,
                    observations_total,
                    history_block_records,
                    history_unique_hashes,
                    history_represented_tokens,
                    history_estimated_bytes,
                    history_capacity_bytes,
                    history_capacity_blocks,
                }
                });
                Arc::new(Self {
                    requests_total,
    ...
    pub fn observe_cache_loss_input(&self, prompt_tokens: u64) {
        let Some(metrics) = &self.cache_loss else {
            return;
        };
    ...
    pub fn cache_loss_funnel_enabled(&self) -> bool {
        self.cache_loss.is_some()
    }
