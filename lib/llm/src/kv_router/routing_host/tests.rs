...
use crate::{
    ...
    kv_router::routing_host::kv_selection::WorkerSelection,
    ...
    protocols::common::timing::WORKER_TYPE_DECODE,
};
...
async fn builtin_host_constructs_only_declared_capabilities() {
    ...
    let reservation = load_state
        .select_and_reserve(&host.push_router, "request-1", &request, None)
        .unwrap();
...
async fn builtin_affinity_uses_shared_dispatch_and_cleanup_lifecycle() {
    ...
    .enumerate()
    {
        ...
        let expected_target = AffinityTarget::new(worker_id, None);
