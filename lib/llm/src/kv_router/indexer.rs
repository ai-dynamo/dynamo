...
use bytes::Bytes;
use dynamo_runtime::{component::Component, metrics::MetricsRegistry};
...
impl KvIndexerMetrics {
    pub fn new(kv_cache_events_applied: IntCounterVec) -> Self {
        Self {
    ...
    pub fn from_component(
        ...
        let kv_cache_events_applied = component.create_intcountervec(
            "kv_cache_events_applied",
    ...
    pub fn get_event_type(event_data: &KvCacheEventData) -> &'static str {
        match event_data {
            KvCacheEventData::Stored(_) => "stored",
            KvCacheEventData::Removed(_) => "removed",
            KvCacheEventData::Cleared => "cleared",
        }
    }
        ...
        match result {
            Ok(_) => {
                self.kv_cache_events_applied
                    .with_label_values(&[worker_id_label.as_str(), event_type, "ok"])
                    .inc_by(1);
            }
            Err(e) => {
                let error_label = match e {
                    KvCacheEventError::ParentBlockNotFound => "parent_block_not_found",
                    KvCacheEventError::BlockNotFound => "block_not_found",
                };
                self.kv_cache_events_applied
                    .with_label_values(&[worker_id_label.as_str(), event_type, error_label])
                    .inc_by(1);
            }
        }
    }
...
impl KvIndexer {
                    ...
                    loop {
                        tokio::select! {
                            ...
                            Some(event) = event_rx.recv() => {
                                ...
                                metrics.increment_event_applied(worker_id, event_type, result);
                            }
                        }
                    }
                })
    ...
    pub fn new(token: CancellationToken, kv_block_size: u32, metrics: KvIndexerMetrics) -> Self {
...
mod tests {
    ...
    fn test_increment_event_applied() {
        ...
        assert_eq!(
            metrics
                .kv_cache_events_applied
                .get_metric_with_label_values(&["0", "stored", "ok"])
                .unwrap()
                .get(),
            1
        );

        metrics.increment_event_applied(0, "stored", Err(KvCacheEventError::ParentBlockNotFound));
        assert_eq!(
            metrics
                .kv_cache_events_applied
                .get_metric_with_label_values(&["0", "stored", "parent_block_not_found"])
                .unwrap()
                .get(),
            1
        );

        metrics.increment_event_applied(0, "removed", Err(KvCacheEventError::BlockNotFound));
        assert_eq!(
            metrics
                .kv_cache_events_applied
                .get_metric_with_label_values(&["0", "removed", "block_not_found"])
                .unwrap()
                .get(),
            1
        );
