...
/// RadixUploader handles periodic and on-demand uploads of the radix tree snapshot to NATS object store.
pub struct RadixUploader {
    /// NATS client for uploading snapshots
    nats_client: nats::Client,
    /// Sender for dump requests to get the tree snapshot
    snapshot_event_sender: mpsc::Sender<DumpRequest>,
    /// NATS bucket name for storing snapshots
    bucket_name: String,
...
impl RadixUploader {
    pub fn new(
        ...
        let sender_clone = snapshot_event_sender.clone();
        let bucket_clone = bucket_name.clone();
        let cancel_clone = cancel.clone();

        // Launch background task for periodic uploads
            ...
            loop {
                tokio::select! {
                    _ = interval_timer.tick() => {
                        if let Err(e) = Self::perform_upload(&client_clone, &sender_clone, &bucket_clone).await {
                            tracing::warn!("Failed to upload radix tree snapshot: {:?}", e);
                        } else {
                            tracing::debug!("Successfully uploaded periodic radix tree snapshot");
                        }
                    }
                    _ = cancel_clone.cancelled() => {
                        tracing::info!("RadixUploader background task shutting down");
                        break;
                    }
                }
            }
    ...
    /// Upload a snapshot of the radix tree immediately.
    ///
    /// ### Returns
    ///
    ...
    pub async fn upload_snapshot(&self) -> Result<(), KvRouterError> {
        Self::perform_upload(
            &self.nats_client,
            &self.snapshot_event_sender,
            &self.bucket_name,
        )
        .await
        .map_err(|e| {
            tracing::error!("Failed to upload radix tree snapshot: {:?}", e);
    ...
    async fn perform_upload(
        ...
        // Request the dump from the indexer
        let (resp_tx, resp_rx) = oneshot::channel();
        let dump_req = DumpRequest { resp: resp_tx };
        ...
        let url = Url::parse(&format!("nats://{}/{}/radix_tree", nats_addr, bucket_name))?;

        // Upload the serialized events to NATS
        nats_client.object_store_upload_data(&events, url).await?;

        tracing::info!(
            "Uploaded {} radix tree events to NATS bucket {} at path radix_tree",
            events.len(),
            bucket_name
        );
        Ok(())
    ...
    /// Shutdown the uploader and wait for background task to complete.
...
mod tests {
    ...
    #[ignore]
    #[tokio::test]
    async fn test_kv_indexer_radix_uploader_round_trip() {
        setup();

        // Configuration
        let kv_block_size = 32;

        // Set up NATS client for testing
        let nats_url =
        ...
        let nats_client = client_options
            .connect()
            .await
            .expect("Failed to connect to NATS");

        // Build indexer1 with events
        let token1 = CancellationToken::new();
        let mut indexer1 = KvIndexer::new(token1.clone(), kv_block_size);

        let worker_0 = 0;
        let worker_1 = 1;
        let worker_2 = 2;

        // Apply events to indexer1
        indexer1
            .apply_event(create_store_event(worker_0, 0, vec![1, 2, 3], None))
            .await;
        ...
        // Get dump channel for RadixUploader
        let dump_tx = indexer1.dump_tx.clone();

        // Create RadixUploader for indexer1
        let bucket_name = format!("test-radix-snapshot-{}", uuid::Uuid::new_v4());
        let upload_interval = Duration::from_secs(3600); // Set long interval since we'll trigger manually
        ...
        // Upload snapshot to NATS object store
        uploader
            .upload_snapshot()
            .await
            .expect("Failed to upload snapshot");

        // Download the snapshot from NATS
        let download_url = url::Url::parse(&format!(
            "nats://{}/{}/radix_tree",
            nats_client.addr(),
            bucket_name
        ))
        .expect("Failed to parse NATS URL");

        let downloaded_events: Vec<RouterEvent> = nats_client
            .object_store_download_data(download_url.clone())
            .await
            .expect("Failed to download snapshot");

        println!("Downloaded {} events from NATS", downloaded_events.len());

        // Create indexer2 and apply all downloaded events
        let token2 = CancellationToken::new();
        let mut indexer2 = KvIndexer::new(token2.clone(), kv_block_size);

        for event in &downloaded_events {
            indexer2.apply_event(event.clone()).await;
        }

        // Allow some time for events to be processed
        tokio::time::sleep(Duration::from_millis(100)).await;

        // Dump both indexers for comparison
        let dump1 = indexer1.dump_events().await.unwrap();
        let dump2 = indexer2.dump_events().await.unwrap();
        ...
        for test_seq in [
            ...
            let scores1 = indexer1.find_matches(test_seq.clone()).await.unwrap();
            let scores2 = indexer2.find_matches(test_seq.clone()).await.unwrap();
        ...
        // Note: NATS object store doesn't provide individual object deletion,
        // so we leave the test object in place. It will be cleaned up when
        // the bucket is eventually deleted or expires.
