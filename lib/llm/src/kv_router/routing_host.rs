    ...
    Sel: WorkerSelector<ModelRuntimeConfig> + Send + 'static,
{
    ...
    fn validate_explicit_worker(
        ...
        let target = match phase {
            // A continuation dispatches to the worker its prefill was routed to,
            // so it validates the prefill field, as kv_selection resolves it.
            RequestPhase::Prefill | RequestPhase::Continuation => routing
                .prefill_worker_id
