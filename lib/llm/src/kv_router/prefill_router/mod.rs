...
use crate::{
    ...
    local_model::runtime_config::PrefillCancelUntil,
    protocols::common::{
    ...
    > for PrefillRouter
{
    async fn generate(
        ...
        let mut cancel_link: Option<(PrefillCancelUntil, Arc<PrefillCancelLink>)> = None;
        let prefill_result: Result<(PrefillOutcome, Option<RoutingConstraints>)> = async {
            ...
            if let Some(link) = &link {
                cancel_link = Some((cancel_policy, link.clone()));
            }
            ...
            let outcome = if let Some(bootstrap_info) = prepared.bootstrap_info {
                ...
                prefill_completion = Some(self.spawn_prefill_task(
                    prefill_stream,
                    tracker,
        ...
        // rather than moved. An Anytime link is dropped here instead; if a
        // drain still holds it, cancellation keeps reaching prefill.
        if let Some((cancel_policy, link)) = cancel_link.take()
...
mod tests {
    ...
    async fn precomputed_bootstrap_observes_initial_and_late_prefill_failures() {
        for after_handoff in [false, true] {
            ...
            let task = router.spawn_prefill_task(prefill, Some(tracker.clone()), permit, None);
