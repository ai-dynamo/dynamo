    ...
    async fn generate(
            ...
            .await
        {
            PrefillResolveDecision::Resolved {
                ...
                Ok(PrefillOutcome::Bootstrap(bootstrap_info, worker_id))
            }
            PrefillResolveDecision::NoBootstrapEndpoint {
                ...
                let (result, _) = Self::execute_prefill(
                ...
                .await?;
                Ok(PrefillOutcome::Completed(result, Some(resolved_wid)))
            }
            PrefillResolveDecision::Unavailable | PrefillResolveDecision::NotActivated => {
                ...
                let (result, prefill_worker_info) = Self::execute_prefill(
                    self.prefill_router.get().cloned(),
                    prefill_context,
                    preselected_worker,
                    None,
                ...
                .await?;
                let prefill_worker_id = prefill_worker_info
                    .map(|(wid, _)| wid)
                    .or(preselected_worker);
                Ok(PrefillOutcome::Completed(result, prefill_worker_id))
            }
        };
        ...
        match prefill_result {
            Ok(outcome) => {
                ...
                let selected_prefill_worker_id = match outcome {
                    PrefillOutcome::Bootstrap(info, prefill_wid) => {
                        decode_req.bootstrap_info = Some(info);
                        decode_req.routing_mut().prefill_worker_id = Some(prefill_wid);
                        Some(prefill_wid)
                    }
                    PrefillOutcome::Completed(result, prefill_wid) => {
                        decode_req.prefill_result = Some(result);
                        if let Some(wid) = prefill_wid {
