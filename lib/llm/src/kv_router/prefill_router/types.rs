...
//! Types specific to the live `PrefillRouter` operator.
//!
//! The conditional-prefill policy machinery (trait, decision input, concrete
//! policy impls, factory) lives in `dynamo_kv_router::conditional_prefill`
//! since it's pure decision logic with no runtime dependencies and is shared
//! with offline replay (`lib/mocker/src/replay/offline/`). The remaining
//! types here (`PrefillError`, `PrefillOutcome`, `PrefillResolveDecision`,
//! `build_decode_router_override`) are operator-level concepts coupled to
//! the live runtime.
...
pub(super) use dynamo_kv_router::conditional_prefill::{
