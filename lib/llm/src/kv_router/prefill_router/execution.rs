...
impl PrefillRouter {
    ...
    pub(super) async fn execute_prefill(
        ...
        let mut prefill_response = router
            ...
            .await
            .map_err(|e| {
                // DIS-2105: sibling fix to addressed_router.rs:259-275. When a
                // prefill worker rejects on the request-plane ACK (queue full),
                // addressed_router synthesises a `DynamoError(ResourceExhausted)`
                // wrapped in `anyhow::Error`. The decode side propagates this
                // chain straight to the HTTP layer, where `request_was_rejected`
                // downcasts and maps it to HTTP 503.
                //
