    ...
    > for EncoderRouter
{
    async fn generate(
        ...
        let encode_result = if let Some(router) = self.router.get().cloned() {
            let encode_future = async move {
                let response = router.generate(encode_context).await?;
                Self::consume_encode_stream(response).await
            };
            await_encode_with_source_guards(encode_future, source_guards).await
        } else {
            Err(anyhow::anyhow!(
                "Encoder router is active but not initialized"
            ))
        };
