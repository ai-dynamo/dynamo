...
impl Model {
    ...
    pub(crate) fn get_generate_engine_for_capability_with_routing(
        ...
        // The vLLM/SGLang raw-generate paths record no per-response metrics, so
        // the selected namespace has no consumer here. Routing metadata still
        // comes from the same WorkerSet as the engine, which is the invariant
        // this accessor exists to preserve.
        .map(Selected::into_value)
        .ok_or_else(|| self.engine_error(self.has_generate_engine_for_capability(capability)))
    }
