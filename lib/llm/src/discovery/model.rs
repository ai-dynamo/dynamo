...
impl Model {
    ...
    pub fn worker_sets_in_namespace(&self, namespace: &str) -> Vec<Arc<WorkerSet>> {
        self.worker_sets
            .iter()
            .filter_map(|entry| {
                let worker_set = entry.value();
                (worker_set.namespace() == namespace).then(|| worker_set.clone())
            })
            .collect()
    }
    ...
    {
        // Collect eligible sets with worker counts and priorities, skipping sets with no
        // workers or sets whose prefill router has died under enforce_disagg.
