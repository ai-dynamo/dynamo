...
impl ModelWatcher {
        ...
        while let Some(result) = discovery_stream.next().await {
            ...
            match event {
                DiscoveryEvent::Added(instance) => {
                    ...
                    let ws_key = worker_set_key(&mcid.namespace, card.model_type, priority);
                    ...
                    let handle = tokio::spawn(async move {
                        match watcher.handle_put(&mcid, &mut card, priority).await {
                            Ok(()) => {
                                tracing::info!(
                                    model_name = card.name(),
                                    namespace = mcid.namespace,
    ...
    async fn handle_put(
        ...
        self.do_worker_set_registration(mcid, card, priority).await
    }
    ...
    async fn recover_concurrent_registration(
            ...
            .is_none_or(|m| !m.has_worker_set(ws_key))
        {
            ...
            {
                // Save the model card so handle_delete can find it for cleanup.
                self.manager.save_model_card_with_priority(
                    &mcid.to_path(),
                    card.clone(),
                    priority,
                )?;
