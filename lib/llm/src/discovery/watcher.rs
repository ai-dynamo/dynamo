...
mod tests {
    ...
    fn test_worker_set_key_encode_gets_separate_key() {
        // Encode workers must key to a namespace-scoped key distinct from
        // aggregated workers. With the `{ns}:{mt}:{wt}` format, an encode
        // worker (ModelType::empty()) lands at `{ns}::encode` — distinct from
        // an aggregated worker's `{ns}:chat|completions:aggregated`.
        ...
        assert_eq!(prefill_key, "myns:prefill:prefill");
        assert_eq!(agg_key, "myns:chat|completions:aggregated");
