...
use dynamo_llm::block_manager::distributed::{
    ...
    G4PutPayloadRequest, G4QueryHit, G4QueryRequest, G4StorageAgent, G4TransferBlock,
    G4TransferExecutor,
};
...
async fn main() -> Result<()> {
    let args = Args::parse()?;
    let (agent, payloads) = build_backend(&args).await?;
...
impl G4TransferExecutor for NoopTransferExecutor {
    async fn execute_transfer(
        &self,
        _request: dynamo_llm::block_manager::distributed::BlockTransferRequest,
    ) -> Result<()> {
        Ok(())
    }
}
