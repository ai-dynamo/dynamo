...
pub(crate) struct SglangRequestTrace(Option<Box<ActiveSglangRequestTrace>>);
...
impl SglangRequestTrace {
    pub(crate) fn runtime_config_needed() -> bool {
    ...
    pub(crate) fn prepare(
        ...
        if !Self::runtime_config_needed() {
            return (Self::disabled(), context);
        }
        ...
        let Some(request_end) = super::integration::build_new_request_end_trace_state(
            ...
            return (Self::disabled(), context.map(|_| request));
        };
        ...
        (
            Self(Some(Box::new(ActiveSglangRequestTrace {
                request_end,
                tool_trace,
            }))),
...
mod tests {
    ...
    fn active_trace(
        ...
        SglangRequestTrace(Some(Box::new(ActiveSglangRequestTrace {
            request_end: state,
            tool_trace,
        })))
    }
    ...
    async fn disabled_trace_returns_original_engine_stream() {
        ...
        let wrapped = SglangRequestTrace::disabled().wrap(stream, "unused".to_string());
        let returned =
