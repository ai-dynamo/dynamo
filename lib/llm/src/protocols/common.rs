...
impl FinishReason {
    ...
    pub(crate) fn to_openai_chat_finish_reason(
        &self,
        ...
        match self {
            ...
            Self::Error(message) => Err(anyhow::anyhow!(message.clone())),
        }
    }
}
...
mod tests {
    ...
    fn test_finish_reason_uses_chat_completion_normalization() {
        ...
        for (backend, expected) in [
            ...
            assert_eq!(backend.to_openai_chat_finish_reason().unwrap(), expected);
        }
        assert!(
            FinishReason::Error("boom".to_string())
                .to_openai_chat_finish_reason()
                .is_err()
        );
    }
