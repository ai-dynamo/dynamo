...
pub fn session_affinity_from_headers(
    headers: &HeaderMap,
    configured_header_name: &HeaderName,
) -> Option<SessionAffinityId> {
    session_affinity_header_value(headers, configured_header_name).map(SessionAffinityId::new)
}
...
mod tests {
    ...
    #[test]
    fn session_affinity_requires_canonical_or_configured_header() {
        let mut headers = HeaderMap::new();
    ...
    #[test]
    fn agent_headers_do_not_enable_session_affinity_by_default() {
        let mut headers = HeaderMap::new();
        let header_name = HeaderName::from_static(HEADER_DYNAMO_SESSION_ID);
        headers.insert(
            HEADER_CLAUDE_CODE_SESSION_ID,
    ...
    #[test]
    fn session_affinity_canonical_header_precedes_configured_header() {
        let mut headers = HeaderMap::new();
        let header_name = HeaderName::from_static("x-session-id");
        headers.insert(HEADER_DYNAMO_SESSION_ID, "canonical".parse().unwrap());
        headers.insert(&header_name, "configured".parse().unwrap());
        headers.insert(
            HEADER_CLAUDE_CODE_SESSION_ID,
            "claude-session".parse().unwrap(),
        );
