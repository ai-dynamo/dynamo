    ...
    if let Some(choice) = choice {
            ...
            && !content_text.is_empty()
        {
            ...
            if !parsed_calls.is_empty() {
                if let Some((name, _)) = parsed_calls
                    .iter()
                    .find(|(name, _)| !params.function_is_allowed(name))
                {
                    anyhow::bail!("Backend returned function '{name}' outside allowed_tools");
                }
...
{"name": "func_b", "arguments": {"y": 2}}
    ...
    fn test_text_tool_calls_parsed_when_tools_are_enabled() {
        ...
        let error =
            chat_completion_to_response(make_chat_resp_with_text(text), &params, None).unwrap_err();
        assert!(error.to_string().contains("outside allowed_tools"));
    }
