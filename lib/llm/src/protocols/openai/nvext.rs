...
pub struct NvExtResponse {
    ...
    /// Output token IDs generated by the engine (RL inference path).
    /// Populated when client requests `extra_fields: ["completion_token_ids"]`
    /// or auto-enabled under `DYN_ENABLE_RL=true` for the chat-completions path.
    /// For RL: `len(completion_token_ids) == len(logprobs.content)` is a hard invariant.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl NvExtResponseFieldSelection {
        ...
        if let Some(fields) = ext.extra_fields.as_ref() {
            for field in fields {
                match field.as_str() {
                    ...
                    "token_ids" => selection.token_ids = true,
                    _ => {}
                }
            }
        }
    ...
    /// - `completion_token_ids` requires the selection flag **and** a `"completion_token_ids"` key on
    ///   `disaggregated_params` that deserializes into `Vec<u32>`; malformed values silently fall back to `None`.
    ...
    pub fn build_response_nvext(
        ...
        let completion_token_ids = if self.completion_token_ids {
            disaggregated_params
                .and_then(|params| params.get("completion_token_ids"))
                .and_then(|v| serde_json::from_value::<Vec<u32>>(v.clone()).ok())
        } else {
            None
        };
