...
mod tests {
    ...
    use dynamo_runtime::discovery::{
        DiscoverySpec, DiscoveryStream, MockDiscovery, ModelTaintsUpdate, SharedMockRegistry,
    };

    /// A backend that removes one instance when a watch opens. The state that a list before the
    /// watch showed is stale by the time the stream starts.
    ...
    /// The periodic reconcile ticks at once after the streams open, so a list before the watch
    /// cannot leave a stale entry behind.
    #[tokio::test]
    async fn the_watch_lists_again_after_it_opens() {
        ...
        let stale = inner
            .register(DiscoverySpec::Model {
                ...
                card_json: serde_json::to_value(card("llama", "meta/llama", 64)).unwrap(),
                model_suffix: None,
            })
            .await
            .unwrap();
        ...
        let watch = DcMembershipWatch::start(discovery, config, CancellationToken::new())
            .await
            .unwrap();
        let mut view = watch.subscribe();
        assert_eq!(view.borrow_and_update().endpoints.len(), 1);

        tokio::time::timeout(Duration::from_secs(1), async {
            while !view.borrow_and_update().endpoints.is_empty() {
                view.changed().await.unwrap();
            }
        })
        .await
        .expect("the stale instance must leave the membership after the watch opens");
    }
