...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct PublicationHubHealth {
    pub(crate) ready: bool,
    pub(crate) sequence: u64,
    pub(crate) subscriber_count: usize,
    pub(crate) lagged_subscribers: u64,
...
impl PublicationHub {
    pub(super) async fn start(
        ...
        let lagged_subscribers = Arc::new(AtomicU64::new(0));
        ...
        let task = tokio::spawn(run_hub(HubTask {
            ...
            lagged_subscribers: lagged_subscribers.clone(),
            terminal_failure,
            cancel: cancel.clone(),
    ...
    pub(crate) fn health(&self) -> PublicationHubHealth {
        let state = self.inner.state.lock();
        PublicationHubHealth {
...
impl PublicationHubSubscription {
    pub(crate) const fn snapshot(&self) -> &HubSnapshot {
        &self.snapshot
    }
    ...
    loop {
        ...
        match delta {
            ...
            Ok(delta) => {
                ...
                fan_out(&mut state, frame, &task.lagged_subscribers);
            }
...
fn fan_out(state: &mut HubState, frame: Arc<PublicationFrame>, lagged: &AtomicU64) {
    state
        .subscribers
...
mod tests {
    ...
    async fn one_actor_lease_fans_out_one_contiguous_frame_instance() {
        ...
        actor
            .admit_event(SourceEpoch::new(1), stored(1, 99))
            .await
            .unwrap();
        ...
        assert_eq!(first_frame.base_sequence, first.snapshot().sequence());
        assert_eq!(first_frame.sequence, first_frame.base_sequence + 1);

        let third = hub.subscribe().unwrap();
        assert_eq!(third.snapshot().sequence(), first_frame.sequence);
    ...
    async fn slow_subscriber_is_removed_without_affecting_fast_subscriber() {
        ...
        for event_id in 1..=3 {
            actor
                .admit_event(SourceEpoch::new(1), stored(event_id, event_id + 100))
                .await
                .unwrap();
            actor.flush().await.unwrap();
    ...
    async fn oversized_delta_resnapshots_and_closes_existing_subscribers() {
        ...
        actor
            .admit_event(SourceEpoch::new(1), stored(1, 777))
            .await
            .unwrap();
        actor.flush().await.unwrap();
        assert!(old.recv().await.is_err());
        let replacement = hub.subscribe().unwrap();
        assert_eq!(replacement.snapshot().sequence(), 1);
        assert!(hub.health().ready);
    ...
    async fn internal_actor_lag_resnapshots_and_closes_existing_subscribers() {
        ...
        for event_id in 1..=70 {
            actor
                .admit_event(SourceEpoch::new(1), stored(event_id, event_id + 1_000))
                .await
                .unwrap();
            actor.flush().await.unwrap();
        }
        ...
        let replacement = hub.subscribe().unwrap();
        assert_eq!(replacement.snapshot().sequence(), 70);
        assert!(hub.health().ready);
