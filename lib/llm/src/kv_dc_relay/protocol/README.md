...
`NOT_FOUND`; a detected producer mismatch returns `FAILED_PRECONDITION` without initializing the
replacement pool's hub for the stale request. If the selected generation retires while admission
or hub initialization is waiting, cancellation can instead return `UNAVAILABLE`. Refresh the
catalog before retrying; a stale request never attaches to the replacement generation.
...
An initialized hub retains its CKF mirror and its hub-capacity permit while idle, but admission
pressure can evict a hub with no subscribers and reclaim its permit. Eviction releases publication
state without retiring the pool generation or withdrawing it from the catalog; a later subscription
can initialize a new hub for the same generation. The initialized-hub limit bounds resident mirrors
across sequential subscribe/drop requests. A subscription that
...
The CBI1 delta body's `base_epoch` equals the enclosing `FilterUpdate.base_sequence`; the CBI1
header's `epoch` equals `FilterUpdate.sequence`. These are the same publication sequence values,
not separate generation counters. Snapshot chunks and heartbeats also carry the current publication
sequence in the envelope; heartbeats have no CBI1 payload.
...
## Build and Listener Configuration

The standard build includes the generated client, messages, wire helpers, and plaintext gRPC
server. Set `--bind` to start the listener; without it, the Relay remains local-only. Deployments
terminate transport security outside Relay and restrict direct access to its listener.
The independent `ckf-diagnostics` Cargo feature enables the optional diagnostic API.
