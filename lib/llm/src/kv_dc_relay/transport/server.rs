...
mod tests {
    ...
    fn tls_test_config(temp: &TempDir, pki: &TestPki) -> KvDcRelayTransportConfig {
    ...
    #[tokio::test]
    async fn mtls_rejects_missing_and_wrong_client_certificates() {
        let pki = test_pki();
        let temp = TempDir::new().unwrap();
        let config = tls_test_config(&temp, &pki);
        let (tls, _, _) = build_tls_config(&config).unwrap();
        ...
        reporter
            .set_service_status("relay-test", tonic_health::ServingStatus::Serving)
            .await;
        ...
        assert!(health_channel(address, &pki.ca_pem, None).await.is_err());
        assert!(
            health_channel(
                address,
                &pki.ca_pem,
