...
impl LocalModelBuilder {
    ...
    pub async fn build(&mut self) -> anyhow::Result<LocalModel> {
        ...
        if let Some(source_path) = self.source_path.take() {
            ...
            .model_name
            .as_deref()
            .is_some_and(|name| name != card.source_path())
        {
            // Preserve the local source before renaming; leave unrenamed cards' checksums unchanged.
...
mod metadata_publish_tests {
    ...
    #[tokio::test]
    async fn build_local_model_unchanged_name_preserves_source_representation() {
        let canonical = fs::canonicalize(sample_model_dir()).unwrap();
        let name = canonical.display().to_string();

        for model_name in [None, Some(name.clone())] {
            ...
            assert_eq!(model.card().source_path(), name);
            // An explicit source would change mdcsum relative to existing workers.
            assert!(model.card().source_path.is_none());
        }
    }

    #[tokio::test]
    async fn build_local_model_explicit_source_path_wins() {
        for source in [PathBuf::from("Qwen/Qwen3-0.6B"), sample_model_dir()] {
            let model = LocalModelBuilder::default()
                .model_path(sample_model_dir())
                .source_path(source.clone())
                .model_name(Some("served-name".to_string()))
                .build()
                .await
                .unwrap();

            assert_eq!(model.display_name(), "served-name");
            assert_eq!(model.card().source_path(), source.display().to_string());
        }
    }
}
