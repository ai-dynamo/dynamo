...
- **Kernels are optional and FFI-gated.** `kvbm-kernels` produces
  ...
  kernels are built only when `KVBM_ENABLE_XPU_KERNELS=1` is set at build
  time.
...
| `kvbm-kernels` | `xpu_permute_kernels` | Links `libkvbm_kernels_xpu.so` built by `icpx -fsycl` when `KVBM_ENABLE_XPU_KERNELS=1` at build. |
| `kvbm-physical` | `cuda` (default) | Compiles `device/cuda`. Pulls in `kvbm-kernels`. |
| `kvbm-physical` | `xpu-sycl` | Compiles `device/sycl`. Pulls in `kvbm-kernels/xpu_permute_kernels`, `oneapi-rs`, and `dynamo-memory/sycl`. |
