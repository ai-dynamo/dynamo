...
function hubAppData() {
  return {
    ...
    async testRegister() {
      // Smoke value — caller is expected to use the CLI for real hashes.
      const req = { sequence_hashes: [] };
      const r = await fetch(`/v1/instances/${this.selectedId}/control/test/register_test_blocks`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(req),
      });
      const body = await r.text();
      this.actionResults.test_register = `${r.status}: ${body.slice(0, 200)}`;
    },
