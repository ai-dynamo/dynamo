...
async def main():
    ...
    try:
        ...
        if not await debugger.test_connection():
            print(
                "Cannot connect to backend. Make sure it's running at http://localhost:8001"
            )
            return
        ...
        print(f"\nRunning full load test ({TOTAL_REQUESTS} requests, payload={PAYLOAD_SIZE}, concurrency={CONCURRENCY})...")
        start_time = time.time()
        await debugger.run_load_test(total_requests=TOTAL_REQUESTS, concurrent=CONCURRENCY)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--payload-size", type=int, default=10, help="Size of the payload in each request")
    parser.add_argument("--concurrency", type=int, default=48, help="Number of concurrent requests")
    parser.add_argument("--requests", type=int, default=20000, help="Total number of requests to send")
