...
from memory_monitor import create_monitor, setup_background_monitor, dump_arena_info, dump_malloc_stats
...
class MockEngine:
    ...
    def generate(self, request):
        ...
        # Track request start in debug mode
        if monitor:
            monitor.request_start(request_id, request)
        ...
        async def generator():
            ...
            try:
                for _ in range(5):
                    ...
                    print(f"Failed to contact pipeline after retries for request {chat_id}")
                ...
                async for output in stream:
                    ...
                    # On first yield, inspect generator scope (debug mode)
                    if items_yielded == 1 and monitor:
                        # Note: Can't inspect our own generator while running,
                        # but we can log what variables are in scope
                        pass

                    yield chunk
            finally:
                # Track request end in debug mode
                if monitor:
                    monitor.request_end(request_id)
...
async def worker(runtime: DistributedRuntime):
    ...
    try:
        ...
        if monitor:
            ...
            # Print arena info to diagnose glibc fragmentation
            dump_arena_info("FRONTEND-FINAL")
            # Print comprehensive report including per-request stats if debug mode
            if monitor.debug:
                monitor.comprehensive_memory_report()
        if monitor_task:
            monitor_task.cancel()
