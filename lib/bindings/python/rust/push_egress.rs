...
//! `@response_handler_capable` decorator. There is no environment variable:
...
impl ResponseSender {
    ...
    /// the other request streams; its index is returned so the response
    /// handler can stop tracking that request.
    #[staticmethod]
