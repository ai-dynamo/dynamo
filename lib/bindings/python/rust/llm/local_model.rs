...
use dynamo_kv_router::protocols::{
    KvTransferEnforcement as RsKvTransferEnforcement,
    KvTransferPreferredWeight as RsKvTransferPreferredWeight,
    RoutingConstraints as RsRoutingConstraints,
};
...
impl ModelRuntimeConfig {
    ...
    fn kv_transfer_preferred_weight(&self) -> Option<f32> {
        self.inner
            ...
            .map(RsKvTransferPreferredWeight::get)
    }
    ...
    fn set_kv_transfer_preferred_weight(
        ...
        self.inner.kv_transfer_preferred_weight = match kv_transfer_preferred_weight {
            Some(value) => {
                Some(RsKvTransferPreferredWeight::try_from(value).map_err(PyValueError::new_err)?)
            }
            None => None,
        };
        Ok(())
