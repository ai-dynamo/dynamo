...
#[pyfunction]
#[pyo3(signature = (args))]
pub fn make_engine<'p>(py: Python<'p>, args: EntrypointArgs) -> PyResult<Bound<'p, PyAny>> {
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        ...
        let inner = select_engine(args.engine_type, local_model)
            .await
            .map_err(to_pyerr)?;
...
async fn select_engine(
    ...
    let inner = match engine_type {
        ...
        EngineType::MistralRs => RsEngineConfig::StaticFull {
...
#[pyfunction]
#[pyo3(signature = (input, distributed_runtime, engine_config))]
pub fn run_input<'p>(
    py: Python<'p>,
    input: &str,
    ...
    pyo3_async_runtimes::tokio::future_into_py(py, async move {
        dynamo_llm::entrypoint::input::run_input(
            Input::Text,
            distributed_runtime.inner.runtime().clone(),
            engine_config.inner,
        )
        .await
        .map_err(to_pyerr)?;
