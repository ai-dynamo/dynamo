...
impl KvRouterConfig {
    ...
    fn new(
        ...
        router_queue_depth_by_missing_isl: Option<Vec<(usize, usize)>>,
    ) -> PyResult<Self> {
        ...
        let router_queue_depth_by_missing_isl = router_queue_depth_by_missing_isl
            .unwrap_or_else(|| DEFAULT_ROUTER_QUEUE_ISL_TOKENS_TIERS.to_vec());
        ...
        let inner = RsKvRouterConfig {
            ...
            router_queue_depth_by_missing_isl:
                dynamo_kv_router::scheduling::config::RouterQueueDepthTiers::try_from(
                    router_queue_depth_by_missing_isl,
                )
                .map_err(PyValueError::new_err)?,
