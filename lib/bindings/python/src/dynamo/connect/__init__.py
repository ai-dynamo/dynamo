...
class AbstractOperation(ABC):
    ...
    def __init__(
        ...
        self._remote_desc_list: Optional[Descriptor | list[Descriptor]] = None if remote_descriptors is None else remote_descriptors
    ...
    def _create_desc_tlist(
        ...
        if isinstance(descriptors, list):
            ...
            descriptor_tuples.append((descriptors.ptr, descriptors.size, descriptors.device.id))
...
class Connector:
    ...
    def __init__(
        ...
        """
        worker_id = worker_id if worker_id is not None else str(uuid.uuid4()).replace("-", "")
    ...
    async def initialize(self) -> None:
        ...
        logger.debug(f"dynamo.connect.{self.__class__.__name__}: Initialized {{ name: '{self._worker_id}' }} completed.")
...
class ReadOperation(ActiveOperation):
    ...
    def results(self) -> list[Descriptor]:
        ...
        return self._local_desc_list if isinstance(self._local_desc_list, list) else [self._local_desc_list]
