...
try:
    ...
    # `pub mod scheduler` is currently commented out in
    # lib/bindings/kvbm/src/v2/mod.rs (TODO: requires scheduler integration
    # types not yet ported into the decomposed kvbm-* crates), so the
    # RustScheduler/SchedulerConfig/RequestStatus attributes are missing
    # from kvbm._core.v2 on this build. The DynamoScheduler degrades
    # cleanly to a vLLM passthrough; KV transfer offload still routes
    # through the kvbm v2 ConnectorLeader/Worker.
    ...
    print(
        "Warning: kvbm.v2 was built without the Rust scheduler module "
        "(see lib/bindings/kvbm/src/v2/mod.rs); falling back to the vLLM "
        "scheduler with KVBM connector offload only."
    )
