...
impl Leader for KvConnectorLeaderRecorder {
    ...
    fn create_slot(&mut self, request: KvbmRequest, tokens: Vec<u32>, extra_block_hashes: Option<Vec<Option<u64>>>) -> anyhow::Result<()> {
        let input_copy = CreateSlotInput {
            request: request.clone(),
            tokens: tokens.clone(),
        };
        let _ = self.connector_leader.create_slot(request, tokens, extra_block_hashes);
        let _ = self
            .unbounded_tx
