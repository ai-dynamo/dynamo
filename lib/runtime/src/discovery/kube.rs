...
mod tests {
    ...
    fn registration_client(kube_client: KubeClient) -> KubeDiscoveryClient {
        KubeDiscoveryClient {
            instance_id: 7,
            ...
            kube_client,
            pod_info: PodInfo {
                pod_name: "test-worker".to_string(),
                pod_namespace: "test-namespace".to_string(),
                pod_uid: "test-uid".to_string(),
                system_port: 9090,
                mode: KubeDiscoveryMode::Pod,
                target: utils::KubeDiscoveryTarget::Pod("test-worker".to_string()),
            },
        }
    }

    fn registration_spec() -> DiscoverySpec {
    ...
    async fn stalled_metadata_apply_rolls_back_and_releases_registration_lock() {
        let mut is_first_request = true;
        let service = tower::service_fn(move |request: Request<Body>| {
            let should_stall = std::mem::replace(&mut is_first_request, false);
            async move {
                assert_eq!(request.method(), "PATCH");
                assert_eq!(
                    request.uri().path(),
                    "/apis/nvidia.com/v1alpha1/namespaces/test-namespace/dynamoworkermetadatas/test-worker"
                );
                if should_stall {
        ...
        let client = registration_client(KubeClient::new(service, "test-namespace"));

        let error = tokio::time::timeout(
            Duration::from_secs(31),
            client.register_internal(registration_spec()),
        )
        .await
        .expect("registration must have an operation deadline")
        .unwrap_err();
        assert!(error.to_string().contains(
            "Timed out after 30s applying DynamoWorkerMetadata test-namespace/test-worker"
        ));
        assert!(client.metadata.try_read().unwrap().get_all().is_empty());

        // A healthy subsequent write must still succeed after the timeout rollback.
        let registered = tokio::time::timeout(
            Duration::from_secs(1),
            client.register_internal(registration_spec()),
        )
        .await
        .expect("the previous registration must release the metadata lock")
        .unwrap();
        assert_eq!(client.metadata.read().await.get_all(), vec![registered]);
    }

    #[tokio::test]
    async fn metadata_apply_api_error_preserves_context_and_rolls_back() {
        let service = tower::service_fn(|_: Request<Body>| async {
            let status = serde_json::json!({
                "apiVersion": "v1",
                "kind": "Status",
                "status": "Failure",
                "message": "metadata patch denied",
                "reason": "Forbidden",
                "code": 403
            });
            Ok::<_, Infallible>(
        ...
        let client = registration_client(KubeClient::new(service, "test-namespace"));
        let error = client
            .register_internal(registration_spec())
            .await
            .unwrap_err();
        let message = format!("{error:#}");
        assert!(
            message.contains("Failed to apply DynamoWorkerMetadata test-namespace/test-worker")
        );
        assert!(message.contains("metadata patch denied"));
        assert!(client.metadata.try_read().unwrap().get_all().is_empty());
    }
