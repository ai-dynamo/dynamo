...
mod tests {
    ...
    async fn dropping_last_client_releases_routing_state() {
        ...
        tokio::time::timeout(Duration::from_secs(1), watcher.changed())
            .await
            .expect("standalone watcher stopped after its Client was dropped")
            .unwrap();
        ...
        tokio::time::timeout(Duration::from_secs(1), watcher.changed())
            .await
            .expect("standalone watcher did not observe endpoint removal")
            .unwrap();
        ...
        endpoint.register_endpoint_instance().await.unwrap();
        tokio::time::timeout(Duration::from_secs(1), raw_watcher.changed())
            .await
            .expect("standalone instance source stopped after its Client was dropped")
            .unwrap();
        assert_eq!(raw_watcher.borrow_and_update().len(), 1);
        wait_for_discovery_event(&mut event_watcher, |event| {
            matches!(event, DiscoveryEvent::Added(DiscoveryInstance::Endpoint(_)))
        })
        .await;
        ...
        tokio::time::timeout(Duration::from_secs(1), raw_watcher.changed())
            .await
            .expect("standalone instance source did not observe endpoint removal")
            .unwrap();
        assert!(raw_watcher.borrow_and_update().is_empty());
        wait_for_discovery_event(&mut event_watcher, |event| {
            matches!(event, DiscoveryEvent::Removed(_))
