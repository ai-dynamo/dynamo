...
// Event publishing methods for NatsQueue (used by KV router)
impl NatsQueue {
    /// The base subject for this NatsQueue (stream name)
    pub fn event_subject(&self) -> String {
        self.stream_name.clone()
    }

    /// Publish an event to the NATS JetStream.
    /// The `event_name` is appended to the base subject.
    ...
    /// Publish raw bytes to the NATS JetStream.
    /// The `event_name` is appended to the base subject.
