...
pub struct ResponseStreamPrologue {
    ...
    /// The same failure as `error`, keeping the worker's [`crate::error::ErrorType`]
    /// so the frontend can classify a pre-stream refusal instead of guessing from
    /// display text.
    ///
    /// `Option` plus `#[serde(default)]` is the mixed-version contract: worker and
    /// frontend roll independently, so an old sender omits the field and a new
    /// sender may emit a variant an old receiver does not know. A required field
    /// would break the handshake in both directions; an absent or unrecognized
    /// field becomes `None` and the untyped path remains.
    #[serde(
        default,
...
/// `typed_error` is the worker's own [`DynamoError`] when the worker sent one.
...
/// the worker sent one, its [`crate::error::ErrorType`].
    ...
    pub async fn send_prologue_typed(
        ...
        if let Some(_prologue) = self.prologue.take() {
            ...
            let header_bytes: Bytes = match serde_json::to_vec(&prologue) {
                Ok(b) => b.into(),
                Err(err) => {
                    tracing::error!(%err, "send_prologue: ResponseStreamPrologue did not serialize");
...
mod tests {
    ...
    #[test]
    fn prologue_unknown_typed_error_variant_keeps_the_message() {
        let encoded = br#"{"error":"Generate Error: boom","typed_error":{"error_type":"VariantFromTheFuture","message":"boom"}}"#;
        let prologue: ResponseStreamPrologue = serde_json::from_slice(encoded)
            .expect("an unknown typed-error variant must not fail the prologue");

        assert_eq!(prologue.error.as_deref(), Some("Generate Error: boom"));
