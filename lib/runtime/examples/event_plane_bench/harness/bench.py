...
"""DIS-2172 TRANSPORT-LAYER event-plane benchmark: single-node OR multi-node.
...
and COUNTS delivered events + per-publisher seq-gap loss in the (stock-transport,
count-only) `event_plane_bench_sub` subscribers. Writes one JSON per (cell,
subscriber) and an aggregate summary.json. Feed the output dir to report.py for
the throughput + drop_rate + resource tables.

STOCK TRANSPORT: this harness builds against an image whose `EventEnvelope` has
NO `published_at_ns` field — the publish hot path is un-perturbed by the bench.
There is therefore NO latency measured here; the metrics are the clock-free
transport metrics (throughput events/s, seq-gap drop_rate, resource footprint),
which are valid across nodes WITHOUT PTP/NTP sync.

Launchers (same sweep logic, different process placement):
  local : every process on this host via subprocess. Traffic is loopback (no
          NIC). Good for dev / quick A/B.
  slurm : processes spread across the job's allocated nodes via `srun --nodelist`.
          ...
          events cross the real NIC. Run inside an salloc/sbatch allocation.
...
(DYN_EVENT_CLOCK / DYN_BENCH_CLOCK are still set below for back-compat with the
latency build, but are inert no-ops against the stock-transport binaries.)
"""
import argparse
import json
import os
...
# DIS-2172 FIX (#2): each subscriber now anchors its warmup+measure window to the
# FIRST event it actually receives (instead of a fixed wall-clock that could start
# before its subscription handshake completed -> the "s=16 zero events" bug). A
# subscriber will wait up to FIRST_EVENT_TIMEOUT seconds for event #1 before it
# gives up and reports received=0. This is passed to the sub via
# DYN_BENCH_FIRST_EVENT_TIMEOUT, and folded into bench.py's sub-wait timeout so a
# validly-waiting subscriber is never killed before its window can even start.
FIRST_EVENT_TIMEOUT = int(os.environ.get("DYN_BENCH_FIRST_EVENT_TIMEOUT", "120"))
...
def run_cell(lr, transport, p, s, topics, gpus_per_node, duration, warmup,
    ...
    # The load must stay on for the WHOLE span of the latest-starting subscriber's
                ...
                for s in subs:
                    ...
                    for r in rows:
                        ...
                        print(f"    {r.get('topic'):>22} recv={r.get('received')} "
                              f"thr={rps:.0f}/s gaps={r.get('gaps')} "
                              f"drop={r.get('drop_rate')} {r.get('error','')}"
                              if rps is not None else
                              f"    {r.get('topic'):>22} {r.get('error','')}", flush=True)
