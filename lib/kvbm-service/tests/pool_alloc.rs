...
//!
//! CUDA tests are gated behind `testing-cuda` (requires a working
//! CUDA driver + at least one visible device). SYCL/XPU tests are gated
//! behind `testing-xpu-sycl` (requires Intel GPU + oneAPI runtime).
//! The UCX NIXL plugin must also be installed in the test environment —
//! tests fail loudly if UCX is missing rather than silently falling back
//! to local-only slabs.

#![cfg(any(feature = "testing-cuda", feature = "testing-xpu-sycl"))]

use dynamo_memory::HugepageMode;
use kvbm_service::{HostMemoryPool, PoolConfig};
use std::sync::Arc;
...
use dynamo_device::cuda::CudaHostRegistrar;

#[cfg(feature = "testing-xpu-sycl")]
use dynamo_device::sycl::SyclHostRegistrar;
...
async fn pool_allocates_one_slab_per_host_memory_node() {
    let pool = HostMemoryPool::new(
        &small_pool_config(),
        "test-instance",
        Arc::new(CudaHostRegistrar::new(0).expect("CudaHostRegistrar")),
    )
...
async fn http_v1_pool_returns_snapshot() {
    ...
    let registrar = Arc::new(CudaHostRegistrar::new(0).expect("CudaHostRegistrar"));
...
async fn pool_local_only_builder_succeeds_without_nixl() {
    ...
    .expect("HostMemoryPool::new local-only");
    let snapshot = pool.snapshot();
...
#[cfg(feature = "testing-xpu-sycl")]
/// Small NIXL-backed pool config for SYCL tests. UCX is on the default
/// backend list so production / test parity is preserved.
fn small_pool_config_sycl() -> PoolConfig {
    PoolConfig::builder()
        .per_node_bytes(SMALL_SLAB_BYTES)
        .hugepage_mode(HugepageMode::Disabled)
        .with_ucx()
...
#[cfg(feature = "testing-xpu-sycl")]
fn small_pool_config_sycl_local() -> PoolConfig {
    PoolConfig::builder()
        .per_node_bytes(SMALL_SLAB_BYTES)
        .hugepage_mode(HugepageMode::Disabled)
        .local_only()
...
#[cfg(feature = "testing-xpu-sycl")]
#[tokio::test]
async fn pool_allocates_one_slab_per_host_memory_node_sycl() {
    let registrar = Arc::new(SyclHostRegistrar::new(0).expect("SyclHostRegistrar"));
    let pool = HostMemoryPool::new(&small_pool_config_sycl(), "test-sycl-instance", registrar)
        .expect("HostMemoryPool::new with SYCL registrar");
    let snapshot = pool.snapshot();
    ...
    for slab in &snapshot.slabs {
        ...
        let name = slab
            ...
            .expect("registered slab must have an agent_name");
        assert!(
            name.starts_with("kvbm-svc:test-sycl-instance:n"),
            "unexpected agent name {name}",
        );
    }
...
#[cfg(feature = "testing-xpu-sycl")]
#[tokio::test]
async fn http_v1_pool_returns_snapshot_sycl() {
    let tmp = tempfile::tempdir().unwrap();
    let cfg = ServiceConfig {
        http_addr: "127.0.0.1:0".parse().unwrap(),
        uds_path: None,
        uds_dir: tmp.path().to_path_buf(),
        shutdown_grace_ms: None,
        pool: small_pool_config_sycl(),
    };

    let registrar = Arc::new(SyclHostRegistrar::new(0).expect("SyclHostRegistrar"));
    let svc = KvbmService::start_with_pool(cfg, Arc::new(kvbm_service::NoopContainer), registrar)
        .await
        .expect("start_with_pool");
    let http = svc.http_addr;
...
#[cfg(feature = "testing-xpu-sycl")]
#[tokio::test]
async fn http_v1_pool_503_without_pool_sycl() {
    let tmp = tempfile::tempdir().unwrap();
    let cfg = ServiceConfig {
        http_addr: "127.0.0.1:0".parse().unwrap(),
        uds_path: None,
...
#[cfg(feature = "testing-xpu-sycl")]
#[tokio::test]
async fn pool_local_only_sycl_succeeds_without_nixl() {
    let cfg = small_pool_config_sycl_local();
    let registrar = Arc::new(SyclHostRegistrar::new(0).expect("SyclHostRegistrar"));
    let pool = HostMemoryPool::new(&cfg, "test-sycl-local", registrar)
        .expect("HostMemoryPool::new local-only SYCL");
    let snapshot = pool.snapshot();
    assert!(!snapshot.slabs.is_empty());
    for slab in &snapshot.slabs {
        assert!(
