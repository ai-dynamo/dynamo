...
async fn http_sidecar_reports_ready_and_discovers_socket() {
    ...
    // /live always 200.
    let r = client.get(format!("http://{http}/live")).send().await.unwrap();
    assert_eq!(r.status(), 200);
    assert_eq!(r.text().await.unwrap(), "ok");

    // /ready should be 200 since uds was set before serve() returned.
    let r = client.get(format!("http://{http}/ready")).send().await.unwrap();
    assert_eq!(r.status(), 200);
    ...
    let r = client.get(format!("http://{http}/metrics")).send().await.unwrap();
    assert_eq!(r.status(), 200);
    ...
    assert_eq!(r.status(), 200);
    let body: serde_json::Value = r.json().await.unwrap();
    assert!(body.is_object(), "/v1/discovery/resources must be an object");
