...
pub struct CommonArgs {
    ...
    /// `endpoint_types`; decode and encode workers do not advertise a local
    /// KV indexer. Encode workers register as `WorkerType::Encode` and are
    /// not exposed on the public chat/completions surface.
    #[arg(
        long,
        value_enum,
