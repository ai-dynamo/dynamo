...
use crate::common::protocols::{
    DirectRequest, EngineType, MockEngineArgs, OutputSignal, SglangArgs,
};
...
fn test_chunked_prefill_budget_is_page_aware() {
    ...
    let mut kv_manager = SglangKvManager::new(10000, 4, None, 0);
...
fn test_decode_allocation_is_page_aware() {
    ...
    let mut kv_manager = SglangKvManager::new(64, 4, None, 0);
...
fn test_decode_speedup_ratio_scales_sglang_decode_time() {
    ...
    let mut base_kv_manager = SglangKvManager::new(64, 4, None, 0);
    ...
    let mut fast_kv_manager = SglangKvManager::new(64, 4, None, 0);
...
fn test_check_decode_mem_preserves_generated_output_on_retract() {
    ...
    let mut kv_manager = SglangKvManager::new(8, 4, None, 0);
...
async fn test_mixed_chunk_decode_retract_reprefill_complete_events_apply_cleanly() {
    ...
    loop {
        let admit = get_new_batch_prefill(&mut waiting, &mut kv_manager, &config, 0.7, &running);
        ...
        let decode = simulate_decode_step(&mut running, &mut kv_manager, &config, now_ms, false);
