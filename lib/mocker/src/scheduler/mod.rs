...
/// wall-clock time to `kv_manager`. Caller must be inside an ambient
/// tokio runtime (the live scheduler spawns its loop under
/// `tokio::spawn`, so this is the natural case). Returns `Ok(None)`
/// when `args.kv_bytes_per_token` is unset.
#[cfg(feature = "kvbm-offload")]
pub async fn init_kvbm_live(
    args: &crate::common::protocols::MockEngineArgs,
    kv_manager: &mut crate::kv_manager::KvManager,
) -> anyhow::Result<
    Option<std::sync::Arc<std::sync::Mutex<crate::kvbm_offload::MockOffloadEngine>>>,
> {
    ...
    let engine = MockOffloadEngine::new(config, ClockSource::Real).await?;
    Ok(Some(kv_manager.attach_new_offload_engine(engine)))
}

/// Attach a [`crate::kvbm_offload::MockOffloadEngine`] driven by
/// virtual (replay) time to `kv_manager`. The offline replay entry
/// path is pure-sync, so this helper owns the tokio runtime required
/// for engine construction + kvbm-engine's background pipeline /
/// session tasks. One worker thread is sufficient for the current
/// task load; the runtime is moved onto the engine via
/// `attach_runtime` so it outlives the initial `block_on` and keeps
/// those tasks draining across the rest of the replay.
#[cfg(feature = "kvbm-offload")]
...
> {
    ...
    let rt = tokio::runtime::Builder::new_multi_thread()
        ...
        .enable_all()
        .build()?;
    let mut engine = rt.block_on(MockOffloadEngine::new(config, ClockSource::Virtual))?;
...
mod offload_init_tests {
    ...
    fn args_with_bpt(bpt: usize) -> MockEngineArgs {
    ...
    #[tokio::test]
    async fn init_kvbm_live_attaches_engine_when_bpt_set() {
        let args = args_with_bpt(131_072);
        let mut kv = make_kv_manager();
        assert!(!kv.has_offload_engine());
        let engine = init_kvbm_live(&args, &mut kv)
            .await
            .expect("init must succeed")
            .expect("engine built with bpt present");
        assert!(kv.has_offload_engine());
    ...
    async fn init_kvbm_live_returns_none_without_bpt() {
        ...
        let mut kv = make_kv_manager();
        let result = init_kvbm_live(&args, &mut kv).await.expect("init must succeed");
