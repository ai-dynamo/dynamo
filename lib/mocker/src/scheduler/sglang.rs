...
fn check_decode_mem(
    ...
    while available + evictable + freed < needed - retracted.len() {
        if running.len() - retracted.len() <= 1 {
    ...
    // Actually remove retracted requests from running (reverse order for swap_remove)
