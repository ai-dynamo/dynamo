...
mod offload {
    ...
    use super::super::core::{AwaitingSwapIn, VllmCore};
    ...
    /// attached — otherwise PS accountants never advance and swap-ins
    ...
    /// Retain-and-promote: an `AwaitingSwapIn` whose handle reports
    /// complete must be removed from `requests_awaiting_swap_in` during
    /// the next pass; a handle that's still pending must survive.
    #[tokio::test]
    async fn ready_swap_ins_drain_on_pass_entry() {
        ...
        // Register a G2 block so try_onboard_prefix actually reserves.
        let plh = PositionalLineageHash::new(99, None, 0);
        seed_g2_blocks(engine.g2_manager(), &[plh]);

        let handle = engine
            .try_onboard_prefix(&[plh], Some(0.0))
            .expect("G2 prefix match must produce a handle");

        // Insert the corresponding request in scheduler state so the
        // promote path's debug_assert on known uuid is satisfied.
        ...
        core.kv_manager.attach_new_offload_engine(engine);
        core.push_awaiting_swap_in_for_test(AwaitingSwapIn {
            uuid,
            handle,
            skip_blocks: 0,
        });
    ...
    async fn cold_request_with_g2_prefix_swaps_in_then_admits() {
        ...
        let engine = MockOffloadEngine::new(config, ClockSource::Virtual)
            .await
            .expect("engine build");
        engine.tick(0.0);
    ...
    /// Live (`ClockSource::Real`) and offline (`ClockSource::Virtual`)
    /// must produce byte-identical scheduler behaviour when driven by
    /// the same trace and the same `now_ms` sequence — the engine's
    /// hot path doesn't branch on `ClockSource`, so any divergence
    /// indicates accidental clock-mode coupling. This test guards
    /// against that regression by running a synthetic G2-offload trace
    /// twice (once per mode) and asserting that admission stream +
    /// KV event stream + parked-swap-in counts match exactly.
    #[tokio::test]
    async fn equivalence_real_vs_virtual_clock_with_g2_offload() {
        use std::sync::{Arc, Mutex};
        ...
        async fn run_mode(clock: ClockSource) -> ModeReport {
            let args = MockEngineArgs::builder()
                .num_gpu_blocks(32)
                .block_size(BLOCK_SIZE)
            ...
            let engine = MockOffloadEngine::new(config, clock).await.unwrap();
            engine.tick(0.0);
        ...
        let report_real = run_mode(ClockSource::Real).await;
        let report_virtual = run_mode(ClockSource::Virtual).await;

        // Sanity: the trace must actually have exercised G2 offload —
        // otherwise this test reduces to "G1 mode parity" and gives a
        // false sense of coverage.
        assert!(
            report_real.swap_in_admissions > 0,
            "trace should exercise at least one G2 swap-in admission"
        );
        assert_eq!(
            report_real, report_virtual,
            "Real and Virtual clock modes must produce identical scheduler behaviour\nReal:    {report_real:?}\nVirtual: {report_virtual:?}",
        );
    }
}
