...
pub struct MockEngineArgs {
    ...
    /// KVBM G2 (host DRAM) block capacity. Only consulted by
    /// `KvbmOffloadConfig::from_args` when the `kvbm-offload` feature
    /// is enabled; otherwise ignored. Falls back to
    /// `KvbmOffloadConfig::default().num_g2_blocks` when unset.
    #[builder(default = "None")]
    #[validate(range(min = 1))]
