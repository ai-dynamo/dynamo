...
lifecycle, and report collector live in `aisimulate-replay`. This directory
...
- `entrypoints.rs` converts existing `MockEngineArgs` and workload inputs into
  a canonical `ReplaySpec`, then calls `aisimulate_replay::Replayer`.
...
See [`aisimulate-replay`](../../../../../aisimulate/crates/replay/README.md) for
