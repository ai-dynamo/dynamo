...
//! Processor-sharing bandwidth accountant for a single-link simulation.
//!
...
//! speed up to `B/(N-1)`. Matches how CUDA streams and PCIe channels
//! fair-share bandwidth across concurrent copies.
//!
//! ## Id uniqueness
//!
//! Accountants do not mint ids themselves. A [`TransferId`] counter is
//! handed in at construction — when two accountants share a single
//! counter (e.g. one for offload, one for onboard), ids are globally
//! unique across both links. That is required whenever ids coexist in a
//! shared map: different accountants dispensing overlapping ids would
//! cause completion signals to cross-fire into the wrong pending
//! transfer.

use std::sync::Arc;
use std::sync::atomic::{AtomicU64, Ordering};

/// Opaque handle returned by [`BandwidthAccountant::reserve`]. Callers
/// correlate completions returned by [`advance_to`](BandwidthAccountant::advance_to)
...
/// [`reserve`](Self::reserve) when time has moved forward while transfers
/// are active.
#[derive(Debug, Clone)]
pub struct BandwidthAccountant {
    ...
    /// non-decreasing.
    last_update_ms: f64,
    /// Source of [`TransferId`]s. When shared across accountants (via
...
struct ActiveTransfer {
    id: TransferId,
    /// Bytes still to transmit at `BandwidthAccountant::last_update_ms`.
    remaining_bytes: f64,
}

impl BandwidthAccountant {
    /// Build an accountant for a link with throughput `gbps` GB/s
    /// (1e9 bytes/s) with a private id counter. Non-positive `gbps`
    /// means "no bandwidth cap" — transfers complete instantly. Use
    /// [`with_gbps_and_counter`](Self::with_gbps_and_counter) when
    /// coupling multiple accountants that share an id namespace.
    pub fn with_gbps(gbps: f64) -> Self {
        Self::with_gbps_and_counter(gbps, Arc::new(AtomicU64::new(0)))
    }

    /// Build an accountant with an externally-owned id counter. Pass an
    /// `Arc` clone of the same counter to every accountant that must
    /// share a [`TransferId`] namespace.
    pub fn with_gbps_and_counter(gbps: f64, id_counter: Arc<AtomicU64>) -> Self {
        let bandwidth_bytes_per_ms = if gbps > 0.0 { gbps * 1e6 } else { f64::INFINITY };
        Self {
    ...
    /// Reserve a new transfer of `bytes` bytes arriving at `now_ms`. Adds
    ...
    ///
    /// `now_ms` is clamped up to `last_update_ms` so the accountant's
    ...
    pub fn reserve(&mut self, now_ms: f64, bytes: usize) -> TransferId {
    ...
    /// prior pass's `end_ms`), the call is a no-op — the accountant stays
    ...
    /// Remove a transfer from the active set by id. No-op if the id is not
    /// present (already drained or unknown).
...
mod tests {
    ...
    fn single_transfer_finishes_at_duration() {
        ...
        let mut acc = BandwidthAccountant::with_gbps(1.0);
        let id = acc.reserve(0.0, 1_000_000);
        approx_eq(acc.earliest_finish().unwrap(), 1.0);
        assert!(acc.advance_to(0.5).is_empty());
        approx_eq(acc.earliest_finish().unwrap(), 1.0);
        let completed = acc.advance_to(1.0);
    ...
    fn two_concurrent_share_bandwidth_under_ps() {
        ...
        let mut acc = BandwidthAccountant::with_gbps(1.0);
        let id1 = acc.reserve(0.0, 1_000_000);
        let id2 = acc.reserve(0.0, 1_000_000);
        // Rate/each = 0.5 MB/ms, so each needs 2 ms to finish.
        approx_eq(acc.earliest_finish().unwrap(), 2.0);
        assert!(acc.advance_to(1.99).is_empty());
        let completed = acc.advance_to(2.0);
    ...
    fn staggered_later_arrival_inherits_remaining_bandwidth() {
        ...
        let mut acc = BandwidthAccountant::with_gbps(1.0);
        let _id1 = acc.reserve(0.0, 1_000_000);
        let c1 = acc.advance_to(1.0);
        assert_eq!(c1.len(), 1);
        assert!(acc.is_idle());

        // Scheduler idles to 5 ms then admits T2.
        assert!(acc.advance_to(5.0).is_empty());
        let id2 = acc.reserve(5.0, 1_000_000);
        approx_eq(acc.earliest_finish().unwrap(), 6.0);
        let c2 = acc.advance_to(6.0);
        assert_eq!(c2, vec![id2]);
    }
    ...
    fn zero_bandwidth_is_infinite_throughput() {
        ...
        let mut acc = BandwidthAccountant::with_gbps(0.0);
        let id = acc.reserve(10.0, 1_000_000_000);
        approx_eq(acc.earliest_finish().unwrap(), 10.0);
        let completed = acc.advance_to(10.0);
        assert_eq!(completed, vec![id]);
    }

    #[test]
    fn earliest_finish_is_none_when_idle() {
        let acc = BandwidthAccountant::with_gbps(1.0);
        assert!(acc.is_idle());
        assert!(acc.earliest_finish().is_none());
    }

    #[test]
    fn shared_counter_yields_globally_unique_ids_across_accountants() {
        // Two accountants sharing a single `Arc<AtomicU64>` counter must
        ...
        let mut a = BandwidthAccountant::with_gbps_and_counter(1.0, counter.clone());
        let mut b = BandwidthAccountant::with_gbps_and_counter(1.0, counter);
        let mut ids = Vec::new();
        for _ in 0..5 {
            ids.push(a.reserve(0.0, 1));
            ids.push(b.reserve(0.0, 1));
        }
        ids.sort();
        ids.dedup();
    ...
    fn many_simultaneous_arrivals_all_complete_at_nx() {
        ...
        let mut acc = BandwidthAccountant::with_gbps(1.0);
        for _ in 0..10 {
            let _id = acc.reserve(0.0, 1_000_000);
        }
        approx_eq(acc.earliest_finish().unwrap(), 10.0);
        let completed = acc.advance_to(10.0);
