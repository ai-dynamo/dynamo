...
//! bandwidth model ([`super::accountant::BandwidthAccountant`]) and the
...
impl KvbmOffloadConfig {
    ...
    ///
    /// Returns `None` when `args.kv_bytes_per_token` is unset — without it
    /// `block_size_bytes` cannot be computed and every transfer would
    /// complete instantly, which is rarely what the caller wants. Caller
    /// should interpret `None` as "don't attach an offload engine for this
    /// run".
    pub fn from_args(args: &MockEngineArgs) -> Option<Self> {
        let bpt = args.kv_bytes_per_token?;
        let defaults = Self::default();
        Some(Self {
            num_g2_blocks: args.num_g2_blocks.unwrap_or(defaults.num_g2_blocks),
...
mod tests {
    ...
    fn from_args_none_when_kv_bytes_per_token_missing() {
        let args = MockEngineArgs::builder().build().unwrap().normalized().unwrap();
    ...
    fn from_args_computes_block_size_bytes() {
        ...
        // Defaults preserved.
        assert_eq!(cfg.num_g2_blocks, 100_000);
