...
mod tests {
    use super::*;
    use dynamo_mocker::loadgen::{SessionTrace, TurnTrace};
    ...
    #[tokio::test]
    async fn test_generate_sequence_events_uses_monotonic_completion_driven_timestamps() {
        let traces = vec![multiturn_trace()];
        let seq_traces = generate_sequence_events(&traces, 1024, 2, 5).await.unwrap();
        let entries = &seq_traces[0];
        ...
        let first_free_us = entries
            .iter()
            .find_map(|entry| match &entry.entry {
                SequenceTraceEntry::Free { request_id } if !request_id.is_empty() => {
                    Some(entry.timestamp_us)
                }
                _ => None,
            })
            .unwrap();
        let second_add_us = entries
            .iter()
            .filter_map(|entry| match &entry.entry {
                SequenceTraceEntry::Add { request_id, .. } if !request_id.is_empty() => {
                    Some(entry.timestamp_us)
                }
                _ => None,
            })
