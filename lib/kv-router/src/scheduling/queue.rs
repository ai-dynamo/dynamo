...
use super::request_progress::RequestProgressUpdater;
...
enum AdmissionCommand {
    Enqueue {
        ...
        block_hashes: Option<Vec<LocalBlockHash>>,
        request_progress: Option<RequestProgressUpdater>,
        lifecycle_lease: Option<RequestLifecycleLease>,
        ack_tx: oneshot::Sender<Option<RequestLifecycleLease>>,
    },
    Update {
...
struct LifecycleCleanup {
    /// Present only when the lease remains owned after worker selection.
...
struct LifecycleCleanupQueue {
    dirty: SegQueue<LifecycleCleanup>,
    pending: AtomicBool,
}

impl LifecycleCleanupQueue {
    fn enqueue(&self, cleanup: LifecycleCleanup) -> bool {
        self.dirty.push(cleanup);
        !self.pending.swap(true, AtomicOrdering::AcqRel)
    }

    fn drain(&self) -> Vec<LifecycleCleanup> {
...
/// Owns one opt-in tracked request across selection and response streaming.
#[derive(Debug)]
#[must_use = "dropping the lease releases scheduler state"]
pub struct RequestLifecycleLease {
    cleanup: Arc<LifecycleCleanupQueue>,
    actor_tx: mpsc::Sender<AdmissionCommand>,
    /// `None` avoids generation tracking for the stock selection-only lease.
    generation: Option<u64>,
...
impl Drop for RequestLifecycleLease {
    fn drop(&mut self) {
        ...
        if self.cleanup.enqueue(LifecycleCleanup {
...
> {
    /// Generations are tracked only for leases retained across response streaming.
    active_lifecycles: HashMap<String, ActiveLifecycle>,
...
> SchedulerQueue<P, C, Sel, RF>
{
    ...
    fn new_with_policy_profile_and_capacity(
        ...
        let cleanup = Arc::new(LifecycleCleanupQueue::default());
        let actor = SchedulerQueueActor {
            active_lifecycles: HashMap::new(),
            next_lifecycle_generation: 1,
...
> SchedulerQueue<P, C, Sel, RF>
{
    ...
    pub async fn enqueue_with_block_hashes(
        ...
        let _ = self
            .enqueue_with_block_hashes_and_lease(request, block_hashes, None, None)
            .await;
    }
...
> SchedulerQueueActor<P, C, Sel, RF>
{
    async fn run(mut self, mut rx: mpsc::Receiver<AdmissionCommand>) {
        ...
        while let Some(command) = rx.recv().await {
            let periodic_cleanup = if !self.cleanup.pending.load(AtomicOrdering::Acquire) {
                commands_since_cleanup = 0;
                false
            } else {
                commands_since_cleanup += 1;
                let periodic_cleanup = rx.is_empty() || commands_since_cleanup == 256;
                if periodic_cleanup {
                    commands_since_cleanup = 0;
                }
                periodic_cleanup
            ...
            let cleanup_changed = match command {
                AdmissionCommand::Enqueue {
                    ...
                    let generation_guarded = request_progress.is_some();
                    let lifecycle_request_id = lifecycle_lease
                        .as_ref()
                        .and_then(|_| request.mode.lifecycle_request_id().map(str::to_owned));
                    let lifecycle_phase = if lifecycle_request_id
                        .as_ref()
                        .is_some_and(|request_id| self.active_lifecycles.contains_key(request_id))
                    {
                        let request_id = lifecycle_request_id.as_deref().unwrap();
                        request.respond(Err(KvSchedulerError::BookingFailed(format!(
                            "lifecycle request {request_id:?} is already active"
                        ))));
                        None
                    } else {
                        self.handle_enqueue(request, block_hashes, request_progress)
                    };

                    if let Some(request_id) = lifecycle_request_id
                        && let Some(phase) = lifecycle_phase
                    {
                        if generation_guarded {
                            let generation = self.next_lifecycle_generation;
                            self.next_lifecycle_generation = generation.wrapping_add(1);
                            self.active_lifecycles
                                .insert(request_id.clone(), ActiveLifecycle { generation, phase });
                    ...
                    let _ = ack_tx.send(lifecycle_lease);
                    false
                }
                ...
                AdmissionCommand::Cleanup => self.drain_lifecycle_cleanup(),
            };

            if cleanup_changed || (periodic_cleanup && self.drain_lifecycle_cleanup()) {
                self.handle_update(None).await;
            }
        }
        self.drain_lifecycle_cleanup();
    ...
    fn drain_lifecycle_cleanup(&mut self) -> bool {
        let cleanups = self.cleanup.drain();
        if cleanups.is_empty() {
            return false;
        }

        let mut changed = false;
        let mut queued_stock_ids = FxHashSet::default();
        let mut queued_progress_ids = FxHashSet::default();
        for cleanup in cleanups {
            let (free_slot, remove_stock_pending, remove_progress_pending) =
                match cleanup.generation {
                    Some(generation) => {
                        let Some(active) = self.active_lifecycles.get(&cleanup.request_id).copied()
                        else {
                            continue;
                        };
                        ...
                        self.active_lifecycles.remove(&cleanup.request_id);
                        match active.phase {
                            LifecyclePhase::Pending => (false, false, true),
                            LifecyclePhase::Booked => (true, false, false),
                        }
                    }
                    None => match self
                        .active_lifecycles
                        .get(&cleanup.request_id)
                        .map(|active| active.phase)
                    {
                        // The slot belongs to the newer response-owned request.
                        Some(LifecyclePhase::Booked) => (false, true, false),
                        // The newer response-owned request is queued, so any slot with this ID
                        // still belongs to the cancelled stock request.
                        Some(LifecyclePhase::Pending) => (true, true, false),
                        None => {
                            let booked = self.slots.request_worker(&cleanup.request_id).is_some();
                            (booked, !booked, false)
                        }
                    },
                };

            if free_slot && self.slots.request_worker(&cleanup.request_id).is_some() {
                if let Err(error) = self.slots.free(&cleanup.request_id, Instant::now()) {
                    tracing::error!(
                        request_id = %cleanup.request_id,
                        %error,
                        "Failed to release scheduler state"
                    );
                }
                changed = true;
            }
            if remove_progress_pending {
                queued_progress_ids.insert(cleanup.request_id);
            } else if remove_stock_pending {
                queued_stock_ids.insert(cleanup.request_id);
            }
        }

        if queued_stock_ids.is_empty() && queued_progress_ids.is_empty() {
            return changed;
        }
        ...
        let removed: Vec<_> = self
            ...
            .entries()
            .filter_map(|entry| {
                let request_id = entry.payload().request.mode.lifecycle_request_id()?;
                let queued_ids = if entry.payload().request_progress.is_some() {
                    &queued_progress_ids
                } else {
                    &queued_stock_ids
                };
                queued_ids
                    .contains(request_id)
                    .then_some((entry.class_index(), entry.snapshot()))
            })
            .collect();
        if removed.is_empty() {
            return changed;
        ...
        self.pending.retain(|queued| {
            let queued_ids = if queued.request_progress.is_some() {
                &queued_progress_ids
            } else {
                &queued_stock_ids
            };
            queued
                ...
                .is_none_or(|request_id| !queued_ids.contains(request_id))
        });
        for (class_index, snapshot) in removed {
            self.pending_count.fetch_sub(1, AtomicOrdering::Relaxed);
            self.pending_isl_tokens
                .fetch_sub(snapshot.raw_isl_tokens, AtomicOrdering::Relaxed);
    ...
    fn handle_enqueue(
        ...
        block_hashes: Option<Vec<LocalBlockHash>>,
        request_progress: Option<RequestProgressUpdater>,
    ) -> Option<LifecyclePhase> {
        let owns_lifecycle = request.mode.lifecycle_request_id().is_some();
        let eligibility = request.eligibility();
        ...
        if !should_queue {
            return self
                .admit_one(request, decay_now, request_progress)
                .then_some(LifecyclePhase::Booked);
        }
        ...
        owns_lifecycle.then_some(LifecyclePhase::Pending)
    }
    ...
    async fn handle_update(&mut self, worker: Option<WorkerWithDpRank>) {
        ...
        loop {
            ...
            let lifecycle_request_id = queued.request_progress.as_ref().and_then(|_| {
                queued
                    .request
                    .mode
                    .lifecycle_request_id()
            ...
            let booked = self.admit_one(request, admit_now, queued.request_progress);
            if let Some(request_id) = lifecycle_request_id {
                if booked {
                    if let Some(active) = self.active_lifecycles.get_mut(&request_id) {
                        active.phase = LifecyclePhase::Booked;
                    }
                } else {
                    self.active_lifecycles.remove(&request_id);
                }
            }
        }
    }
    ...
    fn admit_one(
        ...
        owns_lifecycle && delivered
    }

    /// Completes the tracked-request ownership handoff.
    ///
...
mod tests {
    ...
    use crate::scheduling::{RefreshedOverlap, RequestProgress, RouterPolicyConfig};
    ...
    fn make_lifecycle_request(
        request_id: &str,
        isl_tokens: usize,
    ) -> (SchedulingRequest, SchedulingResponseReceiver) {
    ...
    async fn enqueue_lifecycle(
        ...
        isl_tokens: usize,
        track_progress: bool,
    ) -> (RequestLifecycleLease, SchedulingResponseReceiver) {
        let (request, response) = make_lifecycle_request(request_id, isl_tokens);
        let lease = queue
            .new_request_lifecycle_lease(Some(request_id))
            .expect("lifecycle request must create a lifecycle lease");
        let updater = track_progress.then(|| RequestProgress::new(isl_tokens).1);
        let lease = queue
            .enqueue_with_block_hashes_and_lease(request, None, updater, Some(lease))
            .await
            .expect("actor must return an accepted lifecycle lease");
        (lease, response)
    }

    #[tokio::test]
    async fn disabled_queueing_has_no_lifecycle_lease() {
    ...
    async fn lifecycle_lease_releases_booked_request() {
        let request_id = "booked-lifecycle".to_owned();
        let (queue, slots) = make_queue(1, 16, 512, Some(0.0));
        let (lease, response) = enqueue_lifecycle(&queue, &request_id, 512, true).await;

        response.await.unwrap().unwrap();
        assert!(slots.request_worker(&request_id).is_some());
        drop(lease);

        tokio::time::timeout(Duration::from_secs(1), async {
            while slots.request_worker(&request_id).is_some() {
                tokio::task::yield_now().await;
            }
        })
        .await
        .expect("dropped lifecycle lease did not free booked state");
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn unaccepted_lifecycle_lease_cannot_suppress_next_cleanup() {
        let (queue, slots) = make_queue(1, 16, 512, Some(0.0));
        let (mut rejected, rejected_response) = make_lifecycle_request("rejected-lifecycle", 512);
        rejected.pinned_worker = Some(WorkerWithDpRank::new(0, 0));
        rejected.allowed_worker_ids = Some(HashSet::from([1]));

        let mut unaccepted = queue
            .new_request_lifecycle_lease(Some("rejected-lifecycle"))
            .unwrap();
        assert!(unaccepted.request_id.is_none());
        let (full_tx, _full_rx) = mpsc::channel(1);
        assert!(full_tx.try_send(AdmissionCommand::Cleanup).is_ok());
        unaccepted.actor_tx = full_tx;
        let unaccepted = queue
            .enqueue_with_block_hashes_and_lease(rejected, None, None, Some(unaccepted))
            .await
            .unwrap();
        assert!(rejected_response.await.unwrap().is_err());
        drop(unaccepted);

        assert!(!queue.cleanup.pending.load(AtomicOrdering::Acquire));
        assert!(queue.cleanup.dirty.is_empty());

        let request_id = "accepted-after-rejection".to_owned();
        let (accepted, accepted_response) = enqueue_lifecycle(&queue, &request_id, 512, true).await;
        accepted_response.await.unwrap().unwrap();
        drop(accepted);
        ...
        .await
        .expect("unaccepted lease suppressed the next accepted cleanup");
    }
    #[tokio::test(flavor = "multi_thread")]
    async fn lifecycle_lease_removes_cancelled_pending_request_and_counters() {
        let isl = 512;
        let (queue, slots) = make_queue(1, 16, isl, Some(0.0));
        let (blocker, blocker_response) = make_request("lifecycle-blocker", isl);
        queue.enqueue(blocker).await;
        blocker_response.await.unwrap().unwrap();

        let (lease, response) = enqueue_lifecycle(&queue, "cancelled-lifecycle", isl, false).await;
        ...
        .await
        .expect("dropped lifecycle lease did not remove pending request");
        ...
        slots
            .free(&"lifecycle-blocker".to_owned(), decay_now())
            .unwrap();
        slots.assert_completely_drained(decay_now());
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn stock_cleanup_preserves_same_id_pending_progress_request() {
        let isl = 512;
        let request_id = "shared-lifecycle".to_owned();
        let (queue, slots) = make_queue(1, 16, isl, Some(0.0));
        let (blocker, blocker_response) = make_request("lifecycle-blocker", isl);
        queue.enqueue(blocker).await;
        blocker_response.await.unwrap().unwrap();

        let (stock_lease, stock_response) =
            enqueue_lifecycle(&queue, &request_id, isl, false).await;
        let (progress_lease, progress_response) =
            enqueue_lifecycle(&queue, &request_id, isl, true).await;
        ...
        .await
        .expect("stock cleanup did not leave exactly the progress-owned request");

        slots
            .free(&"lifecycle-blocker".to_owned(), decay_now())
            .unwrap();
        queue.update().await;
        progress_response.await.unwrap().unwrap();
        assert!(slots.request_worker(&request_id).is_some());
        ...
        .await
        .expect("progress-owned request did not release its booking");
        slots.assert_completely_drained(decay_now());
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn stale_lifecycle_cleanup_does_not_free_reused_request_id() {
        let request_id = "reused-lifecycle".to_owned();
        let (queue, slots) = make_queue(1, 16, 512, Some(0.0));

        let (first_lease, first_response) = enqueue_lifecycle(&queue, &request_id, 512, true).await;
        let stale_generation = first_lease.generation.unwrap();
        ...
        let (second_lease, second_response) =
            enqueue_lifecycle(&queue, &request_id, 512, true).await;
        assert_ne!(stale_generation, second_lease.generation.unwrap());
        second_response.await.unwrap().unwrap();

        if queue.cleanup.enqueue(LifecycleCleanup {
            generation: None,
            request_id: request_id.clone(),
        }) {
            queue
                .admission_tx
                .send(AdmissionCommand::Cleanup)
                .await
                .unwrap();
        }
        queue.update().await;
        assert!(
            slots.request_worker(&request_id).is_some(),
            "stale stock cleanup freed the response-owned request ID"
        );
        ...
        assert!(
            slots.request_worker(&request_id).is_some(),
            "stale generation freed the reused request ID"
        );
        ...
        .await
        .expect("current lifecycle lease did not free reused request");
    }
    #[tokio::test(flavor = "multi_thread")]
