...
struct AdmissionCleanupEntry {
    request_id: String,
    /// Worker this attempt booked, so cleanup releases the booking it acquired
    /// rather than whichever worker currently owns the id.
    worker: Option<WorkerWithDpRank>,
}
...
impl RequestLifecycleLease {
    ...
    /// Record the worker this request booked. Set after selection, which is the
    /// first point both the lease and the chosen worker are in hand.
    pub(crate) fn set_worker(&mut self, worker: WorkerWithDpRank) {
    ...
    fn drain_cleanup(&mut self) -> bool {
        ...
        for cleanup in dirty {
            ...
            if self.slots.request_worker(request_id).is_some() {
                ...
                // currently owns the id — after a same-id re-dispatch that is the
                // replacement's booking. Paths that never recorded a worker fall
                // back to the untargeted release so nothing leaks.
                let release = match cleanup.worker {
                    Some(worker) => self
                        .slots
                ...
                if let Err(error) = release {
                    tracing::error!(%request_id, ?cleanup.worker, %error, "Failed to release dropped scheduler booking");
                }
                made_ready = true;
            }
