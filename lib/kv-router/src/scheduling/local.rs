    ...
    RF: OverlapScoresRefresh + 'static,
{
    ...
    pub async fn schedule_request(
        ...
        let mut lifecycle_lease = self
            .queue
            .enqueue_with_block_hashes_and_lease(request, block_hashes, None, lifecycle_lease)
            .await;
        ...
        match &mut response {
            Ok(response) if response.request_progress.is_some() => {
                response.lifecycle_lease = lifecycle_lease.take();
            }
            Ok(_) | Err(_) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn stock_queueing_does_not_retain_lifecycle_lease_after_selection() {
        ...
        let request_id = "stock-lifecycle".to_owned();
        ...
        assert!(response.lifecycle_lease.is_none());
