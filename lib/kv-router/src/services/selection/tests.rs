...
async fn empty_token_potential_loads_equal_current_load_snapshot() {
    ...
    // One live reservation makes the snapshot non-trivial.
    let response = post(
        app.clone(),
        "/select_and_reserve",
        r#"{"model_name":"model","token_ids":[1,2,3,4],"selection_id":"empty-probe"}"#,
    )
    .await;
    assert_eq!(response.status(), StatusCode::OK);
    ...
    let loads_response = app
        ...
        .unwrap();
    let loads = response_json(loads_response).await;
    let snapshot = loads[0]["loads"][0].clone();

    // No tokens means no blocks to hash or match: the projection must be
    // exactly the current per-worker snapshot, with no cache-hit credit and
    // no indexer involvement (#10566).
    let response = post(
        app,
        "/potential_loads",
    ...
    assert_eq!(response.status(), StatusCode::OK);
    let potential = response_json(response).await;
    assert_eq!(potential.as_array().map(|rows| rows.len()), Some(1));
    assert_eq!(
        potential[0]["potential_decode_blocks"],
        snapshot["potential_decode_blocks"]
    );
    assert_eq!(
        potential[0]["potential_prefill_tokens"],
        snapshot["potential_prefill_tokens"]
    );
}
