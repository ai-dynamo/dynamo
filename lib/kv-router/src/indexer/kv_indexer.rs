...
fn apply_routing_decision_with_prune_tracking(
    ...
    *event_id_counter += 1;
    ...
    // Pad mm_infos with `None` so a shorter vector can't truncate the
    // synthetic `Stored` event; every block in `local_hashes` survives even
    // if the sender supplied only partial MM info. The outer `zip` stops at
    // the shorter side, so any trailing entries past `local_hashes.len()`
    // are dropped — also intentional.
    ...
    let stored_event = KvCacheEventData::Stored(KvCacheStoreData {
        ...
        blocks: hashes
            .map(
                |((local_hash, sequence_hash), mm_info)| KvCacheStoredBlockData {
