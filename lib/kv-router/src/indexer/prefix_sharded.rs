...
impl<T: SyncIndexer> KvIndexerInterface for PrefixShardedIndexer<T> {
    ...
    fn timing_report(&self) -> String {
        ...
        let avg_routing_ns = self.timing_sum_routing_ns.load(Ordering::Relaxed) / dispatched;
        let avg_shard_us = self.timing_sum_shard_ns.load(Ordering::Relaxed) / dispatched / 1000;
        format!(
            "PrefixShardedIndexer find_matches ({dispatched} total, no early-exit):\n  \
             avg routing    = {avg_routing_ns}ns  (prefix hash -> shard)\n  \
