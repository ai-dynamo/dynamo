...
class PinnedCopySlot:
    ...
    def copy_to_device_async(self, dst_ptr: int, size: int) -> None:
        ...
        self._enqueue_copy(
            "h2d",
            size,
            lambda: self._vmm.memcpy_h2d_async(
    ...
    def copy_from_device_async(self, src_ptr: int, size: int) -> None:
        ...
        self._enqueue_copy(
            "d2h",
            size,
            lambda: self._vmm.memcpy_d2h_async(
        ...
        with self._profile.aggregate(
            ...
            **self._profile_fields,
            ...
            if self._start_event is not None:
                self._vmm.event_record(self._start_event, self.stream)
            copy()
            self.busy = True
            if self._end_event is not None:
                self._vmm.event_record(self._end_event, self.stream)
    ...
    def wait(self) -> None:
        ...
        if self._end_event is not None:
            ...
            duration_ns = self._vmm.event_elapsed_ns(
                self._start_event,
                self._end_event,
            )
