...
def _file_groups_allocation_ids(
    file_groups: Sequence[NixlFileGroup],
) -> List[str]:
    return [
...
class _NixlPosixStagingTransferSession:
    ...
    def finish_restore(self) -> None:
        ...
        now = time.monotonic()
        first_transfer_at = self._stream_first_transfer_at or now
        ...
        logger.info(
            "%s transfers complete: %.2f GiB in %.3fs "
            "(%.2f GiB/s, %s_groups=%d, streaming=True, "
    ...
    def close(self) -> None:
        ...
        if (
            ...
            and threading.current_thread() is not self._stream_scheduler_thread
        ):
            self._stream_scheduler_thread.join()
        self._active = False
        if self._stream_transfer_pool is not None:
    ...
    def _ensure_streaming_started(self) -> None:
        if not self._work_groups:
            self._active = False
            return
        if self._stream_scheduler_thread is not None:
            return
        self._stream_started_at = time.monotonic()
        prep_overlap_s = self._stream_started_at - self._prep_started_at
        logger.info(
            "%s streaming restore targets started after %.3fs of background "
            "staging prep",
            self._backend_name,
            prep_overlap_s,
        )
        self._stream_transfer_pool = ThreadPoolExecutor(max_workers=self._worker_count)
        self._stream_scheduler_thread = threading.Thread(
            target=self._run_streaming_scheduler,
            name=f"{self._backend_name}-streaming-scheduler",
            daemon=True,
        )
        self._stream_scheduler_thread.start()

    def _run_streaming_scheduler(self) -> None:
        try:
            assert self._stream_transfer_pool is not None
            for prep_future in as_completed(self._prep_futures):
    ...
    def _mark_first_streaming_transfer_started(self) -> None:
        with self._stream_condition:
            ...
            logger.info(
                "%s first streaming transfer started after %.3fs from first "
                "target submission",
                self._backend_name,
                self._stream_first_transfer_at - self._stream_started_at,
            )

    def _restore_prepared_group_when_targets_ready(
        self,
        prepared: _PreparedNixlGroup,
    ) -> None:
        try:
            group_targets = self._wait_for_group_targets(prepared)
            self._mark_first_streaming_transfer_started()
            self._restore_prepared_group(prepared, group_targets)
        except Exception:
            self._close_prepared_group(prepared)
            raise
