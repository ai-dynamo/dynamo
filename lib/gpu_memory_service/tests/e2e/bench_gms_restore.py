...
Phase timing is extracted from log timestamps in the subprocess output:
    Phase 0 (disk read):  "Loading GMS state" → "Loaded N allocations from disk"
    Phase A (GMS alloc):  "Loaded N allocations" → "Phase A complete"
...
_MARKERS = {
    ...
    "disk_end": re.compile(r"Loaded \d+ allocations from disk"),
...
def parse_phase_times(log_text: str) -> dict[str, float]:
    """Return {disk_s, phaseA_s, phaseB_s, total_s} from subprocess log text."""
    ...
    if "disk_start" in times and "disk_end" in times:
        result["disk_s"] = (times["disk_end"] - times["disk_start"]).total_seconds()
    if "disk_end" in times and "phaseA_end" in times:
        result["phaseA_s"] = (times["phaseA_end"] - times["disk_end"]).total_seconds()
...
def bench_restore(
    ...
    try:
        ...
        logger.info(
            "workers=%3d  disk=%.2fs  phaseA=%.2fs  phaseB=%.2fs  total=%.2fs  "
            "B_throughput=%.2f GiB/s",
            workers,
            times.get("disk_s", float("nan")),
            times.get("phaseA_s", float("nan")),
...
def main() -> None:
    ...
    print()
    print(
        f"{'workers':>8}  {'disk_s':>8}  {'phaseA_s':>9}  {'phaseB_s':>9}  "
    ...
    for r in results:
        print(
            f"{r['workers']:>8}  "
            f"{r.get('disk_s', float('nan')):>8.2f}  "
            f"{r.get('phaseA_s', float('nan')):>9.3f}  "
