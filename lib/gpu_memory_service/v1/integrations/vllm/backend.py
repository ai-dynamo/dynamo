...
class GMSV1SleepModeBackend(SleepModeBackend):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            self._weights = GMSClientMemoryManager(
                device_identity.get_socket_path(self._device, _WEIGHTS),
                vmm,
                self._device,
            )
            self._kv_cache = GMSClientMemoryManager(
                device_identity.get_socket_path(self._device, _KV_CACHE),
                vmm,
                self._device,
            )
    ...
    def resume(self, tags: list[str] | None = None) -> None:
        ...
        try:
            ...
            weights_socket_path = device_identity.get_socket_path(
                self._device, _WEIGHTS
            )
            kv_cache_socket_path = device_identity.get_socket_path(
                self._device, _KV_CACHE
            )
            self._weights.refresh_socket_path(weights_socket_path)
            self._kv_cache.refresh_socket_path(kv_cache_socket_path)
