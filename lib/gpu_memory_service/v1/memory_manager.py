...
from gpu_memory_service.v1.mapping import (
    LocalMapping,
    install_mapping,
...
class GMSClientMemoryManager:
    ...
    def refresh_socket_path(self, socket_path: str) -> None:
        """Update the target socket path while this manager is disconnected."""
        with self._lock:
            if self._session is not None:
                raise RuntimeError("cannot refresh the GMS socket path while connected")
            self._socket_path = socket_path
