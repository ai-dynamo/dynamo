...
class GMSWorker(Worker):
    ...
    def sleep(self, level: int = 1) -> None:
        ...
        # Sleep GMS weights (VA-stable unmap, no CPU backup needed)
        manager = get_gms_client_memory_manager()
        assert manager is not None, "GMS client is not initialized"
        assert not manager.is_sleeping, "GMS weights are already sleeping"
        manager.sleep()
    ...
    def wake_up(self, tags: Optional[List[str]] = None) -> None:
        ...
        if "weights" in tags:
            manager = get_gms_client_memory_manager()
            assert manager is not None, "GMS client is not initialized"
            assert manager.is_sleeping, "GMS weights are not sleeping"
            manager.wake()
