...
class _ScratchMapping:
    ...
    In scratch-backed mode, n_chunks granules of VA all alias the same physical
    chunk (scratch_handle). One physical page, many virtual mappings.
...
class GMSClientMemoryManager:
    ...
    def unmap_all_vas(self) -> None:
        ...
        # cuMemUnmap over that prefix covers all aliases in one call.
        for scratch in self._scratch_mappings.values():
            if scratch.scratch_handle == 0:
                continue
            mapped_size = scratch.mapped_size or scratch.aligned_size
            cumem_unmap(scratch.base_va, mapped_size)
    ...
    def create_scratch_mapping(
        ...
        *,
        ...
        The scratch-backed path lets torch.zeros on the full kv_cache size
        succeed without paying the real memory cost. The reserve-only path is
        cheaper and is safe when the caller will not touch the tensor before a
        later remap to real backing. The shadow then sleeps (unmap_all_vas drops
        any scratch physical, preserves VAs) and wakes by promoting entries into
        _mappings via prepare_scratch_for_reallocation.
        ...
        if map_scratch:
            ...
            max_bytes = self._scratch_mapped_prefix_bytes(aligned_size)
            mapped_size = min(aligned_size, max(self.granularity, max_bytes))
            mapped_size = align_to_granularity(mapped_size, self.granularity)
            mapped_size = min(aligned_size, mapped_size)
            for offset in range(0, mapped_size, self.granularity):
        ...
        if map_scratch:
            logger.info(
                "[GMS] Reserved %d MiB VA at 0x%x, aliased 1x %d MiB scratch across %d/%d granules",
                aligned_size // (1 << 20),
                va,
                self.granularity // (1 << 20),
                mapped_size // self.granularity,
                n_chunks,
            )
        else:
            logger.info(
                "[GMS] Reserved %d MiB VA at 0x%x without scratch backing across %d granules",
                aligned_size // (1 << 20),
                va,
                n_chunks,
            )
