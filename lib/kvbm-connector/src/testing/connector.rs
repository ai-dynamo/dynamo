...
impl ConnectorTestConfig {
    ...
    /// ```rust,ignore
    /// let config = ConnectorTestConfig::new()
    ///     .leader_path("messenger.backend.tcp_port", 9090u16);
    /// ```
    #[must_use]
    ...
    /// ```rust,ignore
    /// let config = ConnectorTestConfig::new()
    ///     .worker_path("messenger.backend.tcp_port", 9091u16);
    /// ```
    #[must_use]
...
pub struct TestConnectorWorker {
    ...
    /// Messenger instance ID for this worker.
    pub instance_id: InstanceId,
    /// Messenger worker address.
...
impl TestConnectorInstance {
    ...
    /// Get the Messenger instance ID for this instance.
...
/// - Cache configuration
///
/// Messenger instances are cross-registered to enable inter-instance communication.
...
impl TestConnectorCluster {
    ...
    /// 3. Cross-registers all Messenger instances
...
impl TestConnectorClusterBuilder {
    ...
    /// Build the cluster with cross-registered Messenger instances.
    pub async fn build(self) -> Result<TestConnectorCluster> {
        ...
        // Cross-register all Messenger instances
        ...
        // Give Messenger time to establish connections
...
impl TestConnectorInstanceBuilder {
    ...
    /// 1. Messenger mesh with leader + workers (cross-registered)
    ...
    pub async fn build(self) -> Result<TestConnectorInstance> {
        // 1. Create Messenger instances
        let leader_nova = messenger::create_messenger_tcp().await?;

        let mut worker_novas = Vec::with_capacity(self.num_workers);
        for _ in 0..self.num_workers {
            worker_novas.push(messenger::create_messenger_tcp().await?);
        }
        ...
        let _ = tracing_subscriber::fmt()
            .with_env_filter(
                EnvFilter::try_from_default_env()
                    .unwrap_or_else(|_| EnvFilter::new("warn,velo=error,kvbm=debug")),
            )
            .with_test_writer()
            .try_init();
