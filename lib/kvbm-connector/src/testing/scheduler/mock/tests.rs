...
// TODO: Entire file disabled — uses MockEngineCore which depends on
// v2::integrations::scheduler::Scheduler which has no workspace equivalent.
...
mod tests_disabled {
    ...
    fn test_projection_choke_point_detection() {
        ...
        assert!(
            usage > 0.0,
            "Should have non-zero cache usage with active requests"
        );
    }

    #[test]
    fn test_empty_engine_step_returns_none() {
        let mut engine = MockEngineCore::new(default_config()).expect("Should create engine");
