...
The full 4-site enumeration above (3 spawn-replays + Drop) plus both cleanup_failed_request paths means every async-spawned `coordinator.release` / `release_request` / direct `states.remove` site in the CD pipeline is now identity-checked. By-name `coordinator.release` and `release_request` remain on the engine-core thread only (e.g., `CdRequestStatePayload::Drop`, `maybe_complete`'s eager release).
