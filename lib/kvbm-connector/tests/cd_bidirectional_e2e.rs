...
//! Bidirectional disagg end-to-end test.
//!
...
//! against the same `MockInnerLeaderShim` / `MockP2pBlockTransport` /
//! `MockP2pWorkerHook`.
//!
//! # Instance layout
//!
...
//!   - shared `MockInnerLeaderShim`, `MockP2pBlockTransport`, `MockP2pWorkerHook`
...
use kvbm_engine::p2p::session::MockSessionFactory;
...
async fn drive_one_exchange(
    ...
    assert_eq!(
        q.num_provided_tokens,
        (COMPUTED_BLOCKS + LOCAL_BLOCKS) * BLOCK_SIZE
    );
