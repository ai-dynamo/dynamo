...
//!
//! Implement [`WorkerSelectorPlugin`] and invoke [`export_worker_selector_plugin!`] from a
//! `cdylib`. The safe API hides the raw C ABI; use the raw `*V1` types only when implementing a
//! host or a plugin in another language.
//!
//! # Lifecycle
//!
//! 1. Dynamo calls [`WorkerSelectorPlugin::from_config`] once for each decode or prefill router
//!    role. The opaque configuration bytes belong to the deployment.
//! 2. Dynamo calls [`WorkerSelectorPlugin::required_worker_inputs`] once and caches the result.
//!    Only the requested worker columns are materialized. Any nonempty request automatically
//!    includes [`WorkerInputs::IDENTITY`].
//! 3. Dynamo calls [`WorkerSelectorPlugin::select`] for each unpinned request. Calls for one plugin
//!    instance are serialized; separate router-role instances may run on different threads.
//! 4. The plugin returns [`Selection::Worker`], [`Selection::UseDefault`], or an error. Dynamo
//!    bounds-checks and revalidates a selected worker before reservation and dispatch.
//!
//! Input borrows end when `select` returns. A plugin must not retain them, block, perform network
//! I/O, or unwind across the ABI. Plugins are trusted native code loaded into the frontend process.
//!
//! # Request inputs
//!
//! [`SelectionInput`] always exposes the following request-level data without requiring a
//! [`WorkerInputs`] flag:
//!
//! | Accessor | Meaning |
//! |---|---|
//! | [`SelectionInput::block_size`] | KV block size in tokens |
//! | [`SelectionInput::isl_tokens`] | Input sequence length in tokens |
//! | [`SelectionInput::expected_output_tokens`] | Optional expected output length in tokens |
//! | [`SelectionInput::request_id`] | Optional request ID |
//! | [`SelectionInput::session_id`] | Optional session ID |
//! | [`SelectionInput::selection_mode`] | Query-only, tracked, or tracked-with-admission mode |
//! | [`SelectionInput::tracks_prefill_tokens`] | Whether Dynamo tracks active prefill tokens |
//! | [`SelectionInput::has_shared_cache_hits`] | Whether the request has shared-cache overlap data |
//!
//! # Worker inputs
//!
//! Worker columns contain only workers that are eligible when selection runs. Every present column
//! has [`SelectionInput::worker_count`] entries and uses the same unspecified order.
//!
//! | [`WorkerInputs`] flag | Accessor | Per-worker value |
//! |---|---|---|
//! | [`WorkerInputs::NONE`] | None | No workers are enumerated; the plugin can inspect request data and delegate with [`Selection::UseDefault`] |
//! | [`WorkerInputs::IDENTITY`] | [`SelectionInput::worker_ids`], [`SelectionInput::dp_ranks`] | Runtime worker ID and data-parallel rank |
//! | [`WorkerInputs::CACHED_TOKENS`] | [`SelectionInput::cached_tokens`] | Effective cached prompt tokens |
//! | [`WorkerInputs::CACHE_TIERS`] | [`SelectionInput::cache_tiers`] | Effective, device, host-pinned, disk, and shared-cache overlap in KV blocks |
//! | [`WorkerInputs::LOAD`] | [`SelectionInput::loads`] | Active prefill tokens, active decode blocks, and additional blocks introduced by this request |
//! | [`WorkerInputs::CAPACITY`] | [`SelectionInput::capacities`] | Published total KV-block and maximum batched-token capacities; either may be unavailable |
//! | [`WorkerInputs::ROUTING`] | [`SelectionInput::routing`], [`SelectionInput::worker_stable_routing_id`] | Optional restart-stable routing ID and preferred-taint cost multiplier |
//! | [`WorkerInputs::DEFAULT_COST`] | [`SelectionInput::default_costs`] | Dynamo's complete configured cost before temperature sampling; lower is preferred |
//! | [`WorkerInputs::DEFAULT_KV_OVERLAP`] | [`SelectionInput::default_kv_overlaps`] | Weighted and decay-adjusted KV-overlap credit used by the default cost, in blocks |
//! | [`WorkerInputs::DEFAULT_DECODE_LOAD`] | [`SelectionInput::default_decode_loads`] | Projected decode load used by the default cost, in blocks |
//!
//! A plugin may combine flags with `|`. Accessors for unrequested columns return `None`.
//!
//! # Example
//!
//! ```no_run
