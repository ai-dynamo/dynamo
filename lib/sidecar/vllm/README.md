...
The sidecar supports both a frontend hosting the complete data-parallel group and hybrid load balancing with one frontend and sidecar per node. In hybrid mode, each vLLM frontend must report `ParallelismInfo.data_parallel_size_local` through Control, alongside the global `data_parallel_size` and the first locally hosted `data_parallel_rank`. For example, a node hosting ranks 4–7 of an eight-rank group reports global size 8, local size 4, and starting rank 4. Dynamo registers that worker for ranks 4–7 and divides the frontend's aggregate KV capacity by four. The local range must be nonempty and contained in the global group.

Dynamo forwards the selected absolute rank unchanged as `x-data-parallel-rank` gRPC metadata. When KV routing is enabled, Control must return one unique ZMQ event source for every locally hosted rank, using those same absolute ranks. Global model world size remains based on the complete data-parallel group. Older servers that omit the local-size field retain complete-group behavior starting at rank 0; they cannot describe hybrid ownership.

Launch vLLM on each node with the global DP size, its local size and starting rank, and `--data-parallel-hybrid-lb`. Every node must expose its own native Rust gRPC frontend; do not use `--headless` for these endpoints. Point each colocated sidecar's `--grpc-endpoint` at that node's frontend. This requires a vLLM build implementing the local-size Control field; updating Dynamo alone does not add it to an older server.
