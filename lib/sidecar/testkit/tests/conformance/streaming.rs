...
type StreamCheck<P> = fn(&RequestHandle<P>, &PreprocessedRequest, &Outputs);

pub(super) async fn streaming<F: SidecarFixture>(
    request: PreprocessedRequest,
    check: Option<StreamCheck<F::Protocol>>,
) {
    ...
    for plan in [
        ...
        let outputs = collect(&engine, request.clone(), GenerateContext::new(ctx, None)).await;
        ...
        if let Some(check) = check {
            ...
            expected.model = metadata.model.clone();
            check(&handle, &expected, &outputs);
        }
...
async fn vllm_tokens_terminal_logprobs_and_usage() {
    let mut req = request("request-model", vec![10, 20, 30, 40, 50], 7);
    ...
    bounded(
        ...
        streaming::<vllm::Fixture>(req, Some(vllm::Fixture::assert_stream)),
    )
    .await;
}
