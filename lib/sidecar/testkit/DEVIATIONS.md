...
| Original requirement | Change | Justification | Affected PRs/tests |
| --- | --- | --- | --- |
| Prior #15089 kept new backend unit bodies entirely vLLM-specific; the first sharing revision added generalized adapters | Keep complete shared scenarios in one case file, native tests in another, and plain native setup in support files | User requires maximum useful sharing without significant complexity; remove observation models and capability matrices while preserving assertions | Consolidated source layout and shared/native assertion mapping in UNITS.md; SGLang activation deferred |
| Reusable native fixtures were under `tests/unit/` | Move them to `tests/support/fixtures/`; reuse common builders from `src/fixtures.rs` | Unit and integration suites share data without importing unit cases or requiring native sidecar dependencies in the testkit library | #15089 fixture consumers; test-only vLLM dependency on testkit |
| Permanent tests validated the lane runner itself | Remove that suite and its CI invocation; validate its five checks temporarily for this change | Explicit user preference for one-time validation of the framework; preserve actual lane behavior and inventory guards | Temporary compiled lane/workspace validation; runner and CI unchanged except collection names and self-test removal |
...
| Treat a currently unsupported option as an omitted shared case | Keep support/rejection assertions in the backend-specific case file when contracts differ | Preserve coverage without introducing a capability matrix or silently skipping shared assertions | Typed gRPC versus native HTTP distinctions and SGLang follow-up obligations in UNITS.md |
...
| R03 construction discovers a real engine | Extract private production `from_discovered(args, model)` and invoke it with in-memory metadata | Executes actual WorkerConfig construction without registration, sockets or downloads | #15089 shared/native worker groups |
| R04 deadline testing through fake transport | Inject the attempt callback into the existing retry/pool policy and use paused time | Exercises the production loop and one absolute deadline once for both tonic versions, without networking or wall-clock sleeps | #15089 common transport group in `tests/unit/shared.rs` |
