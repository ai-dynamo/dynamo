...
| Process integration plan primarily adds sidecar tests | Fix TCP pre-prologue cancellation and local cancellation error classification in shared runtime | Reproduced supported vLLM cancellation deadlock and false worker inhibition require production corrections; pending-header assertions remain enabled | PR3 runtime `tcp/server.rs`, `egress/addressed_router.rs`, focused runtime regression and process C8/C12 |
