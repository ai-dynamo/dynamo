...
```sh
python3 lib/sidecar/testkit/run.py --level native --export /tmp/sidecar-native
DYNAMO_SIDECAR_NATIVE_TEST=/tmp/sidecar-native/native_engine \
...
All three Python cases collect in the pinned vLLM test image. The final native
...
PR3 cannot merge with this pinned-engine failure unresolved.

Local handoff used two engine processes on the workstation's one GPU (explicit
`SIDECAR_NATIVE_GPUS=0,0`). CI still reserves two GPUs; that topology and current-head
CI have not executed. The failed local run took 182.78 seconds and peaked at
6.3 GiB. Final per-commit and current-head CI evidence remains pending.
