...
| R13 direct cleanup | CPU wire `lifecycle::vllm_cleanup_before_start_and_during_read` | Before-start error, repeatable cleanup, live-stream cancellation, post-cleanup generation cannot submit native work. Passed in the 9-case conformance run (see evidence below). |
