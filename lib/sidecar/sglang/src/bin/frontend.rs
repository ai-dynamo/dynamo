...
//! Dynamo OpenAI-compatible frontend with the SGLang direct-gRPC dispatch
//! provider registered (the composition root).
//!
//! It discovers `dynamo-sglang-sidecar --direct` workers via etcd and — because
//! their model cards advertise `runtime_data["direct_backend"] = "sglang"` —
//! dispatches inference straight to each worker's `SglangService` gRPC server,
...
//! bindings) keeps the generic `ai-dynamo` wheel free of the sglang/tonic dep.
...
use dynamo_sglang_sidecar::SglangDirectDispatchProvider;

#[derive(Parser, Debug)]
#[command(
    about = "Dynamo OpenAI frontend with the SGLang direct-gRPC dispatch provider registered"
)]
struct Args {
...
fn main() -> anyhow::Result<()> {
    ...
    secondary.block_on(async move {
        // Composition root: register the sglang provider BEFORE the discovery
        // watcher builds any routing, so direct-backend models resolve to a
        // GrpcDispatch instead of the request-plane router.
        register_direct_dispatch_provider(Arc::new(SglangDirectDispatchProvider::new()));
