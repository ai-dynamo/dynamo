...
//! Thin SMG SGLang gRPC client: connect-with-backoff, discovery, and error
...
use crate::proto::scheduler::sglang_scheduler_client::SglangSchedulerClient;

/// Connected SMG SGLang client over a tonic [`Channel`].
pub type Client = SglangSchedulerClient<Channel>;

/// Engine discovery snapshot: model metadata plus server / scheduler metadata.
#[derive(Clone, Debug)]
            ...
            Err(e) => {
                ...
                if Instant::now() >= deadline {
                    return Err(cannot_connect(format!(
                        "could not reach SMG SGLang scheduler at {uri} within {:?}: {last_err}",
                        cfg.deadline
                    )));
                }
...
pub fn backend_unknown(msg: impl Into<String>) -> DynamoError {
    backend(BackendError::Unknown, msg)
}
...
/// Map a structured SMG Generate error event to a typed error.
pub fn generate_error_to_dynamo(err: &pb::GenerateError) -> DynamoError {
    let status = err.http_status_code.trim();
    let message = if err.details.is_empty() {
        err.message.clone()
    } else {
        format!("{}: {}", err.message, err.details)
    };
    if status.starts_with('4') {
        invalid_arg(format!("SMG Generate error {status}: {message}"))
    } else {
        backend_unknown(format!("SMG Generate error {status}: {message}"))
    }
}
