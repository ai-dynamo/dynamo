...
//! engine over the engine RPC v1 gRPC contract.
//!
//! # Endpoint-only configuration
//!
//! The sidecar takes a single engine-specific input: the engine RPC endpoint.
...
/// A Dynamo backend that proxies inference to a vLLM engine RPC server.
pub struct VllmSidecarEngine {
    ...
    pool: OnceCell<Pool>,
    /// Primary model name accepted by the engine RPC server.
...
impl VllmSidecarEngine {
    ...
    pub fn from_args(argv: Option<Vec<String>>) -> Result<(Self, WorkerConfig), DynamoError> {
        ...
        let endpoint =
            normalize_endpoint(&args.engine_rpc_endpoint).map_err(client::invalid_arg)?;
            ...
            loop {
                tokio::select! {
                    ...
                    msg = stream.message() => {
                        match msg {
                            Ok(Some(resp)) => {
                                let response = match validate_generate_response(
                                    resp,
                                    &request_id,
                                    is_prefill,
                                ) {
    ...
    async fn watch(&self) -> Result<(), DynamoError> {
        ...
        loop {
            ...
            .await
            {
                ...
                Ok(Err(status)) => {
                    return Err(client::engine_shutdown(format!(
                        "engine RPC Health RPC failed after startup: {} ({:?})",
                        status.message(),
                        status.code(),
                    )));
                }
                Err(_) => {
                    return Err(client::engine_shutdown(format!(
                        "engine RPC Health RPC timed out after {:?}",
                        self.transport.connect_timeout,
                    )));
                }
            }
