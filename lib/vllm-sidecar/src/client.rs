...
//! Thin vLLM gRPC client: connect-with-backoff, two-RPC discovery,
    ...
    loop {
        ...
        if remaining.is_zero() {
            return Err(cannot_connect(format!(
                "could not reach vLLM gRPC at {uri} within {:?}",
                cfg.deadline
            )));
        }
        match try_connect_once(uri, cfg, remaining.min(cfg.connect_timeout)).await {
            ...
            Err(e) => {
                ...
                if Instant::now() >= deadline {
                    return Err(cannot_connect(format!(
                        "could not reach vLLM gRPC at {uri} within {:?}: {last_err}",
                        cfg.deadline
                    )));
                }
                let remaining = deadline.saturating_duration_since(Instant::now());
...
/// A fixed-size pool of independent vLLM gRPC connections.
///
