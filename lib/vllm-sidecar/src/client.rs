...
pub async fn discover(client: &mut Client) -> Result<Discovery, DynamoError> {
    ...
    let model_name = engine.supported_models.first().cloned().unwrap_or_default();
    let model = client
        .get_model_info(pb::GetModelInfoRequest { model: model_name })
        .await
...
fn validate_schema(engine: &pb::EngineInfo) -> Result<(), DynamoError> {
    if engine.schema_revision < OPENENGINE_SCHEMA_REVISION
        || engine.minimum_client_revision > OPENENGINE_SCHEMA_REVISION
    {
        return Err(cannot_connect(format!(
            "incompatible OpenEngine schema: server revision {}, minimum client {}; client revision {}",
            engine.schema_revision, engine.minimum_client_revision, OPENENGINE_SCHEMA_REVISION
        )));
    }
    if engine.schema_release.trim().is_empty() {
        return Err(cannot_connect(
            "OpenEngine server did not advertise an immutable schema_release",
        ));
    }
    Ok(())
}
