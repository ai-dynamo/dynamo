...
//! `--engine-rpc-endpoint`. Model identity, disaggregation role, parallelism,
//! KV block sizing, and context length are all *discovered* from the engine
//! over the engine RPC API — there is deliberately no `--model` or
//! `--disaggregation-mode`.
//!
...
#[command(
    ...
    about = "Dynamo vLLM sidecar — drives an out-of-process vLLM engine over its private RPC API. \
             Configure with only --engine-rpc-endpoint; everything else is discovered."
)]
pub struct Args {
    /// `host:port` (or full URL) of the vLLM engine RPC v1 gRPC server.
    ///
    /// A bare `host:port` is normalised to `http://host:port`. This is the
    /// only engine-specific configuration the sidecar takes.
    #[arg(long, env = "ENGINE_RPC_ENDPOINT")]
    pub engine_rpc_endpoint: String,
    ...
    #[arg(long, env = "ENGINE_RPC_CONNECTIONS", default_value_t = 8)]
    pub engine_rpc_connections: usize,
...
impl Args {
    ...
    pub fn transport(&self) -> TransportConfig {
        TransportConfig {
            ...
            connections: self.engine_rpc_connections.max(1),
        }
    }
}
...
pub fn normalize_endpoint(raw: &str) -> Result<String, String> {
    ...
    if trimmed.starts_with("http://") {
        ...
        Err(format!(
            "unsupported engine RPC endpoint scheme in `{trimmed}`; use pod-local http://"
        ))
    } else {
