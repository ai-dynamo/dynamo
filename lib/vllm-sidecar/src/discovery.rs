...
pub(crate) fn bootstrap_discover(
    ...
    rt.block_on(async {
        ...
        .await
        .map_err(|_| client::cannot_connect("engine RPC bootstrap timed out"))?
    })
}
...
pub(crate) fn validate_discovery(discovery: &Discovery) -> Result<pb::EngineRole, DynamoError> {
    if discovery.engine.engine_name != "vllm" {
        return Err(client::invalid_arg(format!(
            "engine RPC endpoint reported unsupported engine `{}`",
            discovery.engine.engine_name
        )));
    }
    if discovery.engine.api_version != "vllm.engine.v1" {
        return Err(client::invalid_arg(format!(
            "engine RPC API version `{}` is not supported",
