...
//! Compiles vLLM's private engine RPC schema into client stubs.
//!
//! The vendored schema matches `vllm/rust/proto/engine.proto` at the vLLM
//! revision used by this sidecar.

fn main() -> Result<(), Box<dyn std::error::Error>> {
    // `--experimental_allow_proto3_optional` lets older protoc (the container's
    // apt protoc 3.12) compile the proto3 `optional` fields in engine.proto
