...
/// event converter can strip it entirely (data included). It exists solely to
/// carry usage to the audit aggregator and is never sent to the client.
...
impl LLMMetricAnnotation {
    ...
    pub fn from_annotation<T>(
        ...
        // for per-chunk metrics and `ANNOTATION_AUDIT_USAGE` for the audit-only
        // final usage chunk. Both carry serialized metrics as their one comment.
...
/// Encode one Prometheus registry in the text exposition format.
...
/// Observe typed response metrics, falling back to annotation-encoded metrics.
///
/// Passing the typed payload keeps the production chat path allocation-free. The
/// annotation fallback supports producers that carry metrics in the response envelope.
pub fn observe_response_metrics<T>(
    annotated: &Annotated<T>,
    typed_metrics: Option<&LLMMetricAnnotation>,
    response_collector: &mut ResponseMetricCollector,
    http_queue_guard: &mut Option<HttpQueueGuard>,
) -> bool {
    if let Some(metrics) = typed_metrics {
        observe_llm_metrics(metrics, response_collector, http_queue_guard);
        true
    } else if let Ok(Some(metrics)) = LLMMetricAnnotation::from_annotation(annotated) {
        observe_llm_metrics(&metrics, response_collector, http_queue_guard);
        true
    } else {
        false
...
mod tests {
    ...
    #[test]
    fn typed_and_annotation_observation_match() {
        let typed_metrics = LLMMetricAnnotation {
            input_tokens: 12,
            output_tokens: 3,
            chunk_tokens: 3,
        ...
        for typed in [true, false] {
            let metrics = Arc::new(Metrics::new());
            let mut collector = metrics.clone().create_response_collector("model");
            let mut queue = Some(metrics.clone().create_http_queue_guard("model"));
            let annotated = if typed {
                Annotated::<()>::from_data(())
            } else {
                typed_metrics.to_annotation::<()>().unwrap()
            };
            observe_response_metrics(
                &annotated,
                typed.then_some(&typed_metrics),
                &mut collector,
                &mut queue,
            );
