...
mod tests {
    ...
    #[test] // PARSER.10 — non-streaming basic case
    ...
    #[test] // PARSER.3 — no reasoning markers, pass through
    ...
    #[test] // PARSER.5 — reasoning open without close (truncation): everything after
    ...
    #[test] // PARSER.13 — text before AND after the reasoning span preserved
    ...
    #[test] // PARSER.5, PARSER.13 — dangling end marker, missing start (upstream INVALID_SIMPLE)
    ...
    #[test] // PARSER.5, PARSER.13 — dangling end + thought prefix on the head (upstream INVALID_COMPLETE)
    ...
    #[test] // PARSER.8 — streaming arrival, single chunk
    ...
    #[test] // PARSER.8 — streaming with `thought\n` split across deltas
    ...
    #[test] // PARSER.8 — start marker split across deltas
    ...
    #[test] // PARSER.8 — end marker split across deltas
    ...
    #[test] // PARSER.8 — diverged accumulated text (no `thought\n` prefix at all)
    ...
    #[test] // PARSER.3 — streaming with no markers at all
    ...
    #[test] // PARSER.2 — multiple reasoning spans back-to-back
    ...
    #[test] // PARSER.9 — paired reasoning + tool call. The reasoning parser
    ...
    //
    // PARSER.1, PARSER.4, PARSER.6, PARSER.7  — Tool-call-only categories. N/A for
    //          a reasoning parser.
    // PARSER.11, PARSER.12 — `tool_choice` and `finish_reason`: tool-call concerns,
    //          N/A for reasoning. (Universal cross-parser gap regardless;
    //          see notes in `tool_calling/gemma4/parser.rs`.)
    // PARSER.14 — Empty / null content: empty input is covered implicitly
    //          via the no-markers passthrough cases (`detect_no_markers_*`,
    //          `streaming_no_markers`).
    // PARSER.15 — Duplicate calls: tool-call concept; N/A. Multi-span
    //          reasoning is the analog and is covered by
    //          `streaming_multiple_reasoning_spans`.
    // PARSER.xml1 / PARSER.xml2 — XML-family only. N/A.
