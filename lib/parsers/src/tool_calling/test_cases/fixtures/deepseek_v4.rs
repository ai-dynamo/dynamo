...
/// DeepSeek V4 (DSML) — same DSML grammar as V3.2 but uses
/// `<｜DSML｜tool_calls>` instead of `<｜DSML｜function_calls>`.
pub struct DeepseekV4Fixture;
...
impl ToolCallFixture for DeepseekV4Fixture {
    ...
    fn case_1_single_call(&self, function_name: &str, arguments: &Value) -> FixtureCase<String> {
        ...
        FixtureCase::Sample(format!("<｜DSML｜tool_calls>{invoke}</｜DSML｜tool_calls>"))
    }
    ...
    fn case_5_missing_end_token_recovery(
        ...
        FixtureCase::KnownBroken {
            input: format!("<｜DSML｜tool_calls>{invoke}"),
