...
pub async fn try_tool_call_parse(
    ...
    match config.format {
        ...
        ToolCallParserType::Harmony => {
            let (results, normal_content) = parse_tool_calls_harmony_complete(message, &config.json).await?;
            Ok((results, normal_content))
        }
...
mod tests {
    ...
    async fn parses_single_parameters_object() {
        ...
        let (result, content) = try_tool_call_parse(input, &ToolCallConfig::default()).await.unwrap();
...
<TOOLCALL>[{"name": "get_weather", "arguments": {"location": "San Francisco, CA", "unit": "fahrenheit"}}]</TOOLCALL>"#;
        let (result, content) = detect_and_parse_tool_call(input, Some("nemotron_deci")).await.unwrap();
        assert!(!result.is_empty());
        assert_eq!(result.len(), 1);
...
{"name": "get_weather", "arguments": {"location": "San Francisco, CA", "unit": "fahrenheit"}}
        ...
        let (result, content) = detect_and_parse_tool_call(input, Some("hermes")).await.unwrap();
        assert_eq!(content, Some("".to_string()));
        assert!(!result.is_empty());
        assert_eq!(result.len(), 1);
        let (name, args) = extract_name_and_args(result[0].clone());
...
"#;
    ...
    async fn test_meta_llama_llama31_8b_instruct_simple() {
        ...
        let (result, content) = try_tool_call_parse(input, &ToolCallConfig::mistral()).await.unwrap();
        assert_eq!(content, Some("".to_string()));
        assert!(!result.is_empty());
        assert_eq!(result.len(), 1);
        let (name, args) = extract_name_and_args(result[0].clone());
    ...
    async fn test_meta_llama_llama31_8b_instruct_with_new_lines() {
        ...
        "#;
        let (result, content) = detect_and_parse_tool_call(input, Some("llama3_json")).await.unwrap();
        assert_eq!(content, Some("".to_string()));
        assert!(!result.is_empty());
        assert_eq!(result.len(), 1);
        let (name, args) = extract_name_and_args(result[0].clone());
...
Remember, San Francisco weather can be quite unpredictable, particularly with its famous fog, which can significantly lower temperatures. Always check a local weather forecast for the most accurate and up-to-date information."#;
    ...
    async fn test_phi4_single_function_call() {
        ...
        let (result, content) = detect_and_parse_tool_call(input, Some("phi4")).await.unwrap();
        assert_eq!(content, Some("".to_string()));
        assert_eq!(result.len(), 1);
        let (name, args) = extract_name_and_args(result[0].clone());
  ...
  {"name": "get_population", "arguments": {"city": "Warsaw"}}
    ...
    async fn test_phi4_token_leak_edge_case() {
        ...
        let input = r#"functools"#;
        let (result, _content) = detect_and_parse_tool_call(input, Some("phi4")).await.unwrap();
    ...
    async fn test_phi4_funk_variations() {
        ...
        for test_input in test_cases {
            let (result, content) = detect_and_parse_tool_call(test_input, Some("phi4")).await.unwrap();
            assert_eq!(
                result.len(),
                0,
    ...
    async fn test_pythonic_parser_basic_with_constants() {
        ...
        let (result, content) = detect_and_parse_tool_call(input, Some("pythonic")).await.unwrap();
        assert_eq!(content, Some("".to_string()));
        assert_eq!(result.len(), 2);
        let (name, args) = extract_name_and_args(result[0].clone());
        assert_eq!(name, "get_weather");
    ...
    async fn test_harmony_parser_basic() {
        ...
        let (result, content) = detect_and_parse_tool_call(input, Some("harmony")).await.unwrap();
        assert_eq!(
            content,
    ...
    async fn test_deepseek_v3_1_parser_basic() {
        ...
        let (result, content) = detect_and_parse_tool_call(input, Some("deepseek_v3_1")).await.unwrap();
        assert_eq!(content, Some("".to_string()));
        assert_eq!(result.len(), 2);
        let (name, args) = extract_name_and_args(result[0].clone());
        assert_eq!(name, "get_current_weather");
