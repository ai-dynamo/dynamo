...
mod tests {
    ...
    #[test] // PARSER.1 (with declared `ToolDefinition` tools provided)
    ...
    #[test] // PARSER.2, PARSER.4, PARSER.5
    ...
    #[test] // PARSER.7 — special characters in arg values
    ...
    #[test] // PARSER.2 — parallel calls, zero-spacing edge case
    ...
    #[test] // PARSER.7 — newlines in arg values
    ...
    /// `PARSER.4` — call missing its `<|tool_call_end|>` while the outer
    ...
    ///
    /// TODO(PARSER.4) — BUG, NEEDS FIX: a real call is silently dropped when
    ...
    /// `PARSER.4` — middle-call truncation. Three calls A, B, C inside a
    /// complete section; B is missing its `<|tool_call_end|>`. Does B's
    /// body bleed into C, or are both lost? Pin whichever today does.
    ///
    /// TODO(PARSER.4) — BUG, NEEDS FIX: B's args swallow all of C's raw
    ...
    /// catch accidental in-parser filtering. Real PARSER.11 coverage lives at
    ...
    /// stream-end reason. Real PARSER.12 coverage (stop / tool_calls / length
    ...
    /// `PARSER.14` — empty / null content variants. Empty section already
    ...
    /// `PARSER.15` — duplicate calls (same function name twice in one section).
