...
fn commentary_block_regex() -> &'static Regex {
    COMMENTARY_BLOCK_REGEX.get_or_init(|| {
        ...
        // (`\z`, the bare-envelope PARSER.5 variant where the model never
...
mod tests {
    ...
    /// PARSER.6 — empty args. A no-arg harmony call (`{}`) must still surface
    /// the function name.
    #[tokio::test] // PARSER.6 — gpt-oss
