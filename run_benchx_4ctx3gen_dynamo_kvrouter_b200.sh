#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.4ctx3gen.dynamo.kvrouter
#SBATCH --nodes=1
...
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 4 ctx worker(s) (GPUs 0-3) + 3 gen worker(s) (GPUs 4-6)
#
...
# --- 3 gen worker(s) on $NODE0 GPUs 4-6 (decode) ---
GEN_PIDS=()
for GPU in 4 5 6; do
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE0}:8084,${NODE0}:8085,${NODE0}:8086,${NODE0}:8087" \
