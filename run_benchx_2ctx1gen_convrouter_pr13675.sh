#!/bin/bash
#SBATCH --job-name=core_dlfw_ci-benchx.2ctx1gen.convrouter.pr13675
#SBATCH --nodes=2
#SBATCH --partition=gb200
#SBATCH --account=core_dlfw_ci
#SBATCH --time=04:00:00
#SBATCH --output=bench/logs/run_benchx_2ctx1gen_convrouter_pr13675_%j.log
#SBATCH --error=bench/logs/run_benchx_2ctx1gen_convrouter_pr13675_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 2 ctx + 1 gen with ConversationRouter,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend) instead of
# trtllm-serve disaggregated.
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 2 ctx workers (GPUs 0-1)
...
#
# Submit:
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0 bench/run_benchx_2ctx1gen_convrouter_pr13675.sh
# =============================================================================
...
EXP_NAME="run_benchx_2ctx1gen_convrouter_pr13675_${HCTAG}_c${CONCURRENCY}"
...
# --- 2 ctx workers on $NODE0 GPUs 0-1 (prefill) ---
CTX_PIDS=()
for GPU in 0 1; do
...
python3 "$REPO_DIR/bench/capture_metrics.py" \
  --endpoints "${NODE0}:${DYN_SYS_PORT_CTX_0},${NODE0}:${DYN_SYS_PORT_CTX_1},${NODE1}:${DYN_SYS_PORT_GEN}" \
