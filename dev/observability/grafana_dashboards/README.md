...
- `dynamo.json` - Engine-agnostic per-model view across the Dynamo stack: frontend KPIs (RPS, TTFT/ITL/E2E percentiles, ISL/OSL distributions, outcome breakdown), KV-router internals (per-worker active blocks, hit rate, routing-overhead breakdown), and worker-side request rate/duration. Filterable by `model`. Includes content mirrored from the NVCF deploy team's per-model dashboard.
