#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.3ctx3gen.dynamo.kvrouter
#SBATCH --nodes=1
...
#SBATCH --output=bench/logs/run_benchx_3ctx3gen_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_3ctx3gen_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 3 ctx + 3 gen with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 3 ctx worker(s) (GPUs 0-2) + 3 gen worker(s) (GPUs 3-5)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx3gen_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx3gen_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_3ctx3gen_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
# --- 3 gen worker(s) on $NODE0 GPUs 3-5 (decode) ---
GEN_PIDS=()
for GPU in 3 4 5; do
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE0}:8085,${NODE0}:8086,${NODE0}:8087" \
