...
对于 Kubernetes，请在 Frontend service 上设置 `DYN_ROUTER_MODE=kv`。对于事件驱动的 KV 状态，请使用 [Router Operations](/dynamo/dev/knowledge-base/modular-components/router/router-operations#additional-notes) 中描述的后端专用 flag，配置 backend worker 发布 KV cache 事件。仅当你希望使用近似的 cache 状态预测时，才使用 `--no-router-kv-events`。
...
有关部署模式和快速开始步骤，请参阅 [Router Guide](/dynamo/dev/knowledge-base/modular-components/router/router-guide)。有关 CLI 参数和调优指南，请参阅 [Configuration and Tuning](/dynamo/dev/knowledge-base/modular-components/router/configuration-and-tuning)。有关 A/B 基准测试，请参阅 [KV Router A/B Benchmarking Guide](/dynamo/dev/recipes/benchmarks/kv-router-ab-testing)。
...
- **[Router Guide](/dynamo/dev/knowledge-base/modular-components/router/router-guide)**：部署模式、快速开始和页面地图
- **[Routing Concepts](/dynamo/dev/knowledge-base/modular-components/router/routing-concepts)**：成本模型和 worker 选择行为
- **[Configuration and Tuning](/dynamo/dev/knowledge-base/modular-components/router/configuration-and-tuning)**：Router flag、传输模式和指标
- **[分离式服务](/dynamo/dev/knowledge-base/modular-components/router/disaggregated-serving)**：Prefill 和 decode 路由设置
- **[Router Operations](/dynamo/dev/knowledge-base/modular-components/router/router-operations)**：副本、持久化和恢复
- **[Router Examples](/dynamo/dev/knowledge-base/modular-components/router/router-examples)**：Python API 用法、K8s 示例和自定义路由模式
- **[Router Testing](/dynamo/dev/knowledge-base/modular-components/router/router-testing)**：从 Rust 单元测试到基于 fixture 的 replay 和完整进程 E2E 的测试层级
- **[Standalone Indexer](/dynamo/dev/knowledge-base/modular-components/router/standalone-indexer)**：将 KV indexer 作为单独服务运行，以便独立扩缩容
- **[Router Design](/dynamo/dev/knowledge-base/modular-components/router/router-design)**：架构细节、算法和事件传输模式
