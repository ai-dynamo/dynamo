...
事件驱动的 KV routing 需要同时配置 frontend 和每个执行 prefill 的 worker：
...
2. 在 backend worker 上启用 KV 事件发布。对于 vLLM，请将 `--kv-events-config` 传递给每个 aggregated worker 或 disaggregated prefill worker：
...
不要将 `--kv-events-config` 传递给 vLLM 的 decode-only worker。对于 SGLang，请传递其 backend 专用的 `--kv-events-config`；对于 TensorRT-LLM，请传递 `--publish-events-and-metrics`。有关所有 backend 设置，请参阅 [Router Operations](../../../../../../docs/components/router/router-operations.md#additional-notes)。

> [!TIP]
> 对于 vLLM worker，建议从 `--stream-interval 20` 开始，以减少负载下 host 侧 engine 和 bridge 的开销。此设置用于提高 host 效率，并非 KV routing 的必要条件。有关 streaming 粒度的权衡，请参阅 [Recommended Stream Interval](../../../../../../docs/backends/vllm/vllm-reference-guide.md#recommended-stream-interval)。

对于 Kubernetes，请在 Frontend service 上设置 `DYN_ROUTER_MODE=kv`，并将 backend 事件 flag 添加到执行 prefill 的 worker。对于不使用 worker 事件的近似 routing，请省略 backend 事件 flag，并在 frontend 上设置 `--no-router-kv-events` 或 `DYN_ROUTER_USE_KV_EVENTS=false`。
