...
事件驱动的 KV routing 需要同时配置 frontend 和参与 KV routing 的 backend worker：
...
不要将 `--kv-events-config` 传递给 vLLM 的 decode-only worker。对于共享 network namespace 的 worker，请使用唯一的 ZMQ endpoint 端口。对于 TensorRT-LLM，请传递 `--publish-events-and-metrics`。有关所有 backend 设置，请参阅 [Router Operations](../../../../../../docs/components/router/router-operations.md#additional-notes)。

> [!TIP]
> 对于 vLLM 和 SGLang worker，建议从 `--stream-interval 20` 开始，以减少负载下 host 侧 engine 和 bridge 的开销。此设置用于提高 host 效率，并非 KV routing 的必要条件。有关 streaming 粒度的权衡，请参阅 [vLLM](../../../../../../docs/backends/vllm/vllm-reference-guide.md#recommended-stream-interval) 和 [SGLang](../../../../../../docs/backends/sglang/sglang-reference-guide.md#recommended-stream-interval) reference guide。

对于 Kubernetes，请在 Frontend service 上设置 `DYN_ROUTER_MODE=kv`，并按照上述说明将 backend 事件 flag 添加到相应的 vLLM 或 SGLang worker。对于不使用 worker 事件的近似 routing，请省略 backend 事件 flag，并在 frontend 上设置 `--no-router-kv-events` 或 `DYN_ROUTER_USE_KV_EVENTS=false`。
