...
| 内存管理 | Dynamo KV Block Manager、[LMCache](/dynamo/dev/cli/kv-cache-offloading/overview)、[SGLang HiCache](/dynamo/dev/backends/sg-lang/hi-cache)、[FlexKV](/dynamo/dev/cli/kv-cache-offloading/overview) |
...
- [KV cache offloading](/dynamo/dev/kubernetes/kv-cache-offloading/overview) 通过将 KV 缓存从 HBM 移动到主机内存、本地磁盘或远程存储等成本更低的存储层，扩展可用上下文窗口。复用预计算状态可改善 TTFT、降低总体拥有成本（TCO），并支持更长上下文处理。
...
Dynamo 提供内置指标、分布式追踪和日志，用于监控推理部署。请参阅 [Observability Guide](https://github.com/ai-dynamo/dynamo/blob/0718004e5d075c6eed352ae8a53fb48df4554067/docs/fern/pages/kubernetes/installation/observability.md) 了解设置详情。
...
- [KV Cache Offloading](/dynamo/dev/kubernetes/kv-cache-offloading/overview) -- 设置多层内存管理
