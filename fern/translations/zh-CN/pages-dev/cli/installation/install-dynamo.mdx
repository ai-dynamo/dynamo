...
如需运行说明，请参阅各后端指南：[SGLang](/dynamo/dev/backends/sg-lang/overview) |
[TensorRT-LLM](/dynamo/dev/backends/tensor-rt-llm/overview) | [vLLM](/dynamo/dev/backends/v-llm/overview)
...
本指南使用 `--discovery-backend file`。如需设置 etcd，请参阅[服务发现](/dynamo/dev/kubernetes/kubernetes-operator/service-discovery)。
...
请参阅 [Kubernetes 中的模型缓存](/dynamo/dev/kubernetes/model-deployment/model-loading/model-caching#option-2-modelexpress-p2p-distribution)
    了解正确配置。
...
运行 `nvidia-smi` 检查你的驱动版本。Dynamo 对 CUDA 12 需要驱动 575.51.03+，对 CUDA 13 需要驱动 580.00.03+。B300/GB300 GPU 需要 CUDA 13。完整要求请参阅[支持矩阵](/dynamo/dev/reference/compatibility)。
    ...
    **容器运行但未检测到 GPU**
    ...
    # Wrong -- no GPU access
    docker run --network host --rm -it nvcr.io/nvidia/ai-dynamo/sglang-runtime:1.4.0
    ```
...
- [后端指南](/dynamo/dev/backends/sg-lang/overview) -- 后端特定配置和功能
