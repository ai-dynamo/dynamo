...
docker run --gpus all --rm -it \
  nvcr.io/nvidia/ai-dynamo/vllm-runtime:1.4.1 \
  /bin/bash
```
...
To build KVBM from source, see the detailed instructions in the [KVBM bindings README](https://github.com/ai-dynamo/dynamo/tree/v1.4.1/lib/bindings/kvbm/README.md#build-from-source).
...
```bash
docker run --gpus all --network host --rm -it nvcr.io/nvidia/ai-dynamo/vllm-runtime:1.4.1
```
...
```bash
docker run --gpus all --network host --rm -it nvcr.io/nvidia/ai-dynamo/tensorrtllm-runtime:1.4.1
```
