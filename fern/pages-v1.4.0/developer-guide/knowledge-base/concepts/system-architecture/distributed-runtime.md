...
[Request Migration](../../../../kubernetes/fault-tolerance/request-migration.md#tune-temporary-worker-inhibition).
...
The user can decide which load balancing strategy to use when calling the `Endpoint` from the `Client`, which is done in [push_router.rs](https://github.com/ai-dynamo/dynamo/tree/v1.4.0/lib/runtime/src/pipeline/network/egress/push_router.rs). Dynamo supports three load balancing strategies:
