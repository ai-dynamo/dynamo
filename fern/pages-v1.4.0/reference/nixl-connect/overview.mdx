...
subtitle: Python library for dynamic, RDMA-accelerated data transfers between workers in a Dynamo graph.
---
...
All operations using the NIXL Connect library begin with the [`Connector`](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-connector) class and the type of operation required.
...
In the diagram below, Local creates a [`WritableOperation`](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-writableoperation) intended to receive data from Remote.
Local then sends metadata about the requested operation to Remote.
Remote then uses the metadata to create a [`WriteOperation`](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-writeoperation) which will perform the GPU Direct RDMA memory transfer, when available, from Remote's GPU memory to Local's GPU memory.
...
In the case of the [Dynamo Multimodal Disaggregated Example](../../developer-guide/knowledge-base/modular-components/backends/vllm/multimodal.md):
...
See [NixlReadEmbeddingSender](https://github.com/ai-dynamo/dynamo/blob/v1.4.0/components/src/dynamo/common/multimodal/embedding_transfer.py),
for how they coordinate directly with the Encode Worker by creating a [`ReadableOperation`](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-readableoperation),
...
See [NixlReadEmbeddingReceiver](https://github.com/ai-dynamo/dynamo/blob/v1.4.0/components/src/dynamo/common/multimodal/embedding_transfer.py),
for how the resulting embeddings are registered with the NIXL subsystem by creating a [`Descriptor`](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-descriptor),
a [`ReadOperation`](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-readoperation) is created using the metadata provided by the requesting worker,
and the worker awaits for the data transfer to complete for yielding a response.


## Python Classes

  - [Connector](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-connector)
  - [Descriptor](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-descriptor)
  - [Device](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-device)
  - [ReadOperation](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-readoperation)
  - [ReadableOperation](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-readableoperation)
  - [WritableOperation](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-writableoperation)
  - [WriteOperation](../api/python/nixl_connect.mdx#api-dynamo-nixl-connect-writeoperation)
...
## References
  ...
  - [Dynamo Multimodal Example](https://github.com/ai-dynamo/dynamo/tree/v1.4.0/examples/backends/vllm/launch)
