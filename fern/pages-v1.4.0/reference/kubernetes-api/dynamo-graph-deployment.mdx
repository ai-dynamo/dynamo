...
A DGD is the resource that persists and serves traffic. Author one directly when you have a known-good configuration or a tuned [recipe](https://github.com/ai-dynamo/dynamo/tree/v1.4.0/recipes); generate one from intent with a [`DynamoGraphDeploymentRequest`](dynamo-graph-deployment-request.mdx) (DGDR).
