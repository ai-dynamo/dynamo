...
<Steps>
  <Step title="Prerequisites">
    ...
    Full manifests and MinIO setup: [Kubernetes LoRA deployment example](https://github.com/ai-dynamo/dynamo/tree/v1.4.0/examples/backends/vllm/deploy/lora).
  </Step>
  ...
  <Step title="Deploy a LoRA-enabled worker">
    ...
    Adapted from the [`agg_lora.yaml` example](https://github.com/ai-dynamo/dynamo/blob/v1.4.0/examples/backends/vllm/deploy/lora/agg_lora.yaml):
    ...
    **vLLM worker arguments**
    ...
    <Warning>Set `--max-lora-rank` to at least your adapter's rank. A lower value causes load failures.</Warning>
...
<Steps>
  <Step title="Prerequisites">
    ...
    See the [local LoRA example](https://github.com/ai-dynamo/dynamo/tree/v1.4.0/examples/backends/vllm/launch/lora) for the full workflow.
  </Step>
...
<AccordionGroup>
  <Accordion title="How LoRA-aware KV routing works">
    ...
    For a local two-worker demo with KV-aware routing, run [`agg_lora_router.sh`](https://github.com/ai-dynamo/dynamo/blob/v1.4.0/examples/backends/vllm/launch/lora/agg_lora_router.sh) and load the adapter on both worker system ports.
  </Accordion>
</AccordionGroup>
