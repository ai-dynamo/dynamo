...
To create the operation, NIXL metadata ([RdmaMetadata](rdma-metadata.mdx)) from a remote worker's [`WritableOperation`](writable-operation.mdx)
along with a matching set of local [`Descriptor`](descriptor.mdx) objects which reference memory to be transferred to the remote worker must be provided.
...
Returns [`OperationStatus`](operation-status.mdx) which provides the current state (aka. status) of the operation.
