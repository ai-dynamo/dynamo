...
**For metrics reference**, see [Metrics Documentation](metrics.mdx).
...
Start the observability stack (Prometheus, Grafana, Tempo, exporters). See [Observability Getting Started](README.mdx#getting-started-quickly) for instructions and prerequisites.
...
- **Tempo** (tracing): Accessible through Grafana's Explore view. See [Tracing Guide](tracing.mdx) for details.
...
After making changes to prometheus.yml, restart the Prometheus service. See [Observability Getting Started](README.mdx#getting-started-quickly) for Docker Compose commands.
...
For specific Docker Compose commands, see [Observability Getting Started](README.mdx#getting-started-quickly).

## Developer Guide

For detailed information on creating custom metrics in Dynamo components, see:

- [Metrics Developer Guide](metrics-developer-guide.mdx)
