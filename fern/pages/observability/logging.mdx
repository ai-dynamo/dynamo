...
For collecting and visualizing logs with Grafana Loki (Kubernetes), or viewing trace context in logs alongside Grafana Tempo, start the observability stack. See [Observability Getting Started](README.mdx#getting-started-quickly) for instructions.
...
**Note:** This section has overlap with [Distributed Tracing with Tempo](tracing.mdx). For trace visualization in Grafana Tempo and persistent trace analysis, see [Distributed Tracing with Tempo](tracing.mdx).
...
- [Log Aggregation in Kubernetes](../kubernetes/observability/logging.mdx)
