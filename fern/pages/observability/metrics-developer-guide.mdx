...
The metrics API is accessible through the `.metrics()` method on runtime, namespace, component, and endpoint objects. See [Runtime Hierarchy](metrics.mdx#runtime-hierarchy) for details on the hierarchical structure.
...
- [Metrics Overview](metrics.mdx)
- [Prometheus and Grafana Setup](prometheus-grafana.mdx)
