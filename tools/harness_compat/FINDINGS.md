...
- Codex 0.144.0 advertised `roots`, answered the fixture's native `roots/list` request with a valid empty roots list, then completed the MCP tool/result continuation and shell-derived file effect through Dynamo in two clean runs: `/ephemeral/harness-compat-artifacts/20260802T0010Z-codex-mcp-roots-rerun` and `/ephemeral/harness-compat-artifacts/20260802T0020Z-codex-mcp-roots-confirmation`. Promote C1l1 to the opt-in weekly sentinel set.
