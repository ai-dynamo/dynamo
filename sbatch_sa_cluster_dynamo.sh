...
# Periodically scrape the frontend's /metrics and append matching rows to a CSV.
# Output format: timestamp,metric_with_labels,value
# Captures: queue depth, inflight, tokenizer latency, TTFT, ITL, throughput,
# per-worker prefill/decode load. Histograms emit _sum and _count rows so the
# user can derive per-interval averages by taking deltas.
start_frontend_metrics_monitor() {
    local output_path="$1"
    local interval="${2:-2}"
    local target_url="http://${HEAD_NODE}:${FRONTEND_PORT}/metrics"
    (
    ...
    SRUN_PIDS+=($!)
    echo "Frontend metrics monitor started (pid=${SRUN_PIDS[-1]}, interval=${interval}s, log=${output_path})"
}
...
start_bg srun \
    ...
    --no-container-mount-home \
    --container-mounts "${SCRIPT_MOUNTS}" \
    bash -c "export ETCD_ENDPOINTS=http://${HEAD_NODE}:${ETCD_PORT} && export NATS_SERVER=nats://${HEAD_NODE}:${NATS_PORT} && export DYN_REQUEST_PLANE=${DYNAMO_REQUEST_PLANE} && export DYN_TCP_POOL_SIZE=${DYN_TCP_POOL_SIZE} && python3 -m dynamo.frontend --http-port ${FRONTEND_PORT} --request-plane ${DYNAMO_REQUEST_PLANE}"
...
start_frontend_metrics_monitor "${LOG_DIR}/frontend_metrics.csv" 2
