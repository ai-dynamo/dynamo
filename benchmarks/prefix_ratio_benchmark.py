...
def aggregate_results(results: List[Dict]) -> Dict:
    ...
    throughput_values = [r["output_token_throughput"]["avg"] for r in results if r is not None]
...
def run_benchmark(
    ...
    logger.info(f"Running benchmark with prefix_ratio={prefix_ratio}, seed={seed}, URLs={urls}")
    ...
    for process, url, artifact_dir in processes:
        ...
        if process.returncode == 0:
            ...
            logger.error(f"Genai-perf failed for URL {url} with error code: {process.returncode}")
...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="6a6f4aa4197940add57724a7707d069478df56b1",
        help="Model name",
    )
    parser.add_argument(
        "--tokenizer",
