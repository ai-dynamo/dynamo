...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Convert CSV file to mooncake format"
    )
    parser.add_argument(
        "--input-file",
        type=str,
...
def apply_speed_ratio(df, speed_ratio):
    ...
    print(f"Original timestamps: {df['Timestamp'].min():.2f} - {df['Timestamp'].max():.2f}")
    print(f"Adjusted timestamps: {adjusted_df['Timestamp'].min():.2f} - {adjusted_df['Timestamp'].max():.2f}")
...
def convert_to_mooncake(df, block_size, num_hash_blocks):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        hash_ids = [random.randint(0, num_hash_blocks) for _ in range(hash_array_length)]
...
def print_statistics(df):
    ...
    if len(df) > 1:
        ...
        if duration_s > 0:
            ...
            print(f"\nRequest Rate:")
            print(f"  Total requests: {len(df)}")
            print(f"  Duration: 0 seconds (all requests at same timestamp)")
            print(f"  Average RPS: N/A")
    else:
        print(f"\nRequest Rate:")
        print(f"  Total requests: {len(df)}")
        print(f"  Average RPS: N/A (only 1 request)")
...
def main():
    ...
    mooncake_df = convert_to_mooncake(adjusted_df, args.block_size, args.num_hash_blocks)
