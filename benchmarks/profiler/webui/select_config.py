...
def generate_config_data(prefill_data, decode_data, args):
    ...
    if prefill_data.num_gpus:
        ...
        chart_data = []
        for i, (gpu, ttft, thpt, label) in enumerate(zip(
    ...
    if decode_data.num_gpus:
        ...
        gpu_groups = {}
        for i, (gpu, itl, thpt, label) in enumerate(zip(
    ...
    if prefill_data.num_gpus and decode_data.num_gpus:
        ...
        data["cost"]["index_mapping"] = {str(k): list(v) for k, v in cost_index_mapping.items()}
...
def pick_config_with_webui(prefill_data, decode_data, args):
            ...
            if plot_type == "cost":
                ...
                if mapping_entry:
                    ...
                    if prefill_idx is not None and decode_idx is not None:
                        logger.info(f"Cost selection determines: Prefill={prefill_idx}, Decode={decode_idx}")
                ...
                if decode_selection["idx"] is not None:
                    logger.info(f"Both selections complete: Prefill={row_idx}, Decode={decode_selection['idx']}")
                ...
                if prefill_selection["idx"] is not None:
                    logger.info(f"Both selections complete: Prefill={prefill_selection['idx']}, Decode={row_idx}")
    ...
    with gr.Blocks(title="Configuration Selection") as demo:
        ...
        demo.load(fn=load_data, inputs=[], outputs=[json_data], js=load_profiling_javascript())
    ...
    logger.info(f"WebUI launched. Waiting for user selection on http://0.0.0.0:{args.webui_port}")
    logger.info("Please select a row from the Cost Analysis table")
    
    # Block and wait for selection
    try:
        selected_prefill_idx, selected_decode_idx = selection_queue.get(timeout=WEB_UI_SELECTION_TIMEOUT)
        logger.info(f"User selected: Prefill={selected_prefill_idx}, Decode={selected_decode_idx}")
