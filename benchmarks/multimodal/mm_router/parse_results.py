...
def find_result_json(artifact_dir: str) -> dict | None:
    ...
    # Prefer the known aiperf export filename first
    for candidate in ["profile_export_aiperf.json", "profile_export.json"]:
        path = os.path.join(artifact_dir, candidate)
        if os.path.exists(path):
            try:
                ...
                pass
    # Fallback: any JSON containing request_latency
...
def extract_metrics(data: dict) -> dict | None:
    ...
    def get(keys, factor=1.0):
        for k in keys:
            ...
            for sub in ("request_latency", "latency", "stats", "results", "metrics"):
                if sub in data and isinstance(data[sub], dict) and k in data[sub]:
    ...
    metrics = {
        ...
        "tput": get_float(
            [
                ...
                "req_per_sec",
                "output_throughput",
                "avg",  # aiperf: request_throughput.avg
            ],
