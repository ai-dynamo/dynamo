...
#
# Automated nsys profiling comparison: frontend routing vs MM router worker.
# Runs 4 cases: {frontend, mm} x {conc1, conc8}
# Each case: launch server under nsys, warmup, send 25 measured requests, stop.
#
# Usage:
#   bash run_nsys_compare.sh
#   CONC_LEVELS=1 bash run_nsys_compare.sh        # only conc=1
#   SCENARIOS=frontend bash run_nsys_compare.sh    # only frontend
#
# Output: ./nsys_compare/
#   frontend_conc1.nsys-rep, frontend_conc8.nsys-rep
#   mm_conc1.nsys-rep, mm_conc8.nsys-rep
...
IFS=',' read -ra SCENARIOS <<< "${SCENARIOS:-frontend,mm}"
...
# Map scenario to launch script
get_launch_script() {
    case "$1" in
        frontend) echo "launch_frontend_routing.sh" ;;
        mm)       echo "launch_mm_router_worker.sh" ;;
        rr)       echo "launch_rr_baseline.sh" ;;
        *)        echo ""; return 1 ;;
    esac
}
...
for scenario in "${SCENARIOS[@]}"; do
    launch_script=$(get_launch_script "${scenario}")
