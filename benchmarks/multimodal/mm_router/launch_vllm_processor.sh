...
#
# Benchmark launch script: MM-aware KV routing without mm_router_worker.
# Matches the worker/model configuration of launch_workers.sh (zd-branch)
# for apples-to-apples comparison.
#
# Architecture (new):
#   Frontend (--dyn-chat-processor vllm, --router-mode kv) --> N vLLM backends
#
# Architecture (old, for reference):
#   Frontend (round-robin) --> MM Router Worker --> N vLLM backends
#
# Usage:
#   ./launch_benchmark_no_mm_worker.sh
#
#   # Override defaults:
#   NUM_WORKERS=4 MODEL=Qwen/Qwen3-VL-8B-Instruct ./launch_benchmark_no_mm_worker.sh
...
# ---------------------------------------------------------------------------
# Configuration — defaults match launch_workers.sh (zd-branch)
# ---------------------------------------------------------------------------
...
# Per-worker ports (same scheme as launch_workers.sh):
#   system port : 18079 + i*2  (18081, 18083, 18085, ...)
...
echo "=== MM Benchmark: No MM Router Worker (${NUM_WORKERS} workers) ==="
echo "Architecture: Frontend(vllm+kv) --> ${NUM_WORKERS}x vLLM backend  [no mm_router_worker]"
...
# ---------------------------------------------------------------------------
# Frontend: vllm chat processor + KV router (no mm_router_worker)
# ---------------------------------------------------------------------------
echo "=== Starting frontend (--dyn-chat-processor vllm --router-mode kv) ==="
env "${COMMON_ENV[@]}" \
