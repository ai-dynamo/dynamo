...
- **Zero-config deploy ([DGDR](https://docs.nvidia.com/dynamo/dev/kubernetes-deployment/deployment-guide/dgdr-reference))** *(beta):* Specify model, HW, and SLA in one YAML — AIConfigurator auto-profiles the workload, Planner optimizes the topology, and Dynamo deploys
...
For production multi-node clusters, install the [Dynamo Platform](https://docs.nvidia.com/dynamo/dev/kubernetes-deployment/deployment-guide/model-deployment-guide) and deploy with a single manifest:
...
- **[Service Discovery](https://docs.nvidia.com/dynamo/dev/kubernetes-deployment/deployment-guide/service-discovery)** — K8s-native vs etcd vs file-based discovery
