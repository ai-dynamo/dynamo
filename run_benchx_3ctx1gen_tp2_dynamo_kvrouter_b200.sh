#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.3ctx1gen_tp2.dynamo.kvrouter
#SBATCH --nodes=1
...
#SBATCH --output=bench/logs/run_benchx_3ctx1gen_tp2_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_3ctx1gen_tp2_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 3 ctx TP1 + 1 gen TP2 with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 3 ctx worker(s) (GPUs 0-2) TP1
#   NODE0 — gen worker 0 GPUs 3,4 (TP2)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx1gen_tp2_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx1gen_tp2_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_3ctx1gen_tp2_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
GEN_PIDS=()
# --- decode worker 0 on $NODE0 GPUs 3,4 (TP2) ---
GEN_PORT=8085
echo "[$(date +%H:%M:%S)] Starting gen worker 0 on $NODE0 GPUs 3,4 (DYN_SYSTEM_PORT=${GEN_PORT})..."
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE0}:8085" \
