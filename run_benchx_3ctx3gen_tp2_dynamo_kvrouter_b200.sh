#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.3ctx3gen_tp2.dynamo.kvrouter
#SBATCH --nodes=2
...
#   NODE1 — gen worker 2 GPUs 4,5 (TP2)
#
...
# --- decode worker 2 on $NODE1 GPUs 4,5 (TP2) ---
GEN_PORT=8087
echo "[$(date +%H:%M:%S)] Starting gen worker 2 on $NODE1 GPUs 4,5 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE1 --mpi=pmix \
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE1}:8085,${NODE1}:8086,${NODE1}:8087" \
