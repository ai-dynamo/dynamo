#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.2ctx3gen_tp2.dynamo.kvrouter
#SBATCH --nodes=1
...
# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 2 ctx TP1 + 3 gen TP2 with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 2 ctx worker(s) (GPUs 0-1) TP1
#   NODE0 — gen worker 0 GPUs 2,3 (TP2)
#   NODE0 — gen worker 1 GPUs 4,5 (TP2)
#   NODE0 — gen worker 2 GPUs 6,7 (TP2)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_2ctx3gen_tp2_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_2ctx3gen_tp2_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_2ctx3gen_tp2_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
GEN_PIDS=()
# --- decode worker 0 on $NODE0 GPUs 2,3 (TP2) ---
GEN_PORT=8085
echo "[$(date +%H:%M:%S)] Starting gen worker 0 on $NODE0 GPUs 2,3 (DYN_SYSTEM_PORT=${GEN_PORT})..."
...
# --- decode worker 1 on $NODE0 GPUs 4,5 (TP2) ---
GEN_PORT=8086
echo "[$(date +%H:%M:%S)] Starting gen worker 1 on $NODE0 GPUs 4,5 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
...
# --- decode worker 2 on $NODE0 GPUs 6,7 (TP2) ---
GEN_PORT=8087
echo "[$(date +%H:%M:%S)] Starting gen worker 2 on $NODE0 GPUs 6,7 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8085,${NODE0}:8086,${NODE0}:8087" \
