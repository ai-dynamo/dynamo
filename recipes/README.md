...
| **[DeepSeek-V4.1-Flash](deepseek-v4.1-flash/vllm/)** | vLLM | Agg + Disagg | 8x / 12x B200 or GB200 | ✅ | ✅ | MoE (1M ctx), MXFP4 experts + MXFP4 sparse-indexer KV, TP4 + EP + EPLB, DSpark spec dec, prefix-cache retention tuning, KV-aware routing with decode-load balancing, reasoning + tool calling; plus disagg 2P1D over NIXL/UCX; agentic trace profile | ❌ |
| **[DeepSeek-V4.1-Flash](deepseek-v4.1-flash/sglang/)** | SGLang | Agg + Disagg | 8x GB200 | ✅ | ❌ | Day-0, not benchmarked — MoE, FP8 dense + FP4 experts + FP8 KV, 1M ctx, TP4 + EP4, KV-aware routing, DSpark spec dec (agg only), reasoning + tool calling; plus disagg 1P1D with Mooncake KV transfer over TCP (RDMA on GKE) | ❌ |
