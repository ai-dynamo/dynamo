...
- Tolerations may be needed depending on node taints (check `kubectl describe node`)
- Model download: ~865 GB, ensure PVC has sufficient space

### Other IB Clusters

The UCX configuration should work on any IB cluster with ConnectX NICs. Adjust `UCX_NET_DEVICES` if your primary IB device has a different name.
