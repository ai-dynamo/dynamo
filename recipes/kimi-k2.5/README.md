...
3. **HuggingFace token** with access to `nvidia/Kimi-K2.5-NVFP4` and `nvidia/Kimi-K2.5-Thinking-Eagle3`:
   ...
   kubectl create secret generic hf-token-secret \
     ...
     -n ${NAMESPACE}
   ```
4. A Dynamo TRT-LLM runtime image (i.e. v1.1.1~) that supports Kimi-K2.5 + Eagle3. The manifests ship with `nvcr.io/nvidia/ai-dynamo/tensorrtllm-runtime:1.5.0` as a placeholder — update the `image:` fields in each `deploy.yaml` before applying.
