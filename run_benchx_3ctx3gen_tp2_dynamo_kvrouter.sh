#!/bin/bash
#SBATCH --job-name=core_dlfw_ci-benchx.3ctx3gen_tp2.dynamo.kvrouter
#SBATCH --nodes=3
#SBATCH --partition=gb200
#SBATCH --account=core_dlfw_ci
#SBATCH --time=04:00:00
#SBATCH --output=bench/logs/run_benchx_3ctx3gen_tp2_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_3ctx3gen_tp2_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 3 ctx TP1 + 3 gen TP2 with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 3 ctx worker(s) (GPUs 0-2) TP1
#   NODE1 — gen worker 0 GPUs 0,1 (TP2)
#   NODE1 — gen worker 1 GPUs 2,3 (TP2)
#   NODE2 — gen worker 2 GPUs 0,1 (TP2)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx3gen_tp2_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx3gen_tp2_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_3ctx3gen_tp2_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
GEN_CONFIG_SRC="${REPO_DIR}/bench/gen_config_tp2.yaml"
...
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE1 --mpi=pmix \
  ...
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=0,1 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
...
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE1 --mpi=pmix \
  ...
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=2,3 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
    export DYN_SYSTEM_PORT=${GEN_PORT} && \
    trtllm-llmapi-launch python3 -m dynamo.trtllm \
      --model-path $MODEL_PATH --served-model-name $MODEL \
      --disaggregation-mode decode \
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE1}:8085,${NODE1}:8086,${NODE2}:8087" \
