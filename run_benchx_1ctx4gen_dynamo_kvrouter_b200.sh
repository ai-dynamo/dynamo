#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.1ctx4gen.dynamo.kvrouter
#SBATCH --nodes=1
...
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 1 ctx worker(s) (GPUs 0-0) + 4 gen worker(s) (GPUs 1-4)
#
...
# --- 4 gen worker(s) on $NODE0 GPUs 1-4 (decode) ---
GEN_PIDS=()
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8085,${NODE0}:8086,${NODE0}:8087,${NODE0}:8088" \
