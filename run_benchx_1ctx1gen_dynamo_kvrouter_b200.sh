#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.1ctx1gen.dynamo.kvrouter
#SBATCH --nodes=1
...
# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 1 ctx + 1 gen with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# 1:1 with kv router is mostly a sanity check (single ctx → no real routing
# choice), but exercises the same code path as multi-ctx and serves as a baseline
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_1ctx1gen_dynamo_kvrouter.sh
