#!/bin/bash
#SBATCH --job-name=core_dlfw_ci-benchx.2ctx4gen.dynamo.kvrouter
#SBATCH --nodes=2
#SBATCH --partition=gb200
#SBATCH --account=core_dlfw_ci
#SBATCH --time=04:00:00
#SBATCH --output=bench/logs/run_benchx_2ctx4gen_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_2ctx4gen_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 2 ctx + 4 gen with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 2 ctx worker(s) (GPUs 0-1)
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_2ctx4gen_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_2ctx4gen_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_2ctx4gen_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
# --- 4 gen worker(s) on $NODE1 GPUs 0-3 (decode) ---
GEN_PIDS=()
...
# ==============================================================================
echo "[$(date +%H:%M:%S)] Waiting for servers (2 prefill + 4 decode)..."
if ! wait_for_dynamo_workers "${NODE0}" "${FRONTEND_PORT}" 2 4 2700 60; then
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE1}:8085,${NODE1}:8086,${NODE1}:8087,${NODE1}:8088" \
    --labels "ctx_g0,ctx_g1,gen_g0,gen_g1,gen_g2,gen_g3" \
