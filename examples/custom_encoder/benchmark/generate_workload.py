...
"""Generate the nine-image, 100-request Qwen3-VL CustomEncoder workload."""
...
IMAGE_SIZES = [(299, 299)] * 5 + [(500, 500)] * 4
...
def _make_images(image_dir: Path) -> list[Path]:
    ...
    for index, size in enumerate(IMAGE_SIZES):
        color = ((37 * index) % 256, (83 * index) % 256, (149 * index) % 256)
        ...
        draw.text((size[0] // 3, size[1] // 2), f"image {index}", fill="white")
        path = image_dir / f"image_{index:02d}_{size[0]}x{size[1]}.jpg"
...
def _calibrate_prompt(
    ...
    for repeat_count in range(1025):
        ...
        candidates.append((abs(mean_isl - target_mean_isl), prompt, mean_isl))
        if mean_isl >= target_mean_isl:
            break
    _, prompt, mean_isl = min(candidates, key=lambda candidate: candidate[0])
    return prompt, mean_isl
...
def generate_workload(
    ...
    dataset_path = output_dir / "qwen3_vl_9image_100req.jsonl"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target-mean-isl", type=int, default=515)
