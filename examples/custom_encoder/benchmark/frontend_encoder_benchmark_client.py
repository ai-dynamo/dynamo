...
"""Retry-free OpenAI client for the inline-versus-frontend encoder benchmark."""
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if audit["unique_images"] != len(requests):
        raise ValueError("every request must use a unique image")
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        ...
        async def worker() -> None:
            while True:
                ...
                observed_tokens = (payload.get("usage") or {}).get("completion_tokens")
        ...
        request_window_started = time.perf_counter()
        await asyncio.gather(*(worker() for _ in range(args.concurrency)))
        request_window_s = time.perf_counter() - request_window_started

    successes = sum(completions)
    if successes != len(requests):
        raise RuntimeError(f"only {successes}/{len(requests)} requests completed")
    client_wall_time_s = time.perf_counter() - client_started
    result = {
        ...
        "request_throughput": len(requests) / request_window_s,
        "client_wall_time_s": client_wall_time_s,
        "client_wall_throughput": len(requests) / client_wall_time_s,
        "latency_s": {
