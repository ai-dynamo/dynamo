...
# Compare inline custom encoding with frontend -> E -> aggregated PD. Each cell
# starts fresh server processes; the two topologies alternate across 3 repeats.

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
RESULT_ROOT="${OUTPUT_DIR:-/dynamo-tmp/logs/$(date -u +%m-%d)/custom-encoder-frontend-comparison-textisl644}"
...
: "${DYNAMO_BENCHMARK_DIFF_SHA256:?set DYNAMO_BENCHMARK_DIFF_SHA256}"
...
metadata = {
    ...
    "working_diff_sha256": os.environ["DYNAMO_BENCHMARK_DIFF_SHA256"],
...
if [[ ! -f "$WORKLOAD_ROOT/measured/workload_manifest.json" ]]; then
    python -m examples.custom_encoder.benchmark.fixed_text_image_workload generate \
...
run_cell() {
    ...
    wait_ready
    grep -q '^BENCHMARK_GPU_MEMORY_UTILIZATION=0.4$' "$output_dir/server.log"

    python -m examples.custom_encoder.benchmark.frontend_encoder_benchmark_client \
        --input "$WARMUP_INPUT" \
...
orders=("inline frontend" "frontend inline" "inline frontend")
...
python -m examples.custom_encoder.benchmark.summarize_frontend_encoder_comparison \
    "$RESULT_ROOT" | tee "$RESULT_ROOT/summary.log"
