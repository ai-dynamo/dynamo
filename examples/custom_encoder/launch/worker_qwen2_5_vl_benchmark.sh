...
#   frontend -> aggregated Qwen2.5-1.5B PD -> custom Encode worker -> PD
# E owns both the vision encoder and LinearEmbedsAdapter. PD sends each complete
# prompt to exactly one E and consumes the same handoff used by frontend routing.
...
python -m dynamo.vllm \
  ...
  --custom-encoder-routing-mode worker \
