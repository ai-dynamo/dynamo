...
Aggregated and disaggregated DeepSeek-V3.1 (FP8) on B200 GPUs, using **Dynamo-native KV routing** (no Inference Gateway).
...
|---|---|---|
| `dgd-agg.yaml` | DynamoGraphDeployment | Aggregated: Frontend (KV router) + Worker (TP=8, EP=8, 8 GPUs) |
| `dgd-disagg-1p1d.yaml` | DynamoGraphDeployment | Disaggregated 1P1D: Frontend + Prefill + Decode, NIXL/RoCE, EAGLE spec decode |
| `perf-pod.yaml` | Pod | Long-lived pod with aiperf for benchmarking |
| `nccl-test-2node.yaml` | MPIJob | 2-node NCCL all_reduce test to validate RDMA/RoCE |
| `install-mpi-operator.sh` | Script | Installs Kubeflow training-operator for MPIJob support |
...
## Networking Checkpoint (NCCL Test)

Before deploying disaggregated workloads, validate that multi-node RDMA/RoCE is working. This step is optional for aggregated (single-node) deployments.
...
```bash
kubectl apply -f nccl-test-2node.yaml -n dynamo-system
...
| Component | Image |
|---|---|
| Frontend + Worker | `nvcr.io/nvidia/ai-dynamo/sglang-runtime:0.9.1` |
