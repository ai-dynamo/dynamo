...
# Three independent processes: dynamo.frontend, sglang.launch_server,
# dynamo.sglang_grpc. Mirrors agg.sh's CLI surface.
# GPUs: 1
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo "Usage: $0 [--model-path <name>] [--enable-otel] [-- <extra sglang flags>]"
            exit 0
            ;;
        *) EXTRA_ARGS+=("$1"); shift ;;
    esac
