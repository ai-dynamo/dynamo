...
On an 8-GPU B200 node serving `Qwen/Qwen3-VL-30B-A3B-Instruct-FP8` with concurrent HTTP image requests and moderate image reuse across workers, MM-aware routing delivers significant throughput and latency improvements over round-robin(the default router mode):
...
- **~1.6× lower average latency** and **~3× lower median latency** — cache-warm requests complete substantially faster
