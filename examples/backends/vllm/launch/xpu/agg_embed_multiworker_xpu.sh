...
#
# Spawns 1 frontend + 2 embedding workers, each on its own XPU and its own
...
print_launch_banner --no-curl "Launching Multi-Worker Embeddings (2 XPUs)" "${MODEL1} + ${MODEL2}" "$HTTP_PORT"
...
print_curl_footer <<CURL
  ...
  # Per-worker metrics:
  curl http://localhost:${SYSTEM_PORT1}/metrics  # worker on XPU 0 (${MODEL1})
  curl http://localhost:${SYSTEM_PORT2}/metrics  # worker on XPU 1 (${MODEL2})
CURL
...
# is the same; only the per-worker model + XPU + system port differ.
common_worker_args=(
    --embedding-worker
    --runner pooling
    --dtype float32
