...
#
# Disaggregated 1P1D multimodal (image) serving via the engine RPC sidecar
# (2 GPUs).
#
# Sidecar analogue of disagg_multimodal_p_d.sh. Five processes:
#   1. Dynamo frontend (HTTP ingress)
#   2. Native vLLM decode engine  (kv_role=kv_consumer) + engine RPC gRPC server
#   3. Dynamo decode sidecar worker -> talks to (2)
#   4. Native vLLM prefill engine (kv_role=kv_producer) + engine RPC gRPC server
...
# engine RPC gRPC ports (one per engine).
DECODE_RPC_PORT="${DECODE_RPC_PORT:-50051}"
PREFILL_RPC_PORT="${PREFILL_RPC_PORT:-50052}"
...
print_launch_banner --no-curl "Launching Sidecar Disaggregated Multimodal Serving (2 GPUs)" "$MODEL" "$HTTP_PORT" \
    "Backend:     dynamo-vllm-sidecar 1P1D over engine RPC (URL-passthrough)" \
...
# 2. Decode engine: vllm-rs serve, kv_consumer, engine RPC on DECODE_RPC_PORT.
...
CUDA_VISIBLE_DEVICES=0 vllm-rs serve "$MODEL" \
    ...
    --engine-rpc-port "$DECODE_RPC_PORT" \
    --enforce-eager \
...
DYN_SYSTEM_PORT=${DYN_SYSTEM_PORT1:-8081} \
    dynamo-vllm-sidecar \
    --engine-rpc-endpoint "127.0.0.1:${DECODE_RPC_PORT}" &

# 4. Prefill engine: vllm-rs serve, kv_producer, engine RPC on PREFILL_RPC_PORT.
...
CUDA_VISIBLE_DEVICES=1 VLLM_NIXL_SIDE_CHANNEL_PORT=20097 vllm-rs serve "$MODEL" \
    ...
    --engine-rpc-host 127.0.0.1 \
    --engine-rpc-port "$PREFILL_RPC_PORT" \
    --enforce-eager \
...
DYN_SYSTEM_PORT=${DYN_SYSTEM_PORT2:-8082} \
    dynamo-vllm-sidecar \
    --engine-rpc-endpoint "127.0.0.1:${PREFILL_RPC_PORT}" &
