...
#
# Aggregated serving via the engine RPC sidecar + KVBM multi-tier KV cache (1 GPU).
#
# Same three-process shape as sidecar_agg.sh (Dynamo frontend + `vllm-rs serve`
# engine RPC engine + dynamo-vllm-sidecar worker), with KVBM offload enabled.
#
...
# engine RPC gRPC server and the KVBM connector. `--kv-transfer-config` and the
...
# 3. Dynamo sidecar worker (no vllm/kvbm import; engine RPC client only). It
# receives ONLY the endpoint; model, role, and KV config are discovered from the
# engine. KVBM is invisible here; the handshake reports the engine's GPU KV
# block capacity and does not include connector-managed offloaded tiers.
