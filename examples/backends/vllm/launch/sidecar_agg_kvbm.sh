...
#
# Aggregated serving via the vLLM gRPC sidecar + KVBM multi-tier KV cache (1 GPU).
#
# Same three-process shape as sidecar_agg.sh (Dynamo frontend + `vllm-rs serve`
# vLLM gRPC engine + dynamo-vllm-sidecar worker), with KVBM offload enabled.
#
...
# vLLM gRPC server and the KVBM connector. `--kv-transfer-config` and the
