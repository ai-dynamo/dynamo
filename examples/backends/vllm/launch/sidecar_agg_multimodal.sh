...
#
# Aggregated multimodal (image) serving via the engine RPC sidecar (1 GPU).
#
...
#   2. A native `vllm-rs serve` engine (a VLM) exposing the engine RPC gRPC
#      server
...
print_launch_banner --no-curl "Launching Sidecar Aggregated Multimodal Serving (1 GPU)" "$MODEL" "$HTTP_PORT" \
    "Backend:     dynamo-vllm-sidecar over engine RPC (URL-passthrough)" \
...
# engine RPC gRPC server, loading the VLM. The model is positional;
