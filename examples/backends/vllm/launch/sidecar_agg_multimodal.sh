...
#
# Aggregated multimodal (image) serving via the vLLM gRPC sidecar (1 GPU).
#
...
#   2. A native `vllm-rs serve` engine (a VLM) exposing the vLLM gRPC
#      server
...
print_launch_banner --no-curl "Launching Sidecar Aggregated Multimodal Serving (1 GPU)" "$MODEL" "$HTTP_PORT" \
    "Backend:     dynamo-vllm-sidecar over vLLM gRPC (URL-passthrough)" \
...
# vLLM gRPC server, loading the VLM. The model is positional;
