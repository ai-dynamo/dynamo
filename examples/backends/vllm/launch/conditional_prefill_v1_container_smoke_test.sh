...
IMAGE="${IMAGE:-nvcr.io/nvidian/dynamo-dev/karenc:dynamo-condp-refactor-50c15f0dcd-vllm}"
MODEL="${MODEL:-Qwen/Qwen3-0.6B}"
HF_CACHE_DIR="${HF_CACHE_DIR:-${HOME}/.cache/huggingface}"
PROBE_WARMUP_SECONDS="${PROBE_WARMUP_SECONDS:-90}"
...
CONTAINER_NAME="${CONTAINER_NAME:-dynamo-condp-vllm-smoke-$$}"
LOG_FILE="${LOG_FILE:-/tmp/${CONTAINER_NAME}.log}"

if [[ ! -f "$INNER_SMOKE" ]]; then
    echo "Missing inner smoke test: $INNER_SMOKE" >&2
    exit 1
fi
...
echo "PASS: conditional-disagg vLLM container smoke test"
echo "Log: $LOG_FILE"
