...
- **XPU**: Intel XPU with sufficient VRAM (8 GB+ for 2B models, 24 GB+ for 7B models)
...
```bash
cd examples/backends/vllm/launch/lora/xpu/multimodal
./lora_agg_xpu.sh
```

This starts the frontend (port 8000) and vLLM worker (port 8081) with `Qwen/Qwen3-VL-2B-Instruct` as the base model. On XPU, this model uses `--gpu-memory-utilization 0.78`, `--max-model-len 8192`, and `--max-num-batched-tokens 8192` by default.
...
```bash
./lora_agg_xpu.sh --model llava-hf/llava-1.5-7b-hf            # Use a different base model
./lora_agg_xpu.sh -- --enforce-eager                            # Pass extra vLLM args
./lora_agg_xpu.sh -- --mm-processor-kwargs '{"max_pixels": 1003520}'  # Cap image resolution
```
...
| `ZE_AFFINITY_MASK` | `0` | GPU device index |

### base models supported by this script

| Model | Notes |
|---|---|
| `Qwen/Qwen3-VL-2B-Instruct` | Default. Uses `--gpu-memory-utilization 0.78` for single-XPU testing. |
...
- Set a different port: `DYN_HTTP_PORT=8001 ./lora_agg_xpu.sh`

### OOM during inference
- Reduce `--max-model-len` via extra args: `./lora_agg_xpu.sh -- --max-model-len 4096`
- Cap image resolution: `./lora_agg_xpu.sh -- --mm-processor-kwargs '{"max_pixels": 1003520}'`
- Lower GPU memory utilization: `./lora_agg_xpu.sh -- --gpu-memory-utilization 0.80`
