...
#
# Usage:
#   ./lora_agg_xpu.sh                                             # Qwen3-VL-2B (default)
#   ./lora_agg_xpu.sh --model llava-hf/llava-1.5-7b-hf           # LLaVA 1.5
#   ./lora_agg_xpu.sh -- --enforce-eager                          # Pass extra args to vLLM
...
Environment variables:
  ...
  ZE_AFFINITY_MASK       XPU device index (default: 0)
...
case "$MODEL_NAME" in
    Qwen/Qwen2.5-VL-7B-Instruct)
        MODEL_SPECIFIC_ARGS=("--gpu-memory-utilization" "0.85" "--max-model-len" "8192" "--max-num-seqs" "1024" "--kv-cache-memory-bytes" "4G")
        ;;
    Qwen/Qwen3-VL-2B-Instruct)
        MODEL_SPECIFIC_ARGS=("--gpu-memory-utilization" "0.78" "--max-model-len" "8192" "--max-num-batched-tokens" "8192")
        ;;
    llava-hf/llava-1.5-7b-hf)
        MODEL_SPECIFIC_ARGS=("--gpu-memory-utilization" "0.85" "--max-model-len" "4096" "--kv-cache-memory-bytes" "4G")
        ;;
esac
