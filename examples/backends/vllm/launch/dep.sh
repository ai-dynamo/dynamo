...
# --no-enable-flashinfer-autotune: the FlashInfer MoE autotune warmup hits a CUDA illegal memory
# access on Blackwell (sm100) for this BF16 MoE under data parallel expert parallelism, so the
# worker never reaches a serving state. Matches the Blackwell recipes under recipes/
