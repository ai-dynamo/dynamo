...
The `simple-filter-score-pick` policy shows the complete pipeline. It filters on minimum device overlap and scores active requests. Tool-result turns select the worker with the most device overlap through `session_context().input_trigger()`. With `--router-session-affinity-mode soft`, other turns preserve an eligible `affinity_target()`, then select the lowest cost when no affinity target remains.
