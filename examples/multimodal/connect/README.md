...
There are four types of supported operations:
  ...
  1. Enable remote worker reads of one or more local memory buffers.

  2. Enable remote worker writes to one or more local memory buffers.

  3. Read one or more remote buffers into a matching set of local memory buffers.

  4. Write to one or more remote buffers from a matching set of local memory buffers.
...
In the case of the [Dynamo Multimodal Example](../README.md):
...
with which to utilize GPU Direct RDMA accellerated data transfers between models hosted by different workers in a Dynamo pipeline.
...
The RMDA metadata must be provided to the remote worker expected to complete the operation.
...
> Disposale of the object reference will instruct the RDMA subsystem to cancel the read operation,
...
> Disposale of the object reference will instruct the RDMA subsystem to cancel the write operation,
> therefore the operation should be awaited until complete or and deleted prior to completion when cancellation is intended.
...
When a system contains multple GPU devices, specific GPU devices can be identified by including their ordinal index number.
...
> Blocks the caller until the operation has recieved a completion signal from a remote worker.
...
An operation which enables a remote work to write data to the local worker.
...
A Pydantic type intended to provide JSON serlialized RDMA metadata about a [`ReadableOperation`](#readableoperation) or [`WritableOperation`](#writableoperation) object.
