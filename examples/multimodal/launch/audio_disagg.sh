...
CUDA_VISIBLE_DEVICES=1 \
    ...
    python3 components/worker.py --model $MODEL_NAME --worker-type prefill --enable-disagg $GPU_MEM_ARGS "${EXTRA_ARGS[@]}" &
CUDA_VISIBLE_DEVICES=2 \
    DYN_SYSTEM_PORT=${DYN_SYSTEM_PORT2:-8082} \
    DYN_VLLM_KV_EVENT_PORT=20082 \
    VLLM_NIXL_SIDE_CHANNEL_PORT=20099 \
    python3 components/worker.py --model $MODEL_NAME --worker-type decode --enable-disagg $GPU_MEM_ARGS "${EXTRA_ARGS[@]}" &
