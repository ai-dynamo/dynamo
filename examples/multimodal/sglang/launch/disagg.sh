...
# run SGLang processor
python3 components/processor.py --model $MODEL_NAME --chat-template "$CHAT_TEMPLATE" &

# run SGLang encode worker
CUDA_VISIBLE_DEVICES=0 python3 components/encode_worker.py --model $MODEL_NAME --chat-template "$CHAT_TEMPLATE" &

# run SGLang prefill worker
...
# run SGLang decode worker
CUDA_VISIBLE_DEVICES=2 python3 components/worker.py \
  --model $MODEL_NAME \
  --page-size 16 \
  --tp 1 \
  --trust-remote-code \
