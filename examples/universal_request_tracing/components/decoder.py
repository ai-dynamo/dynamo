...
class Decoder(RequestTracingMixin):
    ...
    async def decode(self, hidden_states: List[float], request_id: Optional[str] = None) -> AsyncIterator[Dict[str, any]]:
        """
        ...
        try:
            ...
            if sequence_id in self.active_sequences:
                ...
                self.log_with_request_id("info", f"Decoding completed. Generated {sequence_info['tokens_generated']} tokens")

    async def _decode_tokens(self, hidden_states: List[float], sequence_id: str) -> AsyncIterator[Dict[str, any]]:
        """
        ...
        for step in range(max_tokens):
            ...
            self.log_with_request_id("debug", f"Generated token: '{token_text}' (id: {token_id})")
    ...
    async def batch_decode(self, batch_hidden_states: List[List[float]], request_id: Optional[str] = None) -> AsyncIterator[Dict[str, any]]:
        """
        Batch decode multiple sequences with request tracking.
        """
        request_id = self.ensure_request_id(request_id)
        self.log_with_request_id("info", f"Starting batch decode for {len(batch_hidden_states)} sequences")
        
        for batch_idx, hidden_states in enumerate(batch_hidden_states):
            self.log_with_request_id("debug", f"Processing batch item {batch_idx + 1}/{len(batch_hidden_states)}")
    ...
    async def get_stats(self, request_id: Optional[str] = None) -> Dict[str, any]:
        ...
        return {
            ...
            "avg_tokens_per_sequence": (
                self.decode_stats["tokens_generated"] / max(1, self.decode_stats["sequences_completed"])
            )
        }

    @dynamo_endpoint(name="stop_sequence")
    async def stop_sequence(self, sequence_id: str, request_id: Optional[str] = None) -> Dict[str, any]:
        """
        ...
        if sequence_id in self.active_sequences:
            sequence_info = self.active_sequences.pop(sequence_id)
            self.log_with_request_id("debug", f"Stopped sequence with {sequence_info['tokens_generated']} tokens")
