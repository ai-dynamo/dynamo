...
fn main() -> anyhow::Result<()> {
        ...
        if let Some(sglang_flags) = flags.internal_sglang_process {
            ...
            if cfg!(feature = "sglang") {
                #[cfg(feature = "sglang")]
                {
                    ...
                    let node_config = dynamo.llm::engines::MultiNodeConfig {
    ...
    let rt_config = dynamo.runtime::RuntimeConfig::from_settings()?;

    // One per process. Wraps a Runtime with holds two tokio runtimes.
    let worker = dynamo.runtime::Worker::from_config(rt_config)?;

    worker.execute(wrapper)
}

async fn wrapper(runtime: dynamo.runtime::Runtime) -> anyhow::Result<()> {
    let mut in_opt = None;
