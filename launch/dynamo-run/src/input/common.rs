...
use dynamo_llm::{
    backend::Backend,
    kv_router::{indexer::KvIndexer, metrics_aggregator::KvMetricsAggregator},
    ...
    match engine_config {
        EngineConfig::Dynamic(endpoint_id) => {
            ...
            match &flags.router_mode {
                RouterMode::KV => {
                    // 0. The other side must publish the KV events, lib/llm/src/engines/vllm
                    //
                    // 1. Here, instantiate a KvRouter. That should handle KvIndexer and
                    //    KvMetricsAggregator
