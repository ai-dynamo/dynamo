#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.4ctx1gen.convrouter.pr13675
#SBATCH --nodes=2
...
# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 4 ctx + 1 gen TRT-LLM disagg with
# ConversationRouter on the ctx pool.  ConversationRouter uses raw-text
# unicode hashing by default (use_token_ids=false), so it sidesteps the
# HarmonyAdapter chat-template bug that breaks KvCacheAwareRouter on gpt-oss.
#
# RWLT sends X-Session-ID + X-Correlation-ID headers via
# send_conversation_routing_headers=true.  If the server understands those
# headers it gets EXPLICIT sticky routing; otherwise the router falls back
# to IMPLICIT block-hash prefix matching, which still reuses the same
# context worker for repeat turns of one trajectory because RWLT keeps the
# system-prompt cache-bust id stable within a trajectory.
#
# Env:
...
#
# Submit:
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0 bench/run_benchx_4ctx1gen_convrouter.sh
# =============================================================================
...
# --- gen worker on $NODE1 GPU 0 port 8005 ---
echo "[$(date +%H:%M:%S)] Starting gen worker on $NODE1 GPU 0 port 8005..."
srun --overlap --ntasks=1 --nodes=1 --nodelist=$NODE1 --mpi=pmix \
...
# --- 4 ctx workers on $NODE0 GPUs 0-3, ports 8001-8004 ---
CTX_PIDS=""
for GPU in 0 1 2 3; do
  PORT=$((8001 + GPU))
  echo "[$(date +%H:%M:%S)] Starting ctx worker on $NODE0 GPU $GPU port $PORT..."
...
# --- health wait ---
echo "[$(date +%H:%M:%S)] Waiting for servers (4 ctx + 1 gen)..."
HEALTHY=0
...
if [ "$HEALTHY" -ne 1 ]; then
  ...
  kill $MASTER_PID $GEN_PID $CTX_PIDS 2>/dev/null; wait 2>/dev/null; exit 1
fi

# --- /kv_cache_events probe (initial) ---
for ENDPOINT in $NODE0:8001 $NODE0:8002 $NODE0:8003 $NODE0:8004 $NODE1:8005; do
...
python3 "$REPO_DIR/bench/capture_metrics.py" \
  --endpoints "${NODE0}:8001,${NODE0}:8002,${NODE0}:8003,${NODE0}:8004,${NODE1}:8005" \
...
# --- RWLT @ CONCURRENCY (with conversation routing headers) ---
...
kill $MASTER_PID $GEN_PID $CTX_PIDS 2>/dev/null
wait 2>/dev/null
