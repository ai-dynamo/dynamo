#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.2ctx1gen.dynamo.kvrouter
#SBATCH --nodes=1
...
#SBATCH --output=bench/logs/run_benchx_2ctx1gen_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_2ctx1gen_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 2 ctx + 1 gen with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 2 ctx worker(s) (GPUs 0-1) + 1 gen worker(s) (GPUs 2-2)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_2ctx1gen_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_2ctx1gen_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_2ctx1gen_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
# --- 1 gen worker(s) on $NODE0 GPUs 2-2 (decode) ---
GEN_PIDS=()
for GPU in 2; do
...
# ==============================================================================
echo "[$(date +%H:%M:%S)] Waiting for servers (2 prefill + 1 decode)..."
if ! wait_for_dynamo_workers "${NODE0}" "${FRONTEND_PORT}" 2 1 2700 60; then
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8085" \
    --labels "ctx_g0,ctx_g1,gen_g0" \
