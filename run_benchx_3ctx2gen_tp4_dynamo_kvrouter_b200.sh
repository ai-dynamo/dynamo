#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.3ctx2gen_tp4.dynamo.kvrouter
#SBATCH --nodes=2
...
#SBATCH --output=bench/logs/run_benchx_3ctx2gen_tp4_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_3ctx2gen_tp4_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 3 ctx TP1 + 2 gen TP4 with kv router,
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx2gen_tp4_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_3ctx2gen_tp4_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_3ctx2gen_tp4_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
# ==============================================================================
echo "[$(date +%H:%M:%S)] Waiting for servers (3 prefill + 2 decode)..."
if ! wait_for_dynamo_workers "${NODE0}" "${FRONTEND_PORT}" 3 2 2700 60; then
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE0}:8085,${NODE1}:8086" \
    --labels "ctx_g0,ctx_g1,ctx_g2,gen_g0,gen_g1" \
