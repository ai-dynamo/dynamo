...
- **零配置部署（[DGDR](docs/kubernetes/dgdr.md)）** *(beta)：* 在一个 YAML 中指定模型、硬件和 SLA；AIConfigurator 自动分析工作负载，Planner 优化拓扑，然后由 Dynamo 完成部署
...
对于生产级多节点集群，安装 [Dynamo Platform](docs/kubernetes/model-deployment-guide.md)，并使用单个 manifest 部署：
