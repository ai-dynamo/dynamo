...
- `validation`: final report template. Raw V1 data belongs in `artifacts/`.
...
Pod, inventory it read-only, then collect the 10 cold starts:
...
hash must be used as checkpoint ID and retained with the campaign:
...
implementation/bin/create-checkpoint --snapshotctl dist/snapshotctl \
  ...
  --checkpoint-id "$COMPATIBILITY_HASH" --output artifacts/i1/checkpoint.json
...
only `/health`, so the KV cache remains empty. Copy `COMPATIBILITY_HASH` into
`campaign.json`; V1 refuses to run when it differs from the checkpoint ID.
