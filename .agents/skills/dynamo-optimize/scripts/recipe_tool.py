...
def repo_root(start: Path) -> Path:
    for path in [start, *start.parents]:
        if (path / "recipes").is_dir() and (path / ".agents" / "skills").is_dir():
            return path
...
def gpu_count_hint(text: str) -> int | None:
    values = [
        int(match) for match in re.findall(r"nvidia\.com/gpu:\s*[\"']?(\d+)", text)
    ]
...
def main() -> int:
    ...
    list_parser.add_argument("--framework", choices=sorted(FRAMEWORKS))
