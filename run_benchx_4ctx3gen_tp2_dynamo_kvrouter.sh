#!/bin/bash
#SBATCH --job-name=core_dlfw_ci-benchx.4ctx3gen_tp2.dynamo.kvrouter
#SBATCH --nodes=3
#SBATCH --partition=gb200
#SBATCH --account=core_dlfw_ci
#SBATCH --time=04:00:00
#SBATCH --output=bench/logs/run_benchx_4ctx3gen_tp2_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_4ctx3gen_tp2_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 4 ctx TP1 + 3 gen TP2 with kv router,
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_4ctx3gen_tp2_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_4ctx3gen_tp2_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_4ctx3gen_tp2_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE0}:8084,${NODE1}:8085,${NODE1}:8086,${NODE2}:8087" \
