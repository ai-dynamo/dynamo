#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.1ctx2gen.dynamo.kvrouter
#SBATCH --nodes=1
...
#SBATCH --output=bench/logs/run_benchx_1ctx2gen_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_1ctx2gen_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 1 ctx + 2 gen with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 1 ctx worker(s) (GPUs 0-0) + 2 gen worker(s) (GPUs 1-2)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_1ctx2gen_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_1ctx2gen_dynamo_kvrouter.sh
# =============================================================================
...
# --- 2 gen worker(s) on $NODE0 GPUs 1-2 (decode) ---
GEN_PIDS=()
for GPU in 1 2; do
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8085,${NODE0}:8086" \
