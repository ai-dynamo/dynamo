...
# its p2p dependency and pins the CD role). All deployment-wide config (tokio
# workers, nixl backends, control dev+metrics, onboard mode) is seeded into the
# hub's base_config via KVBM_HUB_KVBM in start-hub.sh. Only the per-instance
# disagg role (--role prefill) is supplied here.
#
# Env vars:
...
#   NOTE: block_layout, onboard.mode, control.dev are all hub-authoritative now
#   (seeded at start-hub via KVBM_HUB_LAYOUT / KVBM_ONBOARD_MODE / KVBM_HUB_KVBM);
#   they are no longer launcher knobs.
set -eu
