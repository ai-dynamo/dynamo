#!/bin/bash
#SBATCH --job-name=coreai_comparch_aarwlt-benchx.4ctx2gen_tp2.dynamo.kvrouter
#SBATCH --nodes=1
...
#SBATCH --output=bench/logs/run_benchx_4ctx2gen_tp2_dynamo_kvrouter_%j.log
#SBATCH --error=bench/logs/run_benchx_4ctx2gen_tp2_dynamo_kvrouter_%j.err

# =============================================================================
# benchx (feat/bench_x sha 11e16c) — 4 ctx TP1 + 2 gen TP2 with kv router,
# driven by dynamo.trtllm (etcd + nats + dynamo frontend).
#
# Layout:
#   NODE0 — etcd + nats + dynamo frontend + 4 ctx worker(s) (GPUs 0-3) TP1
#   NODE0 — gen worker 0 GPUs 4,5 (TP2)
#   NODE0 — gen worker 1 GPUs 6,7 (TP2)
#
...
#
# Submit:
#   sbatch --export=ALL,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_4ctx2gen_tp2_dynamo_kvrouter.sh
#   sbatch --export=ALL,CONCURRENCY=48,HOSTCACHE=0,WORKER_METRICS=0 bench/run_benchx_4ctx2gen_tp2_dynamo_kvrouter.sh
# =============================================================================
...
EXP_NAME="run_benchx_4ctx2gen_tp2_dynamo_kvrouter_${HCTAG}_c${C_TAG}"
...
MODEL_PATH="/lustre/fsw/portfolios/coreai/projects/coreai_comparch_aarwlt/users/rihuo/openai_gpt-oss-120b"
...
GEN_CONFIG_SRC="${REPO_DIR}/bench/gen_config_tp2_b200.yaml"
...
GEN_PIDS=()
# --- decode worker 0 on $NODE0 GPUs 4,5 (TP2) ---
GEN_PORT=8085
echo "[$(date +%H:%M:%S)] Starting gen worker 0 on $NODE0 GPUs 4,5 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
  --output="$RESULTS_DIR/gen_worker_g0.log" \
  --container-image="$CONTAINER_IMAGE" --container-mounts="$CONTAINER_MOUNTS" \
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=4,5 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
...
# --- decode worker 1 on $NODE0 GPUs 6,7 (TP2) ---
GEN_PORT=8086
echo "[$(date +%H:%M:%S)] Starting gen worker 1 on $NODE0 GPUs 6,7 (DYN_SYSTEM_PORT=${GEN_PORT})..."
start_bg srun --overlap --ntasks=2 --ntasks-per-node=2 --nodes=1 --nodelist=$NODE0 --mpi=pmix \
  --output="$RESULTS_DIR/gen_worker_g1.log" \
  --container-image="$CONTAINER_IMAGE" --container-mounts="$CONTAINER_MOUNTS" \
  --no-container-entrypoint \
  --no-container-mount-home \
  bash -c "cd $REPO_DIR && export CUDA_VISIBLE_DEVICES=6,7 && $COMMON_ENV && $DYNAMO_WORKER_ENV && \
    export DYN_SYSTEM_PORT=${GEN_PORT} && \
    trtllm-llmapi-launch python3 -m dynamo.trtllm \
      --model-path $MODEL_PATH --served-model-name $MODEL \
      --disaggregation-mode decode \
...
# --- 4 ctx worker(s) on $NODE0 GPUs 0-3 (prefill, TP1) ---
CTX_PIDS=()
...
# ==============================================================================
echo "[$(date +%H:%M:%S)] Waiting for servers (4 prefill + 2 decode)..."
if ! wait_for_dynamo_workers "${NODE0}" "${FRONTEND_PORT}" 4 2 2700 60; then
...
if [ "$WORKER_METRICS" = "1" ]; then
  ...
  python3 "$REPO_DIR/bench/capture_metrics.py" \
    --endpoints "${NODE0}:8081,${NODE0}:8082,${NODE0}:8083,${NODE0}:8084,${NODE0}:8085,${NODE0}:8086" \
    --labels "ctx_g0,ctx_g1,ctx_g2,ctx_g3,gen_g0,gen_g1" \
